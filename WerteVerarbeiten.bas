   ...
   Select Case fldName
    ...
    Case "größe", "gewicht"
     ...
     If fldName = "gewicht" Then
      ...
      Else
       Add = myEFrag("SELECT COALESCE((SELECT REPLACE(REGEXP_REPLACE(inhalt,'[^0-9,.]',''),'.',',')FROM eintraege WHERE art IN('gew','gw') AND pat_id=" & Pat_id & " ORDER BY zeitpunkt DESC),'?')").Fields(0)
      End If
      On Error Resume Next
      bmival = 0
