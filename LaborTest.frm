...
'  db.Open "PROVIDER=MSDASQL;driver={MySQL ODBC 3.51 Driver};server=linux1;uid=praxis;pwd=***REMOVED***;database=quelle;"
  ...
  If erg <> 0 Then
   adoRS.Open "SELECT FID,`dokumente`.Pat_ID AS pat_id,`namen`.nachname AS nachname, `namen`.vorname AS vorname, ZeitPunkt,`dokumente`.DokPfad AS dokpfad,DokArt,DokName,Quelldatum, -abgehakt AS abgehakt, `dokumente`.absPos AS abspos,`dokumente`.AktZeit AS aktzeit,DokGroe,QS,QT,`dokumente`.StByte AS stbyte FROM ((`dokumente` LEFT JOIN `namen` ON `dokumente`.pat_id = `namen`.pat_id) LEFT JOIN `dokumente abgehakt` ON `dokumente`.dokpfad = " & "`" & "dokumente abgehakt" & "`" & ".dokpfad) WHERE `dokumente`.pat_id = " & erg & " ORDER BY ZeitPunkt DESC", DBCn, adOpenStatic, adLockOptimistic
  Else
   adoRS.Open "SELECT FID,`dokumente`.Pat_ID AS pat_id,`namen`.nachname AS nachname, `namen`.vorname AS vorname, ZeitPunkt,`dokumente`.DokPfad AS dokpfad,DokArt,DokName,Quelldatum, abgehakt, `dokumente`.absPos AS abspos,`dokumente`.AktZeit AS aktzeit,DokGroe,QS,QT,`dokumente`.StByte AS stbyte FROM ((`dokumente` LEFT JOIN `namen` ON `dokumente`.pat_id = `namen`.pat_id) LEFT JOIN `dokumente abgehakt` ON `dokumente`.dokpfad = `dokumente abgehakt`.dokpfad) WHERE abgehakt <> 0 ORDER BY ZeitPunkt DESC", DBCn, adOpenStatic, adLockOptimistic
  End If
