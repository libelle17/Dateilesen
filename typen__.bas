...
Public Type Diagnosen
 ...
 Ausnahme As String 'Ausnahme 130 '3677 Ausnahme / Begründung für abweichendes Geschlecht
...
'   sql = sql0 & "(" & rBr(i).FID & "," & rBr(i).Pat_ID & "," & datform(rBr(i).ZeitPunkt) & ",'" & rBr(i).Pfad & "','" & rBr(i).Art & "','" & rBr(i).Name & "','" & _
   rBr(i).Typ & "'," & datform(rBr(i).AktZeit) & "," & rBr(i).DokGroe & "," & datform(rBr(i).DokAenD) & ",'" & rBr(i).QS & "','" & rBr(i).QT & "'," & _
   rBr(i).absPos & "," & rBr(i).StByte & ")"
  If Lese.dlg.SammelInsert = 0 Or i = 1 Then
   csql.Append csql0
  End If
...
'   sql = sql0 & "(" & rDo(i).FID & "," & rDo(i).Pat_ID & "," & datform(rDo(i).ZeitPunkt) & ",'" & rDo(i).DokPfad & "','" & rDo(i).DokArt & "','" & rDo(i).DokName & "'," & datform( _
   rDo(i).Quelldatum) & "," & rDo(i).absPos & "," & datform(rDo(i).AktZeit) & "," & rDo(i).DokGroe & "," & datform(rDo(i).DokAenD) & ",'" & _
   rDo(i).QS & "','" & rDo(i).QT & "'," & rDo(i).StByte & ")"
  If Lese.dlg.SammelInsert = 0 Or i = 1 Then
   csql.Append csql0
  End If
...
'   sql = sql0 & "(" & rLw(i).RefNr & ",'" & rLw(i).Abkü & "','" & rLw(i).Langname & "','" & rLw(i).Quelle & "','" & rLw(i).QSpez & "'," & datform(rLw(i).AbnDat) & ",'" & _
   ...
   rLw(i).Erklärung & "','" & rLw(i).Normbereich & "','" & rLw(i).NormU & "','" & rLw(i).NormO & "','" & rLw(i).AuftrHinw & "')"
    ...
    For j = 1 To i - 1
     ...
     If rLw(i).Normbereich <> rLw(j).Normbereich Then GoTo nextj
     If rLw(i).normu <> rLw(j).normu Then GoTo nextj
     If rLw(i).NormO <> rLw(j).NormO Then GoTo nextj
     If rLw(i).AuftrHinw <> rLw(j).AuftrHinw Then GoTo nextj
     GoTo nexti
nextj:
     Dim rsdop As New ADODB.Recordset
     Set rsdop = Nothing
     rsdop.Open "select 0 from laborxwert where refnr = " & rLw(i).RefNr & " and abkü = '" & rLw(i).Abkü & "'", DBCn, adOpenStatic, adLockReadOnly
