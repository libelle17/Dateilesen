 ...
 If Cstumm = 0 Then
  ...
  If Li1 <> "" Then
   For i = cRow(MFGTyp) To cRowSel(MFGTyp)
    MFG.col = 0
    MFG.Row = i
    Call DBCn.Execute("UPDATE `diagreihe` SET gi1 = (SELECT MIN(lfdnr) FROM diagg1 WHERE gruppe='" & Li1 & "') WHERE icd = '" & MFG.TextMatrix(i, 0) & "'", rAF)
    If rAF <> 1 Then
 ...
 If Cstumm = 0 Then
  ...
  If Text1 <> "" Then
   For i = cRow(MFGTyp) To cRowSel(MFGTyp)
    MFG.col = 0
    MFG.Row = i
    Call DBCn.Execute("UPDATE diagreihe SET gi2 = '" & Text1 & "' WHERE icd = '" & MFG.TextMatrix(i, 0) & "'", rAF)
    If rAF <> 1 Then
...
Public Sub callMachDMPBogen(Pat_id&, Optional VorDoku$, Optional obmitauswahl%, Optional immeranhaeng%, Optional obStumm%, Optional Datei$)  ' Erstelle
 ...
 FNr = 16
 myFrag rTyp, "SELECT icd FROM `diagnosen` d WHERE d.pat_id = " & Pat_id & " AND d.diagsicherheit <> 'A' AND COALESCE(d.f6010,0)=0 AND d.icd LIKE 'E1%' ORDER BY icd"
...
' Dim RRsyst%, RRdiast%
 ...
 Dim WagSt$, ArmSt$
 Dim ab315%, ab317%, ab0418%, ab1118%, ab0619%, ab0921%
 ...
 Datenerfassung$ = IIf(ab0921, "July_2021", IIf(ab0619, "April_2019", IIf(ab1118, "Okt_2018", IIf(ab0418, "Jan_2018", IIf(ab317, "July_2017", "15_07")))))
 FeldFormVersion = Datenerfassung
 FeldFormVersion = IIf(ab0921, "July_2021", IIf(ab0619, "April_2019", IIf(ab1118, "Okt_2018", IIf(ab0418, "April_2018", Datenerfassung))))
 
 FNr = 17
 On Error GoTo fehler
 erg = Dir(Datei)
 If LenB(erg) = 0 Then erg = REPLACE$(Datei, "u:", "\\linux1\daten\eigene Dateien")
 If LenB(erg) <> 0 Then
...
' Ermittlung der 'Kasse' aus Rezepten oder Überweisungen oder vorherigen DMP-Dokus usw.
 ...
 Else
  Set rform = Nothing
  myFrag rform, "SELECT kurzname, rname FROM `kassenliste` WHERE vknr = '" & rfal!VKNr & "' AND ik = '" & rfal!IK & "'" 'aktDC.VKNr
  If Not rform.BOF Then
...
' auskommentiert am 30.6.15, da bei Woltmann Fehler dadurch erzeugt ("Wenn eine Angabe im Feld KVK-Einlesedatum erfolgte, dann darf die Angabe der Statusergänzung nicht fehlen.")
   If ab0921 Then
    ZsD = ZsD & "<KVKEinlesedatum>" & Format(MAX(QAnf(ZQuart(DokuDat)), rfal!lVorl), "yyyy-mm-dd") & "</KVKEinlesedatum>"
   End If
   ...
   ZsD = ZsD & "</Krankenversicherung><Software><KBVPruefnummer>F0312265</KBVPruefnummer><Name>TurboMed EDV GmbH</Name><Version>" & IIf(ab0921, "21.3.1.5080", IIf(ab0619, "19.2.1.4087", IIf(ab1118, "18.4.1.3886", IIf(ab0418, "18.2.2.3686", "15.2.1.2774")))) & "</Version></Software></ZusatzDaten>"
   BDT.FIAdd ZsD
...
Private Sub MFGrefresh()
 ...
 rs.Open "SELECT COUNT(0) ct FROM `diagreihe`", DBCn, adOpenStatic, adLockOptimistic
 MFG.Rows = rs!ct + 30
 Set rs = Nothing
 rs.Open "SELECT MAX(zahl) AS mz FROM (SELECT icd, COUNT(0) AS zahl FROM (SELECT d.icd AS icd,diagtext FROM `diagnosen` d LEFT JOIN `diagreihe` r ON d.icd = r.icd GROUP BY d.icd, diagtext) AS innen GROUP BY icd) AS innen;", DBCn, adOpenStatic, adLockOptimistic
 MFG.Cols = rs!mZ + 4
 ...
 Next i
 Set rs = Nothing
 rs.Open "SELECT d.icd AS icd,g1.gruppe dg1, gi2, diagtext FROM `diagnosen` d LEFT JOIN `diagreihe` r ON d.icd = r.icd LEFT JOIN `diagg1` g1 ON r.gi1 = g1.lfdnr GROUP BY d.icd, diagtext ORDER BY g1.rf ,gi2 DESC,icd", DBCn, adOpenStatic, adLockOptimistic
 aicd = vNS
 MFG.Row = 1
 Do While Not rs.EOF
     ...
     If Not angefangen Then
       If UBound(Feld) > 0 Then
        If Feld(0) = "Patient" Then
         ...
         DBCn.Execute ("DROP TABLE `" & labxtb & "`")
         On Error GoTo fehler
         DBCn.Execute "CREATE TABLE `" & labxtb & "` (id integer(10) auto_increment key, patient varchar(255), fehlerart varchar(3000))", rAF
         DBCn.Execute "BEGIN"
        End If
       End If
     Else
...
Sub LabordateiAnzeig(Datei$)
 ...
 Dim pid&, vorPID&, gschl$, DateiDat As Date, vorFarbe&, vorDPneu$, vorEinh$
 ...
 ",l.*,COALESCE(lp.langtext,l.parameter) lt " & vbCrLf & _
 "FROM labpatel p LEFT JOIN labpath l ON l.elID=p.id LEFT JOIN laborparameter lp ON l.Parameter=lp.`Abkü` AND l.Einheit=lp.Einheit " & vbCrLf & _
 ...
 If Not rc.BOF And True Then
  ...
  Do While Not rc.EOF
    If Not IsNull(rc!Pat_id) Then
     If rc!Pat_id <> vorPID Then vorFarbe = IIf(vorFarbe = vbWhite, vbGräulich, vbWhite) '&H8000000F&=vbgelblichgrau
     vorPID = rc!Pat_id
     .Row = i
     .col = Pat_IDSp:    .Text = rc!Pat_id:      .CellBackColor = vorFarbe
     ...
     If aktw < 45 Then
      ...
      If aktDC.obmetf Then
       ...
       Else ' also aktw zw. 30 und 44
        On Error Resume Next
        ' metdos = DBCn.Execute("SELECT (REPLACE(REPLACE(mo,'½',0.5),'1/2',0.5)+REPLACE(REPLACE(mi,'½',0.5),'1/2',0.5)+REPLACE(REPLACE(nm,'½',0.5),'1/2',0.5)+REPLACE(REPLACE(ab,'½',0.5),'1/2',0.5)+REPLACE(REPLACE(zn,'½',0.5),'1/2',0.5))*nr,i.* FROM " & _
              ...
              "and metf " & _
              ") i;").Fields(0)
'        metdos = DBCn.Execute("SELECT ROUND((zubruch(mo)+zubruch(mi)+zubruch(nm)+zubruch(ab)+zubruch(zn))*zunr(mp.medikament)) metdos " & vbCrLf & _
         ...
         ";").Fields(0) & vbCrLf
         metdos = DBCn.Execute("SELECT metfdosis(" & CStr(pid) & ")").Fields(0)
        On Error GoTo fehler
     ...
     Select Case vorDp
      ...
      Case "HB"
       If aktw < IIf(gschl = "w", 12, 14) Then
         ...
         Set radg = DBCn.Execute("SELECT icd FROM `diagnosen` d WHERE d.pat_id = " & .TextMatrix(i, 0) & " AND d.diagtext LIKE '%anämie%' AND d.diagsicherheit not IN ('A','Z') AND COALESCE(d.f6010,0)=0 AND d.obdauer<>0")
 '        DBCn.Execute "SET GROUP_CONCAT_MAX_LEN = 255"
 '        SET radg = DBCn.Execute("SELECT GROUP_CONCAT(diagtext) diag FROM `diagnosen` d WHERE d.pat_id = " & .TextMatrix(i, 0) & " AND d.diagsicherheit not IN ('A','Z') AND COALESCE(d.f6010,0)=0 AND d.obdauer<>0")
...
mehrdeutig:
  ...
  rs.Move 1
 Loop
 DBCn.Execute "DROP TABLE IF EXISTS `" & labxtb & "`"
 End If
 .col = 1
 .Row = 1
 .Visible = True
...
Private Sub Form_Load()
 Dim sql$, i%, erg$, pCol As New Collection, j&, gehezu&, rAF&
 Me.Visible = True
 FNr = 1
  ...
  Case Else ' artPat, artDiag
   ...
   Select Case Me.PLArt
    ...
    Case artDiag
     ...
     DBCn.Execute "INSERT INTO `diagreihe`(`icd`) SELECT `icd` FROM `diagnosen` WHERE not `icd` IN (SELECT icd FROM `diagreihe`) GROUP BY icd ", rAF
     Call MFGrefresh
...
'     sql = "SELECT f.bhfb, dmpbeg AS notiz, dmpklass, tkz, n.pat_id, n.nachname, n.vorname, icd, kurzname FROM `namen` n LEFT JOIN `diagnosen` d ON n.pat_id = d.pat_id AND icd RLIKE '^E1[0-4]' AND diagsicherheit not IN ('Z','A') LEFT JOIN (SELECT f.pat_id, f.fid, f.schgr, f.bhfb, f.ik, f.vknr, f.quartal FROM (SELECT * FROM (SELECT BhFB AS mbhfb, bhfe1, Pat_ID AS pid, quartal FROM `faelle` f WHERE bhfb < " & DatFor_k(min(NOW(), qend(ZQuart(NOW() - Verspätung)))) & " ORDER BY pid, bhfb DESC) l GROUP BY pid DESC) l LEFT JOIN `faelle` f ON f.pat_id = l.pid AND f.bhfb = l.mbhfb GROUP BY pat_id) f ON n.pat_id = f.pat_id LEFT JOIN `anamnesebogen` a ON n.pat_id = a.pat_id LEFT JOIN `kassenliste` k ON k.ik = f.ik AND k.vknr = f.vknr WHERE (notiz LIKE '%DMP hier%' OR dmpklass = 3) AND NOT ISNULL (f.bhfb) GROUP BY n.pat_id ORDER BY n.pat_id"
     sql = "SELECT f.bhfb, dmpbeg notiz, dmpklass, tkz, n.pat_id, n.nachname, n.vorname, icd, kurzname " & _
      "FROM `namen` n " & _
      "LEFT JOIN `diagnosen` d ON n.pat_id = d.pat_id AND icd RLIKE '^E1[0-4]' AND d.diagsicherheit not IN ('Z','A') AND COALESCE(d.f6010,0)=0 " & _
     ...
     myFrag rDPat, "SELECT COUNT(0) FROM (" & sql & ") AS innen"
     With MFG
...
' IN der ersten Spalte die schon exportierten färben
 ...
 '      .Text = replace$(replace$(replace$(rDPat!Notiz, "DMP", ""), &HA, " "), &HD, " ")
      ...
      If ZQuart(rDPat!BhFB) = ZQuart(Now() - Verspätung) Then .CellBackColor = DunkelRosa Else .CellBackColor = vbWhite
      Dim begcol%, obraus%, obrot%, obhellrot%, oberst%
      obraus = 0 ' Patient rausgeflogen
      obrot = 0
      obhellrot = 0
      oberst = 0
'      rDok.Open "SELECT `DokuDatum`, `Art`, `ausgedruckt`, `OK`, `exportiert` FROM `dmpreihe` dr WHERE pat_id = " & rDPat!Pat_id & " AND (dr.Abk LIKE 'eDMPDM%' OR dr.Abk LIKE 'DMPDTYP%') ORDER BY `DokuDatum` DESC", DBCn, adOpenDynamic, adLockReadOnly
      myFrag rDok, "SELECT `DokuDatum`, `Art`, `ausgedruckt`, `OK`, `exportiert` FROM `dmpreihe` dr WHERE pat_id = " & rDPat!Pat_id & " AND (dr.Abk LIKE 'eDMPDM%' OR dr.Abk LIKE 'DMPDTYP%') ORDER BY `DokuDatum` DESC"
      If Not rDok.BOF Then
       begcol = .col + 1
       If ZQSort(Now() - 120) > ZQSort(rDok!DokuDatum) Then ' Now() - rDok!DokuDatum > 120 THEN ' 21. des übernä Monats
        obraus = True
       ...
       If ZQuart(Now() - Verspätung) <> ZQuart(rDok!DokuDatum) Then
        begcol = begcol + 1
        If Not obraus And ZQuart(rDPat!BhFB) = ZQuart(Now() - Verspätung) Then ' 2.Bedingung eingefügt: 31.12.15
         obrot = True
       ...
       For j = begcol To .Cols - 1
        ...
        If rDok!Ok And rDok!ausgedruckt Then
         .Text = .Text & " ok"
        ElseIf j = begcol And Not obraus And ZQuart(rDPat!BhFB) = ZQuart(Now() - Verspätung) Then ' letzte Bedingungen eingefügt 31.12.15
      ...
      End If ' not rdok!bof
      Set rDok = Nothing
      If Not oberst And Not obraus And ZQuart(rDPat!BhFB) = ZQuart(Now() - Verspätung) Then ' letzte Bedingung eingefügt 31.12.15
       .col = begcol
       .CellBackColor = DunkelRot
       obhellrot = True
       .toolTipText = "Doku fehlt"
      End If
      If (obrot Or obhellrot) And rDPat!dmpklass = 3 And rDPat!Tkz = 0 Then
       Dim ttt$
       ttt = .toolTipText
       .col = DNamSp
...
Sub AlleMark(ob%) ' Alle Markieren, alle Demarkieren
 ...
 Else      ' Gesendete Demarkieren
  ...
  DBCn.Execute "UPDATE `faxeinp`.`outa` SET pid = mid(docname,instr(docname,'PID ')+4,locate(', ',docname,instr(docname,'PID '))-instr(docname,'PID ')-4) WHERE instr(docname,'PID ')<>0 AND (ISNULL(pid) OR pid=0) AND mid(docname,instr(docname,'PID ')+4,locate(', ',docname,instr(docname,'PID '))-instr(docname,'PID ')-4)<>''", rAF
  For i = 1 To PatZuHASL.COUNT
...
'   Stop
  ...
  Case artLab
   If IsNumeric(Me.MFG.TextMatrix(Me.MFG.MouseRow, Pat_IDSp)) Then
    Select Case Me.MFG.MouseCol
     Case 0: Call FertigStellen(Me.MFG.MouseRow, True) ' IN TurbomedAnzeigen
 ...
 Else ' IF obtb = 0 THEN
  If MFG.col = GruSp Then
   Set ctrl = Me.Li1
   ctrl.Clear
   rs.Open "SELECT * FROM `diagg1` ORDER BY rf", DBCn, adOpenStatic, adLockOptimistic
   Do While Not rs.EOF
...
Private Sub MFG_MouseMove(Button As Integer, Shift As Integer, x As Single, Y As Single)
    ...
    Case parsp
     If Me.MFG.TextMatrix(Me.MFG.MouseRow, 8) = "x" Then
      myFrag rs, "SELECT bez1.abkü, bez2.abkü, CONCAT(GROUP_CONCAT(' ',IF(ISNULL(bez1.abkü),''," & vbCrLf & _
                ...
                "WHERE idypbez = " & IDS(1, MFG.MouseRow)
      If Not rs.BOF Then
 ...
 Case artLab
  Me.MFG.toolTipText = vNS
  If Me.MFG.toolTipText = "" Then
  If IsNumeric(Me.MFG.TextMatrix(Me.MFG.MouseRow, Pat_IDSp)) Then
   Select Case Me.MFG.MouseCol
     Case namsp
'      myfrag rs, "SELECT GROUP_CONCAT(CONCAT_WS(' ',DATE_FORMAT(diagdatum,'%d.%m.%y'), CONCAT('(',diagsicherheit,')'), diagtext, icd),char(13,10)) diag FROM `diagnosen` WHERE pat_id = " & Pat_id & " AND diagsicherheit <> 'A' ORDER BY DATE(diagdatum)"
     ...
     Case parsp
      ...
      myFrag rs, hsql
      If Not rs.BOF Then
       Me.MFG.toolTipText = rs.Fields(0)
      End If
     Case medsp
      Me.MFG.toolTipText = Me.MFG.TextMatrix(MFG.MouseRow, labhwsp) ' Zeilenumbruch statt '|' geht nicht nicht chr(10), chr(13), chr(10) & chr(13) oder chr(13) & chr(10)
   End Select
   End If
...
' Verknüpfung nach 2) löschen
      DBCn.Execute "DELETE FROM `laborypgl` WHERE `idypneu` = " & cb, rAF
      If rAF <> 1 Then
...
' Verknüfung nach 3) überbrücken
      If Me.MFG.TextMatrix(MR, 8) = "x" Then
       DBCn.Execute "UPDATE `laborypgl` SET `idypbez` = " & cb & " WHERE `idypbez` = " & IDS(1, MR), rAF
       For i = 0 To UBound(IDS, 2)
