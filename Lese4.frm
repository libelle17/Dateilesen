...
Private Sub DokumenteInDatenbank_Click()
 ...
 DBCn.Execute "create database if not exists `" & TMDok & "` default character set 'latin1' collate 'latin1_german2_ci'"
 DBCn.Execute "use `" & TMDok & "`"
 DBCn.Execute "CREATE TABLE if not exists `tmdok`.`Dokumente` (  `id` int(11) NOT NULL AUTO_INCREMENT,  `Pfad` varchar(255),  `Datei` varchar(255),  `Größe` int(11) DEFAULT NULL,  `geändert` datetime DEFAULT NULL,  PRIMARY KEY (`id`),  KEY `Name` (`Pfad`,`Datei`),  KEY `Größe` (`Größe`),  KEY `geändert` (`geändert`)) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_german2_ci ROW_FORMAT=DYNAMIC"
 Call dverz(DPfad)
End Sub
Private Sub dverz(DPfad$)
 Dim FSOPfad As Folder, Fil As File, SubF As Folder
 Set FSOPfad = FSO.GetFolder(DPfad)
 For Each Fil In FSOPfad.Files
  DBCn.Execute "insert into `Dokumente`(Pfad,Datei,größe,geändert) values('" & doUmwfSQL(DPfad, True) & "','" & doUmwfSQL(Fil.Name, True) & "'," & Fil.size & "," & DatForm(Fil.DateLastModified) & ")"
 Next Fil
