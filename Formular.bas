...
' 2) Methode wie in doPatvonMO
 ...
 If inf.KVNr <> "" Then
  infos(0, infi) = IIf(InStrB(inf.Überschr, "Herr") <> 0, "Herrn", "Frau") ' inf.anrede
  infos(1, infi) = inf.gesName
  infos(2, infi) = inf.Straße
  infos(3, infi) = inf.plz & " " & inf.ort
  infos(4, infi) = inf.Faxnr
  infos(5, infi) = inf.anrede ' inf.Überschr
  If InStrB(inf.anrede, "Frau") Then infos(5, infi) = "Sehr geehrte Frau Kollegin" Else If InStrB(inf.anrede, "Herr") Then infos(5, infi) = "Sehr geehrter Herr Kollege" Else If inf.anrede = "" Then infos(5, infi) = inf.Überschr Else infos(5, infi) = inf.anrede
  infos(6, infi) = inf.DmpTyp2
  infos(7, infi) = inf.DmpTyp1
  infos(8, infi) = inf.NiederlGb
  infos(9, infi) = inf.Vorname
  infos(10, infi) = inf.Funktion
  infos(11, infi) = inf.Fachrtg
  infos(12, infi) = inf.KVNr
...
' dim fs
 ...
 If Lese.MOBetr And Not Lese.pataw.ohneÜbertr Then
  syscmd acSysCmdSetStatus, "Erstelle Brief; 0) Übertrage Patientendaten aus Medical Office ..."
...
'  rsNa.Seek "=", raAn!Pat_id
 Dim NachNa$, VorNa$, GName$, G1Name$, tit$, GebDat As Date, dieder$, bhb As Date, behs$
 myFrag rsNa, "SELECT n.*,COALESCE(titel,'') tit, CONCAT(IF(geschlecht='m','Herrn','Frau'),' ',gesname(pat_id),', *',DATE_FORMAT(gebdat,'%e.%c.%y')) gname,IF(geschlecht='m','der','die') dieder, DATE(IF(qad=0,(SELECT fanf FROM faelle WHERE pat_id=n.pat_id AND fanf>20040701 ORDER BY fanf LIMIT 1),qad))bhb, IF(DATE(IF(qad=0,(SELECT fanf FROM faelle WHERE pat_id=n.pat_id AND fanf>20040701 ORDER BY fanf LIMIT 1),qad))=DATE(lbeh),CONCAT('am ',DATE_FORMAT(lbeh,'%e.%c.%y')),CONCAT('vom ',DATE_FORMAT(IF(qad=0,(SELECT fanf FROM faelle WHERE pat_id=n.pat_id AND fanf>20040701 ORDER BY fanf LIMIT 1),qad),'%e.%c.%y'),' bis zum ',DATE_FORMAT(lbeh,'%e.%c.%y'),' mehrmals'))behs FROM namenlb n WHERE pat_id = " & Pat_ID
...
' ermitteln, ob ein oder zwei Adressaten
   ...
   For ii = 1 To UBound(infos, 2)
    If infos(4, ii) <> infos(4, 0) And infos(4, ii) <> "" Then
     Index = ii
     Exit For
    End If
   Next ii
...
' Einleitung: zuh(replace$(GName, ", *", ", geb. ")), dieder, behd
   ...
   Call einzEintr(Pat_ID, "Sonogramm", "", "'sono','sd'", VorDat, True, "Abdomen|Schilddrüse|Weichteile")
...
'  Epi = Epi + myEFrag(sql1).Fields(0)
  ...
  sql1 = "SELECT IF(COUNT(0)=0,'',CONCAT(CHR(13),'Weitere Termine haben wir noch vereinbart, Ihr freundliches Einverständnis voraussetzend: '," & vbCrLf & _
         "GROUP_CONCAT(CONCAT(DATE_FORMAT(zp,'%d.%m.%y %H:%i'),IF(raum RLIKE 'abor|^Seminar',CONCAT(' (',raum,')'),'')) SEPARATOR ', '),'.',CHR(13)))" & vbCrLf & _
...
' dc.Range(Tabl.Range.End, Tabl.Range.End) = "Vor dem " & Matr2(0, 6, 0) & " wurden noch bestimmt: " & ZusText
 If LenB(ZusText) > 2 Then
  ...
  End If ' briefNeu
 Else
   ag.Append "<w:p><w:pPr><w:pStyle w:val=""Normal""/><w:ind w:hanging=""0"" w:left=""-1077"" w:right=""0""/></w:pPr><w:r><w:rPr><w:sz w:val=""24""/></w:rPr><w:br/></w:r></w:p>"
 End If
