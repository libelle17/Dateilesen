...
Public Sub doplink()
  ...
  With ShExInfo
    ...
    .lpParameters = "-pw " & pwd & " root@" & LiName & " mariadb-dump --defaults-extra-file=~/.modbpwd medoff epraxis |mysql --defaults-extra-file=~/.mariadbpwd quelle"
...
' bis hier 0,4s
 ...
 Dim i&, Gewicht!, Wert$, WertNum!, j&, k&, ianf&, iend&, FlbZ&, aktlwx&  ', lSQL$ ' bmi!, bmiS$,
 Dim angefx&, gefunden&
...
' 9.7.22: da das offenbar statt für Nachschulung für erneute Schulung verwendet wird, Erhöhung von 1 auf > 4 Jahre
      ...
      Case Tailli
       Taille = MachNumerisch(rs(Tailli).Fields(1)): obTaille = True
     End Select
     Set rszahl = Nothing
...
' Parameter für Fettleber-Index, FIB-4-Index und NFS (NAFLD-Fibrose-Score)
  Dim obGGT%, obGPT%, obGOT%, obTHR%, obTG%, obAlb%, GGT#, GPT#, GOT#, Thr#, FLI#, FIB4#, NFS#, TG#, Alb#
  FLI = -1
  FIB4 = -1
  NFS = -1
  If obLabzugew Then
...
'  doMarkierungen CLng(Pat_id), True ' in doPatvonMO verschoben 20.9.25
  Dim notiz$
  notiz = Trim$(REPLACE$(REPLACE$(rnam!notiz, Chr(13), ""), Chr(10), ""))
...
Sub LaborInsPLZ(Pat_id$, SelbstStatus%, raDatBOF%, ByRef Matr$(), ByRef MForm%(), ByRef mBreiten$(), Optional ohnebr%) 'Word.Document, Pat_id&)
...
' sql2 = "SELECT lab.* FROM (SELECT @patid:= " & Pat_id & " nix) nul, geslab lab"
 ...
 Set raLw = hollabor(CLng(Pat_id), "", 0, 0, obnachgruppe:=True, Zahl:=lz, Einheit:="", gz:=gz, dzz:=dz, dSL:=dSL, obneu:=True) ' , obUpdate:=True)
...
#Else
 myFrag raLw, "CALL geslabdp" & IIf(ohnebr, "ohnebr", "") & "(" & CStr(Pat_id) & ",'')", adOpenStatic
' myFrag raLw, "CALL geslabdp(" & CStr(Pat_ID) & ",'WHERE (reihe <> 999 OR ISNULL(reihe)) GROUP BY zeitpunkt DESC,abkü,einheit')", adOpenStatic
#End If
 'm = 6: Tj(m) = Timer: For p = 0 To m - 1: Tj(m) = Tj(m) - Tj(p): Next p
' raLw.Open sql2, DBCn, adOpenDynamic, adLockReadOnly
 Do While Not raLw.EOF
'  If raLw!Wert = "6.9" Then Stop
  If Not IsNull(raLw!Abkü) Then
