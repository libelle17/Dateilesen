...
AusS.Append "  var f=document.getElementById('emailNeuFeld'); if (f) f.focus();" & vbCrLf
...
AusS.Append "   $emrolletxt = ($emrow['rolle']=='h') ? 'Haupt' : (($emrow['rolle']=='n') ? 'weitere' : 'Neben');" & vbCrLf
AusS.Append "   if ($emrow['rolle']=='h') {" & vbCrLf
AusS.Append "    echo '<span style=""display:inline-block;width:1rem;"">&nbsp;</span>';" & vbCrLf
AusS.Append "   } else {" & vbCrLf
AusS.Append "    echo '<form method=""post"" action=""../php/emailspei.php"" style=""display:inline"">';" & vbCrLf
AusS.Append "    echo '<input type=""hidden"" name=""aktion"" value=""hauptsetzen"">';" & vbCrLf
AusS.Append "    echo '<input type=""hidden"" name=""email"" value=""'.htmlspecialchars($emrow['email']).'"">';" & vbCrLf
AusS.Append "    echo '<button type=""submit"" style=""width:1rem;font-size:0.7rem;height:1.3rem"" title=""als Hauptadresse (patstamm.FEmail) waehlen"">&uarr;</button>';" & vbCrLf
...
AusS.Append "  echo '<input type=""email"" name=""email"" id=""emailNeuFeld"" size=25 placeholder=""neue Adresse"" style=""color:blue"">';" & vbCrLf
...
AusS.Append "  echo '<script>if(location.hash==""#emailoffen""){document.getElementById(""emailAdrBox"").style.display=""block"";document.getElementById(""emailAdrBtn"").style.color=""crimson"";document.getElementById(""emailAdrBtn"").style.backgroundColor=""cornsilk"";var f=document.getElementById(""emailNeuFeld"");if(f)f.focus();}</script>';" & vbCrLf
...
Function dodoplz(Pat_id$, plzVerz$, Optional Datum As Date, Optional Uhrzeit$, Optional mitanzeig%, Optional Arzt$, Optional zmaxges% = 8, Optional obphp% = 0) ' Patientenlaufzettel
 ...
 Dim haAnam$ ' frei eingegebener Hausarzt aus anamnesebogen, fuer Kopfzeile
...
'  MsgBox ("Keinen Patienten zu " & Pat_id & " gefunden!")
 ...
 myFrag rAn, "SELECT COALESCE(`diabetes seit`,'') dmseit,COALESCE(vorgestellt,(SELECT MIN(bhfb) FROM faelle WHERE pat_id=" & Pat_id & ")) vorg,Diabetestyp,COALESCE(Hausarzt,'') Hausarzt FROM `anamnesebogen` WHERE pat_id = " & Pat_id
 If rAn.State <> 0 Then
  If Not rAn.BOF Then
   dmseit = rAn!dmseit
'   Vorgestellt = rAn!vorg
   If Not IsNull(rAn!vorg) Then Vorgestellt = rAn!vorg
   haAnam = Trim$(rAn!Hausarzt)
  End If
 End If
...
"ORDER BY bhfb DESC LIMIT 1;"
 ...
 If rFl.State <> 0 Then
  If Not rFl.BOF Then
   ...
   rFlKateg = rFl!kateg
   On Error GoTo fehler
   If rFlSchGr = 90 Or rFlSchGr = 89 Then obLeist = False
   If rFlSchGr = 90 Or rFlSchGr = 89 Or rFlKateg = "PBe" Or rFlKateg = "SHV" Then obDMPPrüf = 0 ' rFlKateg = "LKK" Or
...
'  doMarkierungen CLng(Pat_id), True ' in doPatvonMO verschoben 20.9.25
  Dim notiz$
  notiz = rnam!info & IIf(rnam!info = "", "", ", ") & rnam!notiz
 notiz = REPLACE$(REPLACE$(REPLACE$(notiz, vbCrLf, Chr(1)), Chr(13), Chr(1)), Chr(10), Chr(1))
 Do While InStrB(notiz, Chr(1) & Chr(1)) <> 0
  notiz = REPLACE$(notiz, Chr(1) & Chr(1), Chr(1)) ' mehrere aufeinanderfolgende Zeilenumbrueche (Leerzeilen) zu einem Trenner zusammenfassen
 Loop
 notiz = REPLACE$(notiz, Chr(1), ", ")
 Do While Right$(notiz, 1) = "," Or Right$(notiz, 1) = " "
  notiz = left$(notiz, Len(notiz) - 1)
 Loop
 Do While left$(notiz, 1) = "," Or left$(notiz, 1) = " "
  notiz = Mid$(notiz, 2)
 Loop
...
'  IF Not obdmp THEN
  ...
  AusS.AppVar (Array("<div class='unauff'", IIf(obpath(0), " class='cave'", ""), " style=""margin-left:1ch"">", vbCrLf))
  For i = 0 To UBound(üwerg, 2)
  ...
  If MOtot Then
   AusS.AppVar (Array("<div class='cave' style=""margin-left:1ch"">Dieser Laufzettel wurde ohne Echtzeit-Verbindung zur MO-Datenbank erstellt!</div>", vbCrLf))
  Else ' MOtot Then Else
   If obpath(0) Then AusS.AppVar (Array("<div class='cave' style=""margin-left:1ch"">Hausarzt nicht richtig in Medical Office eingetragen</div>", vbCrLf))
  End If ' MOtot Then
  ...
  If obLeist Then
   If pKVNR = KVNr And PAlter > 35 Then
    ...
    End If
    If obGU = True Then
     AusS.AppVar (Array("<div class='cave' style=""margin-left:1ch"">Gesundheitsuntersuchung fällig</div>", vbCrLf))
...
' Barthel-Index
  ...
  If rFlSchGr <> 90 And rFlSchGr <> 89 And (PAlter >= 70 Or Not rsdd.BOF) Then
   ...
   End If
  End If ' rFlSchGr <> 90 And rFlSchGr <> 89 AND (PAlter >= 70 OR Not rsdd.BOF) THEN
  Set rsdd = Nothing
...
' TH:GLP1-Agonist
  ...
  If rFlSchGr <> 90 And rFlSchGr <> 89 Then ' nicht bei Privaten
...
#End If
  ...
  ÜS(i) = "ADL"
  If rFlSchGr <> 90 And rFlSchGr <> 89 And PAlter >= 70 Then
