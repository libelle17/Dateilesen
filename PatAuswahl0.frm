...
'  Case 1: arztbed = "(e.art IN ('gs','doppler','dop','duplex') OR e.inhalt LIKE '%(gs)%')"
' END SELECT
'  DoCmd.Maximize
 Me.Faelle = ""
' Me.Faelle.Clear
...
' Me.FaellepHA.Clear
 Me.Vorbriefe = ""
' Me.Vorbriefe.Clear
' myFrag rs, "SELECT f.pat_id, LEFT(CONCAT(n.nachname,',',n.vorname,IF(n.titel='','',','),n.titel,IF(n.nvorsatz='','',' '),n.nvorsatz),24) AS name, DATE_FORMAT(f.bhfb,'%d.%m.%y') BhFB, IF(e.art IN ('gs','doppler','dop','duplex') OR e.inhalt LIKE '%(gs)%','gs','tk') arzt, f.auftrag, f.verdacht, f.befund FROM faelle f LEFT JOIN namen n ON f.pat_id = n.pat_id LEFT JOIN tmbrie b ON f.pat_id = b.pat_id AND name LIKE '%brief%' AND b.zeitpunkt > f.bhfb LEFT JOIN eintraege e ON e.pat_id = f.pat_id AND " & arztbed & " WHERE (auftrag LIKE '%beten%' OR auftrag LIKE '%bitte%' OR verdacht LIKE '%beten%' OR verdacht LIKE '%bitte%' OR befund LIKE '%beten%' OR befund LIKE '%bitte%') AND ISNULL(b.name) GROUP BY f.pat_id ORDER BY arzt, f.bhfb, f.pat_id, e.zeitpunkt DESC"
' myFrag rs, "SELECT f.pat_id, LEFT(CONCAT(n.nachname,',',n.vorname,IF(n.titel='','',','),n.titel,IF(n.nvorsatz='','',' '),n.nvorsatz),24) AS name, DATE_FORMAT(f.bhfb,'%d.%m.%y') BhFB, IF(e.art IN ('gs','doppler','dop','duplex') OR e.inhalt LIKE '%(gs)%','gs','tk') arzt, f.auftrag, f.verdacht, f.befund FROM faelle f LEFT JOIN namen n ON f.pat_id = n.pat_id LEFT JOIN tmbrie b ON f.pat_id = b.pat_id AND name LIKE '%brief%' AND b.zeitpunkt > f.bhfb LEFT JOIN eintraege e ON e.pat_id = f.pat_id AND " & arztbed & " LEFT JOIN `faxeinp`.`outa` o ON o.pid = f.pat_id AND o.transe > f.bhfb AND o.docname LIKE '%brief%' WHERE (auftrag LIKE '%beten%' OR auftrag LIKE '%bitte%' OR verdacht LIKE '%beten%' OR verdacht LIKE '%bitte%' OR befund LIKE '%beten%' OR befund LIKE '%bitte%') AND ISNULL(b.name) AND ISNULL(o.transe) GROUP BY f.pat_id ORDER BY f.pat_id, e.zeitpunkt DESC"
 myFrag rs, "SELECT * FROM (SELECT f.pat_id, LEFT(CONCAT(n.nachname,',',n.vorname,IF(n.titel='','',','),n.titel,IF(n.nvorsatz='','',' '),n.nvorsatz),24) AS name, DATE_FORMAT(f.bhfb,'%d.%m.%y') BhFB, (SELECT COUNT(0) FROM eintraege e WHERE pat_id = f.pat_id AND (e.art IN ('gs','doppler','dop','duplex','dup') OR (e.art='tb' AND e.ersteller='gs') OR e.inhalt LIKE '%(gs)%')) gsz, (SELECT MAX(zeitpunkt) FROM eintraege e WHERE pat_id = f.pat_id AND (e.art IN ('gs','doppler','dop','duplex','dup') OR (e.art='tb' AND e.ersteller='gs') OR e.inhalt LIKE '%(gs)%')) gsl, (SELECT COUNT(0) FROM eintraege e WHERE pat_id = f.pat_id AND (e.art IN ('tk') OR e.inhalt LIKE '%(tk)%')) tkz, (SELECT MAX(zeitpunkt) FROM eintraege e WHERE pat_id = f.pat_id AND (e.art IN ('tk') OR e.inhalt LIKE '%(tk)%')) tkl," & _
...
Private Sub Verfasser_Click()
 Select Case Verfasser
  Case "Kothny"
   Me.Vorbriefe = "AccessBriefK.dot"
  Case "Hammerschmidt"
   Me.Vorlage = "Accessbriefh.dot"
  Case Else ' "Schade"
   Me.Vorlage = "Accessbriefa.dot"
 End Select
End Sub ' Verfasser_Click

'Private Sub Verfasser_ItemCheck(Item As Integer)
' Select Case Verfasser
'  Case "Kothny"
'   Me.Vorbriefe = "AccessBriefK.dot"
'  Case "Hammerschmidt"
...
'End Sub
'
'Private Sub Verfasser_LostFocus()
' Select Case Verfasser
'  Case "Kothny"
'   Me.Vorbriefe = "AccessBriefK.dot"
'  Case "Hammerschmidt"
...
'End Sub
'
'Private Sub Verfasser_Validate(Cancel As Boolean)
' Select Case Verfasser
'  Case "Kothny"
'   Me.Vorbriefe = "AccessBriefK.dot"
'  Case "Hammerschmidt"
...
' 28.8.25: Notwendigkeit der manuellen Aufrufbarkeit nicht mehr verstanden, deshalb inaktiviert
...
' in OKButton_Click und Abr_Click
Private Sub setzBriefReg()
  Me.Visible = False
  cR.WriteKey Me.Vorlage, "Vorlage", RegPos, HKEY_CURRENT_USER, REG_SZ
  cR.WriteKey Me.Verfasser, "Verfasser", RegPos, HKEY_CURRENT_USER, REG_SZ
  cR.WriteKey Me.Programm, "Programm", RegPos, HKEY_CURRENT_USER, REG_SZ
End Sub ' setzBriefReg()
...
'  Me.ohneÜbertr.Visible = False ' in der Hoffnung, dass man das eigentlich nicht brauche
  DoEvents
...
#Else
  Call AuswPat_id(Me)
#End If
    Me.VorlageLab.Visible = False ' True
    Me.Vorlage.Visible = False ' True
    Me.VerfasserLbl.Visible = True
    Me.Verfasser.AddItem "Hammerschmidt"
    Me.Verfasser.AddItem "Kothny"
    Me.Verfasser.AddItem "Schade"
    Me.Verfasser.Visible = True
    Me.ProgrammLbl.Visible = True
    Me.Programm.AddItem "LibreOffice"
...
'    sql = "SELECT dbnr, bstelle, anrede, """" AS überschrift, haname, ort, kvnu AS kvnr, tel1, tel2, tel3, tel4, fax1, fax1k,fax2,fax2k, fax3,fax3k, email, zulg, arzttyp, gemmit, beme, dmpt2, dmpt1, geschlecht, titel, vorname, nachname, straße, plz FROM `kvaerzte`.`hae` WHERE kvnr = '" & Left$(rKVNr!KVNr, 2) & "/" & Mid$(rKVNr!KVNr, 3, 5) & "' " & _
     ...
     sql = "SELECT * FROM (" & _
          ...
          "UNION SELECT haname, ort, CAST(kvnu AS char) kvnr, tel1, fax1, titel, vorname, nachname, straße, plz FROM `kvaerzte`.`hae` WHERE kvnr = '" & left$(nz(rKVNr!KVNr,"000000"), 2) & "/" & Mid$(nz(rKVNr!KVNr,"000000"), 3, 5) & "' " & _
          "UNION SELECT haname, ort, CAST(kvnu AS char) kvnr, tel1, fax1, titel, vorname, nachname, straße, plz FROM `kvaerzte`.`haealt` WHERE kvnr = '" & left$(nz(rKVNr!KVNr,"000000"), 2) & "/" & Mid$(nz(rKVNr!KVNr,"000000"), 3, 5) & "' " & _
          "UNION SELECT name haname, ort, CAST(kvnr AS char) kvnr, telefon AS tel1, telefax AS fax1, titel, vorname, nachname, straße, plz FROM `hausaerzte` WHERE kvnr = '" & nz(rKVNr!KVNr,"000000") & "' " & _
...
' '      END IF
'      End If
'      Me.Faelle.AddItem (rFaA!BhFB & " - " & rFaA!BhFE1 & " " & rFaA!SchGr & "  " & Left$(nz(rFaA!name,vNS), 20) & " " & nz(rFaA!Kateg,vNS) & "  " & rFaA!ÜWNNr & " " & rFaA!ÜWNaN & " " & rFaA!ÜWVor)
'      rFaA.Move 1
'     Loop
