...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vNS, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in doPatientenlaufzettel/" + AnwPfad)
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vNS, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in getDiast/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' letztGFR = hollwert(PID, "GFR','GFRCYS','GFRK','GFRM','GFRT','GFRW','CREACL','GFC','GFCK1','GFCK2','GFCK3','GFCK4','GFCM1','GFCW1','GFCW2','MDRD", "ml/m")
 ...
 If lc.Zp - LA.Zp > 1 Or (lc.WertSg <> "" And LA.WertSg = "") Then ' wenn letztes Kreatinin mehr als einen Tag jünger als die letzte eGFR ist
  If pid <> altPID And Palter = 0 Then
   altPID = pid
   altPalter = DBCn.Execute("SELECT patalter(" & pid & ")").Fields(0)
   altobweibl = DBCn.Execute("SELECT IF(geschlecht='w',-1,0) FROM namen WHERE pat_id=" & CStr(pid)).Fields(0)
...
'   hollwert = DBCn.Execute("call ltWert(" & Pat_id & ",""" & Abk & """,""" & Einh & """)")!ltWert
'  END Function
...
' bei Auswahl aller Medikamentenpläne (Pat_id 1564: 4165 Einträge) Dauer: 0,64s
 ...
 If pid <> altPID Then
  obHCT = 0
  hctMed = ""
'  mpz = DBCn.Execute("SELECT COUNT(0) Zahl FROM medplan mp WHERE pat_id = " & PID & "")!Zahl
  mpz = DBCn.Execute("SELECT COUNT(0) Zahl FROM wmedplan mp WHERE pat_id = " & pid & " AND zeitpunkt=(SELECT MAX(zeitpunkt) FROM wmedplan WHERE pat_id=" & pid & ")")!Zahl
' mpz = DBCn.Execute("SELECT COUNT(0) Zahl FROM medplan mp WHERE pat_id = " & Pid & " AND mpnr=(SELECT MAX(mpnr) FROM medplan mpi WHERE pat_id=mp.pat_id AND zeitpunkt=(SELECT MAX(zeitpunkt) FROM medplan WHERE pat_id=mp.pat_id))")!Zahl
  Erase mdpl ' 30.5.20
  If mpz > 0 Then
   ru = 0
   ReDim mdpl(mpz - 1)
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vNS, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in mplan/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vNS, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in UKPDS/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' END Enum
 ...
 Dim DateiNameRoh$, DateiName$, DateiNameAktRoh$, DateiNameAkt$
...
' Dim wasAvan$ ' was genau verordnet wurde
 ...
 Dim zmax%(pzl), rszmax& ' maximal dargestellte Zahl an Ausprägungen jedes Parameters, Maximum davon je Großzeile
 ...
 Dim obart%, obherzi%, obneph%, icdh$ ' Arteriosklerose, Herzinsuffizienz, Nephropathie, ICD-Hauptteil
 Dim ru% ' Runde
 Dim dmseit$
...
' dmtyp = DBCn.Execute("SELECT dmtyp(" & Pat_ID & ")").Fields(0)
...
' myfrag rFl, "SELECT k.name KrKasse, k.*, f.* FROM (SELECT f.* FROM faelle f WHERE f.pat_id = " & Pat_id & " AND qanf = (SELECT MAX(qanf) FROM faelle WHERE pat_id = " & Pat_id & ")) f LEFT JOIN `kassenliste` k ON f.VKNr=k.vknr AND f.IK=k.IK ORDER BY MID(qanf,2) DESC, qanf DESC" ' schgr 21.6.16
 myFrag rFl, "SELECT schgr,kateg,k.name krkasse" & vbCrLf & _
          ...
          "ORDER BY MID(qanf,2) DESC, qanf DESC"  ' 11.4.22
 m = 5: TI(m) = Timer: For p = 0 To m - 1: TI(m) = TI(m) - TI(p): Next p
  ...
  If obphp Then
   DateiNameRoh = rnam!Nachname & "_" & rnam!Vorname & ",Pid_" & rnam!Pat_id & IIf(LenB(Arzt) <> 0, "," & Arzt, vNS) ' replace$(CDate(Uhrzeit), ":", ".") & "_" & Format(Datum, "dd.mm.yyyy") &
  ...
  Else
   DateiNameRoh = rnam!Nachname & " " & rnam!Vorname & ",   Pid " & rnam!Pat_id & IIf(LenB(Arzt) <> 0, ", " & Arzt, vNS) & ", Patientenlaufzettel" ' replace$(CDate(Uhrzeit), ":", ".") & " " & Format(Datum, "dd.mm.yyyy") &
  End If
...
'  PAlter = (NOW() - rnam!GebDat) * 2.73792574745373E-03 ' 1/365,24
  ...
  If obphp Then
   ...
   AusS.AppVar (Array("    <TITLE>" & rnam!Nachname & " " & rnam!Vorname & " (Pat_id:" & rnam!Pat_id & "): Patientenlaufzettel vom " & Now & "</TITLE>", vbCrLf))
...
'  sql(i) = "SELECT DATE(zeitpunkt) zp, wert FROM `laborneu` ln WHERE abkü RLIKE 'hba[c1]' AND einheit = '%' AND pat_id = " & Pat_ID & " UNION SELECT DATE(u.eingang) zp, w.wert FROM `laborxus` u LEFT JOIN laborxwert w ON u.refnr = w.refnr WHERE abkü RLIKE 'hba[c1]' AND einheit = '%' AND pat_id = " & Pat_ID & " GROUP BY zp ORDER BY zp DESC"
  Hba1ci = i
  kritnr(i) = LA_HbA1c
  sql(i) = vNS
...
'  sql(i) = "SELECT * FROM (SELECT zeitpunkt zp, CAST(inhalt AS char(100)) wert FROM `eintraege` WHERE pat_id = " & Pat_ID & " AND art LIKE 'urin%' UNION SELECT eingang zp, CAST(keimzahl AS char(16)) wert FROM `laboryus` us LEFT JOIN `laborybakt` b ON us.id=b.usid WHERE pat_id = " & Pat_ID & " AND keimzahl<> '')  u2 ORDER BY zp DESC"
  ...
  Üs(i) = "Alb/U"
  Titel(i) = "Albuminurie [mg/g Kreatinin], NB < 30 mg/gCrea, bei Nichterrechnen Albumin [mg/l]"
  Albi = i
  kritnr(i) = LA_AlbCre
...
'  sql(i) = "SELECT zeitpunkt zp, IF(ISNULL(wert),IF(ISNULL(kommentar),'',kommentar),wert) wert FROM (" & lsql & ") AS innen WHERE abkü LIKE 'FT3%' ORDER BY zp DESC"
  ...
  If obdm Then
   ...
   sql(i) = "SELECT zeitpunkt zp, concat (art, ' <- ', inhalt) wert FROM `eintraege` WHERE pat_id = 1789 AND inhalt LIKE '%" & UmwfSQL(rnam!Nachname) & "%' ORDER BY zp DESC"
   nurpath(i) = 3
   i = i + 1
  End If ' obdm Then
...
' Parameter für Fettleber-Index, FIB-4-Index und NFS (NAFLD-Fibrose-Score)
  Dim obGGT%, obGPT%, obGOT%, obTHR%, obTG%, obTaille%, obAlb%, GGT#, GPT#, GOT#, Thr#, FLI#, FIB4#, NFS#, TG#, Taille#, Alb#
  FLI = -1
  FIB4 = -1
  NFS = -1
  If obLabzugew Then
...
'Beginn mit Schreiben der Daten
  ...
  If Not rFl.EOF Then
   If rFl!SchGr = 90 Or rFl!Kateg = "LKK" Or rFl!Kateg = "PBe" Or rFl!Kateg = "SHV" Then obDMPPrüf = 0
  End If
  If obDMPPrüf Then
   DmPStr = ",  DMP-Diab: "
...
' 24.4.20 auch noch vorgezogen
   ...
   Call UKPDS(aRisk, Pat_id, rnam!GebDat, dmseit, falDiabDau, obweibl)
...
#End If
  Dim vorET As Date
  If Not rFl.BOF Then If Not IsNull(rFl!vorET) Then vorET = rFl!vorET
  ' 255,204,229 = #ffcce5 rötlich; 229,204,255 = #E5CCFF bläulich; #ffffcc gelblich
  AusS.AppVar (Array(" ", IIf(dmtyp = "1" Or dmtyp = "2" Or dmtyp = "g", "<span style='background-color:" & IIf(dmtyp = "1", "#ffd9e8", IIf(dmtyp = "g", "#ffffde", "#efe0ff")) & "'", ""), "<B><span title='", rnam!Vorname, " ", rnam!Nachname, ", ", rnam!strasse, ", ", rnam!plz, " ", rnam!ort, ", Tel1: ", rnam!PrivatTel, ", Tel2: ", rnam!PrivatTel_2, ", Mobil:", rnam!PrivatMobil, ", Fax: ", rnam!PrivatFax, ", Diensttel: ", rnam!DienstTel & ", Email: ", rnam!email, "'>", IIf(rFl!vorET > Now(), "<span class='schwanger'>", ""), _
  GesNamFn(rnam), "</span></B>, *", Format(rnam!GebDat, "d.m.yy"), " (", Palter, "a), <span style='color:blue'><span class='unauff'>&nbsp;&nbsp;Pat_id: </span>", Pat_id, "</span><span id = 'unauff'>,", IIf(obdm, "&nbsp;&nbsp;D.m.seit: ", ""), IIf(obdm, dmseit, ""), ",&nbsp;&nbsp;vorgestellt: ", Format(rAn!Vorgestellt, "d.m.yy"), ",&nbsp;&nbsp;für: </span>", Format(Datum, "d.m.yy"), " <span style='font-size:smaller'>", Format(Uhrzeit, "hh:mm"), ",</span>&nbsp;&nbsp;&nbsp;<span class='unauff'>", rnam!Notiz, ",&nbsp;&nbsp;&nbsp;", IIf(obdm, "Therapie zuletzt: ", ""), "</span>", IIf(obdm, therart, ""), "<span " & dmpfarbe & ">", DmPStr, "</span></h1>", vbCrLf))
' TherapieArtEinzelnFestlegen(CLng(Pat_ID), rAn) & "</span></h1>" ' rnam!Vorname, " ", rnam!Nachname
...
'  Wend
  ...
  For i = 0 To UBound(üwerg, 2)
   If üwerg(10, i) <> vNS Then
    ...
    AusS.AppVar (Array(üwerg(10, i), ": ", üwerg(1, i), IIf(i < 2, " (T:" & üwerg(13, i) & IIf(üwerg(6, i) = "X", ", DMP 2", "") & IIf(üwerg(7, i) = "X", ", DMP 1", "") & ")", ""), ", ", vbCrLf))
...
'      AusS.AppVar (Array("<div class='unauff'><span id = 'cave'>Nicht im DMP-Diabetes</span>: ", IIF(rnam!dmpklass = 0, "(&Oslash;&nbsp;Eintrag)", "DMP nein"), "</div>", vbCrLf))
'     END IF
'    Case 2
...
#End If
  Set rFlb = Nothing
' rFlb.Open "SELECT `dokumente`.pat_id, `dokumente`.zeitpunkt AS zp, `dokumente`.dokpfad, `dokumente`.dokname, `dokumente`.quelldatum, `dokumente`.dokgroe, kvnr, LEFT(IF(übwvbsnr='',IF(übwvkvnr = '', andüw, übwvkvnr),übwvbsnr),7) übwv, IF(`dokumente abgehakt`.abgehakt,'X',' ') AS abgeh FROM ((`dokumente` LEFT JOIN `dokumente abgehakt` ON `dokumente`.DokPfad = `dokumente abgehakt`.DokPfad) LEFT JOIN `namen` ON `dokumente`.pat_id = `namen`.pat_id) LEFT JOIN `lfaelle` ON `dokumente`.pat_id = `lfaelle`.pat_id WHERE (((`dokumente`.DokName) LIKE '%Fremdlabor%')) AND (ISNULL(abgehakt) OR abgehakt = 0) AND `dokumente`.pat_id = " & Pat_ID & " ORDER BY `dokumente`.pat_id DESC, zp DESC", DBCn, adOpenDynamic, adLockReadOnly
'  rFlb.Open "SELECT `dokumente`.pat_id, `dokumente`.zeitpunkt AS zp, `dokumente`.dokpfad, `dokumente`.dokname, `dokumente`.quelldatum, `dokumente`.dokgroe, kvnr, übwr, IF(`dokumente abgehakt`.abgehakt,'X',' ') AS abgeh FROM ((`dokumente` LEFT JOIN `dokumente abgehakt` ON `dokumente`.DokPfad = `dokumente abgehakt`.DokPfad) LEFT JOIN `namen` ON `dokumente`.pat_id = `namen`.pat_id) LEFT JOIN `lfaelle` ON `dokumente`.pat_id = `lfaelle`.pat_id WHERE (((`dokumente`.DokName) LIKE '%Fremdlabor%')) AND (ISNULL(abgehakt) OR abgehakt = 0) AND `dokumente`.pat_id = " & Pat_id & " ORDER BY `dokumente`.pat_id DESC, zp DESC", DBCn, adOpenDynamic, adLockReadOnly
  myFrag rFlb, "SELECT `dokumente`.pat_id, `dokumente`.zeitpunkt AS zp, `dokumente`.dokpfad, `dokumente`.dokname, `dokumente`.quelldatum, `dokumente`.dokgroe, kvnr, übwr, IF(`dokumente abgehakt`.abgehakt,'X',' ') AS abgeh FROM ((`dokumente` LEFT JOIN `dokumente abgehakt` ON `dokumente`.DokPfad = `dokumente abgehakt`.DokPfad) LEFT JOIN `namen` ON `dokumente`.pat_id = `namen`.pat_id) LEFT JOIN `lfaelle` ON `dokumente`.pat_id = `lfaelle`.pat_id WHERE (((`dokumente`.DokName) LIKE '%Fremdlabor%')) AND (ISNULL(abgehakt) OR abgehakt = 0) AND `dokumente`.pat_id = " & Pat_id & " ORDER BY `dokumente`.pat_id DESC, zp DESC"
...
' Barthel-Index
  ...
  If Not rFl.EOF() Then
   If rFl!SchGr <> 90 And (Palter >= 70 Or Not rsdd.BOF) Then
    ...
    End If
   End If ' rFl!SchGr <> 90 AND (PAlter >= 70 OR Not rsdd.BOF) THEN
  End If ' Not rFl.EOF() THEN (TUG)
  Set rsdd = Nothing
...
'  SET rTh = DBCn.Execute("SELECT (REPLACE(REPLACE(mo,'½',0.5),'1/2',0.5)+REPLACE(REPLACE(mi,'½',0.5),'1/2',0.5)+REPLACE(REPLACE(nm,'½',0.5),'1/2',0.5)+REPLACE(REPLACE(ab,'½',0.5),'1/2',0.5)+REPLACE(REPLACE(zn,'½',0.5),'1/2',0.5))*nr mg,i.* FROM " & _
...
'  obmetf = DBCn.Execute("SELECT COUNT(0) zl FROM medplan mp LEFT JOIN medarten ma ON mp.medanfang = ma.medikament WHERE mp.pat_id=" & Pat_ID & " AND metf")!zl
  ' 0s
  If obdm And dmtyp = "2" And therart <> "Diät" Then
' TH:Metformin
   If lGFR.WertSg > 45 Then
    Dim obmetf%
    ...
    obmetf = DBCn.Execute("SELECT COALESCE(-(SELECT 1 FROM medplan mp LEFT JOIN medarten ma ON mp.medanfang = ma.medikament " & vbCrLf & _
   ...
   If lGFR.WertSg > 59 And (obneph Or obherzi) Then
' TH:SGLT2-Hemmer
    Dim sglt%
    sglt = DBCn.Execute("SELECT COALESCE(" & vbCrLf & _
...
' TH:GLP1-Agonist
    Dim glp%
    glp = DBCn.Execute("SELECT COALESCE(" & vbCrLf & _
     "-(SELECT 1 FROM medplan mp LEFT JOIN medarten ma ON mp.medanfang = ma.medikament " & vbCrLf & _
...
' TH:kürzlich aut-idem-Kreuz
   Set rdpp4 = Nothing
   myFrag rdpp4, "SELECT pat_id, CONCAT('cave: kürzlich aut-idem-Kreuz bei: ',GROUP_CONCAT(LEFT(medikament,instr(medikament,' ')) SEPARATOR ',')) Anw FROM rezepteintraege WHERE auti=0 AND rezklkurz<>'prp' AND zeitpunkt> SUBDATE(NOW(),90) AND pat_id=" & Pat_id
...
' TH:Brilique 60 mg
   ...
   Set rdpp4 = Nothing
  End If ' IF rFl!SchGr <> 90 THEN ' nicht bei Privaten

' TH:Metformin-Dosierung
  If lGFR.WertSg < 30 And metdos > 0 Then
   If SafeArrayGetDim(thh) = 0 Then ReDim thh(0) Else ReDim Preserve thh(UBound(thh) + 1)
   thh(UBound(thh)) = "eGFR " & lGFR.WertSg & " " & lGFR.Einheit & " und Metformindosis " & metdos & " mg/d => evtl. absetzen"
  ElseIf lGFR.WertSg < 45 And metdos > 1000 Then
   If SafeArrayGetDim(thh) = 0 Then ReDim thh(0) Else ReDim Preserve thh(UBound(thh) + 1)
...
'    obKHE = diI("I20 I21 I22 I23 I24 I25 Z95.5", Pat_ID, 0, True)
    ...
    lRRÄnd = rsMB!länd
    RRMedAktZ = DBCn.Execute("SELECT COALESCE(" & vbCrLf & _
...
'         Debug.Print rs(i)!Wert
         ...
         If RowSp(i) <> 0 Then
          Pause(i) = RowSp(i) - 1
         End If
         If sql(i) <> vNS Then If Not rs(i).EOF Then rs(i).MoveNext
        Else
         Pause(i) = Pause(i) - 1
        End If
...
'  Close #336
  ...
  For j = 0 To UBound(Matr, 3)
   ...
   For i = 3 To UBound(Matr, 2)
    ...
    End Select
    If i = 3 Then
     AusS.AppVar Array("  <td title=""", Matr(1, 0, j), """")
    Else
     If LenB(Matr(1, iumw, j)) <> 0 Then ' Kommentar
      AusS.AppVar Array("  <td title=""", Matr(1, iumw, j), """")
     Else
      AusS.Append ("  <td")
     End If
    End If
    AusS.AppVar Array(" class='", Formg, "'>", Matr(0, iumw, j), "</td>", vbCrLf)
   Next i
   AusS.AppVar (Array(" </tr>", vbCrLf))
  Next j
  AusS.AppVar (Array("</Table>", vbCrLf))
...
' Einträge
  ...
  DoEvents
  Lese.Ausgeb "Patientenlaufzettel für " & rnam!Nachname & ", " & rnam!Vorname & " (Pat_id: " & Pat_id & ") erstellt.", True
  syscmd 4, "Fertig mit dem Patientenlaufzettel für: " & rnam!Nachname & ", " & rnam!Vorname & " (Pat_id: " & Pat_id & ")"
...
'   Print #339, Str$(ZZ) + ": '" + Str$(raLw!gruppe) + "'"
'   ZZ = ZZ + 1
'   altgru = raLw!gruppe
'  END IF
'  Print #339, Str$(ZZ) + ": Abkü: '" + raLw!abkü + "', Einheit: '" + raLw!Einheit + "'"
'  ZZ = ZZ + 1
'  raLw.MoveNext
' Loop
' Close #339
...
#If problematisch Then
 ...
 If dz = 0 Then dz = DBCn.Execute("SELECT COUNT(0) ct FROM (" & sqldat & ") innen")!ct
#Else
 If dz = 0 Then dz = DBCn.Execute("call geslabkatz(" & CStr(Pat_id) & ",'zeitpunkt')")!Zahl
#End If
...
' GoTo nochmal1
 If dz = 0 Then dz = 1
 ReDim Matr(1, dz + 5, ZZ + 2) ' Spaltenzahl, Zeilenzahl, matr(x,y)= Wert ' (1, = für Tooltip, 27.3.10, Umkehrung 6.4.11
 ReDim MForm(dz + 5, ZZ + 2) ' Formatierung (1 = schräg, 2 = fett)
...
#End If
 ...
 raLw.MoveFirst
 Do While Not raLw.EOF
  If Not IsNull(raLw!Abkü) Then
  If raLw!gruppe <> altgru Then
   Matr(0, 0, j) = "Gruppe: " & raLw!gruppe & "|Reihe: " & raLw!reihe & "|Abkü: " & raLw!Abkü & "|NB: " & raLw!uNG & "-" & raLw!oNG
   altgru = raLw!gruppe
   j = j + 1
  End If
  Matr(0, 0, j) = raLw!Abkü
  Matr(1, 0, j) = "Gruppe: " & raLw!gruppe & "|Reihe: " & raLw!reihe & "|Abkü: " & raLw!Abkü & "|NB: " & raLw!uNG & "-" & raLw!oNG
...
'    IF NOT ISNULL(raLw!Nb) THEN Matr(0,5, j) = raLw!Nb
  ...
  End Select
  j = j + 1
  End If 'NOT ISNULL(raLw!Abkü) THEN
  raLw.Move 1
 Loop
 End If
...
' raLw.Open sql2, DBCn, adOpenDynamic, adLockReadOnly
 Dim SD As New SortierDatum
 Do While Not raLw.EOF
  If Not IsNull(raLw!Abkü) Then
   SD.Datum = Int(raLw!Zeitpunkt)
   dSL.scinit
   dSL.Eingrenz SD
   i = dSL.sCa + 5
#If bewaehrt Then
   i = 6
   Do While CDate(IIf(LenB(Matr(0, i, 0)) = 0, 0, Matr(0, i, 0))) < DateValue(raLw!Zeitpunkt)
    i = i + 1
...
#End If
  j = 0
'  IF Left$(raLw!Abkü, 2) = "HB" AND raLw!Einheit = "%" THEN Stop
...
'   myfrag rlpar, "SELECT gruppe, reihe, langtext FROM `laborparameter` lp WHERE abkü = '" & raLw!abkü & "' AND einheit = '" & IIf(ISNULL(raLw!Einheit) OR raLw!Einheit = "", "kA", raLw!Einheit) & "'"
...
'   IF Not rlpar.EOF THEN
    Matr(1, 0, j) = "Gruppe: " & raLw!gruppe & "|Reihe: " & raLw!reihe & "|Abkü: " & raLw!Abkü & "|NB: " & raLw!uNG & "-" & raLw!oNG
    Matr(0, 3, j) = raLw!Langtext ' rlpar!Langtext
'   Else
'    Matr(1, 0, j) = raLw!abkü
...
'   IF o <> -99 AND rlwd > o THEN pKz = "p"
  End If ' IsNumeric(raLw!Wert) OR raLw!Wert LIKE "1:*" OR raLw!Wert LIKE "<*" OR raLw!Wert LIKE ">*" THEN
  On Error Resume Next
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vNS, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LaborInsPLZ/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
