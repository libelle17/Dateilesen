...
'" LEFT JOIN faelle et ON f.pat_id=et.pat_id AND et.voret BETWEEN " & KhtsflZuk() & " " & vbCrLf & _
 ...
 sql(AWlf) = "SELECT d.pat_id,GesNameg(d.pat_id) PName,wer,d.diagdatum Datum,d.diagsicherheit Si, d.DiagText,d.ICD " & vbCrLf & _
             ...
             "WHERE d.gzICDok RLIKE '^E1[234]' AND ISNULL(r.gicd) AND schgr<> 90" ' AND COALESCE(d.Dggel,0)=0
 mins(AWlf) = 5
 maxs(AWlf) = 18
 AWlf = AWlf + 1
...
sql(AWlf) = "SELECT pid, gesnameg(pid) PName, wer, zahl, icd, e.Art " & vbCrLf & _
            ...
            "FROM `aktfvs` f " & vbCrLf & _
            "LEFT JOIN anaktk az ON az.pid=f.pat_id" & vbCrLf & _
            "LEFT JOIN `diageview` d ON d.pat_id = f.pat_id AND ((d.gicd REGEXP '^E1[0-4]\.|^R73' AND d.obdauer<>0 AND d.diagsicherheit='G') OR (d.icd='O24.4' AND d.Dggel=0 AND d.diagsicherheit IN ('G',' ') AND d.diagdatum BETWEEN qbegs(f.quartal) AND qends(f.quartal)))" & vbCrLf & _
 ...
 " LEFT JOIN briefe b ON f.pat_id = d.pat_id AND d.quelldatum BETWEEN fa.qanf AND fa.qend AND d.name RLIKE 'WA [0-5]'" & vbCrLf & _
 ...
 " GROUP BY f.pat_id,e.art, e.inhalt, d.name" & vbCrLf & _
...
"SELECT innen.*, ezp, LEFT(art,7) AS art, inhalt from" & vbCrLf & _
 "(SELECT f.`pid` AS Pat_ID, name, " & vbCrLf & _
     ...
     "(SELECT pat_id AS pid, nachname, vorname, fid,schgr,ik,vknr FROM `faelle` WHERE schgr <> '90' AND NOT goäkatnr IN ('40','41') AND quartal = '" & AktQ & "') AS f " & vbCrLf & _
       "LEFT JOIN briefe b ON pid = b.pat_id AND name LIKE '%sonobild %' " & vbCrLf & _
     ") AS innen " & vbCrLf & _
    "LEFT JOIN " & vbCrLf & _
...
' 72
 ...
 " ,(SELECT COALESCE(MAX(MID(name,INSTR(name,'WA ')+3,2)),'-') FROM briefe WHERE pat_id=f.pat_id AND name RLIKE 'WA [0-5]') WFc" & vbCrLf & _
 " ,(SELECT COALESCE(MAX(MID(name,INSTR(name,'WA ')+3,2)),'-') FROM briefe WHERE pat_id=f.pat_id AND name RLIKE 'WA [0-5]' AND zeitpunkt BETWEEN qanf() AND qend()) WFa" & vbCrLf & _
...
' 73
 AwN(AWlf) = "Fehlende 02311 bzw. 02300 für Fußsyndrom nach Einträgen (vorher 27)"
 sql(AWlf) = _
...
' 74
 ...
 "LEFT JOIN (SELECT * FROM (SELECT MAX(pat_id) AS PID, MAX(MID(name,INSTR(name,""WA ""),5)) AS fotoStad, Quelldatum Fototag FROM `briefe` GROUP BY pat_id, Fototag) AS innen WHERE " & vbCrLf & _
...
"WHERE NOT ISNULL(leistung) AND LfBegr<>'50gr. OGTT beim Gyn erfolgt' AND LfBegr<>'50 g oGTT bei Frauenarzt bereits erfolgt'"
 mins(AWlf) = 10
 maxs(AWlf) = 80
 AWlf = AWlf + 1
...
' "AND NOT notiz LIKE '%DMP%nein%'"
 ...
 "       i.Inhalt , i.name, i.DkZp, i.FID, geschlecht, quartal,kateg " & vbCrLf & _
 "    FROM ( " & vbCrLf & _
 ...
 "     e.inhalt, dk.name name, dk.qdm DkZp, f.fid, f.quartal,f.kateg " & vbCrLf & _
 " FROM aktfv f " & vbCrLf & _
...
#If alt Then
 ...
 "LEFT JOIN (SELECT fid,leistung FROM `leistungen` WHERE leistung IN ('97314','97324')) AS l ON f.fid = l.fid " & vbCrLf & _
 "LEFT JOIN (SELECT * FROM (SELECT MAX(pat_id) AS PID, MAX(MID(name,INSTR(name,""WA ""),5)) fotoStad, Quelldatum zeitpunkt FROM briefe GROUP BY pat_id, zeitpunkt) innen WHERE " & vbCrLf & _
 "fotoStad = 'WA 0D' OR (fotostad LIKE 'WA %' AND NOT fotostad IN ('WA 1A','WA 2A') AND NOT fotostad LIKE 'WA 0%' AND NOT fotostad LIKE 'WA -%' ) " & vbCrLf & _
 ") AS dok ON f.pat_id = dok.pid " & vbCrLf & _
 "AND zeitpunkt BETWEEN " & lQAnfuEnd(FristS) & _
