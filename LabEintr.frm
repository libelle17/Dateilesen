...
Private DatPfad$
'Const AbgehSQL$ = "SELECT tmbrie.Pat_ID, tmbrie.Zeitpunkt, tmbrie.pfad, tmbrie.Name, DATE(tmbrie.DokAenD) AS Quelldt, tmbrie.DokGroe, CONCAT(hkv.nachname, ' ', hkv.ort) AS kv, CONCAT(hüw.nachname,' ',hüw.ort) üb, DATE(bhfb) AS Fallbg, IF(abgehakt <> 0,'X',' ') AS abgeh FROM ((`tmbrie` LEFT JOIN `br_abgehakt` ON `tmbrie`.pfad = `br_abgehakt`.pfad) LEFT JOIN `namen` ON `tmbrie`.pat_id = `namen`.pat_id) LEFT JOIN `lfaelle` ON `tmbrie`.pat_id = `lfaelle`.pat_id LEFT JOIN `hausaerzte` hkv ON `namen`.kvnr = hkv.kvnr LEFT JOIN `hausaerzte` hüw ON übwv = hüw.kvnr WHERE (((`tmbrie`.Name) LIKE '%Fremdlabor%')) AND (ISNULL(abgehakt) OR abgehakt = 0) GROUP BY pfad ORDER BY pat_id DESC, zeitpunkt DESC, übwv DESC"
'Const AbgehSQL$ = "SELECT `tmbrie`.Pat_ID, `tmbrie`.Zeitpunkt, `tmbrie`.pfad, `tmbrie`.Name, DATE(`tmbrie`.DokAenD) AS Quelldt, `tmbrie`.DokGroe, kvnr, übwv, DATE(bhfb) AS Fallbg, IF(abgehakt <> 0,'X',' ') AS abgeh FROM ((`tmbrie` LEFT JOIN `br_abgehakt` ON `tmbrie`.pfad = `br_abgehakt`.pfad) LEFT JOIN `namen` ON `tmbrie`.pat_id = `namen`.pat_id) LEFT JOIN `lfaelle` ON `tmbrie`.pat_id = `lfaelle`.pat_id WHERE (((`tmbrie`.Name) LIKE '%Fremdlabor%')) AND (ISNULL(abgehakt) OR abgehakt = 0) GROUP BY pfad ORDER BY pat_id DESC, zeitpunkt DESC, übwv DESC"
Const AbgehSQL$ = "SELECT d.Pat_ID, Zeitpunkt, d.pfad, Name, intaccdatemy(Quelldatum) Quelldt, DokGroe, kvnr, übwr, intaccdatemy(bhfb) Fallbg, IF(abgehakt <> 0,'X',' ') abgeh, ROW_NUMBER() OVER(ORDER BY intaccdatemy(Quelldatum) DESC, Name) Rang FROM ((`tmbrie` d LEFT JOIN `br_abgehakt` da ON d.pfad = da.dokpfad) LEFT JOIN `namen` n ON d.pat_id = n.pat_id) LEFT JOIN `lfaelle` f ON d.pat_id = f.pat_id WHERE ((Name LIKE '%Fremdlabor%')) AND ((ISNULL(abgehakt) OR abgehakt = 0) OR ungueltig =1 OR Quelldatum < '19000000') GROUP BY d.pfad ORDER BY d.pat_id DESC, zeitpunkt DESC, übwr DESC"
...
' Call hlese.ConstrFestleg(0, hlese)
 ...
 Resume
End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in abhaken/" + AnwPfad)
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in inTurbomedAnzeigen/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in weiter/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in zurück/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LadDaten/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Private Sub FlexAusricht()
 On Error GoTo fehler
    With MSHFlexGrid1
        ...
        .FillStyle = 1 ' flexFillRepeat, in  C:\Windows\SysWow64\MSHFLXGD.oca
        ...
        .FillStyle = 0 ' flexFillSingle, in  C:\Windows\SysWow64\MSHFLXGD.oca
        .Redraw = True
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in FlexAusricht/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in IrfanString/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Irfan_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Photoimpact_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
