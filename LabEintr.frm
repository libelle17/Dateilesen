...
Const AbgehSQL$ = "SELECT d.Pat_ID, Zeitpunkt, d.pfad, Name, intaccdatemy(Quelldatum) Quelldt, DokGroe, kvnr, übwr, intaccdatemy(bhfb) Fallbg, IF(abgehakt <> 0,'X',' ') abgeh, row_number() OVER(ORDER BY intaccdatemy(Quelldatum) DESC, Name) Rang FROM ((`briefe` d LEFT JOIN `dokumente abgehakt` da ON d.pfad = da.dokpfad) LEFT JOIN `namen` n ON d.pat_id = n.pat_id) LEFT JOIN `lfaelle` f ON d.pat_id = f.pat_id WHERE ((Name LIKE '%Fremdlabor%')) AND ((ISNULL(abgehakt) OR abgehakt = 0) OR ungueltig =1 OR Quelldatum < '19000000') GROUP BY d.pfad ORDER BY d.pat_id DESC, zeitpunkt DESC, übwr DESC"
