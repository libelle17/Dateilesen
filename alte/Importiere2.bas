...
'           syscmd acSysCmdSetStatus, "Zeile " & i & " / " & ind - 1
          Call dolies(frm, cKenn(i), ZSU1(doUmwfSQL(cInha(i), lies.obMySQL, False)), obSchluss, i, obmitFormularen) ' false = 13.8.15, um bei zusammenhängenden Zeilen das Leerzeichen zu erhalten
         Next i
#If thaalt Then
...
'       rDo(UBound(rDo)).DokPfad = Datei 'stehen
   ...
   Case 6330 ' eintraege: Art
    ...
    If rInhalt = "DiagTxt" Then
     obEinweisung = True
    ElseIf rInhalt = "lar" Then
     If SafeArrayGetDim(RezZeile) = 0 Then
      Debug.Print "RezInhalt nicht initialisiert in Zeile " & znr & " bei Pat. " & rNa(0).Pat_id
...
Else
 ...
 Select Case q
  ...
  Case 4: DatStr = "1.10."
 End Select
 QAnf = CDate(DatStr & Right$(Quartal, IIf(Len(Quartal) > 4, 4, 2)))
End If
...
Else
 ...
 Select Case q
  ...
  Case 4: DatStr = "31.12."
 End Select
 QEnd = CDate(DatStr & Right$(Quartal, IIf(Len(Quartal) > 4, 4, 2)))
End If
