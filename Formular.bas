...
Public Type InfoTyp
 anrede As String '  0: Frau/Herrn
 gesName As String '  1: Titel+Vorn+Nachn,
 Straße As String '  2: Straße,
 plz As String '  3: PLZ+Ort, s. 16
 Faxnr As String '  4: Faxnr,
 Überschr As String '  5: S.g./Liebe,
 DmpTyp2 As String '  6: DMPTyp2,
 DmpTyp1 As String '  7: DMPTyp1,
 NiederlGb As String '  8: Niederlassungsgebiet 3. Feld für einmal austauschen,
 Vorname As String '  9: Vorname,
 Funktion As String  ' 10: Funktion ("Üw 207, HA"),
 Fachrtg As String  ' 11: Fachrichtung
 KVNr As String  ' 12: KV-Nummer
 TelNr As String  ' 13: Tel'nr.
...
Public fuellThaS$ ' wird im Modul fuellThaS.bas definiert
Dim psql$(10)

' gemeinsame Algorithmusdefinition zum Therapieartenermitteln, in doViewserstellen und TheraErmitt
' 27.12.25: Änderung muss aber offenbar direkt in die Funktion fuellTha eingetragen werden
Function therinit()
...
' psql(1) = "IF inpids IN('','0') THEN DELETE FROM therarten; ELSE DELETE FROM therarten WHERE pat_id IN (inpid); END IF;"
 psql(1) = "DELETE FROM therarten WHERE pat_id=inpid;"
...
" SELECT pat_id pid,-3 MPNr,qdm zp,qdm bis,'ICT' Thart, MID(NAME,p) Gru,-2 ia,x.absPos,x.StByte,0 FROM (SELECT IF(p1>p2,p1,p2) p, b.* FROM (SELECT INSTR(b.name,'insulin') p1, INSTR(b.name,'spritz') p2, b.* FROM tmbrie b) b) x WHERE name RLIKE '(insulin|spritz).*(plan|schema|tabelle)' " & vbCrLf & _
"  AND (inpids IN('','0') OR x.pat_id IN (inpid)) " & vbCrLf & _
...
"   SELECT Pid,MPNr,Zp,Zp bis " & vbCrLf
 ...
 Dim zeile$
 psql(3) = myEFrag("SHOW CREATE FUNCTION fuellTha", rAf, DBCn).Fields(2)
 If Len(psql(3)) > 4000 Then ' 27.12.25: 8035
   Open QuellVerz & fTDatei For Output As #228
'    Print #228, "fuellThaS=""" & REPLACE$(REPLACE$(sql, """", """"""), vbCrLf, """ & vbcrlf" & vbCrLf & "fuellThaS=fuellThaS & """) & """"
   Print #228, "#if vomprogrammeeingelesen then" & vbCrLf & psql(3) & vbCrLf & "#endif ' vomprogrammeeingelesen then"
   Close #228
 Else ' Len(psql(3)) > 4000 Then ' 27.12.25: 8035
   psql(3) = ""
   Open QuellVerz & fTDatei For Input As #228
   Do While Not EOF(228)
    Line Input #228, zeile
    If Not zeile Like "[#]*" Then psql(3) = psql(3) & zeile & vbCrLf
   Loop
   Close #228
...
' psql(5) = "UPDATE anamnesebogen x SET ther1=(SELECT therart FROM therarten WHERE pat_id=x.pat_id ORDER BY zp, mpnr LIMIT 1), therakt =(SELECT therart FROM therarten WHERE pat_id=x.pat_id ORDER BY zp DESC, mpnr DESC LIMIT 1) WHERE inpids IN('','0') OR x.pat_id IN (inpid);"
 psql(5) = "UPDATE anamnesebogen x SET ther1=(SELECT therart FROM therarten WHERE pat_id=x.pat_id ORDER BY zp, mpnr LIMIT 1), therakt =(SELECT therart FROM therarten WHERE pat_id=x.pat_id ORDER BY zp DESC, mpnr DESC LIMIT 1) WHERE (x.pat_id=inpid);"
 Exit Function
fehler:
 Dim AnwPfad$
#If VBA6 Then
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in TheraErmitt/" + AnwPfad)
...
' 29.1.26: noch in testthap,Lese->Therapieartenfürallefestlegeneinernachdemanderen_Click,rufThFestleg
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ICDZeit/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function DtbCreateQueryDef$(QName$, sql$, Optional Conx As ADODB.Connection, Optional obmerge%)
 Dim csql As CString
 syscmd 4, "Erstelle View: " & QName
...
'#END IF
 Else
  ...
  Dim ErrNr&, ErrDes$
  Call myEFrag("CREATE OR REPLACE ALGORITHM=" & IIf(obmerge, "MERGE", "UNDEFINED") & " DEFINER=`" & benu & "`@`%` SQL SECURITY DEFINER VIEW `" & QName & "` AS " & csql, , Conx, , ErrNr, ErrDes)
...
If InStrB(Err.Description, "gone away") <> 0 Then
 ...
 Resume
End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in DtbCreateQueryDef/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function do_Labor_Click(frm As Form)
 ...
 "FROM (tmbrie dl LEFT JOIN `br_abgehakt` da ON dl.Pfad=da.DokPfad) LEFT JOIN `namen` na ON na.Pat_ID=dl.Pat_ID " & _
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Labor_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' Call dtbInit
 ...
 sql = "SELECT """ + frm.Nachname + """ Nachname,""" + frm.Vorname + """ Vorname, b.* FROM tmbrie b WHERE name NOT LIKE ""%labor%"" AND name NOT LIKE ""%heckliste%"" AND name NOT LIKE ""%namnese%"" AND pat_id = " + CStr(frm!Pat_ID) + " ORDER BY zeitpunkt DESC"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_DokDown/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function do_Dokumente_Click(frm As Form)
 On Error GoTo fehler
 Call do_Click_Vorbereit
 sql = "SELECT """ + frm.Nachname + """ nachname,""" + frm.Vorname + """ vorname, b.* FROM " & "`" & QMdbAkt & "`" & ".tmbrie b WHERE name NOT LIKE ""%labor%"" AND name NOT LIKE ""%heckliste%"" AND name NOT LIKE ""%namnese%"" AND NOT (name LIKE ""%BZ%"" OR name LIKE ""%RR%"" OR name LIKE ""%blutdr%"") AND pat_id = " & CStr(frm!Pat_ID) + " ORDER BY zeitpunkt DESC"
 Call DtbCreateQueryDef("LaborDokumente eP", sql)
' DoCmd.OpenForm "LaborDokumente eP"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Dokumente_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function do_Briefe_Click(frm As Form)
 On Error GoTo fehler
 Call do_Click_Vorbereit
 sql = "SELECT """ + frm.Nachname + """ nachname,""" + frm.Vorname + """ vorname, pfad DokPfad, name DokName, art DokArt, b.* FROM `" + QMdbAkt + "`.`tmbrie` b WHERE pat_id = " + CStr(frm!Pat_ID) + " ORDER BY zeitpunkt DESC"
 Call DtbCreateQueryDef("LaborDokumente eP", sql)
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Briefe_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function do_BZKurven_Click(frm As Form)
 On Error GoTo fehler
 Call do_Click_Vorbereit
 Call DtbCreateQueryDef("LaborDokumente eP", "SELECT """ + frm.Nachname + """ nachname,""" + frm.Vorname + """ vorname, b.* FROM `" + QMdbAkt + "`.tmbrie b WHERE (ame LIKE ""%BZ%"" OR name LIKE ""%RR%"" OR name LIKE ""%blutdr%"") AND pat_id = " + CStr(frm!Pat_ID) + " ORDER BY zeitpunkt DESC")
 DoCmd.OpenForm "LaborDokumente eP", acNormal, , "17=17"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_BZKurven_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function do_AugenBefunde_Click(frm As Form)
 On Error GoTo fehler
 Call do_Click_Vorbereit
 Call DtbCreateQueryDef("LaborDokumente eP", "SELECT """ + frm.Nachname + """ nachname,""" + frm.Vorname + """ vorname, zeitpunkt, aktzeit, name dokname, pfad dokpfad, dokart, pat_id FROM `" + QMdbAkt + "`.tmbrie b WHERE (name LIKE ""%augenb`!l`%"" OR name LIKE ""%augen`aä`rzt%"" OR name LIKE ""%aa%"") AND pat_id = " + CStr(frm!Pat_ID) + _
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_AugenBefunde_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Click_Vorbereit/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
If InStrB(Err.Description, "Transaction level 'READ-COMMITTED'") <> 0 Then
 ...
 Resume
End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in dodo_u_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Diagnosen_Reset/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ÖffneAnamnesebogen/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ÖffneFremdlabor/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ÖffneFormular/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ÖffneTabelle/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in AnpassForm/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in tabAnp/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Datenbank_Aufruf_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Ausgabe_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Datenquelle_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in DqC_U/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in DqC_UNZ/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in DqC_A/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Form_Open/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Form_Close/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' AnwPfad = App.Path
'#END IF
'SELECT CASE MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in übrigeRecordsetsSchließen/" + AnwPfad)
' Case vbAbort: Call MsgBox("Höre auf"): Progende
' Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
' Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
'End SELECT
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_form_beforeUpdate/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Form_AfterUpdate/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in formtest/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
'             "LEFT JOIN " & vorsil & "wert w ON u.id = w.usid LEFT JOIN " & vorsil & "hinw t ON w.kommid = t.id " & vbCrLf & _
'             "WHERE ((abkü IN ('ALBCRE','ALBKRE','ALBQ','ALBUM','ALBUP') AND einheit LIKE 'mg/g %') OR (abkü IN ('ALBU','ALBUMU') AND (einheit = 'mg/l' OR einheit = ''))) AND pat_id = " & Pat_id & " " & vbCrLf & _
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Form_Current_AnBog/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' Diagnosen
 ...
 "WHERE pat_id = " + CStr(Pat_id) + " AND NOT EXISTS (SELECT * FROM `laborneu` WHERE pat_id = " & CStr(Pat_id) & " AND abkü = " & vorsil & "wert.Abkü AND wert = " & vorsil & "wert.wert AND zeitpunkt > " & vorsil & "us.Eingang-3 AND zeitpunkt <  " & vorsil & "us.Eingang+6) "
...
'' Aufrufe der Anamnese usw. belegen
 ...
 "FROM (tmbrie dl LEFT JOIN `br_abgehakt` da ON dl.Pfad=da.DokPfad) " & _
...
#End If
 ...
 End If
 sql = "SELECT MAX(zeitpunkt) AS zp FROM `tmbrie` WHERE (name LIKE 'Brief an Dr%' OR name LIKE 'Brief an F%Dr.' OR name LIKE '%Arztbrief%') AND NOT name LIKE '%Entwurf%' AND pat_id = " + CStr(Pat_ID)
 '!zp, "dd/mm/yy")
 Set rsLab = Nothing
 myFrag rsLab, sql
 If Not rsLab.BOF Then
 ...
 End If
' frm.Briefe.Caption = CStr(lz) + " Briefe (&7), zul." + format$(Dtb.OpenRecordset("SELECT MAX(zeitpunkt) AS zp FROM `" + QMdbAkt + "`.`tmbrie` WHERE (name LIKE ""Brief an Dr*"" OR name LIKE ""Brief an F*Dr."" OR name LIKE ""*Arztbrief*"") AND NOT name LIKE ""*Entwurf*"" AND pat_id = " + CStr(Pat_id))!zp, "dd/mm/yy")
...
' und für Blutzuckerkurven
 Dim maxzp As Date
 maxzp = 0
 sql = "SELECT COUNT(0) ct, MAX(zeitpunkt) mzp FROM tmbrie WHERE name LIKE '%BZ%' AND pat_id = " & CStr(Pat_ID)
 Set rsLab = Nothing
 myFrag rsLab, sql
 If Not rsLab.BOF Then
  lz = rsLab!ct
...
' lz = Dtb.OpenRecordset("SELECT COUNT(0) AS ct FROM `" + QMdbAkt + "`.`dokumente` WHERE (dokname LIKE ""*augenb`!l`*"" OR dokname LIKE ""*augen`aä`rzt*"" OR dokname LIKE ""*aa"") AND pat_id = " + CStr(frm.Pat_id))!ct + _
 ...
 maxzp = 0
 Set rsLab = Nothing
 sql = "SELECT COUNT(0) ct,MAX(zeitpunkt) mzp FROM tmbrie b WHERE ((name LIKE '%augenb%' AND NOT name LIKE '%augenbl%') OR name LIKE '%augenarzt%' OR name LIKE '%augenärzt%' OR name LIKE '%aa') AND pat_id = " & CStr(Pat_ID) ' name REGEXP '.*augenb[^l].*'
 Set rsLab = Nothing
 myFrag rsLab, sql
 If Not rsLab.BOF Then
  lz = rsLab!ct
...
#End If
 ...
 zp0 = rMA!mzp
 Set rMA = Nothing
 myFrag rMA, "SELECT COUNT(0) ct, COALESCE(MAX(zeitpunkt),0) mzp FROM `eintraege` WHERE (art IN ('gs','doppler','dop','duplex','dup') OR (art='tb' AND ersteller='gs') OR inhalt LIKE '%(gs)%') AND pat_id = " & CStr(Pat_ID)
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Form_Current2/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in DiabetesDiagnose/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
For i = 177 To 180
  frm.vTextB(i) = vNS
  Select Case i
   Case 177: aktart = "'doppler','dop','duplex','dup'"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ddsono/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in sensib/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
'Call Lese.ProgStart
  sql = "SELECT n.pat_id, " & _
        ...
        "LEFT JOIN eintraege e ON e.pat_id = n.pat_id AND e.art IN ('doppler','dop','duplex','dup') AND e.inhalt LIKE '%beina%' AND e.zeitpunkt=(SELECT MAX(zeitpunkt) FROM eintraege e0 WHERE e0.pat_id = e.pat_id AND e.art IN ('doppler','dop','duplex','dup') AND e.inhalt LIKE '%beina%') " & _
...
#Else
  AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in PStatNeu/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in PStatus/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in obLH/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LegNPFest/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in hkGrund/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in EigTAlter/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in EigT/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' in do_An1Aufruf_click, do_An2Aufruf_click, do_AnAAufruf_click, do_An1Aufruf_click, do_CheckAufruf_click, do_Form_Current2
 Function PfadFestLeg$(art$, muster1$, frm As Form, Optional muster2$)
  ...
  On Error GoTo fehler
  sql = "SELECT * FROM tmbrie WHERE pat_id = " + CStr(frm.anaRS!Pat_ID) + " AND (name LIKE " & "'" & muster1 & "'" & IIf(LenB(muster2) = 0, vbNullChar, " OR name LIKE " & "'" & muster2 + "'") + ") ORDER BY zeitpunkt DESC"
  On Error GoTo fehler
'  Call dtbInit
'  SET rDok = Dtb.OpenRecordset(sql, dbOpenDynaset)
  Set PFrs = Nothing
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in PfadFestLeg/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Urineintraege/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in doVa_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in doZn_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in knöpfeanpassen/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in RRpAll/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in RRParse/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_RRParse/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in dodoRRParse/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in mdTEst/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in mdTest_alt/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_An1Aufruf_click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_An2Aufruf_click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_AnAAufruf_click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_CheckAufruf_click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_do_Aufruf/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in inActi/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Acti/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Hirndurchblutungsstörung_Exit/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in doGilb/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in KRAdd/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LabWert0/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LabWertA/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' sql = "SELECT Pat_ID, ZeitPunkt, FertigStGrad, AbKü, LangText,Wert, Einheit, Kommentar,"""" AS NB FROM (" & laborAbfr & " WHERE pat_id = " & Pat_id & " AND abkü = '" & Name & "' AND wert <> '') AS labor UNION SELECT Pat_ID, Eingang AS zeitpunkt, BefArt AS FertigStGrad, Abkü, langname AS Langtext, Wert, Einheit, Kommentar, Normbereich AS NB " + _
 "FROM `" & vorsil & "us` LEFT JOIN `" & vorsil & "wert` ON `" & vorsil & "us`.RefNr=`" & vorsil & "wert`.RefNr " & _
 "WHERE pat_id = " & CStr(frm.anaRS!Pat_id) & "  AND abkü = '" & Name & "' AND wert <> '' AND NOT EXISTS (SELECT * FROM `laborneu` WHERE pat_id = " & CStr(frm.anaRS!Pat_id) & " AND abkü = " & vorsil & "wert.Abkü AND wert = " & vorsil & "wert.wert AND zeitpunkt > " & vorsil & "us.Eingang-3 AND zeitpunkt <  " & vorsil & "us.Eingang+6) "
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LabWert/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in EinRueck/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
'   dFrs.Open "SELECT * FROM `dokumente` WHERE pat_id = " & Pat_id & " AND dokpfad = '" & frm.anaRS!DokPfad & "'", DBCn, adOpenDynamic, adLockReadOnly
   myFrag dFrs, "SELECT * FROM tmbrie WHERE pat_id = " & Pat_ID & " AND pfad = '" & frm.anaRS!DokPfad & "'"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Fremdlabor_Form_Current/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function makeDatPfad(frm As Form)
 ...
 If (InStrB("$\", left$(frm!DokPfad, 1)) <> 0 And left$(frm!DokPfad, 1) <> vNS) Or Mid$(frm!DokPfad, 2, 1) = ":" Then
   ...
   If InStrB(cmd.CommandText, "FROM `" + QMdbAkt + "`.`tmbrie`") <> 0 Then
'   IF instrb(Dtb.QueryDefs![LaborDokumente ep].sql, "FROM `" + QMdbAkt + "`.`tmbrie`") <> 0 THEN
    Dim Appl$, Bef$
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in makeDatPfad/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_LaborDokumente_Form_Current/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_LaborDokumente_form_load/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Taste/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' AnwPfad = App.Path
'#END IF
'SELECT CASE MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Fremdlabor_Form_Load/" + AnwPfad)
' Case vbAbort: Call MsgBox("Höre auf"): Progende
' Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
' Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
'End SELECT
...
' AnwPfad = App.Path
'#END IF
'SELECT CASE MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_Fremdlabor_Form_Unload/" + AnwPfad)
' Case vbAbort: Call MsgBox("Höre auf"): Progende
' Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
' Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
'End SELECT
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_gescannte_Datei_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_anzeigen_click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_PhotoImpact_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_FDatenquelle_Change/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ReadRegistryGetSubkey/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in GetExcel/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in DetectExcel/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Detectword/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in cmdPreview_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in FaxSend/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in getHausarzt/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' AnwPfad = App.path
'#END IF
'SELECT CASE MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in getHausarztAlt/" + AnwPfad)
' Case vbAbort: Call MsgBox("Höre auf"): Progende
' Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
' Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
'End SELECT
...
#If mo Then
 ...
 If MOConInit(, vonwo & " -> getHausarzt1") Then Exit Function
 ebsnr = "": elanr = "": psur = "": asur = "": ReDim arztnrn(0)
 infi = 0
 
 syscmd acSysCmdSetStatus, "Erstelle Brief: 1) Hausarzt, a) Überweiser ..."
 ' 1) Überweiser aus dem Memofeld von FPatfall, bis zu 3-4 Fälle zurück
 ...
 If Not rHa.BOF Then
  Do While Not rHa.EOF
   Call ParseMemoFast(rHa!fm, FMem(), , "FMemo aus patfall, FPatnr " & Pat_ID)
   For mru = 0 To UBound(FMem)
    Select Case FMem(mru).ENr
     Case "3.4"
      üqu = ZQKurz(stzk(FMem(mru).Text))
     Case "2.2"
      If IsNumeric(FMem(mru).Text) Then
       ob71010 = True
       GoTo l102
      End If
     Case "2.4"
      If ob71010 Then GoTo l104
     Case "2.5"
      If ob71010 Then GoTo l105
     Case "10.2", "1.10.2"
l102:
      ebsnr = FMem(mru).Text
     Case "10.4", "1.10.4"
...
'       If UBound(namspl) > 0 Then infos(9, infi) = namspl(0): infos(14, infi) = namspl(UBound(namspl))
       Call ParseMemoFast(rHi!FA, FMi(), , "FAdresse aus epraxis " & ebsnr)
       For nru = 0 To UBound(FMi)
        Select Case FMi(nru).ENr
         ...
         Case "6" ' Ort
          infos(3, infi) = infos(3, infi) & FMi(nru).Text
         Case "9.2" ' Telefon
          infos(13, infi) = FMi(nru).Text ' Telefonnummer
         Case "10.2": ' Faxnummer
...
' 2) Methode wie in doPatvonMO
 ...
 For iinf = infi To 1
  ...
  If inf.KVNr <> "" Or inf.BSNR <> "" Or inf.Faxnr <> "" Then ' fax 29.9.25 ' BSNR 3.4.26
   ...
   infos(4, iinf) = inf.Faxnr
   infos(5, iinf) = inf.anrede ' inf.Überschr
   If InStrB(inf.anrede, "Frau") Then infos(5, iinf) = "Sehr geehrte Frau Kollegin" Else If InStrB(inf.anrede, "Herr") Then infos(5, iinf) = "Sehr geehrter Herr Kollege" Else If inf.anrede = "" Then infos(5, iinf) = inf.Überschr Else infos(5, iinf) = inf.anrede
   infos(6, iinf) = inf.DmpTyp2
   infos(7, iinf) = inf.DmpTyp1
   infos(8, iinf) = inf.NiederlGb
   infos(9, iinf) = inf.Vorname
   infos(10, iinf) = inf.Funktion
   infos(11, iinf) = inf.Fachrtg
   infos(12, iinf) = inf.KVNr
...
'       Case "Titel":
 ...
 ' b) Arzt in Arztdatei gespeichert
  ...
  infos(10, infi) = "HA"
  Call ParseMemoFast(rHa!FA, FMem(), , "FAdresse aus earzt" & Pat_ID)
  Debug.Print "Ende"
  For mru = 0 To UBound(FMem)
...
'         rHa.Open sql0 & "WHERE LEFT(bsnr,7) = " & Left$(übwerg(0, runde), 7) & " AND a.nachname = '" & übwerg(1, runde) & "' AND a.vorname = '" & übwerg(2, runde) & "' AND a.obehem=0", _
       ...
       End If ' Not HACngef And übwerg(1, runde) <> vNS And übwerg(2, runde) <> vNS Then ' Nachname, Vorname
...
'       IF rHae.BOF THEN
        ...
        End If
      ElseIf Not rhae.BOF And Not rHaBOF Then
        If Not IsNull(rHa!Nachname) Then NN = rHa!Nachname Else NN = ""
        If Not IsNull(rHa!Vorname) Then VN = rHa!Vorname Else VN = ""
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in gethausarzt1/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function doDuplexkontrollieren()
 ...
 If IsDate(begDStr) Then
  begD = CDate(begDStr)
  sql = "SELECT pat_id, zeitpunkt, MIN(name) DokName, quelldatum, COUNT(0) zahl FROM tmbrie b WHERE name LIKE '%SonoBild%' AND quelldatum >= " & DatFor_k(begD) & " GROUP BY pat_id, zeitpunkt,DATE(quelldatum) ORDER BY quelldatum"
...
' in BriefeBerichtspflicht_Click
...
Public Function ConvertToUrl$(strFile$)
    ...
    ConvertToUrl = "file:///" + strFile
End Function ' ConvertToUrl

Public Function ConvertFromUrl$(url$)
  ConvertFromUrl = Mid$(REPLACE$(REPLACE$(REPLACE$(url, "\", "/"), "|", ":"), "%20%", " "), 9)
End Function ' ConvertFromUrl$(url$)

Function OOSchließen()
    Dim oSM                   'Root object for accessing OpenOffice from VB
    Dim oDesk, oDoc As Object 'First objects from the API
Dim oEnum As Object

' Verbindung zu LibreOffice herstellen (Beispiel für OLE-Automatisierung)
' Die genaue Methode hängt von Ihrer Umgebung ab.
    On Error Resume Next ' Fehlerbehandlung für den Fall, dass LibreOffice nicht läuft
    Set oSM = CreateObject("com.sun.star.ServiceManager")
    If Err.Number = 0 Then
     Set oDesk = oSM.createInstance("com.sun.star.frame.Desktop")
     If Err.Number <> 0 Then
      Debug.Print "LibreOffice ist nicht gestartet oder nicht erreichbar."
      Exit Function
     ElseIf oDesk Is Nothing Then
      Debug.Print "oDesk is nothing"
      Exit Function
     End If
    End If
    Dim docs As Object
    ...
    Else
     On Error Resume Next
     Set docs = oDesk.getComponents()
     If Err.Number Then
         oDesk.Terminate
         Exit Function
     End If
     On Error GoTo 0
' Alle geöffneten Dokumente durchgehen
     Set oEnum = docs.createEnumeration()
     While oEnum.hasMoreElements()
        Set oDoc = oEnum.nextElement()
        If oDoc.supportsService("com.sun.star.text.TextDocument") Then ' Oder eine andere Dokumentenart
            ' Dokument schließen
'            oDoc.Close (False) ' True speichert das Dokument, False schließt ohne zu speichern
            If oDoc.isModified() Then
             Dim erg%
             If Not oDoc.isReadOnly Then
              If Not oDoc.hasLocation Then
               oDoc.storeasurl ("file:///LibreOffice_" & Format(Now(), "yyyymmdd_hhmmss") & ".ods")
              Else
               erg = MsgBox("Sollen Änderungen in " & ConvertFromUrl(oDoc.url) & " gespeichert werden?", vbYesNoCancel, "Rückfrage")
               Select Case erg
                Case vbYes
                 oDoc.Store
                Case vbCancel
                 Exit Function
               End Select
              End If ' Not oDoc.hasLocation Then Else
             End If ' Not oDoc.isReadOnly Then
            End If ' oDoc.isModified() Then
            oDoc.Close False
        End If ' If oDoc.supportsService("com.sun.star.text.TextDocument") Then
     Wend
    End If ' False
End Function
...
Public Function mAkePropertyValue2(oSM, CName, uValue)
...
'  Const DT$ = "p:\test\B.docx", vz$ = "p:\test\doc"
  ...
  If FSO.FileExists(zdt) Then OOOpen zdt ' WordDateiOeffnen zdt
End Function ' dozus

Public Function PKennz$(ByRef abz$, Optional reset%)
#Const ohneIDs = True
#If ohneIDs Then
 PKennz = abz
#Else
...
Function einzEintr(Pat_ID$, ÜS$, erlaeut$, arten$, Optional VorDat As Date, Optional obgross%, Optional fett$, Optional neuz$)
 Dim inhlt$
#If False Then
 Dim pos&, wlen%
 Static warr
...
' G1Name = rsNa!G1Name
' myFrag raAn, "SELECT COALESCE(`diabetes seit`,0) dmseit FROM `anamnesebogen` a WHERE pat_id = " & Pat_ID, adOpenStatic
...
'  SET rHa = TabÖff("Hausaerzte", "KVNR")
 ...
 Dim lddat As Date
 myFrag raFa, "SELECT GREATEST(MAX(bhfb),MAX(fanf),MAX(ausgst))lddat FROM `faelle` WHERE pat_id = " & Pat_ID
 If Not raFa.BOF Then
  If Not IsNull(raFa!lddat) Then
   lddat = raFa!lddat
  End If ' Not IsNull(raFa!lddat) Then
...
' If Right$(LCase$(Vorlage), 4) = ".dot" Then If Left$(LCase$(Vorlage), 11) = "accessbrief" Then VBuch = Mid$(Vorlage, 12, Len(Vorlage) - 15)
   syscmd acSysCmdSetStatus, "Erstelle Brief für " & gesName & ": 3) Briefrahmen, erstelle app ..."
   Open zvz & "docProps\app.xml" For Output As #51
   Print #51, app1 & Vorlage & app2
   Close #51
   syscmd acSysCmdSetStatus, "Erstelle Brief für " & gesName & ": 3) Briefrahmen, erstelle core ..."
...
'  oSh.run "powershell ""$dt=\""v:\exp9\word\endnotes.xml\"",\""v:\exp9\word\footnotes.xml\"";$dth=\""v:\exp9\word\header1.xml\"";$nrd=\""\\" & LiName & "\daten\eigene dateien\programmierung\dateilesen\dzahl.txt\"";$nr=[int](get-content -path $nrd)+1;set-content -path $nrd $nr;$repl='${1}'+'{0:X8}' -f $nr+'$2';$qla='(.*w:rsidRDefault=\"")[0-9A-F]*(\""><w:';$qls=$qla+'r><w:';foreach ($dta in $dt){((get-content -path $dta) -replace \""${qls}s.*)\"",$repl) -replace \""${qls}c.*)\"",$repl|set-content -path $dta;};(get-content -path $dth)|foreach-object {$_ -replace \""${qla}p.*)\"",$repl}|set-content -path $dth;""", 0, True
   ...
   syscmd acSysCmdSetStatus, "Erstelle Brief für " & gesName & ": 3) Briefrahmen, fange mit document.xml an ..."
  #If spaeter Then
   ag.Clear
   ...
   For ii = 1 To UBound(infos, 2)
     If infos(4, ii) = infos(4, 0) And infos(4, ii) <> "" Then ' wenn die Faxnummern gleich sind ' 3.4.26: und nicht leer vorziehen, damit
      ...
      For jj = 0 To UBound(infos, 1) ' dann lieber den manuell eingegebenen Arzt nehmen
       ...
       infos(jj, ii) = ""
      Next jj
     Else ' wenn sie verschieden sind, dann den zweiten Arzt extra im Brief aufführen
      obNachr = ii
      Exit For
...
' Straße: zuh(infos(2, 0)), zuh(IIf(obNachr <> 0 And UBound(infos, 2) > 0, "<w:r><w:rPr><w:rStyle w:val=""s24s""/></w:rPr><w:t>" & infos(2, obNachr) & "</w:t></w:r>", ""))
...
' Einleitung: zuh(replace$(GName, ", *", ", geb. ")), dieder, behd
   ...
   If (VorDat0 And (VorDat0 > Now() - 30)) Or myEFrag("SELECT COALESCE((SELECT schgr FROM faelle WHERE pat_id=" & Pat_ID & " ORDER BY bhfb DESC LIMIT 1),'24')schgr", rAf)!SchGr = 0 Then
    satzanf = "ich berichte " & IIf(VorDat0, "erneut", "") & " über"
   Else
    satzanf = "besten Dank für die freundliche Überweisung von"
   End If
   ...
   Call einzEintr(Pat_ID, "Anamnese zu Augenuntersuchungen", "", "'aug','augen'", VorDat)
   ...
   Call einzEintr(Pat_ID, "Gewichtsverlauf", "", "'gewicht','gw','gew','gewi'", VorDat, True)
   Call einzEintr(Pat_ID, "Taille", "", "'taille','hüfte'", VorDat)
   Call einzEintr(Pat_ID, "Blutdruck/Puls", "", "'rr','puls'", VorDat, True)
   Call einzEintr(Pat_ID, "Blutdruckvergleichsmessung", "", "'rrvgl'", VorDat)
   Call einzEintr(Pat_ID, "Urinuntersuchung", "", "'urin'", VorDat)
   Call einzEintr(Pat_ID, "Fußbefund, ggf. Ulcusbeschreibungen", "", "'fuß','fuss','usd','usdm','usdm1','usdm2','wv','ulc','ulcus','kv','debr'", VorDat)
   Call einzEintr(Pat_ID, "Doppleruntersuchung", "", "'dop','doppler'", VorDat, True, "Beinarterien|Beinarterien mit Belastung|Beinvenen")
   Call einzEintr(Pat_ID, "Duplexuntersuchung", "", "'dup','duplex'", VorDat, True, "Halsschlagadern|Nierenarterien|Beinarterien|Beinarterien mit Belastung|Beinvenen")
   Call einzEintr(Pat_ID, "Sonogramm", "", "'son','sono','sd'", VorDat, True, "Abdomen|Schilddrüse|Weichteile")
   Call einzEintr(Pat_ID, "EKG", "", "'ekg'", VorDat, True)
   Call einzEintr(Pat_ID, "Langzeitblutdruckmessung", "", "'lzrr'", VorDat, True)
...
'     Dim ii%, obNachr%
     ...
     If obNachr <> 0 And UBound(infos, 2) > 0 Then
      ...
      Call BMRd(dc, !ha3b, infos(2, obNachr), True) ' raHacall bmrd(dc,!Straße
      ...
      End If
      Call BMRd(dc, !ha10b, infos(10, obNachr)) ' Überweiserinfos
...
'    rlAb.Open "SELECT DISTINCT quelle,Eingang,k.text kommentar FROM `" & vorsil & "us` u INNER JOIN `" & vorsil & "bakt` b ON u.id = b.usid LEFT JOIN " & vorsil & "hinw k ON b.kommid=k.id WHERE pat_id = " & pat_id & " AND NOT ISNULL(keimzahl) AND eingang > " & DatFor_k(VorDat), DBCn, adOpenDynamic, adLockReadOnly
...
' bei Wiederholungsbriefen
    ...
    Call WiederHolungsBrief(NachNa, VorDat, VorDat0, Wapp, dc)
...
''   ag.apph PKennz("<w:sectPr><w:type w:val=""nextPage""/><w:headerReference w:type=""default""r:id=""rId7""/><w:pgSz w:w=""12240""w:h=""15840""/><w:pgMar w:left=""1417""w:right=""850""w:top=""567""w:bottom=""850""w:header=""720""w:footer=""720""w:gutter=""0""/><w:pgNumType w:fmt=""decimal""/><w:formProt w:val=""false""/><w:textDirection w:val=""lrTb""/><w:docGrid w:type=""default""w:linePitch=""100""w:charSpace=""0""/><w:cols w:space=""720""/><w:titlePg/></w:sectPr>", False)
   ag.Append PKennz("<w:sectPr w:rsidR=""00EB40B3"" w:rsidSect=""00EB40B3""><w:headerReference w:type=""default"" r:id=""rId7""/><w:pgSz w:w=""11907"" w:h=""16840""/><w:pgMar w:top=""567"" w:right=""851"" w:bottom=""851"" w:left=""1418"" w:header=""720"" w:footer=""720"" w:gutter=""0""/><w:cols w:space=""720""/><w:titlePg/></w:sectPr>")
...
'    wpfad = getReg(2, "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths", "WinWord.exe")
 ...
 Else ' briefneu
  With Wapp
   ...
   On Error Resume Next
   If obStumm Then
    Call .Quit(0)  '  = wdDoNotSaveChanges
   Else ' obStumm
    Call snie ' sound
   End If ' obStumm
  End With ' wapp
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in fobHAimDMP/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in HAlokal/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Üwrd/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Üw12/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function testWied()
 ...
 VorDat = InputBox("Arztbrief mit Daten ab:", "Rückfrage Daten ab", VorDat0)
 Call WiederHolungsBrief(Forms(Anmnbi), VorDat, VorDat0, Wapp)
 Exit Function
fehler:
 Dim AnwPfad$
#If VBA6 Then
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in testWied/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
'  If True Then
''   Call dtbInit
''   sql = "SELECT COUNT(0) AS ct FROM `" + QMdbAkt + "`.`tmbrie relevant` WHERE pat_id = " & CStr(Pat_id)
'   sql = "SELECT COUNT(0) AS ct FROM `tmbrie` WHERE (name LIKE ""%Brief an %Dr%"" OR (name LIKE ""%Arztbrief%"" AND NOT name LIKE ""%: Arztbrief%"") OR name LIKE ""Brief an HA%"" OR name LIKE ""Brief an HAe%"") AND NOT pfad LIKE '%.pdf' AND name NOT LIKE ""%Entwurf%"" AND pat_id = " & Pat_ID
'   myFrag BRz, sql
...
'    Else
' mit qab oder qea erstellte Arztbriefe
     sql = _
     "SELECT 18900101+INTERVAL p.fdatum DAY+INTERVAL p.fuhrzeit SECOND zp" & vbCrLf & _
     "FROM dbsprot p" & vbCrLf & _
     "LEFT JOIN ltag l ON MID(p.fprimarykey,2,LENGTH(p.fprimarykey)-2)=l.FSurogat" & vbCrLf & _
     "LEFT JOIN datafile d ON d.FReferenznr=l.FSurogat" & vbCrLf & _
     "WHERE p.fpatnr=" & Pat_ID & " AND" & vbCrLf & _
     "     p.FTablename='ltag'" & vbCrLf & _
     "AND feintragsart =848 -- IN (50,51,52,151,848" & vbCrLf & _
     "AND ftext RLIKE '^(eArztbrief|Angiobrief|Gestationsdiabetes)'" & vbCrLf & _
     "AND 18900101+INTERVAL p.fdatum DAY<DATE(NOW())" & vbCrLf & _
     "ORDER BY zp DESC LIMIT 1"
'     "-- ,FEpochtime,p.fsurogat,p.FPatNr,l.fsurogat,18900101+INTERVAL l.fdatum DAY+INTERVAL l.fzeit SECOND zpl,l.feintragsart,l.ftext,d.FSurogat,d.FFilename" & vbCrLf & _
'     "-- , p.*,l.*, d.*" & vbCrLf & _

     If MOConInit(, "GetVorDat") Then Exit Function
     myFrag BRz, sql, adOpenStatic, MOCon
     If Not BRz.EOF Then GetVorDat = BRz!Zp ' den Text auszulesen gelingt mir noch nicht, die Datei steht in FFilename, aber 1) weiß ich noch nicht, wo der Ordner steht, und 2), wie man ".moc" entpackt
     
' alte Arztbriefe aus Turbomed
     sql = _
     ...
     If Not BRz.BOF Then
      ...
      If zeitp1 > GetVorDat Then GetVorDat = zeitp1
      Dim txtdatei$, txtpfad$, zeile$, obfertig%
      txtpfad = FSO.GetParentFolderName(lBrNam)
      txtdatei = txtpfad & "\" & FSO.GetBaseName(lBrNam) & ".txt"
      If Not FSO.FileExists(txtdatei) Then
'       Shell "cmd /c copy /y " + """" + z + """" & " " & """" + REPLACE$(z, ".BDT", ".txt") + """"
       On Error Resume Next
       Call ShellaW("""c:\program files\libreoffice\program\soffice"" --headless --convert-to txt:Text --outdir """ & txtpfad & """ """ & lBrNam & """", vbHide)
       On Error GoTo fehler
      End If
      If FSO.FileExists(txtdatei) Then
       Open txtdatei For Input As #235
       Do While Not EOF(235)
        Input #235, zeile
        If IsDate(zeile) Then
         Dim zeildat As Date
         zeildat = CDate(Trim(zeile))
' wenn Zeildat neuer, bzw. älter als das aktuelle Eintragsdatum, aber jünger als der letzte MO-Arztbrief
         If zeildat > GetVorDat Or (zeildat < zeitp1 And zeitp1 = GetVorDat) Then GetVorDat = CDate(zeile)
         obfertig = True
         Exit Do
        End If ' IsDate(Trim(zeile)) Then
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in GetVorDat/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Function WiederHolungsBrief(Nachname$, VorDat As Date, VorDat0 As Date, Wapp, Optional dc)
 ...
 On Error GoTo fehler
 If VorDat = CDate(0) Or VorDat <= Vgst Then GoTo schluss
...
#Else
      dc.Range(r2.Range.paragraphs(1).Range.Next.Start, pakt.Range.Start).Delete ' 23.10.12: pakt.range.start statt pakt.range.start-1, um Leerzeile unter Überschrift zu vermeiden
#End If
...
#Else
      dc.Range(r2.Range.Start, pakt.Next.Range.Start).Delete
#End If
     End If
     On Error GoTo fehler
'     r2.Range.SELECT
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in WiederHolungsBrief/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' in tuBriefStandalone und ZwischenSpeichern
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ArBName/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "beim Speichern unter: " + ABName + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ZwischenSpeichern/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' in tuBriefStandalone
Function letzteMed(dc, Pat_ID$, briefneu%)
...
'    dc.Range(Tabl.Range.END, Tabl.Range.END) = replace$(bemerk, Chr(171), "½") + Chr(13) + Chr(10)
  ...
  If Med <> "" Then If Asc(Med) = 10 Then Med = Mid$(Med, 1)
  ...
  If IsNull(raDat!mo) Then ab = "" Else ab = REPLACE$(REPLACE$(raDat!ab, "«", "½"), "¬", "¼")
  If IsNull(raDat!mo) Then Zn = "" Else Zn = REPLACE$(REPLACE$(raDat!Zn, "«", "½"), "¬", "¼")
  If raDat!j_bBed = 0 Then bBed = "" Else bBed = "X"
  If briefneu <> 0 Then
   ag.Append "<w:tr>"
   ag.Append "<w:tc><w:tcPr><w:tcW w:w=""0"" w:type=""auto""/></w:tcPr>" & IIf(Med = "", "<w:p/>", "<w:p><w:r><w:rPr><w:rStyle w:val=""s24s""/></w:rPr><w:t>" & zuh(Med) & "</w:t></w:r></w:p>") & "</w:tc>"
   ...
   ag.Append "<w:tc><w:tcPr><w:tcW w:w=""0"" w:type=""auto""/></w:tcPr>" & IIf(ab = "", "<w:p/>", "<w:p><w:r><w:rPr><w:rStyle w:val=""s24s""/></w:rPr><w:t>" & ab & "</w:t></w:r></w:p>") & "</w:tc>"
   ag.Append "<w:tc><w:tcPr><w:tcW w:w=""0"" w:type=""auto""/></w:tcPr>" & IIf(Zn = "", "<w:p/>", "<w:p><w:r><w:rPr><w:rStyle w:val=""s24s""/></w:rPr><w:t>" & Zn & "</w:t></w:r></w:p>") & "</w:tc>"
  ...
  Else ' briefneu <> 0 Then
   ...
   Tabl.cell(runde, 5) = ab
   Tabl.cell(runde, 6) = Zn
   Tabl.cell(runde, 7) = bBed
  End If ' briefneu <> 0 Then else
  runde = runde + 1
  raDat.Move 1
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in letzteMed/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in MedArtenUnausgefüllt/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in FlagInit/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Sub Epikrise(dc, Pat_ID$, VorDat As Date, lddat As Date, obStumm%, briefneu%, Optional dstring$)
  Dim i%
  On Error GoTo fehler
'  nzw = vbCr
...
'    END IF
    ...
    For j = 1 To FEZ
     If Not un(j) And obunter Then
      Folge = Folge + ")"
      obunter = 0
     End If ' Not un(j) And obunter Then
     If flag(j) And Not (j = flNep And obkNeph) Then
      ...
      FZ = FZ + 1
     End If ' flag(j) And Not (j = flNep And obkNeph) Then
    Next j
...
w2:
    ...
    If FZ <> 0 Then
     Ep0 = Ep0 + IIf(FZ = 1, "mit der Folgeerkrankung ", "mit den Folgeerkrankungen ") + Folge
    Else
     Select Case dmtyp
      Case "g", "p"
      Case Else
       Ep0 = Ep0 + "ohne Hinweis auf Folgeerkrankungen"
     End Select
    End If
   Case Else ' dmtyp
  End Select
  If bglz <> 0 Then
   Ep0 = Ep0 + IIf(FZ = 0, ", mit ", " und ") + IIf(bglz = 1, "der Begleiterkrankung ", "den Begleiterkrankungen ") + Begl
  End If
  
  If briefneu Then
   ag.Append "<w:p><w:r><w:rPr><w:b/><w:rStyle w:val=""s24s""/></w:rPr><w:t>Epikrise</w:t></w:r><w:r><w:rPr><w:rStyle w:val=""s24s""/></w:rPr><w:t>: "
  End If
  ...
  ",e.*" & vbCrLf & _
  "FROM sws s" & vbCrLf & _
  "JOIN eintraege e USING (pat_id)" & vbCrLf & _
  "WHERE e.art='ogtt' AND s.pat_id=" & Pat_ID & " AND e.zeitpunkt>=" & Format$(VorDat, "yyyymmdd") & vbCrLf & _
  "AND s.voret=(SELECT MAX(voret) FROM sws WHERE pat_id=s.pat_id)" & vbCrLf & _
  ")i"
  Dim rsws As ADODB.Recordset, swstxt$
  swstxt = myEFrag(swsql, rAf)!erg
  If swstxt <> "" Then
   Epi = Epi & swstxt
   If briefneu Then
    ag.Append zuh(Epi) & "</w:t><w:br/></w:r></w:p>"
    Epi = ""
   End If
  Else ' If swstxt = "" Then ' wenn kein OGTT für Gestationsdiabetes
' Therapieart
...
' Grenze zwischen Diagnostik und Therapie
  ...
  If briefneu Then
   ag.Append zuh(Epi) & "</w:t></w:r></w:p>" ' vorderer Rest vor dem OGTT, IF swstxt = ""
   Epi = ""
  End If ' briefneu
...
'   Epi = Epi + "Für das LDL gilt nach den noch nicht aktualisierten Leitlinien ein Zielwert von 100 mg/dl." + nzw
  ...
  End If ' lLDL > 100 And Alter < 76 Then
...
'   IF sKrea = vNS THEN sKrea = "0"
   ...
   If lKrea <> 0 Then
    ...
    If obNephrologe Then
     Set rDial = Nothing
     myFrag rDial, "SELECT 0 FROM tmbrie WHERE pat_id = " & Pat_ID & " AND (name LIKE '%Arenz%' OR name LIKE '%Stenglein%' OR name LIKE '%Habersetzer%' OR name LIKE '%Nephrol%')"
...
'    END IF
      ...
      Epi = Epi + IIf(dmtyp = "1", " Bei", " Nur bei") + " Typ-1-Diabetes ist außerdem der Nutzen einer Eiweißrestriktion belegt."
     Else
...
'  rKH.Seek "=", Pat_ID
  If Not rKh.BOF Then
   If Not IsNull(rKh!ges) Then
    Epi = Epi & nzw & "Folgende Einweisungen wurden von mir ausgestellt: " & rKh!ges & nzw
   End If
  ...
  sql1 = "SELECT IF(COUNT(0)=0,'',CONCAT(CHR(13),'Weitere Termine haben wir noch vereinbart, Ihr freundliches Einverständnis voraussetzend: '," & vbCrLf & _
         "GROUP_CONCAT(CONCAT(DATE_FORMAT(zp,'%d.%m.%y %H:%i'),CASE WHEN raum RLIKE 'abor|^Seminar' THEN CONCAT(' (',raum,' ',zusatz,')') WHEN raum RLIKE '^(Kothny|Schade|Hammersc)' THEN ' (Sprechstunde)' ELSE '' END) SEPARATOR ', '),'.',CHR(13)))" & vbCrLf & _
...
'  rUebw.Open sql1, DBCn, adOpenDynamic, adLockReadOnly
  ...
  If Not rUebw.BOF Then
   ...
   Do While Not rUebw.EOF
    If Not obkomma Then Epi = Epi & nzw & "Weitere Überweisungen wurden von mir ausgestellt: "
...
'    End If
  ...
  If briefneu Then
   If Epi <> "" Then
    ag.Append "<w:p><w:r><w:rPr><w:rStyle w:val=""s24s""/></w:rPr><w:t>" & zuh(Epi) & "<w:br/><w:br/></w:t></w:r></w:p>"
    Epi = ""
   End If ' if Epi
  Else ' briefneu
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Epikrise/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in zplschul/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in SchutzBest/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
'    READS SQL DATA
'    COMMENT 'vergleicht " & vorsil & "us mit labroneu'
'BEGIN
'drop table IF EXISTS lauswahl;
'create table lauswahl (index (befart,abkü,wert)) AS (SELECT us.id,befart,abkü,wert,0 FROM " & vorsil & "us us LEFT JOIN " & vorsil & "wert w ON w.usid=us.id WHERE us.id=2823 ORDER BY abkü,wert);
'end $$
...
'VN = "labor2"
'Vsql = "SELECT u.Pat_id AS Pat_ID,CAST(u.Eingang As Date) AS zeitpunkt,u.BefArt AS FertigStGrad,w.Abkü AS Abkü,w.Langname AS Langtext,w.Wert AS Wert,w.Einheit AS Einheit,w.Kommentar AS Kommentar,w.Normbereich AS NB FROM (`" & vorsil & "us` u LEFT JOIN " & vorsil & "wert w on((u.RefNr = w.RefNr))) WHERE (not(exists(SELECT 1 AS Not_used FROM `laborneu` WHERE ((`laborneu`.Pat_ID = u.Pat_id) AND (`laborneu`.Abkü = w.Abkü) AND (`laborneu`.Wert = w.Wert) AND (`laborneu`.ZeitPunkt > SUBDATE(u.Eingang,INTERVAL 3 DAY)) AND (`laborneu`.ZeitPunkt < ADDDATE(u.Eingang, INTERVAL 6 DAY))))))  AND wert <> '' AND NOT ISNULL(wert) ;"
'Call DtbCreateQueryDef(VN, Vsql)
'vz = vz + 1
...
'VN = "_labor2a"
'#IF False THEN
''Vsql = "SELECT u.Pat_id Pat_ID,CAST(u.Eingang As Date) zeitpunkt,u.BefArt FertigStGrad,w.Abkü Abkü,w.Langname Langtext,w.Wert Wert,w.Einheit Einheit, CONCAT(IF(erklärung RLIKE '^:[ /\*:]*$','',IF(erklärung RLIKE '^:[ /\*]*:',CONCAT(MID(erklärung,locate(':',erklärung,2)+1),';'),IF(erklärung='.','',IF(erklärung='','',CONCAT(erklärung,';'))))),kommentar) Kommentar, nb.NB NB, nb.uNg uNg, nb.oNg oNg, s.labor Labor FROM (`" & vorsil & "us` u LEFT JOIN `" & vorsil & "wert` w ON u.RefNr = w.RefNr LEFT JOIN `" & vorsil & "pnb` nb ON w.nbid = nb.id) LEFT JOIN `" & vorsil & "saetze` s ON u.satzid = s.satzid WHERE wert <> '' AND NOT ISNULL(wert)"
...
'"FROM (`" & vorsil & "us` u " & _
'"LEFT JOIN `" & vorsil & "wert` w ON u.RefNr = w.RefNr " & _
'"LEFT JOIN `" & vorsil & "pneu` p ON w.abkü = p.abkü AND w.einheit = p.einheit " & _
'"LEFT JOIN `" & vorsil & "pgl` gl ON p.id = gl.idxpbez " & _
'"LEFT JOIN `" & vorsil & "pneu` bez1 ON gl.idxpneu = bez1.id " & _
'"LEFT JOIN `" & vorsil & "pnb` nb ON w.nbid = nb.id) " & _
'"LEFT JOIN `" & vorsil & "saetze` s ON u.satzid = s.satzid WHERE wert <> '' AND NOT ISNULL(wert)"
'#ELSEIF False THEN
...
''Vsql = "SELECT u.Pat_id Pat_ID,CAST(u.Eingang As Date) zeitpunkt,u.BefArt FertigStGrad,w.Abkü Abkü,w.Langname Langtext,w.Wert Wert,w.Einheit Einheit, " & KomStr & " Kommentar, nb.NB NB, nb.uNg uNg, nb.oNg oNg, s.labor Labor FROM (`" & vorsil & "us` u LEFT JOIN `" & vorsil & "wert` w ON u.RefNr = w.RefNr LEFT JOIN `" & vorsil & "pnb` nb ON w.nbid = nb.id) LEFT JOIN `" & vorsil & "saetze` s ON u.satzid = s.satzid" ' WHERE wert <> '' AND NOT ISNULL(wert)"

''Vsql = "SELECT u.Pat_id Pat_ID,CAST(u.Eingang As Date) zeitpunkt,u.BefArt FertigStGrad,IF(ISNULL(n2.abkü),w.abkü,n2.abkü) Abkü, w.Abkü abk_ur,w.Langname Langtext,w.Wert Wert,IF(ISNULL(n2.abkü),w.einheit,n2.einheit) Einheit, w.Einheit Einheit_ur," & KomStr & " Kommentar, IF(ISNULL(n2.abkü),nb.NB,nb2.nb) NB, nb.NB NB_ur, IF(ISNULL(n2.abkü),nb.uNg,nb2.uNg) uNg, nb.uNg uNg_ur, IF(IF(ISNULL(n2.abkü),w.abkü,n2.abkü) = 'LDL' AND IF(ISNULL(n2.abkü),w.einheit,n2.einheit) = 'mg/dl','100',IF(ISNULL(n2.abkü),nb.oNg,nb2.oNg)) oNg, nb.oNg oNg_ur, s.labor Labor FROM (`" & vorsil & "us` u LEFT JOIN `namen` na ON u.pat_id = na.pat_id LEFT JOIN `" & vorsil & "wert` w ON u.RefNr = w.RefNr LEFT JOIN `" & vorsil & "pnb` nb ON w.nbid = nb.id LEFT JOIN `" & vorsil & "pneu` neu ON nb.pid = neu.id LEFT JOIN `" & vorsil & "pgl` gl ON neu.id = gl.idxpneu LEFT JOIN `" & vorsil & "pneu` n2 ON gl.idxpbez = n2.id LEFT JOIN `" & vorsil & "pnb` nb2 ON n2.id = nb2.pid " & _
'"AND nb2.geschlecht IN (3,9,0,IF(na.geschlecht='m',IF(ADDDATE(na.gebdat,INTERVAL 18 YEAR)>CAST(u.eingang As Date),1,4) ,IF(ADDDATE(na.gebdat,INTERVAL 18 YEAR)>CAST(u.eingang As Date),2,5)))  ) LEFT JOIN `" & vorsil & "saetze` s ON u.satzid = s.satzid"
'Vsql = "SELECT u.Pat_id Pat_ID,CAST(u.Eingang As Date) zeitpunkt,u.BefArt FertigStGrad,IF(ISNULL(n2.abkü),w.abkü,n2.abkü) Abkü,w.Abkü abk_ur,w.Langname Langtext,w.Wert Wert,IF(ISNULL(n2.abkü),w.einheit,n2.einheit) Einheit,w.Einheit Einheit_ur," & KomStr & " Kommentar,nb.NB NB,nb.NB NB_ur,nb.uNg uNg,nb.uNg uNG_ur,IF(IF(ISNULL(n2.abkü),w.abkü,n2.abkü) = 'LDL' AND IF(ISNULL(n2.abkü),w.einheit,n2.einheit) = 'mg/dl','100',nb.oNg) oNg,nb.oNg oNg_ur,s.Labor Labor, e.pfad " & _
       ...
       '"LEFT JOIN `" & vorsil & "wert` w ON u.RefNr = w.RefNr " & _
       '"LEFT JOIN `" & vorsil & "pnb` nb ON w.nbid = nb.id " & _
       '"LEFT JOIN `" & vorsil & "pneu` neu ON nb.pid = neu.id " & _
       '"LEFT JOIN `" & vorsil & "pgl` gl ON neu.id = gl.idxpneu " & _
       '"LEFT JOIN `" & vorsil & "pneu` n2 ON gl.idxpbez = n2.id) " & _
       '"LEFT JOIN `" & vorsil & "saetze` s ON u.satzid = s.satzid " & _
       '"LEFT JOIN `" & vorsil & "eingel` e ON s.datid = e.datid " & _
       '"ORDER BY u.pat_id, u.eingang, u.BefArt"
       ''  GROUP BY  u.pat_id, u.Eingang, FertigstGrad, w.abkü, w.wert, w.einheit,nb.nb
'' WHERE (ISNULL(nb2.eingang) OR nb2.eingang = (SELECT MAX(eingang) FROM `" & vorsil & "pnb` WHERE pid = n2.id))
'#ELSE
...
",IF(w.Einheit IN('','\'kA\'')AND Wert=''AND k.text RLIKE'^[<>][^ ]+ .*$',MID(k.text,INSTR(k.text,' ')+1),COALESCE(um.Einhstand,w.Einheit))Einheit,IF(w.Grenzwerti IN('','N'),'',w.Grenzwerti)obpath" & vbCrLf & _
...
",CONCAT('2:',DATE_FORMAT(eingang,'%e.%c.%y'),'|',w.Abkü,'|',w.Langtext,'|',Wert,IF(w.grenzwerti IN('','N'),'',CONCAT('(',w.Grenzwerti,')')),'|',w.Einheit,'|[',NB,'](',uNg,'-',oNg,')|',l.Labor,'|',COALESCE(e.text,''),'|',COALESCE(k.text,''))COLLATE utf8mb4_german2_ci info" & vbCrLf & _
"FROM " & vorsil & "us u " & vbCrLf & _
...
"LEFT JOIN " & vorsil & "dat d ON d.datid=s.datid " & vbCrLf & _
"LEFT JOIN " & vorsil & "plab l ON l.id=s.labid " & vbCrLf & _
"LEFT JOIN laborparameter p ON p.abkü=w.abkü AND IF(p.einheit IN ('','\'kA\''),'kA',p.Einheit)=IF(w.einheit IN ('','\'kA\''),'kA',w.einheit)" & vbCrLf & _
"      AND p.id = (SELECT id FROM laborparameter WHERE abkü=w.`Abkü` AND IF(einheit IN ('','\'kA\''),'kA',Einheit)=IF(w.einheit IN ('','\'kA\''),'kA',w.Einheit) ORDER BY gruppe DESC, reihe DESC,id LIMIT 1)" & vbCrLf & _
...
"FROM " & vorsil & "bakt b " & vbCrLf & _
"LEFT JOIN " & vorsil & "us u ON b.usid=u.id " & vbCrLf & _
"LEFT JOIN " & vorsil & "hinw k ON b.kommid=k.id " & vbCrLf & _
"LEFT JOIN " & vorsil & "hinw e ON b.erklid=e.id " & vbCrLf & _
"LEFT JOIN " & vorsil & "hinw h ON b.hinwid=h.id"
'"LEFT JOIN " & vorsil & "saetze s ON u.satzid=s.satzid " & vbCrLf & _
'"LEFT JOIN " & vorsil & "dat d ON s.datid=d.datid;"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
"UNION SELECT eingang zp, CAST(keimzahl AS CHAR(16)) wert FROM `" & vorsil & "us` us " & vbCrLf & _
"LEFT JOIN `" & vorsil & "bakt` b ON us.id=b.usid WHERE pat_id = " & PID & " AND keimzahl<> '')  u2 ORDER BY zp DESC"
...
",w.Einheit, IF(w.Grenzwerti IN('','N'),'',w.Grenzwerti)Grenzwerti " & _
...
"FROM " & vorsil & "us u " & vbCrLf & _
"LEFT JOIN " & vorsil & "wert w ON u.id=w.usid " & vbCrLf & _
"LEFT JOIN " & vorsil & "hinw e ON w.erklid=e.id " & vbCrLf & _
"LEFT JOIN " & vorsil & "hinw k ON w.kommid=k.id " & vbCrLf & _
"LEFT JOIN " & vorsil & "pnb n ON w.nbid=n.id " & vbCrLf & _
"LEFT JOIN " & vorsil & "saetze s ON u.satzid=s.satzid " & vbCrLf & _
"LEFT JOIN " & vorsil & "dat d ON s.datid=d.datid " & vbCrLf & _
"LEFT JOIN " & vorsil & "plab l ON s.labid=l.id " & vbCrLf & _
"LEFT JOIN laborparameter p ON p.abkü=w.abkü AND IF(p.einheit IN ('','\'kA\''),'kA',p.Einheit)=IF(w.einheit IN ('','\'kA\''),'kA',w.einheit)" & vbCrLf & _
"      AND p.id = (SELECT id FROM laborparameter WHERE abkü=w.`Abkü` AND IF(einheit IN ('','\'kA\''),'kA',Einheit)=IF(w.einheit IN ('','\'kA\''),'kA',w.Einheit) ORDER BY gruppe DESC, reihe DESC,id LIMIT 1)"
...
'"LEFT JOIN `laborkommentar` k ON k.KommentarVW = n.kommentarvw  LEFT JOIN `laborparameter` p ON n.abkü = p.abkü AND n.einheit = p.einheit LEFT JOIN `" & vorsil & "pgl` gl ON p.id = gl.idpara LEFT JOIN `" & vorsil & "pneu` n2 ON gl.idypbez = n2.id  LEFT JOIN `" & vorsil & "pnb` nb2 ON n2.id = nb2.pneuid WHERE wert <> '' AND NOT ISNULL(wert)" ' AND nb2.geschlecht IN (3,9,0,IF(na.geschlecht='m',IF(ADDDATE(na.gebdat,INTERVAL 18 YEAR)>CAST(eingang As Date),1,4) ,IF(ADDDATE(na.gebdat,INTERVAL 18 YEAR)>CAST(eingang As Date),2,5)))
'Call DtbCreateQueryDef(VN, Vsql)
'vz = vz + 1
#End If

VN = "labor1a"
'Vsql = "SELECT n.Pat_ID AS Pat_ID,CAST(n.ZeitPunkt As Date) AS ZeitPunkt,n.FertigStGrad AS FertigStGrad,n.Abkü AS Abkü,l.Langtext AS Langtext,n.Wert AS Wert,n.Einheit AS Einheit,k.Kommentar AS Kommentar,_utf8'' AS nb FROM (`laborneu` n LEFT JOIN `laborlangtext` l ON l.langtextvw = n.langtextvw) LEFT JOIN laborkommentar k ON k.KommentarVW = n.kommentarvw WHERE wert <> '' AND NOT ISNULL(wert) AND NOT EXISTS(SELECT u.Pat_id AS _Pat_ID,CAST(u.Eingang As Date) AS _zeitpunkt,u.BefArt AS _FertigStGrad,w.Abkü AS _Abkü,w.Langname AS _Langtext,w.Wert AS _Wert,w.Einheit AS _Einheit,w.Kommentar AS _Kommentar,nb.NB AS _NB FROM `" & vorsil & "us` u LEFT JOIN " & vorsil & "wert w ON u.RefNr = w.RefNr LEFT JOIN `" & vorsil & "pnb` nb ON w.nbid = nb.id WHERE u.pat_id = n.pat_id AND w.abkü = n.abkü AND w.wert = n.wert AND CAST(n.zeitpunkt As Date) > SUBDATE(CAST(u.Eingang As Date),INTERVAL 3 DAY) AND CAST(n.zeitpunkt As Date) < ADDDATE(CAST(u.Eingang As Date),INTERVAL 6 DAY) )"
'Vsql = "SELECT n.Pat_ID AS Pat_ID,CAST(n.ZeitPunkt As Date) AS ZeitPunkt,n.FertigStGrad AS FertigStGrad,n.Abkü AS Abkü,l.Langtext AS Langtext,n.Wert AS Wert,n.Einheit AS Einheit,k.Kommentar AS Kommentar, CONCAT(IF(na.geschlecht='m' OR ISNULL(uNw),uNm,uNw),'-',IF(na.geschlecht='m' OR ISNULL(oNw),oNm,oNw)) NB, IF(na.geschlecht='m' OR ISNULL(uNw),uNm,uNw) uNg, IF(na.geschlecht='m' OR ISNULL(oNw),oNm,oNw) oNg, 'TM' Labor FROM (`laborneu` n LEFT JOIN `namen` na ON n.pat_id = na.pat_id LEFT JOIN `laborlangtext` l ON l.langtextvw = n.langtextvw) LEFT JOIN laborkommentar k ON k.KommentarVW = n.kommentarvw  LEFT JOIN laborparameter p ON n.abkü = p.abkü AND n.einheit = p.einheit WHERE wert <> '' AND NOT ISNULL(wert)" & _
 " AND NOT EXISTS(SELECT 0 FROM `" & vorsil & "us` u LEFT JOIN `" & vorsil & "wert` w ON u.RefNr = w.RefNr LEFT JOIN `" & vorsil & "pnb` nb ON w.nbid = nb.id WHERE u.pat_id = n.pat_id AND w.abkü = n.abkü AND w.wert = n.wert AND CAST(n.zeitpunkt As Date) > SUBDATE(CAST(u.Eingang As Date),INTERVAL 3 DAY) AND CAST(n.zeitpunkt As Date) < ADDDATE(CAST(u.Eingang As Date),INTERVAL 6 DAY) )"
...
"LEFT JOIN `laborkommentar` k ON k.KommentarVW = n.kommentarvw  LEFT JOIN `laborparameter` p ON n.abkü = p.abkü AND n.einheit = p.einheit AND l.langtext = p.langtext LEFT JOIN `" & vorsil & "pgl` gl ON p.id = gl.idpara LEFT JOIN `" & vorsil & "pneu` n2 ON gl.idxpbez = n2.id  LEFT JOIN `" & vorsil & "pnb` nb2 ON n2.id = nb2.pid AND nb2.geschlecht IN (3,9,0,IF(na.geschlecht='m',IF(ADDDATE(na.gebdat,INTERVAL 18 YEAR)>CAST(eingang As Date),1,4) ,IF(ADDDATE(na.gebdat,INTERVAL 18 YEAR)>CAST(eingang As Date),2,5))) WHERE wert <> '' AND NOT ISNULL(wert) AND NOT EXISTS(SELECT 0 FROM `" & vorsil & "us` u LEFT JOIN `" & vorsil & "wert` w ON u.RefNr = w.RefNr LEFT JOIN `" & vorsil & "pnb` nb ON w.nbid = nb.id WHERE u.pat_id = n.pat_id AND w.abkü = n.abkü AND w.wert = n.wert AND CAST(n.zeitpunkt As Date) > SUBDATE(CAST(u.Eingang As Date),INTERVAL 3 DAY) AND CAST(n.zeitpunkt As Date) < ADDDATE(CAST(u.Eingang As Date),INTERVAL 6 DAY))"
...
"LEFT JOIN `" & vorsil & "pgl` gl ON p.id = gl.idpara " & vbCrLf & _
"LEFT JOIN `" & vorsil & "pneu` n2 ON gl.idypbez = n2.id  " & vbCrLf & _
"LEFT JOIN `" & vorsil & "pnb` nb2 ON n2.id = nb2.pid AND nb2.geschlecht IN (3,9,0,IF(na.geschlecht='m',IF(ADDDATE(na.gebdat,INTERVAL 18 YEAR)>CAST(eingang As Date),1,4) ,IF(ADDDATE(na.gebdat,INTERVAL 18 YEAR)>CAST(eingang As Date),2,5))) " & vbCrLf & _
"WHERE wert <> '' AND NOT ISNULL(wert) AND NOT EXISTS(SELECT 0 FROM `" & vorsil & "us` u " & vbCrLf & _
    "LEFT JOIN `" & vorsil & "wert` w ON u.id = w.usid " & vbCrLf & _
    "LEFT JOIN `" & vorsil & "pnb` nb ON w.nbid = nb.id WHERE u.pat_id = n.pat_id AND w.abkü = n.abkü AND w.wert = n.wert " & vbCrLf & _
            "AND CAST(n.zeitpunkt As Date) > SUBDATE(CAST(u.Eingang As Date),INTERVAL 3 DAY) AND CAST(n.zeitpunkt As Date) < ADDDATE(CAST(u.Eingang As Date),INTERVAL 6 DAY)) AND (ISNULL(n2.eingang) OR n2.eingang = (SELECT MAX(eingang) FROM " & vorsil & "pnb WHERE pid = n2.id))"
...
"              LEFT JOIN `" & vorsil & "pgl` `gl` on((`p`.`id` = `gl`.`idpara`))) " & _
"              LEFT JOIN `" & vorsil & "pneu` `n2` on((`gl`.`idxpbez` = `n2`.`id`))) " & _
"              LEFT JOIN `" & vorsil & "pnb` `nb2` on(((`n2`.`id` = `nb2`.`pid`) AND (`nb2`.`Geschlecht` IN (3,9,0,IF((`na`.`Geschlecht` = 'm'),IF(((`na`.`GebDat` + INTERVAL 18 YEAR) > CAST(`nb2`.`Eingang` As Date)),1,4),IF(((`na`.`GebDat` + INTERVAL 18 YEAR) > CAST(`nb2`.`Eingang` As Date)),2,5))))))) " & _
...
"  AND (ISNULL(`nb2`.`Eingang`) OR (`nb2`.`Eingang` = (SELECT MAX(`" & vorsil & "pnb`.`Eingang`) FROM `" & vorsil & "pnb` WHERE (`" & vorsil & "pnb`.`pid` = `n2`.`id`)))) " & _
...
If LVobMySQL Then
 ...
 "WHERE schgr NOT IN(89,90) AND NOT goäkatnr IN ('40','41') AND n.nachname <> 'Bereitschaftsdienst' AND quartal = (SELECTmy  CONCAT(intacc(((month(SUBDATE(NOW(),INTERVAL " & frist & " DAY))-1) divmy 3) + 1) ¡ YEAR(SUBDATE(NOW(),INTERVAL " & frist & " DAY)) ) AS `lq`) " & vbCrLf & _
...
If LVobMySQL Then
 ...
 Vsql = "SELECT pat_id,fid,schgr,vknr,ik,goäkatnr,lanrid,quartal,kid " & vbCrLf & _
        ...
        "WHERE schgr NOT IN(89,90) AND NOT goäkatnr IN ('40','41')" & vbCrLf & _
...
' Vsql = "SELECT f.*,(SELECT MAX(kateg) FROM kassenliste WHERE id=f.kid) kateg" & vbCrLf & _
        ...
        "GROUP BY pat_id ORDER BY pat_id;"
Vsql = "SELECT f.pat_id,f.fid,f.schgr,f.vknr,f.ik,f.goäkatnr,f.lanrid,f.quartal,f.kid,MAX(k.Kateg)kateg" & vbCrLf & _
       "FROM aktf f" & vbCrLf & _
       "LEFT JOIN kassenliste k ON k.ID = f.kid" & vbCrLf & _
       "WHERE f.quartal = quartal(current_timestamp() - interval " & frist & " day)" & vbCrLf & _
...
'         "AND quartal = quartal(SUBDATE(NOW(), INTERVAL " & fristS & " DAY)) " & _
 ...
 "  AND schgr NOT IN(89,90) AND goäkatnr NOT IN('40','41')" & vbCrLf & _
 "  AND nachname<>'Bereitschaftsdienst'" & vbCrLf & _
 ...
 sql = "DROP FUNCTION IF EXISTS `zuht`"
 myEFrag (sql)

' "-- SET buch=CONVERT(MID(ursp,pos,1)USING LATIN1);" & vbCrLf &
sql = "CREATE DEFINER=`praxis`@`%` FUNCTION `zuht`(" & vbCrLf & _
"    `ursp` TEXT," & vbCrLf & _
"    `mitabsatz` BIT," & vbCrLf & _
"    `obgross` BIT" & vbCrLf & _
")" & vbCrLf & _
"RETURNS varchar(10000) CHARSET utf8mb3 COLLATE utf8mb3_unicode_ci" & vbCrLf & _
"Language sql" & vbCrLf & _
...
"DECLARE ASCi INTEGER(2);" & vbCrLf & _
"DECLARE zul13 BIT;" & vbCrLf & _
"DECLARE maxp INTEGER(11) DEFAULT LENGTH(ursp);" & vbCrLf & _
"parse: WHILE pos <= maxp DO" & vbCrLf & _
" SET buch=MID(ursp,pos,1);" & vbCrLf & _
" SET ASCi=ASCII(buch);" & vbCrLf & _
" SET erg=CONCAT(erg,"
sql = sql & _
"  CASE" & vbCrLf & _
"   WHEN ASCI=13 AND pos=maxp then ''" & vbCrLf & _
"   WHEN ASCi=13 THEN CONCAT('</w:t>',IF(mitabsatz,CONCAT('</w:r></w:p><w:p><w:pPr><w:pStyle w:val=""""hang""""/></w:pPr><w:r><w:rPr><w:rStyle w:val=""""s',IF(obgross,'18','24'),'s""""/></w:rPr>'),'<w:br/>'),'<w:t>')" & vbCrLf & _
"   WHEN ASCI=10 AND zul13 THEN ''" & vbCrLf & _
"   WHEN ASCI IN (38,60,62) OR ASCI>=128 THEN CONCAT('&#',ASCI,';')" & vbCrLf & _
"   Else buch" & vbCrLf & _
" END);" & vbCrLf & _
" IF ASCi=13 then SET zul13=1; ELSE SET zul13=0;END IF;" & vbCrLf & _
" SET pos=pos+1;" & vbCrLf & _
"END WHILE;" & vbCrLf & _
"RETURN erg;" & vbCrLf & _
"End"
...
Vsql = "SELECT n.pat_id, IF(l2.eingang>l1.zeitpunkt OR ISNULL(l1.zeitpunkt),l2.eingang,l1.zeitpunkt) lzp, " & vbCrLf & _
  ...
  "LEFT JOIN (SELECT eingang, wert, abkü, pat_id FROM " & vorsil & "us u INNER JOIN " & vorsil & "wert w ON w.usid = u.id AND w.abkü RLIKE '^hba[1c]' " & vbCrLf & _
       "AND u.eingang = (SELECT MAX(eingang) FROM " & vorsil & "wert wi INNER JOIN " & vorsil & "us ui ON wi.usid = ui.id WHERE abkü RLIKE '^hba[1c]' AND pat_id=u.pat_id)) l2 ON l2.pat_id = n.pat_id " & vbCrLf & _
...
",IF(l2.zeitpunkt>l1.zeitpunkt OR ISNULL(l1.zeitpunkt),l2.wert,l2.wert) letzter FROM namen n " & _
  "LEFT JOIN (SELECT eingang zeitpunkt, wert, abkü, pat_id FROM " & vorsil & "us u INNER JOIN " & vorsil & "wert w ON w.usid = u.id AND " & krit & vbCrLf & _
       "AND u.eingang = (SELECT MAX(eingang) FROM " & vorsil & "wert wi INNER JOIN " & vorsil & "us ui ON wi.usid = ui.id WHERE " & krit & " AND pat_id=u.pat_id)) l2 ON l2.pat_id = n.pat_id " & vbCrLf & _
...
'Vsql = "SELECT n.pat_id," & _
       ...
       "(SELECT MIN(CONVERT(REPLACE(CONCAT('0',TRIM(w.wert)),',','.'),DECIMAL(9,2))) FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON w.usid = u.id WHERE u.pat_id = n.pat_id AND w.abkü RLIKE '^hba[1c]' AND CONVERT(REPLACE(CONCAT('0',TRIM(w.wert)),',','.'),DECIMAL(9,2))<21 AND w.wert RLIKE '^[0-9 .,]*$' AND eingang > CAST((SELECT CONCAT(YEAR(SUBDATE(NOW(),INTERVAL 200 DAY)), '-',((month( SUBDATE(NOW(),INTERVAL 200 DAY) )-1) div 3)*3 + 1,'-01')) As Date)) min2 " & _
       "FROM namen n"
Vsql = "SELECT pat_id" & vbCrLf & _
    ...
    ",(SELECT MIN(CAST(wert AS DECIMAL(9,2))) FROM " & vorsil & "wert w INNER JOIN " & vorsil & "us u ON w.usid = u.id WHERE pat_id = n.pat_id AND abkü RLIKE '^hba[1c]' AND wert REGEXP '^[0-9]+\\.?[0-9]*$' AND wert<21) min2" & vbCrLf & _
    ...
    ",(SELECT MAX(CAST(wert AS DECIMAL(9,2))) FROM " & vorsil & "wert w INNER JOIN " & vorsil & "us u ON w.usid = u.id WHERE pat_id = n.pat_id AND abkü RLIKE '^hba[1c]' AND wert REGEXP '^[0-9]+\\.?[0-9]*$' AND wert<21) max2" & vbCrLf & _
    "FROM namen n"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
'Vsql = "SELECT n.pat_id," & _
       ...
       "(SELECT MAX(CONVERT(REPLACE(CONCAT('0',TRIM(w.wert)),',','.'),DECIMAL(9,2))) FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON w.usid = u.id WHERE u.pat_id = n.pat_id AND w.abkü RLIKE '^hba[1c]' AND CONVERT(REPLACE(CONCAT('0',TRIM(w.wert)),',','.'),DECIMAL(9,2))<21 AND w.wert RLIKE '^[0-9 .,]*$') w2 " & _
       "FROM namen n"
Vsql = "SELECT n.pat_id" & _
       ...
       ",(SELECT MAX(CAST(wert AS DECIMAL(9,2))) FROM " & vorsil & "wert w INNER JOIN " & vorsil & "us u ON w.usid = u.id WHERE pat_id = n.pat_id AND abkü RLIKE '^hba[1c]' AND wert REGEXP '^[0-9]+\\.?[0-9]*$' AND (wert<20 OR (wert<21 AND einheit='%'))) max2" & vbCrLf & _
       "FROM namen n"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
'Vsql = "SELECT n.pat_id," & _
       ...
       "(SELECT MAX(CONVERT(REPLACE(CONCAT('0',TRIM(w.wert)),',','.'),DECIMAL(9,2))) FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON w.usid = u.id WHERE u.pat_id = n.pat_id AND w.abkü RLIKE '^glu' AND w.einheit = 'mg/dl') w2 " & _
       "FROM namen n"
Vsql = "SELECT n.pat_id" & _
       ...
       ",(SELECT MAX(CAST(wert AS DECIMAL(9,2))) FROM " & vorsil & "wert w INNER JOIN " & vorsil & "us u ON w.usid = u.id WHERE pat_id = n.pat_id AND abkü RLIKE '^glu' AND wert REGEXP '^[0-9]+\\.?[0-9]*$' AND einheit='mg/dl') max2" & vbCrLf & _
       "FROM namen n"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
Vsql = "SELECT pat_id" & vbCrLf & _
    ...
    ",(SELECT MIN(CAST(wert AS DECIMAL(9,2))) FROM " & vorsil & "wert w INNER JOIN " & vorsil & "us u ON w.usid = u.id  WHERE pat_id = n.pat_id AND abkü IN ('albcre','albkre','albq','album','albup') AND wert REGEXP '^[0-9]+\\.?[0-9]*$' AND langtext LIKE '%alb%' AND einheit LIKE 'mg/g%' AND eingang > CAST((SELECT CONCAT(YEAR(SUBDATE(NOW(),INTERVAL 200 DAY)), '-',((month( SUBDATE(NOW(),INTERVAL 200 DAY) )-1) div 3)*3 + 1,'-01')) As Date)) min2" & vbCrLf & _
    ...
    ",(SELECT MAX(CAST(wert AS DECIMAL(9,2))) FROM " & vorsil & "wert w INNER JOIN " & vorsil & "us u ON w.usid = u.id WHERE pat_id = n.pat_id AND abkü IN ('albcre','albkre','albq','album','albup') AND wert REGEXP '^[0-9]+\\.?[0-9]*$' AND langtext LIKE '%alb%' AND einheit LIKE 'mg/g%' AND eingang > CAST((SELECT CONCAT(YEAR(SUBDATE(NOW(),INTERVAL 200 DAY)), '-',((month( SUBDATE(NOW(),INTERVAL 200 DAY) )-1) div 3)*3 + 1,'-01')) As Date)) max2" & vbCrLf & _
    "FROM namen n"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
Vsql = "SELECT pat_id, geschlecht" & vbCrLf & _
    ...
    ",(SELECT MIN(CAST(wert AS DECIMAL(9,2))) FROM " & vorsil & "wert w INNER JOIN " & vorsil & "us u ON w.usid = u.id WHERE pat_id = n.pat_id AND abkü IN ('kre02','creat','krea','krea02') AND wert REGEXP '^[0-9]+\\.?[0-9]*$' AND NOT langtext LIKE '%urin%' AND einheit = 'mg/dl' AND eingang > CAST((SELECT CONCAT(YEAR(SUBDATE(NOW(),INTERVAL 200 DAY)), '-',((month( SUBDATE(NOW(),INTERVAL 200 DAY) )-1) div 3)*3 + 1,'-01')) As Date)) min2" & vbCrLf & _
    ",(SELECT MAX(CAST(wert AS DECIMAL(9,2))) FROM labor1a WHERE pat_id = n.pat_id AND abkü IN ('kre02','creat','krea','krea02') AND wert REGEXP '^[0-9]+\\.?[0-9]*$' AND NOT langtext LIKE '%urin%' AND einheit = 'mg/dl' AND zeitpunkt > CAST((SELECT CONCAT(YEAR(SUBDATE(NOW(),INTERVAL 200 DAY)), '-',((month( SUBDATE(NOW(),INTERVAL 200 DAY) )-1) div 3)*3 + 1,'-01')) As Date)) max1" & vbCrLf & _
    ",(SELECT MAX(CAST(wert AS DECIMAL(9,2))) FROM " & vorsil & "wert w INNER JOIN " & vorsil & "us u ON w.usid = u.id WHERE pat_id = n.pat_id AND abkü IN ('kre02','creat','krea','krea02') AND wert REGEXP '^[0-9]+\\.?[0-9]*$' AND NOT langtext LIKE '%urin%' AND einheit = 'mg/dl' AND eingang > CAST((SELECT CONCAT(YEAR(SUBDATE(NOW(),INTERVAL 200 DAY)), '-',((month( SUBDATE(NOW(),INTERVAL 200 DAY) )-1) div 3)*3 + 1,'-01')) As Date)) max2" & vbCrLf & _
    "FROM namen n"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
' vielleicht auch nicht mehr, am 27.12.25 mit und ohne 92311 Einträge
Vsql = "SELECT * FROM medplan mp " & vbCrLf & _
       "WHERE mpnr=(SELECT mpnr FROM medplan WHERE pat_id=mp.Pat_ID ORDER BY pat_id, zeitpunkt DESC,mpnr DESC LIMIT 1)"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
Vsql = _
"SELECT mp.*" & vbCrLf & _
",RANK()OVER(PARTITION BY mp.pat_id ORDER BY mp.mpnr DESC,mp.zeitpunkt DESC,mp.mpnr DESC)rni" & vbCrLf & _
",RANK()OVER(PARTITION BY mp.pat_id ORDER BY mp.mpnr,mp.zeitpunkt,mp.mpnr)rnr" & vbCrLf & _
"FROM (" & vbCrLf & _
" SELECT mp.*" & vbCrLf & _
",RANK()OVER(PARTITION BY mp.pat_id,DATE(mp.zeitpunkt) ORDER BY mp.mpnr DESC, mp.zeitpunkt DESC)rang" & vbCrLf & _
" FROM medplan mp" & vbCrLf & _
")mp WHERE rang=1"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
VN = "_qsumme"
Vsql = "SELECT (COUNT(0) * IF((`e`.`Euro` = 18.75 OR e.euro = 19.05 OR euro = 14.25) AND LEFT(e.ziffer,1)='9',75,`e`.`Euro`)) AS `wert`,COUNT(0) AS `zahl`,`e`.`Euro` AS `euro`,`f`.`Quartal` AS `quartal` FROM ((`faelle` `f` LEFT JOIN `leistungen` `l` on((`f`.`FID` = `l`.`FID`))) LEFT JOIN `EBM2010` `e` on(((`l`.`Leistung` = `e`.`Ziffer`) AND (`e`.`Leistungstext` <> '')))) WHERE ((`f`.`SchGr` NOT IN (89,90)) AND (`l`.`Leistung` <> '00000') AND (`e`.`Euro` IS NOT NULL)) GROUP BY `l`.`Leistung`,`f`.`Quartal`"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
VN = "fallzahlstand_1"
Vsql = "SELECT COUNT(0) AS `zahl`,(COUNT(0) - COUNT(`f`.`erst`)) AS `z0`,`f`.`Quartal` AS `quartal` FROM `faelleverschiedenneu` `f` WHERE ((`f`.`SchGr` NOT IN(89,90)) AND ((to_days(`f`.`Fanf`) - to_days(CONCAT(SUBSTR(`f`.`Quartal`,2,4),'-',(((LEFT(`f`.`Quartal`,1) - 1) * 3) + 1),'-01'))) BETWEEN 0 AND (to_days((NOW() - INTERVAL 0 DAY)) - to_days(CONCAT(YEAR((NOW() - INTERVAL 0 DAY)),'-',((((month((NOW() - INTERVAL 0 DAY)) - 1) DIV 3) * 3) + 1),'-01')))) AND ((`f`.`Pat_ID` < 3044) OR (`f`.`Pat_ID` > 50000))) GROUP BY `f`.`Quartal` ORDER BY SUBSTR(`f`.`Quartal`,2,4),LEFT(`f`.`Quartal`,1)"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
'VN = "test0"
'Vsql = "SELECT `u`.`Pat_id` AS `Pat_ID`,CAST(`u`.`Eingang` As Date) AS `zeitpunkt`,`u`.`BefArt` AS `FertigStGrad`,IF(ISNULL(`n2`.`Abkü`),`w`.`Abkü`,`n2`.`Abkü`) AS `Abkü`,`w`.`Abkü` AS `abk_ur`,`w`.`Langtext` AS `Langtext`,`w`.`Wert` AS `Wert`,IF(ISNULL(`n2`.`Abkü`),`w`.`Einheit`,`n2`.`Einheit`) AS `Einheit`,`w`.`Einheit` AS `Einheit_ur`,CONCAT(IF((erkl.text REGEXP '^:[ /*:]*$'),'',IF((erkl.text REGEXP '^:[ /*]*:'),CONCAT(SUBSTR(erkl.text,(locate(':',erkl.text,2) + 1)),';'),IF((erkl.text = '.'),'',IF((erkl.text = ''),'',CONCAT(erkl.text,';'))))),`w`.`Kommentar`) AS `Kommentar`,IF(ISNULL(`n2`.`Abkü`),`nb`.`NB`,`nb2`.`NB`) AS `NB`,`nb`.`NB` AS `NB_ur`,IF(ISNULL(`n2`.`Abkü`),`nb`.`uNg`,`nb2`.`uNg`) AS `uNg`,`nb`.`uNg` AS `uNg_ur`,IF(ISNULL(`n2`.`Abkü`),`nb`.`oNg`,`nb2`.`oNg`) AS `oNg`,`nb`.`oNg` AS `oNg_ur`,`s`.`Labor` AS `Labor` FROM `" & vorsil & "us` `u` LEFT JOIN `namen` `na` ON `u`.`Pat_id` = `na`.`Pat_ID` LEFT JOIN `" & vorsil & "wert` `w` " & _
'"on `u`.`id` = `w`.`usid` LEFT JOIN `" & vorsil & "pnb` `nb` ON `w`.`nbid` = `nb`.`id` LEFT JOIN `" & vorsil & "pneu` `neu` ON `nb`.`pid` = `neu`.`id` LEFT JOIN `" & vorsil & "pgl` `gl` ON `neu`.`id` = `gl`.`idxpneu` LEFT JOIN `" & vorsil & "pneu` `n2` ON `gl`.`idxpbez` = `n2`.`id` LEFT JOIN `" & vorsil & "pnb` `nb2` ON `n2`.`id` = `nb2`.`pid` AND `nb2`.`Geschlecht` IN (3,9,0,IF((`na`.`Geschlecht` = 'm'),IF(((`na`.`GebDat` + INTERVAL 18 YEAR) > CAST(`u`.`Eingang` As Date)),1,4),IF(((`na`.`GebDat` + INTERVAL 18 YEAR) > CAST(`u`.`Eingang` As Date)),2,5))) LEFT JOIN `" & vorsil & "saetze` `s` ON `u`.`SatzID` = `s`.`SatzID`  LEFT JOIN " & vorsil & "hinw erkl ON erkl.id=w.erklid WHERE (`u`.`Pat_id` = 14)"
'Call DtbCreateQueryDef(VN, Vsql)
'vz = vz + 1

'VN = "test2"
'Vsql = "SELECT `nb2`.`Eingang` AS `eingang`,`u`.`Pat_id` AS `Pat_ID`,CAST(`u`.`Eingang` As Date) AS `zeitpunkt`,`u`.`BefArt` AS `FertigStGrad`,IF(ISNULL(`n2`.`Abkü`),`w`.`Abkü`,`n2`.`Abkü`) AS `Abkü`,`w`.`Abkü` AS `abk_ur`,`w`.`Langtext` AS `Langtext`,`w`.`Wert` AS `Wert`,IF(ISNULL(`n2`.`Abkü`),`w`.`Einheit`,`n2`.`Einheit`) AS `Einheit`,`w`.`Einheit` AS `Einheit_ur`,CONCAT(IF((erkl.text REGEXP '^:[ /*:]*$'),'',IF((erkl.text REGEXP '^:[ /*]*:'),CONCAT(SUBSTR(erkl.text,(locate(':',erkl.text,2) + 1)),';'),IF((erkl.text = '.'),'',IF((erkl.text = ''),'',CONCAT(erkl.text,';'))))),`w`.`Kommentar`) AS `Kommentar`,IF(ISNULL(`n2`.`Abkü`),`nb`.`NB`,`nb2`.`NB`) AS `NB`,`nb`.`NB` AS `NB_ur`,IF(ISNULL(`n2`.`Abkü`),`nb`.`uNg`,`nb2`.`uNg`) AS `uNg`,`nb`.`uNg` AS `uNg_ur`,IF(ISNULL(`n2`.`Abkü`),`nb`.`oNg`,`nb2`.`oNg`) AS `oNg`,`nb`.`oNg` AS `oNg_ur`,`s`.`Labor` AS `Labor` FROM ((((((((`" & vorsil & "us` `u` LEFT JOIN `namen` `na` on((`u`.`Pat_id` = `na`.`Pat_ID`))) " & _
'"LEFT JOIN `" & vorsil & "wert` `w` on((`u`.`id` = `w`.`usid`))) LEFT JOIN `" & vorsil & "pnb` `nb` on((`w`.`nbid` = `nb`.`id`))) LEFT JOIN `" & vorsil & "pneu` `neu` on((`nb`.`pid` = `neu`.`id`))) LEFT JOIN `" & vorsil & "pgl` `gl` on((`neu`.`id` = `gl`.`idxpneu`))) LEFT JOIN `" & vorsil & "pneu` `n2` on((`gl`.`idxpbez` = `n2`.`id`))) LEFT JOIN `" & vorsil & "pnb` `nb2` on(((`n2`.`id` = `nb2`.`pid`) AND (`nb2`.`Geschlecht` IN (3,9,0,IF((`na`.`Geschlecht` = 'm'),IF(((`na`.`GebDat` + INTERVAL 18 YEAR) > CAST(`u`.`Eingang` As Date)),1,4),IF(((`na`.`GebDat` + INTERVAL 18 YEAR) > CAST(`u`.`Eingang` As Date)),2,5))))))) LEFT JOIN `" & vorsil & "saetze` `s` on((`u`.`SatzID` = `s`.`SatzID`))) WHERE ((`u`.`Pat_id` = 14) AND (`n2`.`Abkü` = 'alkp02') AND (`nb2`.`Eingang` = (SELECT MAX(`" & vorsil & "pnb`.`Eingang`) FROM `" & vorsil & "pnb` WHERE (`" & vorsil & "pnb`.`pid` = `n2`.`id`))))  LEFT JOIN " & vorsil & "hinw erkl ON erkl.id=w.erklid GROUP BY `w`.`usid`,`w`.`Abkü` ORDER BY `nb2`.`Eingang` DESC"
'Call DtbCreateQueryDef(VN, Vsql)
'vz = vz + 1

'VN = "test2"
'Vsql = "SELECT `nb2`.`Eingang` AS `eingang`,`u`.`Pat_id` AS `Pat_ID`,CAST(`u`.`Eingang` As Date) AS `zeitpunkt`,`u`.`BefArt` AS `FertigStGrad`,IF(ISNULL(`n2`.`Abkü`),`w`.`Abkü`,`n2`.`Abkü`) AS `Abkü`,`w`.`Abkü` AS `abk_ur`,`w`.`Langtext` AS `Langtext`,`w`.`Wert` AS `Wert`,IF(ISNULL(`n2`.`Abkü`),`w`.`Einheit`,`n2`.`Einheit`) AS `Einheit`,`w`.`Einheit` AS `Einheit_ur`,CONCAT(IF((`w`.`Erklärung` REGEXP '^:[ /*:]*$'),'',IF((`w`.`Erklärung` REGEXP '^:[ /*]*:'),CONCAT(SUBSTR(`w`.`Erklärung`,(locate(':',`w`.`Erklärung`,2) + 1)),';'),IF((`w`.`Erklärung` = '.'),'',IF((`w`.`Erklärung` = ''),'',CONCAT(`w`.`Erklärung`,';'))))),`w`.`Kommentar`) AS `Kommentar`,IF(ISNULL(`n2`.`Abkü`),`nb`.`NB`,`nb2`.`NB`) AS `NB`,`nb`.`NB` AS `NB_ur`,IF(ISNULL(`n2`.`Abkü`),`nb`.`uNg`,`nb2`.`uNg`) AS `uNg`,`nb`.`uNg` AS `uNg_ur`,IF(ISNULL(`n2`.`Abkü`),`nb`.`oNg`,`nb2`.`oNg`) AS `oNg`,`nb`.`oNg` AS `oNg_ur`,`s`.`Labor` AS `Labor` FROM ((((((((`" & vorsil & "us` `u` LEFT JOIN `namen` `na` on((`u`.`Pat_id` = `na`.`Pat_ID`))) " & _
'"LEFT JOIN `" & vorsil & "wert` `w` on((`u`.`id` = `w`.`usid`))) LEFT JOIN `" & vorsil & "pnb` `nb` on((`w`.`nbid` = `nb`.`id`))) LEFT JOIN `" & vorsil & "pneu` `neu` on((`nb`.`pid` = `neu`.`id`))) LEFT JOIN `" & vorsil & "pgl` `gl` on((`neu`.`id` = `gl`.`idxpneu`))) LEFT JOIN `" & vorsil & "pneu` `n2` on((`gl`.`idxpbez` = `n2`.`id`))) LEFT JOIN `" & vorsil & "pnb` `nb2` on(((`n2`.`id` = `nb2`.`pid`) AND (`nb2`.`Geschlecht` IN (3,9,0,IF((`na`.`Geschlecht` = 'm'),IF(((`na`.`GebDat` + INTERVAL 18 YEAR) > CAST(`u`.`Eingang` As Date)),1,4),IF(((`na`.`GebDat` + INTERVAL 18 YEAR) > CAST(`u`.`Eingang` As Date)),2,5))))))) LEFT JOIN `" & vorsil & "saetze` `s` on((`u`.`SatzID` = `s`.`SatzID`))) WHERE ((`u`.`Pat_id` = 14) AND (`n2`.`Abkü` = 'alkp02') AND (`nb2`.`Eingang` = (SELECT MAX(`" & vorsil & "pnb`.`Eingang`) FROM `" & vorsil & "pnb` WHERE (`" & vorsil & "pnb`.`pid` = `n2`.`id`)))) GROUP BY `w`.`usid`,`w`.`Abkü` ORDER BY `nb2`.`Eingang` DESC"
'Call DtbCreateQueryDef(VN, Vsql)
'vz = vz + 1
...
myEFrag ("DROP FUNCTION IF EXISTS `pidueb`")
sql = "CREATE DEFINER=`praxis`@`%` FUNCTION `pidueb`( pid INT(6) UNSIGNED) RETURNS INT(6) UNSIGNED " & vbCrLf & _
 "COMMENT 'liefert MO-Pat_ID aus TM-Pat_ID' " & vbCrLf & _
 "BEGIN" & vbCrLf & _
 "  CASE" & Chr$(13) & _
 ...
 "   WHEN pid=70180 THEN RETURN 70275;" & vbCrLf & _
 "   WHEN pid=70239 THEN RETURN 70277;" & vbCrLf & _
 "   WHEN pid=70245 THEN RETURN 70279;" & vbCrLf & _
 "   WHEN pid=70246 THEN RETURN 70280;" & vbCrLf & _
 "   WHEN pid=70247 THEN RETURN 70281;" & vbCrLf & _
 "   WHEN pid=70252 THEN RETURN 70286;" & vbCrLf & _
 "   WHEN pid=70254 THEN RETURN 70288;" & vbCrLf & _
 "   WHEN pid=70256 THEN RETURN 70290;" & vbCrLf & _
 "   WHEN pid=70261 THEN RETURN 70294;" & vbCrLf & _
 "   WHEN pid=70262 THEN RETURN 70295;" & vbCrLf & _
 "   WHEN pid=70268 THEN RETURN 70300;" & vbCrLf & _
 "   WHEN pid=70270 THEN RETURN 70302;" & vbCrLf & _
 "   WHEN pid=70272 THEN RETURN 70304;" & vbCrLf & _
 "   WHEN pid=70274 THEN RETURN 70306;" & vbCrLf & _
 "   WHEN pid=70275 THEN RETURN 70307;" & vbCrLf
 sql = sql & _
 "   ELSE RETURN pid;" & vbCrLf & _
 "  END CASE;" & vbCrLf & _
 "END"
myEFrag (sql)

myEFrag ("DROP FUNCTION IF EXISTS `pidtm`")
sql = "CREATE DEFINER=`praxis`@`%` FUNCTION `pidtm`( fPatNr INT(6) UNSIGNED) RETURNS INT(6) UNSIGNED " & vbCrLf & _
 "COMMENT 'liefert TM-Pat_ID aus der MO-fPatNr' " & vbCrLf & _
 "BEGIN" & vbCrLf & _
 "  CASE" & Chr$(13) & _
 ...
 "   WHEN fPatNr=70277 THEN RETURN 70239;" & vbCrLf & _
 "   WHEN fPatNr=70279 THEN RETURN 70245;" & vbCrLf & _
 "   WHEN fPatNr=70280 THEN RETURN 70246;" & vbCrLf & _
 "   WHEN fPatNr=70281 THEN RETURN 70247;" & vbCrLf & _
 "   WHEN fPatNr=70286 THEN RETURN 70252;" & vbCrLf & _
 "   WHEN fPatNr=70288 THEN RETURN 70254;" & vbCrLf & _
 "   WHEN fPatNr=70290 THEN RETURN 70256;" & vbCrLf & _
 "   WHEN fPatNr=70294 THEN RETURN 70261;" & vbCrLf & _
 "   WHEN fPatNr=70295 THEN RETURN 70262;" & vbCrLf & _
 "   WHEN fPatNr=70300 THEN RETURN 70268;" & vbCrLf & _
 "   WHEN fPatNr=70302 THEN RETURN 70270;" & vbCrLf & _
 "   WHEN fPatNr=70304 THEN RETURN 70272;" & vbCrLf & _
 "   WHEN fPatNr=70306 THEN RETURN 70274;" & vbCrLf & _
 "   WHEN fPatNr=70307 THEN RETURN 70275;" & vbCrLf
 sql = sql & _
 "   ELSE RETURN fPatNr;" & vbCrLf & _
 "  END CASE;" & vbCrLf & _
 "END"
myEFrag (sql)
...
sql = sql & "   SET i=i+1;" & Chr$(13) & _
    ...
    "   WHEN inh RLIKE 'rabipur' THEN SET erg='16';" & vbCrLf & _
    "   WHEN inh RLIKE 'abrysvo|rsv|arexvy' THEN SET erg='17';"
    sql = sql & _
...
#End If
    ...
    ' ktag fehlerhaft
    Vsql = _
    " SELECT dt,zp,pat_id,fid,schgr,vknr,ik,lanrid,GROUP_CONCAT(Typ ORDER BY Typ SEPARATOR'')Typ,CONCAT(DATE_FORMAT(zp,'%e.%c.: '),GROUP_CONCAT(Art ORDER BY Art SEPARATOR ', '))Art FROM(" & _
    " SELECT dt,zp,pat_id,fid,schgr,vknr,ik,lanrid,Typ,GROUP_CONCAT(CONVERT(concat(Typ,': ',Art) USING UTF8MB4)COLLATE utf8mb4_german2_ci ORDER BY Art)Art FROM(" & vbCrLf & _
    "  SELECT DATE(e.zeitpunkt)dt,e.zeitpunkt zp,f.pat_id,f.fid,schgr,vknr,ik,f.lanrid,'E'Typ,GROUP_CONCAT(DISTINCT Art ORDER BY Art)Art" & vbCrLf & _
    "   FROM aktfvs f" & vbCrLf & _
    ...
    "  UNION " & vbCrLf & _
    "  SELECT DATE(k.zeitpunkt) dt, k.zeitpunkt zp,f.pat_id,f.fid,schgr,vknr,ik,f.lanrid,'F'Typ,GROUP_CONCAT(DISTINCT CONVERT(LEFT(formbez,40)USING UTF8MB4) COLLATE UTF8MB4_GERMAN2_CI)Art" & vbCrLf & _
    "   FROM aktfvs f " & vbCrLf & _
    "   JOIN forminhkopf k ON k.pat_id = f.pat_id " & vbCrLf & _
    "   JOIN formulare u ON k.form_id=u.formid " & vbCrLf & _
    "   AND k.zeitpunkt " & DiesQ() & vbCrLf & _
    "   AND u.form_abk NOT LIKE '%dmp%' AND u.form_abk NOT IN(1,3,4,2017)" & vbCrLf & _
    "   GROUP BY pat_id,DATE(k.zeitpunkt) " & vbCrLf
    ' 1,3,4,2017=Diagnosen
    ' ktag fehlerhaft
    Vsql = Vsql & _
    "  UNION " & vbCrLf & _
    "  SELECT DATE(l.zeitpunkt) dt, l.zeitpunkt zp,f.pat_id,f.fid,schgr,vknr,ik,f.lanrid,'L'Typ,GROUP_CONCAT(DISTINCT CAST(leistung AS CHAR COLLATE UTF8MB4_GERMAN2_CI))Art" & vbCrLf & _
    "   FROM aktfvs f " & vbCrLf & _
    "   JOIN leistungen l ON l.pat_id = f.pat_id " & vbCrLf & _
    "   AND l.zeitpunkt " & DiesQ() & vbCrLf & _
    "   AND NOT leistung IN('','0','88122')AND NOT leistung RLIKE '^([^0-9]|32|00|8690|\\rtf)'" & vbCrLf & _
    "   GROUP BY pat_id,DATE(l.zeitpunkt) " & vbCrLf & _
    "  UNION " & vbCrLf & _
    "  SELECT DATE(m.zeitpunkt) dt, m.zeitpunkt zp,f.pat_id,f.fid,schgr,vknr,ik,f.lanrid,'M'Typ,GROUP_CONCAT(DISTINCT CONVERT('MP'USING UTF8MB4)COLLATE utf8mb4_german2_ci)Art" & vbCrLf & _
    "   FROM aktfvs f " & vbCrLf & _
    "   JOIN medplan m ON m.pat_id = f.pat_id " & vbCrLf & _
    "   AND m.zeitpunkt " & DiesQ() & vbCrLf & _
    "   GROUP BY pat_id,DATE(m.zeitpunkt) " & vbCrLf
    Vsql = Vsql & _
    "   UNION" & vbCrLf & _
    "  SELECT DATE(r.zeitpunkt)dt,r.zeitpunkt zp,f.pat_id,f.fid,schgr,vknr,ik,f.lanrid,'R'Typ,GROUP_CONCAT(DISTINCT CONVERT(r.Medikament USING UTF8MB4)COLLATE utf8mb4_german2_ci)Art" & vbCrLf & _
    "   FROM aktfvs f" & vbCrLf & _
    "   JOIN rezepteintraege r on r.pat_id=f.pat_id" & vbCrLf & _
    "   AND r.zeitpunkt " & DiesQ() & vbCrLf & _
    "   GROUP BY pat_id,DATE(r.zeitpunkt) " & vbCrLf & _
    "  ) i GROUP BY pat_id,dt,i.typ,i.art" & vbCrLf & _
    " )i GROUP BY pat_id,dt" & vbCrLf & _
    "    ORDER BY pat_id,zp"
    ...
    "   JOIN eintraege e ON e.pat_id = f.pat_id " & vbCrLf & _
    "   AND e.zeitpunkt " & DiesQ() & vbCrLf & _
    "   AND art IN ('tb','tn','th'," & artSpezBerat & "," & artSpezEintr & "," & artSpezUS1 & ") AND NOT art IN ('re','ba','doppler','dop','duplex','dup','impf','inj','kva','tv','ufrag','wv','caro','colo','debr','fa','EKG','GPD','kv','LZRR','OAU','pa','pros','puls','rp','sono','ulcus','vac','ADL','284','bar'," & artSpezMA & ") AND art<>'tv'" & vbCrLf & _
    "   GROUP BY pat_id,DATE(e.zeitpunkt) " & vbCrLf & _
    ...
    VN = "aktfkvc" ' von Claude vorgeschlagen
Vsql = _
"WITH aktiv AS (" & vbCrLf & _
"  SELECT pat_id,fid,schgr,vknr,ik,lanrid,quartal" & vbCrLf & _
"  FROM aktf" & vbCrLf & _
"  WHERE vknr<>71800" & vbCrLf & _
"  GROUP BY pat_id" & vbCrLf & _
")," & vbCrLf & _
"roh AS (" & vbCrLf & _
"  SELECT DATE(e.zeitpunkt)dt,e.zeitpunkt zp,e.pat_id,'E'Typ,GROUP_CONCAT(DISTINCT Art ORDER BY Art)Art" & vbCrLf & _
"   FROM eintraege e JOIN aktiv a ON a.pat_id=e.pat_id" & vbCrLf & _
"   WHERE e.zeitpunkt " & DiesQ() & vbCrLf & _
"   AND art IN(" & artspezG & ") AND art NOT IN('tv','bd','ts')" & vbCrLf & _
"   GROUP BY e.pat_id,DATE(e.zeitpunkt)" & vbCrLf & _
"  UNION ALL" & vbCrLf & _
"  SELECT DATE(k.zeitpunkt)dt,k.zeitpunkt zp,k.pat_id,'F'Typ,GROUP_CONCAT(DISTINCT CONVERT(LEFT(formbez,40)USING UTF8MB4)COLLATE UTF8MB4_GERMAN2_CI)Art" & vbCrLf & _
"   FROM forminhkopf k JOIN formulare u ON k.form_id=u.formid JOIN aktiv a ON a.pat_id=k.pat_id" & vbCrLf & _
"   WHERE k.zeitpunkt " & DiesQ() & vbCrLf & _
"   AND u.form_abk NOT LIKE '%dmp%' AND u.form_abk NOT IN(1,3,4,2017)" & vbCrLf & _
"   GROUP BY k.pat_id,DATE(k.zeitpunkt)" & vbCrLf & _
"  UNION ALL" & vbCrLf
Vsql = Vsql & _
"  SELECT DATE(l.zeitpunkt)dt,l.zeitpunkt zp,l.pat_id,'L'Typ,GROUP_CONCAT(DISTINCT CAST(leistung AS CHAR COLLATE UTF8MB4_GERMAN2_CI))Art" & vbCrLf & _
"   FROM leistungen l JOIN aktiv a ON a.pat_id=l.pat_id" & vbCrLf & _
"   WHERE l.zeitpunkt " & DiesQ() & vbCrLf & _
"   AND NOT leistung IN('','0','88122') AND NOT leistung RLIKE '^([^0-9]|32|00|8690|\\rtf)'" & vbCrLf & _
"   GROUP BY l.pat_id,DATE(l.zeitpunkt)" & vbCrLf & _
"  UNION ALL" & vbCrLf & _
"  SELECT DATE(m.zeitpunkt)dt,m.zeitpunkt zp,m.pat_id,'M'Typ,'MP'Art" & vbCrLf & _
"   FROM medplan m JOIN aktiv a ON a.pat_id=m.pat_id" & vbCrLf & _
"   WHERE m.zeitpunkt " & DiesQ() & vbCrLf & _
"   GROUP BY m.pat_id,DATE(m.zeitpunkt)" & vbCrLf & _
"  UNION ALL" & vbCrLf & _
"  SELECT DATE(r.zeitpunkt)dt,r.zeitpunkt zp,r.pat_id,'R'Typ,GROUP_CONCAT(DISTINCT CONVERT(r.Medikament USING UTF8MB4)COLLATE utf8mb4_german2_ci)Art" & vbCrLf & _
"   FROM rezepteintraege r JOIN aktiv a ON a.pat_id=r.pat_id" & vbCrLf & _
"   WHERE r.zeitpunkt " & DiesQ() & vbCrLf & _
"   GROUP BY r.pat_id,DATE(r.zeitpunkt)" & vbCrLf & _
")," & vbCrLf & _
"kontakte AS (" & vbCrLf & _
"  SELECT pat_id,dt,MIN(zp)zp,GROUP_CONCAT(Typ ORDER BY Typ SEPARATOR'')Typ," & vbCrLf & _
"   CONCAT(DATE_FORMAT(MIN(zp),'%e.%c.: '),GROUP_CONCAT(CONCAT(Typ,': ',Art)ORDER BY Typ SEPARATOR ', '))Art" & vbCrLf & _
"  FROM roh GROUP BY pat_id,dt" & vbCrLf & _
")" & vbCrLf
Vsql = Vsql & _
"SELECT a.pat_id,COUNT(DISTINCT k.zp)koz,a.quartal," & vbCrLf & _
"  COALESCE(GROUP_CONCAT(DISTINCT DATE_FORMAT(k.zp,'%e.%c.')ORDER BY k.zp SEPARATOR '•'),'')czp," & vbCrLf & _
"  COALESCE(MIN(k.zp),'')erst,COALESCE(MAX(k.zp),'')letzt," & vbCrLf & _
"  a.fid,a.schgr,a.vknr,a.ik,a.lanrid," & vbCrLf & _
"  COALESCE(GROUP_CONCAT(k.Typ ORDER BY k.zp SEPARATOR '•'),'')Typ," & vbCrLf & _
"  COALESCE(GROUP_CONCAT(k.Art ORDER BY k.zp SEPARATOR ' • '),'')Art" & vbCrLf & _
"FROM aktiv a" & vbCrLf & _
"LEFT JOIN kontakte k ON k.pat_id=a.pat_id" & vbCrLf & _
"GROUP BY a.pat_id;"
Call DtbCreateQueryDef(VN, Vsql, , True)
    vz = vz + 1
    ...
    "   AND NOT art IN ('BMI','GEW','GR',342,345,'bd','bla','re','ba','doppler','dop','duplex','dup','impf','inj','kva','tv','ufrag','wv','caro','colo','debr','fa','EKG','GPD','kv','LZRR','OAU','pa','pros','puls','rp','sono','ulcus','vac','ADL','284','bar'," & artSpezMA & ")" & vbCrLf & _
    "   GROUP BY pat_id,DATE(e.zeitpunkt) " & vbCrLf & _
...
"FROM (SELECT x.*,labor FROM " & vorsil & "pneu x LEFT JOIN " & vorsil & "plab lx ON x.lid=lx.id) x " & vbCrLf & _
"LEFT JOIN (SELECT y.*,ly.labor FROM " & vorsil & "pneu y LEFT JOIN " & vorsil & "plab ly ON y.labid=ly.id) y ON y.abkü=x.abkü AND y.langtext=x.langtext AND (y.einheit=x.einheit OR ( " & vbCrLf & _
"(SELECT MAX(yi.id) FROM (SELECT yii.*,lyii.labor FROM " & vorsil & "pneu yii LEFT JOIN " & vorsil & "plab lyii ON yii.labid=lyii.id) yi WHERE yi.abkü=x.abkü AND yi.langtext=x.langtext AND yi.einheit=x.einheit AND yi.labor=x.labor) IS NULL AND " & vbCrLf & _
...
"SELECT * FROM (" & vbCrLf & _
"SELECT COALESCE(GROUP_CONCAT(DISTINCT CASE WHEN wt<>'' THEN wt END SEPARATOR '<br>'),'') Wert, abkü, einheit,obpath,Zeitpunkt,COUNT(DISTINCT abkü,einheit,zeitpunkt) Zl"",rest,"",COUNT(0) OVER() dszahl FROM " & vbCrLf & _
" (SELECT wert wt,abkü,einheit,obpath,zeitpunkt"",rest,"" FROM labor2a WHERE pat_id="",pid,""" & vbCrLf & _
"  UNION " & vbCrLf & _
"  SELECT wert wt,abkü,einheit,obpath,zeitpunkt"",rest,"" FROM labor1a l1 WHERE pat_id="",pid,"") i " & vbCrLf & _
...
' FROM " & vorsil & "us u LEFT JOIN " & vorsil & "wert w ON w.usid=u.id LEFT JOIN " & vorsil & "hinw e ON e.erklid=w.id LEFT JOIN " & vorsil & "hinw k ON k.kommid=w.id LEFT JOIN " & vorsil & "pnb n ON n.nbid=w.id LEFT JOIN " & vorsil & "saetze s ON s.satzid=u.satzid LEFT JOIN " & vorsil & "dat d ON d.datid=s.datid LEFT JOIN " & vorsil & "plab l ON l.labid=s.id LEFT JOIN laborparameter p ON p.abkü=w.abkü AND p.einheit=IF(w.einheit IN ('','\'kA\''),'kA',w.einheit) HAVING (wert<>'' AND wert IS NOT NULL) OR (kommentar<>'' AND kommentar IS NOT NULL)
...
"BEGIN " & vbCrLf & _
"SET @frag = CONCAT('" & vbCrLf & _
" SELECT i.*,COUNT(0) OVER() dszahl FROM (" & vbCrLf & _
" (SELECT zeitpunkt zp,',katg,' FROM labor2a WHERE pat_id=',pid,')" & vbCrLf & _
" UNION " & vbCrLf & _
" (SELECT zeitpunkt zp,',katg,' FROM labor1a l1 WHERE pat_id=',pid,')i)i ORDER BY ',katg);" & vbCrLf & _
"PREPARE sc1 FROM @frag; " & vbCrLf & _
"execute sc1;" & vbCrLf & _
"DEALLOCATE PREPARE sc1; " & vbCrLf & _
"-- SELECT @frag; " & vbCrLf & _
...
"    READS SQL DATA " & vbCrLf & _
"    DETERMINISTIC " & vbCrLf & _
"    COMMENT 'zur Beurteilung der Sensibilität (0=unauffällig,1=nicht untersucht,2=auffällig)' " & vbCrLf & _
"BEGIN " & vbCrLf & _
...
sql = "DROP FUNCTION IF EXISTS `quelle`.`zquart`"
myEFrag (sql)
sql = "CREATE  DEFINER=`praxis`@`%` FUNCTION `quelle`.`zquart` (IN Datum DATE) RETURNS VARCHAR(5) " & vbCrLf & _
    "NO SQL DETERMINISTIC" & vbCrLf & _
    "COMMENT 'Quartal eines Datums' " & vbCrLf & _
"  BEGIN " & vbCrLf & _
"  DECLARE zq VARCHAR(5); DECLARE monat INTEGER(2) DEFAULT MONTH(Datum);" & vbCrLf & _
"  CASE WHEN monat IN (1,2,3) THEN SET zq=1;" & vbCrLf & _
"       WHEN monat IN (4,5,6) THEN SET zq=2;" & vbCrLf & _
"       WHEN monat IN (7,8,9) THEN SET zq=3;" & vbCrLf & _
"       ELSE SET zq=4;" & vbCrLf & _
"  END CASE;" & vbCrLf & _
"  SET zq=CONCAT(zq,YEAR(Datum));" & vbCrLf & _
"  RETURN zq; " & vbCrLf & _
"END; "
myEFrag (sql)

sql = "DROP FUNCTION IF EXISTS `quelle`.`zqsort`"
myEFrag (sql)
sql = "CREATE  DEFINER=`praxis`@`%` FUNCTION `quelle`.`zqsort` (IN Datum DATE) RETURNS VARCHAR(5) " & vbCrLf & _
    "NO SQL DETERMINISTIC" & vbCrLf & _
    "COMMENT 'Quartal eines Datums zum Sortieren' " & vbCrLf & _
"  BEGIN " & vbCrLf & _
"  DECLARE zq VARCHAR(5); DECLARE monat INTEGER(2) DEFAULT MONTH(Datum);" & vbCrLf & _
"  CASE WHEN monat IN (1,2,3) THEN SET zq=1;" & vbCrLf & _
"       WHEN monat IN (4,5,6) THEN SET zq=2;" & vbCrLf & _
...
"       ELSE SET zq=4;" & vbCrLf & _
"  END CASE;" & vbCrLf & _
"  SET zq=CONCAT(YEAR(Datum),zq);" & vbCrLf & _
"  RETURN zq; " & vbCrLf & _
"END; "
myEFrag (sql)
...
"  DECLARE cur2 CURSOR FOR SELECT eingang zeitpunkt, wert FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON u.id=w.usid WHERE pat_id = pid AND abkü RLIKE '^hba[1c]' AND eingang = (SELECT MAX(eingang) FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON u.id=w.usid WHERE pat_id = pid AND abkü RLIKE '^hba[1c]'); " & vbCrLf & _
"  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE; " & vbCrLf & _
...
"  DECLARE cur2 CURSOR FOR SELECT eingang zeitpunkt, wert FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON u.id=w.usid WHERE pat_id = pid AND abkü IN ('CREAT','KRE02','KREA','KREA02','KRES') AND einheit = 'mg/dl' AND eingang = (SELECT MAX(eingang) FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON u.id=w.usid WHERE pat_id = pid AND abkü IN ('CREAT','KRE02','KREA','KREA02','KRES') AND einheit = 'mg/dl'); " & vbCrLf & _
"  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE; " & vbCrLf & _
"  OPEN cur1; fetch cur1 INTO zp1, Hb1; CLOSE cur1; " & vbCrLf & _
"  OPEN cur2; fetch cur2 INTO zp2, Hb2; CLOSE cur2; " & vbCrLf & _
"  IF NOT ISNULL(zp2) THEN IF zp2 > zp1 OR ISNULL(zp1) THEN SET zp1=zp2; SET Hb1=Hb2; END IF; END IF; " & vbCrLf & _
...
"  SET @sql2 = CONCAT('SELECT eingang zeitpunkt, wert INTO @zp2, @w2 FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON u.id=w.usid WHERE pat_id = ',pid,' AND abkü IN (\'', abk, '\') AND einheit LIKE \'', einh,'%\' ORDER BY eingang DESC LIMIT 1'); " & vbCrLf & _
...
sql = "DROP PROCEDURE IF EXISTS `fuellTha_einzeln`;"
myEFrag (sql)
sql = "CREATE DEFINER=`praxis`@`%` PROCEDURE `quelle`.`fuellTha_einzeln`(IN inpid TEXT) " & vbCrLf & _
"    MODIFIES SQL DATA " & vbCrLf & _
"    COMMENT 'fuellt die Tabelle tharten, pid 0 => alle; Algorithmus definiert in therinit, seit 11.12.20' " & vbCrLf & _
"proc: BEGIN " & vbCrLf & _
" -- SQL bei der Erstellung der Prozedur am " & Format(Now(), "d.m.yy HH:MM:SS") & " von der Funktion `fuellTha` übertragen" & vbCrLf & _
...
'   " DEALLOCATE PREPARE stmt;" & vbCrLf
 End Select ' case iru
Next iru
'sql = sql & " SET sqlt = CONCAT('SELECT * FROM therarten WHERE(pat_id=',inpid,')ORDER BY zp DESC;');" & vbCrLf
'sql = sql & " EXECUTE IMMEDIATE sqlt;" & vbCrLf
...
" UNION -- 3) Insulinpläne " & vbCrLf & _
" SELECT pat_id pid,-3 MPNr,qdm zp,qdm bis,'ICT' Thart, MID(NAME,p) Gru,-2 ia,b.absPos,b.StByte,0 FROM (SELECT IF(p1>p2,p1,p2) p, b.* FROM (SELECT INSTR(b.name,'insulin') p1, INSTR(b.name,'spritz') p2, b.* FROM tmbrie b) b) b WHERE b.name RLIKE '(insulin|spritz).*(plan|schema|tabelle)' " & vbCrLf & _
"  AND (inpid=0 OR pat_id=inpid) " & vbCrLf & _
" UNION -- 4) Medikamentenplan " & vbCrLf & _
"  SELECT Pid,MPNr,Zp,Zp bis,Thart,gru,ia,abspos,stbyte,feldnr FROM ( " & vbCrLf & _
"   SELECT Pid,MPNr,Zp,Zp bis " & vbCrLf
...
"       ,MAX((COALESCE(mp.mo,'')<>'')+(COALESCE(mp.mi,'')<>'')+(COALESCE(mp.nm,'')<>'')+(COALESCE(mp.ab,'')<>'')+(COALESCE(mp.zn,'')<>'')+if(glp1<>0 AND mp.Medanfang RLIKE 'OZEMPIC|TRULICITY|MOUNJARO',1,0)) ez " & vbCrLf & _
"       ,glp1<>0 AND mp.MedAnfang RLIKE 'OZEMPIC|TRULICITY|MOUNJARO' wglp -- Wochen-GLP-1 " & vbCrLf & _
...
myEFrag (sql)
sql = "CREATE DEFINER=`praxis`@`%`PROCEDURE`fuellThaP`(IN`inpid`TEXT)LANGUAGE SQL NOT DETERMINISTIC CONTAINS SQL SQL SECURITY DEFINER COMMENT 'fuellt die Therapieraten für einen oder alle (bei inpid 0) Patienten; ruft fuellTha_einzeln() auf'" & vbCrLf & _
"BEGIN" & vbCrLf & _
" DECLARE sqlt TEXT;" & vbCrLf & _
" DECLARE current_pid TEXT;" & vbCrLf & _
" DECLARE done INT DEFAULT FALSE;" & vbCrLf & _
" DECLARE pid_cursor CURSOR FOR" & vbCrLf & _
"  SELECT n.pat_id FROM namen n JOIN faelle f USING(pat_id)" & vbCrLf & _
"  WHERE fanf=(SELECT MAX(fanf) FROM faelle WHERE pat_id=n.pat_id)" & vbCrLf & _
"  GROUP BY n.pat_id ORDER BY fanf DESC;" & vbCrLf & _
" DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;" & vbCrLf & _
" SET SESSION innodb_lock_wait_timeout = 28800;" & vbCrLf & _
" SET inpid=REPLACE(REPLACE(inpid,'"" ',''),'''','');" & vbCrLf & _
" IF inpid IN ('','0') THEN" & vbCrLf & _
"  TRUNCATE therarten;" & vbCrLf & _
"  OPEN pid_cursor;" & vbCrLf & _
"  pid_loop: LOOP" & vbCrLf & _
"   FETCH pid_cursor INTO current_pid;" & vbCrLf & _
"   IF done THEN LEAVE pid_loop; END IF;" & vbCrLf & _
"   CALL fuellTha_einzeln(current_pid);" & vbCrLf & _
"  END LOOP pid_loop;" & vbCrLf & _
"  CLOSE pid_cursor;" & vbCrLf & _
" ELSE" & vbCrLf
sql = sql & _
"  START TRANSACTION;" & vbCrLf & _
"  SET sqlt = CONCAT('DELETE FROM therarten WHERE pat_id=',inpid,';');" & vbCrLf & _
"  EXECUTE IMMEDIATE sqlt;" & vbCrLf & _
"  CALL fuellTha_einzeln(inpid);" & vbCrLf & _
"  SET sqlt = CONCAT('SELECT * FROM therarten WHERE(pat_id=',inpid,')ORDER BY zp DESC;');" & vbCrLf & _
"  EXECUTE IMMEDIATE sqlt;" & vbCrLf & _
"  COMMIT;" & vbCrLf & _
" END IF;" & vbCrLf & _
"END"
...
"inhalt RLIKE " & BZein & " AND NOT inhalt RLIKE " & BZaus & "))AND NOT inhalt like '%nicht möglich%'"
myEFrag sql
...
"SELECT eingang,wert INTO zp2, gfr2s FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON u.id=w.usid WHERE pat_id = pid AND abkü RLIKE muster AND eingang=(SELECT MAX(eingang) FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON u.id=w.usid WHERE pat_id=pid AND abkü RLIKE muster) LIMIT 1; " & vbCrLf & _
...
"SELECT eingang,wert INTO zp4, cre2s FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON u.id=w.usid WHERE pat_id = pid AND abkü RLIKE muc AND einheit = eic ORDER BY eingang DESC LIMIT 1; " & vbCrLf & _
...
"UNION " & vbCrLf & _
"SELECT eingang zeitpunkt,abkü,COALESCE(wert,0) wert,einheit FROM " & vorsil & "wert w LEFT JOIN " & vorsil & "us u ON u.id=w.usid WHERE pat_id=pid AND abkü IN ('ALBCRE','ALBKRE','ALBQ','ALBUM','ALBUP') AND einheit LIKE 'mg/g%' " & vbCrLf & _
...
syscmd 4, "Views für " & DBCn & " erstellt"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in doViewsErstellen/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' End Select
 ...
 End If ' not radatbof
 If briefneu Then
  If Not raDatBOF Then ag.Append "<w:p><w:r><w:rPr><w:sz w:val=""24""/></w:rPr><w:br w:type=""column""/></w:r><w:r><w:rPr><w:sz w:val=""24""/></w:rPr><w:lastRenderedPageBreak/><w:t>" & LaborÜS & "</w:t></w:r></w:p>"
 Else ' briefNeu Then
  If raDatBOF Then
    dc.bookmarks!LaborÜS.Range = vNS
...
' dc.Range(Tabl.Range.End, Tabl.Range.End) = "Vor dem " & Matr2(0, 6, 0) & " wurden noch bestimmt: " & ZusText
 If LenB(ZusText) > 2 Then
  ...
  Else
   ...
   End With
  End If ' briefNeu
 ElseIf briefneu Then
   ag.Append "<w:p><w:pPr><w:pStyle w:val=""Normal""/><w:ind w:hanging=""0"" w:left=""-1077"" w:right=""0""/></w:pPr><w:r><w:rPr><w:sz w:val=""24""/></w:rPr></w:r></w:p>"
 End If
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LaborIns1/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
'  "FROM `" & vorsil & "us` LEFT JOIN " & vorsil & "wert ON " & vorsil & "us.RefNr=" & vorsil & "wert.RefNr " + _
'  "WHERE pat_id = " + CStr(Pat_id) + " AND NOT (Wert LIKE ""%eine Berechnung%"") AND NOT EXISTS (SELECT * FROM `laborneu` WHERE pat_id = " + CStr(Pat_id) + " AND abkü = " & vorsil & "wert.Abkü AND wert = " & vorsil & "wert.wert AND zeitpunkt > " & vorsil & "us.Eingang-3 AND zeitpunkt < " & vorsil & "us.Eingang+6)"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LaborTabAnp/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LaborTabAnp_alt/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Tabelle/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
  ...
  If Inhalt <> "Blutentnahme" And Inhalt <> "Blutabnahme" Then
   ...
   Else
    ...
    Select Case raVL!art
     Case "caro", "carotis": Übs = "Halsschlagadern"
     Case "sem", "seminar", "schul": Übs = "Schulung"
     Case "ulcus", "ulc": Übs = "Ulcus"
     Case "wv": Übs = "Wundverband"
     Case "debr": Übs = "Debridement"
     Case "fuß": Übs = "Fußbefund"
     Case "kv": Übs = "Kompressionsverband"
     Case "fa", "fam", "familie": Übs = "Familienanamnese"
     Case "rauch", "rauchen", "raucht": Übs = "Rauchanamnese"
     Case "bew", "beweg", "bewegung", "bewg", "bewe": Übs = "Bewegung"
     Case "alko", "alk", "alkohol": Übs = "Alkoholanamnese"
     Case "colo": Übs = "Colo-/Gastroskopien"
     Case "aug", "augen": Übs = "Augen"
     Case "beruf": Übs = "Beruf"
    End Select
    If Übs <> "" Then Übs = Übs & ": "
    Inhalt = REPLACE$(Inhalt, "^", vNS)
    kkeintraege = kkeintraege + vbTab & Übs & Inhalt & vbCrLf
   End If
   lzp = DateValue(Zp)
  End If
  raVL.Move 1
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in kkeintraege/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
End Function ' kkeintraege
...
' AnwPfad = App.Path
'#END IF
'SELECT CASE MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in haakt/" + AnwPfad)
' Case vbAbort: Call MsgBox("Höre auf"): Progende
' Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
' Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
'End SELECT
...
' AnwPfad = App.Path
'#END IF
'SELECT CASE MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in KVÄDateifind/" + AnwPfad)
' Case vbAbort: Call MsgBox("Höre auf"): Progende
' Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
' Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
'End SELECT
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_haakt/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in PiepKurz/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Piep/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#End If
Call PiepKurz
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + " bei DS Nr. " + CStr(DBNr) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Z(neueZeileZeichen abschneiden)/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' aufgerufen in MDIForm_Load und tuBriefStandalone
Function snie()
  Call WD
  Call Sound(WinDir + "\media\Windows XP-Batterie niedrig.wav")
End Function ' snie()
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in WD/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in testdb/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in Datenbankkontrolle/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' Dim q AS DAO.QueryDef
 Call do_Click_Vorbereit
 sql = "SELECT a.nachname +"",""+a.vorname Nachname, cstr(a.diabetestyp) vorname, b.pfad AS DokPfad, b.name DokName, b.art DokArt, b.* FROM `" + QMdbAkt + "`.`tmbrie` b LEFT JOIN `" + QMdbAkt + "`.Anamnesebogen a ON `tmbrie`.pat_id = a.pat_id WHERE `tmbrie`.name LIKE ""Brief an*"" AND NOT cstr(diabetestyp) IN (""1"", ""2"",""g"",""s"") ORDER BY zeitpunkt DESC"
 Call DtbCreateQueryDef("LaborDokumente eP", sql)
 DoCmd.OpenForm "LaborDokumente eP", acNormal, , "17=17"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "Aufgefangener Fehler in do_abgehakt_Click/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "Aufgefangener Fehler in doForm_Load/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "Aufgefangener Fehler in diagexpHerricht/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in GesDiagExp/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in DiagnosenExport/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in dyndiag/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' lFDat = Dtb.OpenRecordset("SELECT MAX(bhfe1) AS bhfe FROM `faelle` WHERE pat_id = " + CStr(Pat_id))!BhFE
 Dim rs As New ADODB.Recordset
 myFrag rs, "SELECT MAX(bhfe1) AS bhfe FROM `faelle` WHERE pat_id = " & Pat_ID & IIf(nurKasse <> 0, " AND schgr NOT IN(89,90)", vNS)
 If Not rs.BOF Then
  lFDat = rs!BhFE
 End If
