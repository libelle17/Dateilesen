...
'        HAStr = rKVA!haname & ", " & rKVA!Ort & IIf(NOT ISNULL(rKVA!Email) AND LenB(rKVA!Email) <> 0, " Email: " & rKVA!Email, "") & _
 ...
 '      END IF
    ...
    myFrag rDi, "SELECT diagsicherheit, icd, diagtext, DATE_FORMAT(diagdatum,'%d.%m.%y') diagdatum, obdauer FROM `diagnosen` d WHERE pat_id = " & Me.PatID & " AND COALESCE(d.f6010,0)=0 GROUP BY diagsicherheit, icd, diagtext, obdauer ORDER BY icd"
    If Not rDi.BOF Then
