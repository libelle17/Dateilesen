...
Public ausaCStr%
Public obQuelle% ' ob aCStr mit quelle o.�. aufgerufen wurde
...
Const �1$ = "Datenbank-Verbindung herstellen"
...
Dim obNurLauf%
Dim Comp ' Laufvariable f�r Computer
Dim lCpt$, lBenutzer$, lPa�wort$, luid$, lpwd$, lODBC$, lDaBa$, altCpt$, altODBC$
Public changeStill% ' Ver�nderungen still vornehmen => keine Folgeereignisse
Dim Opt&
Public RegPos$
...
Function ShowAllDomains(Optional obneu%) As Collection
  ...
  If obneu Or CptN Is Nothing Then zupr�fen = True Else If CptN.Count = 0 Then zupr�fen = True
  If zupr�fen Then
...
Public Sub ShowAllComputers(ByVal strDomain$)
  ...
  For Each oComputer In PrimDomainContr
    Cpts.Add Left$(oComputer.Name & Space$(CptL�nge), CptL�nge) & "| " & HostByName$(oComputer.Name)
...
Private Sub Datei_Click()
 ...
 Dim filefilter$
 CommonDialog1.DialogTitle = "Datenbankdatei ausw�hlen:"
...
Private Sub Pa�wort_Change()
  If Not changeStill Then Call Verbind
End Sub

Private Sub Pa�wort_Click()
 Static altpa�wort$
 If Pa�wort <> altpa�wort Then If Not changeStill Then Call Verbind
 altpa�wort = Pa�wort
End Sub ' Pa�wort_Click
...
'  neuS = fWertLesen(HCU, RegPos, "ODBC")
  ...
  neuS = cR.ReadKey("Pa�wort", RegPos, HKEY_CURRENT_USER)
'  neuS = fWertLesen(HCU, RegPos, "Pa�wort")
'  If neuS <> "" Then ' Kommentar 15.8.09 wg. Faxdopp, f�r Access Pa�wort leer
   Me.Pa�wort = neuS
'  End If
...
'  Call fStSpei(HCU, RegPos, "Server", Me.Cpt)        ' f�r MySQL
  cR.WriteKey Me.Benutzer, "Benutzer", RegPos, HKEY_CURRENT_USER, REG_SZ
'  Call fStSpei(HCU, RegPos, "Benutzer", Me.Benutzer) ' f�r MySQL
  cR.WriteKey Me.pwd, "pwd", RegPos, HKEY_CURRENT_USER, REG_SZ
'  Call fStSpei(HCU, RegPos, "pwd", Me.pwd)         ' f�r MySQL
  cR.WriteKey Me.DaBa, "Datenbank", RegPos, HKEY_CURRENT_USER, REG_SZ
'  Call fStSpei(HCU, RegPos, "Datenbank", Me.DaBa) ' f�r MySQL
  cR.WriteKey Opt, "options", RegPos, HKEY_CURRENT_USER, REG_DWORD
'  Call fdwSpei(HCU, RegPos, "options", Opt)       ' f�r MySQL
  cR.WriteKey Me.uid, "uid", RegPos, HKEY_CURRENT_USER, REG_SZ
'  Call fStSpei(HCU, RegPos, "uid", Me.uid)         ' f�r Access
  cR.WriteKey Me.Pa�wort, "Pa�wort", RegPos, HKEY_CURRENT_USER, REG_SZ
'  Call fStSpei(HCU, RegPos, "Pa�wort", Me.Pa�wort)   ' f�r Access
  cR.WriteKey Me.Datei, "Datei", RegPos, HKEY_CURRENT_USER, REG_SZ
'  Call fStSpei(HCU, RegPos, "Datei", Me.Datei)    ' f�r Access
  cR.WriteKey Me.DBKennw, "DBKennw", RegPos, HKEY_CURRENT_USER, REG_SZ
'  Call fStSpei(HCU, RegPos, "DBKennw", Me.DBKennw) ' f�r Access
 Exit Sub
fehler:
...
Function doVerbind%(Optional Tabelle$, Optional ErrDes$)
  ...
  ' CnStr = ggf. mit Datenbank, CoStr = ohne, ConStr = mit gesterntem Pa�wort
  On Error Resume Next
    ...
    If InStrB(Me.ODBC, "MySQL") <> 0 Then
     CnStr = "DRIVER={" & Me.ODBC & "};server=" & Trim$(Left(Me.Cpt, CptL�nge)) & ";"
     Call rechneOpt
    ...
    ElseIf InStrB(Me.ODBC, "Acc") <> 0 Or InStrB(Me.ODBC, ".Jet.") <> 0 Then
     ...
     If LenB(Me.DBKennw) <> 0 Then
      ...
      Constr = CnStr & String$(Len(Me.Pa�wort), "*") & ";"
      CnStr = CnStr & Me.DBKennw & ";"
     End If
     CnStr = CnStr & "user id=" & Me.Benutzer & ";password="
     Constr = Constr & "user id=" & Me.Benutzer & ";password="
     CoStr = CnStr
     CnStr = CnStr & Me.Pa�wort & ";"
     CoStr = CoStr & Me.Pa�wort & ";"
     Constr = Constr & String$(Len(Me.Pa�wort), "*") & ";"
    End If
'    CnStr = CnStr & "user id=" & Me.Benutzer & ";password=" ' geht auch, aber wer wei�, ob's nicht Fehler macht
'    Constr = Constr & "user id=" & Me.Benutzer & ";password="
...
' If Me.Cpt = "" Then Call RegLaden
 If UCase$(Me.Cpt) <> UCase$(lCpt) Or UCase$(Me.Benutzer) <> UCase$(lBenutzer) Or Me.Pa�wort <> lPa�wort Or UCase$(lODBC) <> UCase$(Me.ODBC) Or UCase$(Me.uid) <> UCase$(luid) Or UCase$(Me.pwd) <> UCase$(lpwd) Then
  Screen.MousePointer = vbHourglass
  On Error Resume Next
...
' changeStill = altChangeStill
 ...
 If Err.Number <> 0 Then ' wnenn wmi nicht l�ut
  On Error GoTo fehler
...
'  Me.ODBC.AddItem "Achtung: WMI-Dienst nicht gestartet; Auswahl k�nnte nicht den verf�gbaren Treibern entsprechen!"
 Else
  On Error GoTo fehler
  ...
  For runde = 0 To 3
   For i = 0 To UBound(arrValueNames)
    sV = arrValueNames(i)
    ' rrunde = richtige Runde f�r jeden Treiber; zuerst die �brigen nach Sortierung, dann die interessanten nach Stand
...
Private Sub Pa�wort_GotFocus()
 Me.Pa�wort.SelStart = 0
 Me.Pa�wort.SelLength = Len(Me.Pa�wort)
End Sub
...
' Stop
 Ende
End Sub
Private Function �berschrift(�$)
 Me.�2 = �
 Me.Caption = App.Title & IIf(LenB(�2) <> 0, " (" & �2 & ")", "") & ": " & �1
' Call Verbind
End Function
Function cnVorb$(DBName$, TBName$, Optional �$, Optional obregneu%, Optional RegNichtLaden%)
  ...
  If Not innen Then
   If TBName <> "--multi" Then
    Call Me.r�cksetzBedTbl
    Call Me.setzBedTbl(TBName)
   End If
   Call �berschrift(�)
   If Not RegNichtLaden Then Call RegLaden(�, nuranfangs:=IIf(obregneu, False, True))
...
'  End If
   ...
   If LCase(Left(�, 5)) = "admin" Then
    cR.WriteKey "", "Pa�wort", RegPos, HKEY_CURRENT_USER, REG_SZ
'    Call fStSpei(HCU, RegPos, "Pa�wort", "")
   End If
  End If
  Exit Function
fehler:
...
Function Auswahl$(DBName$, TBName$, Optional �$)
 Dim altCn$
 On Error Resume Next
