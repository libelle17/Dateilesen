...
'  sql(i) = "SELECT zeitpunkt zp, IF(ISNULL(wert),IF(ISNULL(kommentar),'',kommentar),wert) wert FROM (" & lsql & ") AS innen WHERE abkü = 'CKNAC' ORDER BY zp DESC"
  ...
  sql(i) = "SELECT zeitpunkt zp, inhalt wert FROM `eintraege` WHERE pat_id = " & Pat_ID & " AND (art RLIKE '^(caro|duplex|dup$)' AND (inhalt LIKE '%Halsschlag%' OR inhalt LIKE '%Halsart%' OR inhalt LIKE '%Carotis%' OR inhalt LIKE '%ACC%' OR inhalt LIKE '%ACI%' OR inhalt LIKE '%subcl%')) ORDER BY zp DESC"
  Fqmin(i) = 0.5
  RowSp(i) = 2
  zmax(i) = 4
  i = i + 1
...
' myFrag raDat, "CALL geslabkatg(" & CStr(Pat_ID) & ",'Zeitpunkt')"
 myFrag raDat, "CALL geslabdp(" & CStr(Pat_ID) & ",'')", adOpenStatic
#End If
