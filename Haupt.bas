...
'    replace$(doUmwfSQL(rs!diagtext,lies.obMySQL,false), "≥", "¸")
   ...
   Print #257, " Dim rbeg&, robeg&, ri&, roi&" & IIf(aktTbn = "faelle", ", jj&, fazu%()", "")
   Print #257, " On Error GoTo fehler"
   Print #257, " IF UBound(r" & Tbk(i) & ") > 0 THEN"
   Print #257, "  rbeg = 0"
   Print #257, "  For ri = 1 To UBound(r" & Tbk(i) & ")"
   Print #257, "   IF r" & Tbk(i) & "(ri)." & ZPFeld(i) & " >= qbeg THEN"
   Print #257, "    rbeg = ri"
   Print #257, "    Exit For"
   Print #257, "   END IF"
   Print #257, "  Next ri"
   Print #257, "  IF rbeg <> 0 THEN"
   Print #257, "   For robeg = 0 To UBound(ro" & Tbk(i) & ")"
   Print #257, "    IF ro" & Tbk(i) & "(robeg)." & ZPFeld(i) & " >= qbeg THEN"
   Print #257, "     Exit For"
   Print #257, "    END IF"
   Print #257, "   Next robeg"
   Print #257, "   IF robeg <= UBound(roFa) THEN"
   If aktTbn = "faelle" Then
    Print #257, "    ReDim fazu(robeg To UBound(ro" & Tbk(i) & "))"
    Print #257, "    For roi = robeg To UBound(ro" & Tbk(i) & ")"
   ...
   Print #257, "    ReDim Preserve ro" & Tbk(i) & "(robeg - 1)"
   Print #257, "   END IF ' IF robeg <= UBound(roFa) THEN"
   Print #257, "   ReDim Preserve ro" & Tbk(i) & "(robeg + UBound(r" & Tbk(i) & ") - rbeg)"
   Print #257, "   For ri = rbeg To UBound(r" & Tbk(i) & ")"
   Print #257, "    Call ro" & Tbk(i) & "Zuw(robeg + ri - rbeg, ri)"
   Print #257, "   Next ri"
...
'  Print #257, "  Call myefrag(sql,rAf)', , adAsyncExecute)"
  ...
  Print #257, "    Call ForeignYes1"
  Print #257, "   END IF"
  Print #257, "   IF sammelinsert = 0 THEN csql.m_len = 0"
  ...
  If aktTbn = "faelle" Then
   ...
   Print #257, "   ' SET rs = myefrag(""SELECT FID FROM `faelle` WHERE pat_id = "" & rFa(i).Pat_ID & "" AND quartal = '"" & rFa(i).Quartal & ""' AND bhfb = "" & DatFor_k(rFa(i).BhFB) & "" AND bhfe1 = "" & DatFor_k(rFa(i).BhFE1) & "" AND ausgst = "" & DatFor_k(rFa(i).ausgst))"
   Print #257, "   SET rs = myefrag(""SELECT LAST_INSERT_ID() FID"")"
   Print #257, "   IF rs.BOF THEN"
   Print #257, "    Err.Raise 999, , ""Fehler bei der Fallaktualisierung b.Pat. "" & rFa(i).Pat_ID & "", FID "" & rFa(i).FID"
   Print #257, "   Else"
   Print #257, "    IF rs!FID <> rFa(i).FID THEN"
   Print #257, "     Lese.Ausgeb ""ƒnderung bei der FallID  bei Pat. "" & rFa(i).Pat_ID & "", FID "" & rFa(i).FID & "" -> "" & rs!FID & "" in zu speichernden Tabellen mit fallid"", True "
   Print #257, "     Dim jjj&"
   For ii = 0 To TbZ1
    If Tbk(ii) <> "Fa" Then
     fat = 0
     ...
     If fat Then
      ...
      Print #257, "        r" & Tbk(ii) & "(jjj).FID = rs!FID"
      Print #257, "       END IF"
      Print #257, "      Next jjj"
     End If
    End If
   ...
   Print #257, "     END IF"
   Print #257, "    END IF"
   Print #257, "  END IF ' IF Sammelinsert = 0 "
  End If
...
'  END IF
  ...
  If aktTbn = "namen" Then
   ...
   Print #257, " Resume"
  End If
  Print #257, "END IF ' Err.Number = -2147217833 THEN"
  If aktTbn = "faelle" Then
   Print #257, "IF Err.Number = -2147467259 THEN"
   ...
   Print #257, " Resume"
   Print #257, "END IF ' Err.Number = -2147467259 THEN"
  ...
  Print #257, " AnwPfad = App.Path"
  Print #257, "#END IF"
  Print #257, " SELECT CASE MsgBox(""FNr: "" + CStr(Err.Number) + vbCrLf + ""LastDLLError: "" + CStr(Err.LastDllError) + vbCrLf + ""Source: "" & IIf(ISNULL(Err.Source), vNS, CStr(Err.Source)) + vbCrLf + ""Description: "" + Err.Description, vbAbortRetryIgnore, ""aufgefangener Fehler in " & aktTbn & "Speichern" + "/"" + AnwPfad)"
  Print #257, "  Case vbAbort: Call MsgBox("" Hˆre auf ""): Progende"
  Print #257, "  Case vbRetry: Call MsgBox(""Versuche nochmal""): Resume"
  Print #257, "  Case vbIgnore: Call MsgBox("" Setze fort ""): Resume Next"
  Print #257, " END SELECT"
...
'    ON Error GoTo fehler
   ...
   On Error Resume Next
   myFrag cnA, "SELECT COUNT(0) AS ct FROM " & "`" & LCase(SL.Item(j).Feld) & "`"
...
'    ON Error GoTo fehler
   ...
   If Not cnA Is Nothing Then If cnA.State = 1 Then cnA.Close
   On Error Resume Next
   myFrag cnA, "SELECT " & IIf(lies.obMySQL, vNS, "top 1 ") & "* FROM " & "`" & LCase(tbn(j)) & "`" & IIf(lies.obMySQL, " LIMIT 1", ""), adOpenStatic, aktCn, adLockReadOnly
   If Err.Number <> 0 Then GoTo weiter
   On Error GoTo fehler
...
Function TabFuellSnSh() ' Tabf¸ll; Tabellenf¸llungen ermitteln
 Dim rs As New ADODB.Recordset, Zp As Date, zps$, rsct As New ADODB.Recordset, rAF&, ct&, ErrDes$
 On Error GoTo fehler
...
' rs.Open "SELECT table_name, table_type FROM information_schema.tables WHERE table_schema = '" & CurDB(DBCn) & "'", DBCn, adOpenDynamic, adLockOptimistic
 ...
 Do While Not rs.EOF
  Set rsct = Nothing
  On Error Resume Next
  myFrag rsct, "SELECT COUNT(0) ct FROM `" & rs!table_name & "`"
  If Err.Number <> 0 Then
...
Function getDokPfad$(Optional Abschnitt$)
 ...
 If LenB(Abschnitt) <> 0 Or IsNull(PcDokPfad) Or LenB(PcDokPfad) = 0 Then
  ...
  If LenB(erg) = 0 Then
   getDokPfad = REPLACE$(LCase$(getDokPfad), "x:", "\\linux1\turbomed")
   erg = Dir(getDokPfad, vbDirectory)
  End If
...
Function holAB(frm As Lese) ' kommt vor in AnamnesebogenHolen_Click
 ...
 myFrag rq, "SELECT COUNT(0) AS ct FROM `anamnesebogen`", CStrAcc & frm.snst.QuelleAnamneseBˆgen
 frm.GesBytes = rq!ct
 On Error GoTo fehler
...
Function doLdFD() ' Liste der fehlenden Dokumente
 ...
 Dim aktDP$
 Ers(0) = "\\linux1\daten\turbomed\Dokumente"
...
Function holDA(frm As Lese) ' kommt vor IN DokumenteAbgehaktkopieren_Click()
 ...
 Do While Not rq.EOF
  ...
  For i = 1 To 4
   ...
   If Not rsAnm Is Nothing Then If rsAnm.State = 1 Then rsAnm.Close
   rsAnm.CursorLocation = adUseClient
   myFrag rsAnm, "SELECT -abgehakt AS j_abgehakt, da.* FROM `" & TabN & "` da WHERE `" & sF & "` = '" & Wert & "'", adOpenStatic, DBCn, adLockOptimistic
   If Not rsAnm.BOF Then
...
'   GoTo nochmal:
   ...
   Loop
   Call myEFrag(csql.Value, rAF)
   If rAF <> 1 And Err.Number <> 0 Then
    Call myEFrag("Commit")
    Call myEFrag(csql.Value, rAF)
    If rAF <> 1 And Err.Number <> 0 Then
     Set rq = Nothing
     Call myEFrag(csql.Value, rAF)
     If rAF <> 1 And Err.Number <> 0 Then
      Err.Raise 999, , Err.Number & " " & Err.Description & vbCrLf & "Fehler in anaUpd: Falsche Zahl an Datens‰tzen aktualisiert: " & rAF
     End If
    End If
   End If
Ende:
...
Public Function anaIns(rq As Recordset)
 ...
 Dim i&, sql1$, sql2$, rAF&, Wert$, mprim&, rs As ADODB.Recordset, pos0&
   ...
   sql2 = "values("
   Set rs = myEFrag("SELECT MAX(prim) AS mprim FROM `" & TName & "`")
...
' CtlTp(3) = "Exit"
 ...
 Print #309, "  adoRS.CancelUpdate"
 Print #309, " IF mvBookMark > 0 THEN"
 Print #309, "    adoRS.Bookmark = mvBookMark"
 Print #309, "  Else"
 Print #309, "    adoRS.MoveFirst"
 Print #309, "  END IF"
 ...
 Print #309, "  ON Error GoTo fehler"
 Print #309, "  adoRS.UpdateBatch adAffectAll"
 Print #309, "  IF mbAddNewFlag THEN"
 Print #309, "    adoRS.MoveLast              'Zu neuem Datensatz gehen"
 Print #309, "  END IF"
 Print #309, "  mbEditFlag = False"
 Print #309, "  mbAddNewFlag = False"
 Print #309, "  SetButtons True"
 ...
 Print #309, "  ON Error GoTo fehler"
 Print #309, "  IF Not adoRS.EOF THEN adoRS.MoveNext"
 Print #309, "  IF adoRS.EOF AND adoRS.RecordCount > 0 THEN"
 ...
 Print #309, "  ON Error GoTo fehler"
 Print #309, "  IF Not adoRS.BOF THEN adoRS.MovePrevious"
 Print #309, "  IF adoRS.BOF AND adoRS.RecordCount > 0 THEN"
 Print #309, "    T¸t 1760,1000"
 Print #309, "    'Ende der Zeile wurde erreicht; zur¸ck zum Zeilenanfang"
 Print #309, "    adoRS.MoveFirst"
 Print #309, "  END IF"
...
' IF AccName = "Anamnesebogen" THEN
 ...
 Print #309, "   adoRS.Find "" Pat_id = "" & Me.suche, 0, adSearchForward"
 Print #309, "  Else"
 Print #309, "   IF adoRS.EOF THEN adoRS.MoveFirst"
 Print #309, "   adoRS.Find ""gesname LIKE '"" & Me.suche & ""*'"", 1, adSearchForward"
 Print #309, "  END IF"
 Print #309, " Else"
...
'   Print #309, "  SET adoRS = New Recordset"
...
'' cn.Open "DRIVER={MySQL ODBC 5.1 Driver};database=office;server=linux1;uid=praxis;pwd=...;option=3"
' Cn.Open "DRIVER={" & ODBCStr & "};database=office;server=linux1;uid=praxis;pwd=...;option=3"
' Dim rs As New ADODB.Recordset
...
Call liesexcel("p:\patienten¸bergreifendes\Listenausgabe_Krankenkassen.xls", 5, "kassen")
