...
'Const AbgehSQL$ = "SELECT `briefe`.Pat_ID, `briefe`.Zeitpunkt, `briefe`.pfad, `briefe`.Name, DATE(`briefe`.DokAenD) AS Quelldt, `briefe`.DokGroe, kvnr, übwv, DATE(bhfb) AS Fallbg, IF(abgehakt <> 0,'X',' ') AS abgeh FROM ((`briefe` LEFT JOIN `dokumente abgehakt` ON `briefe`.pfad = `dokumente abgehakt`.pfad) LEFT JOIN `namen` ON `briefe`.pat_id = `namen`.pat_id) LEFT JOIN `lfaelle` ON `briefe`.pat_id = `lfaelle`.pat_id WHERE (((`briefe`.Name) LIKE '%Fremdlabor%')) AND (ISNULL(abgehakt) OR abgehakt = 0) GROUP BY pfad ORDER BY pat_id DESC, zeitpunkt DESC, übwv DESC"
Const AbgehSQL$ = "SELECT d.Pat_ID, Zeitpunkt, d.pfad, Name, intaccdatemy(Quelldatum) AS Quelldt, DokGroe, kvnr, übwr, intaccdatemy(bhfb) AS Fallbg, IF(abgehakt <> 0,'X',' ') AS abgeh FROM ((`briefe` d LEFT JOIN `dokumente abgehakt` da ON d.pfad = da.dokpfad) LEFT JOIN `namen` n ON d.pat_id = n.pat_id) LEFT JOIN `lfaelle` f ON d.pat_id = f.pat_id WHERE ((Name LIKE '%Fremdlabor%')) AND ((ISNULL(abgehakt) OR abgehakt = 0) OR ungueltig =1 OR Quelldatum < '19000000') GROUP BY d.pfad ORDER BY d.pat_id DESC, zeitpunkt DESC, übwr DESC"
...
Private Sub abhaken_Click()
  ...
  Set rAd = Nothing
  myFrag rAd, "SELECT 0 FROM `dokumente abgehakt` WHERE dokpfad = '" & doUmwfSQL(Me.MSHFlexGrid1.Text, lies.obMySQL) & "'"
  If rAd.BOF Then
...
Private Sub dpS_MouseDown(Button As Integer, Shift As Integer, x As Single, Y As Single)
'
End Sub

Private Sub dpS_MouseMove(Button As Integer, Shift As Integer, x As Single, Y As Single)
'
End Sub

Private Sub dpS_MouseUp(Button As Integer, Shift As Integer, x As Single, Y As Single)
'
End Sub
'Private Sub dpS_EndOfRecordset(fMoreData AS Boolean, adStatus AS ADODB.EventStatusEnum, ByVal pRecordset As Adodb.Recordset)
' Stop
'End Sub
...
' PROVIDER=MSDASQL;dsn=MySQLpraxis;uid=praxis;pwd=***REMOVED***;database=quelle;
 ...
 If Not obStart Then
  hlese.Visible = False
 End If
 sql = SqlU(IIf(Me.nurHeuer, "SELECT * FROM (" & AbgehSQL & ") AS i WHERE YEAR(quelldt)>=year(SUBDATE(NOW(),INTERVAL 90 DAY)+1) OR quelldt < '19000000' OR ISNULL(quelldt) ORDER BY quelldt DESC", AbgehSQL), Lese.obMySQL)
