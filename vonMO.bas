...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in exp/" + AnwPfad)
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in ParseMemo/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in WechsMemo/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in MOConInit/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
 ...
 If Not rsMO.BOF Then
  ...
  If rsMO!fm <> "" Then
   ...
   For j = 0 To UBound(FMem)
    ...
    ElseIf FMem(j).ENr Like "*.9" And FMem(j).ENr <> "1.9" Then
     EintS.aKür = FMem(j).Text
    ElseIf FMem(j).ENr Like "*.10" And FMem(j).ENr <> "1.10" Then
     EintS.TKür = FMem(j).Text
     EinL.sCAdd EintS
    End If
   Next j
  End If
...
' insdat=NULL
sql = _
"SELECT FPatnr,CONCAT('UPDATE namen SET obk=0,obs=0,obh=0,antikoag=0,dmt1=0,gdm=0,kdm=0,cgm=0,insAnw=0,insdat=0'," & vbCrLf & _
"GROUP_CONCAT(setz SEPARATOR ''),' WHERE pat_id=',FPatnr) sqls" & vbCrLf & _
"FROM (" & vbCrLf & _
...
#Const schneller = True
#If schneller Then
  myEFrag "TRUNCATE " & Tbl, , DBCn
...
'  Shell """c:\program files\putty\plink.exe"" -pw " & pwd & " -batch root@linux1 ""mariadb-dump --defaults-extra-file=~/.modbpwd medoff " & Tbl & "|mariadb --defaults-extra-file=~/.mariadbpwd quelle"""
  rufauf "c:\program files\putty\plink.exe", " -pw " & pwd & " -batch root@" & LiName & " ""mariadb-dump --defaults-extra-file=~/.modbpwd medoff " & Tbl & "|mariadb --defaults-extra-file=~/.mariadbpwd quelle""", True
' 2.2.26: kriege ich nicht funktionsfähig, immer "Zugriff verweigert"
'  doplink "mariadb-dump --defaults-extra-file=~/.modbpwd medoff " & Tbl & "|mariadb --defaults-extra-file=~/.mariadbpwd quelle", pwd
' 2.2.26: das ginge hingegen auch:
#ElseIf genausoschnell Then
  RunCommandLine "c:\program files\putty\plink.exe" & " -pw " & pwd & " -batch root@" & LiName & " ""mariadb-dump --defaults-extra-file=~/.modbpwd medoff " & Tbl & "|mariadb --defaults-extra-file=~/.mariadbpwd quelle"""
#Else
...
'     DBCn.Execute rMo.Fields(0), rAf
     ...
     If ErrNr Then
      ...
      Open "v:\üsqlfehler.txt" For Output As #200
      Print #200, rMo.Fields(0)
      Close #200
'      Exit Do
     End If
     rMo.MoveNext
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in tbtrans/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 If True Then
#End If ' obhatrans
'#const mitrichtnamen=1
#If mitrichtnamen Then
  Call richtHA
#End If
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in HATrans/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
sqlOut = _
  ...
  labsql & vbCrLf & _
  ") i" & vbCrLf & _
  "INTO OUTFILE '" & csvLokal & "'" & vbCrLf & _
  "FIELDS TERMINATED BY '|~|'" & vbCrLf & _
  "ENCLOSED BY 0x02" & vbCrLf & _
  "ESCAPED BY '`'" & vbCrLf & _
  "LINES TERMINATED BY '|#|'"
  myEFrag sqlOut, rAfGes, MOCon, , ErrNr, ErrDes
  If ErrNr Then
  ...
  Else ' ErrNr
    ...
    If rAfGes Then
      ...
      myEFrag sqlLoad, rAfGes, DBCn, , ErrNr, ErrDes
      If ErrNr Then
        Debug.Print "LOAD DATA Fehler: " & ErrNr & " " & ErrDes
        Exit Sub
      End If ' ErrNr
      T1 = Timer
      Debug.Print ">>> LOAD DATA: " & rAfGes & " Zeilen, " & Format((tg1 - tg0) * 1000, "0") & "ms"
    End If ' rAfGes
    On Error Resume Next
    Kill csvLokal
    On Error GoTo fehler
  ...
  Exit Sub
fehler:
  Debug.Print "Fehler in LaborStagingFuellenOutfile: " & Err.Number & " " & Err.Description
End Sub ' LaborStagingFuellenOutfile

' in doPatvonMO
Private Sub LaborStagingFuellen(labsql$, pid&, ByRef erfolg%, ByRef rAfGes&)
  Dim ErrNr&, ErrDes$
  ' Pfade:
...
'  If False Then
  ...
  LaborStagingFuellenOutfile labsql, pid, erfolg, csvLokal, csvLinux, csvSamba, rAfGes
  If erfolg <> 0 Then Exit Sub
  ' Versuch 2: ges
  laborges labsql, rAfGes, erfolg
  If erfolg <> 0 Then Exit Sub
  ' Versuch 2: VB6 CSV-Schreiben (robust):
  Debug.Print ">>> Fallback auf VB6-CSV"
  LaborStagingFuellenVB6 labsql, pid, erfolg, csvLokal, csvLinux, csvSamba, rAfGes
'  End If
End Sub ' LaborStagingFuellen(labsql$, pid&)

' in LaborStagingFuellen
Private Sub LaborStagingFuellenVB6(labsql$, pid&, ByRef erfolg%, csvLokal$, csvLinux$, csvSamba$, ByRef rAfGes&)
  ...
  Do While Not rsSrc.EOF
    ...
    If chunkLines >= linesPerChunk Or rsSrc.EOF Then
      ...
      bufChunk = ""
      chunkLines = 0
    End If ' chunkLines >= linesPerChunk Or rsSrc.EOF Then
  Loop
  Close #fileNr
  rsSrc.Close
  T1 = Timer
...
fallback:
  ...
  Exit Sub
fehler:
  Debug.Print "Fehler in LaborStagingFuellenVB6: " & Err.Number & " " & Err.Description
  Resume fallback
End Sub ' LaborStagingFuellenVb6

' -- Hilfsfunktion: Feld für TSV formatieren ------------------
' in LaborStagingFuellenVb6
Private Function TsvFeld$(s$)
  Dim r$
  r = s
  ...
  r = """" & r & """"
  TsvFeld = r
End Function ' TsvFeld(s$) As String
...
' in doPatvonMO, für Not ohneLabor, not laborlangsam und # mitClaude
Private Sub LaborAusStaging(pid&, ByRef rAfGes&)
  ...
  rAfGes = 0
  On Error GoTo fehler
    syscmd 4, "Labor: Verarbeite Staging-Daten..."
    ' Auf READ COMMITTED wechseln damit sq(5) die von sq(0..4)
    ' eingefügten Lookup-Werte garantiert sieht
    myEFrag "SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED", rAfIso, DBCn, True, ErrNrIso, ErrDesIso
  sq(0) = _
...
nochmal:
  For i = 0 To 5
    ...
    End If ' i < 5 Then
    meldTxt = "Labor Staging?DB: " & rAf & " Sätze in " & sName(i) & IIf(ErrNr <> 0, " – Fehler " & ErrNr & " " & ErrDes, " – OK")
    Debug.Print meldTxt
    syscmd 4, meldTxt
  ...
  If rAfGes = 0 And retry = 0 Then
    ...
    Set rsChk = Nothing
    myFrag rsChk, "SELECT COUNT(*) FROM labor_staging WHERE FPatNr=" & pid, adOpenStatic, DBCn, adLockReadOnly
    Dim stagingCount&
    ...
    If stagingCount > 0 Then
      ...
      myEFrag "SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED", rAf, DBCn, True, ErrNr, ErrDes
      Debug.Print ">>> Retry: Isolation READ COMMITTED gesetzt"
      GoTo nochmal
    End If ' stagingCount > 0 Then
  End If ' rAfGes = 0 And retry = 0 Then
  If rAfGes = 0 Then
    Debug.Print "LaborAusStaging: laborneu leer nach Retry – Pat." & pid
  End If
  myEFrag "SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ", rAfIso, DBCn, True, ErrNrIso, ErrDesIso
  Exit Sub
fehler:
  Debug.Print "Fehler in LaborAusStaging: " & Err.Number & " " & Err.Description
End Sub ' LaborAusStaging(pid&, ByRef rAfGes&)
#End If
...
'      If EintS.TypNr = 29957 Then Stop
     End If
    ElseIf FMem(j).ENr Like "*.3" And FMem(j).ENr <> "1.3" Then ' Kürzel intern
     EintS.IKür = FMem(j).Text
    ElseIf FMem(j).ENr Like "*.5" And FMem(j).ENr <> "1.5" Then
     EintS.EKür = FMem(j).Text
    ElseIf FMem(j).ENr Like "*.8" And FMem(j).ENr <> "1.8" Then
     EintS.name = FMem(j).Text
    ElseIf FMem(j).ENr Like "*.9" And FMem(j).ENr <> "1.9" Then ' erste drei Buchstaben des internen Kürzels vor der letzten Bearbeitung in der Datenpflege
     EintS.aKür = FMem(j).Text
    ElseIf FMem(j).ENr Like "*.10" And FMem(j).ENr <> "1.10" Then
     EintS.TKür = FMem(j).Text
...
'     End Select
#If mosysdruck Then
     Print #234, EintS.TypNr & " " & EintS.IKür & " " & EintS.EKür & " " & EintS.name & " " & EintS.aKür & " " & EintS.TKür
#End If ' mosysdruck
     EinL.sCAdd EintS
    End If
   Next j
...
'         ' letzteres bei Pat. 70326:
       Case "2.7.2": rFa(UBound(rFa)).bHeilb = FaStr(j).Text
...
'            End If
       Case "5.2.11":
           rFa(UBound(rFa)).privVers = FaStr(j).Text ' oder auch BG-Name
       Case "5.22"
...
' FEintragsart>1000 sind die selbst definierten Kategorien in mosystem
  ...
  If Not rsEi.BOF Then
   Do While Not rsEi.EOF
'    If rsEi!Wert Like "*zum Isthmus hin ein 5,5 x 4 x 3 mm*" Then Stop
...
#If mitclaude Then
    syscmd 4, "Labor: Übertrage Rohdaten nach Staging..."
    LaborStagingFuellen labsql, pid, erfolg, rAfGes
    tg1 = GetTickCount
    Debug.Print ">>> Export: " & (tg1 - tg0) & "ms"
    If rAfGes And (erfolg = 1 Or erfolg = 3) Then
     LaborAusStaging pid, rAfGes
    End If ' erfolg<>0
    ' Isolation Level zurücksetzen
#Else
   Call laborges(labsql, rAfGes, erfolg)
#End If
...
' weiterhin wurde \r\n bei Normtext, Testhinweis und EText durch '' ersetzt
   If erfolg = 0 Then
    ...
    GoTo abermals
   Else ' Erfolg = 0 => <>0
    meldTxt = "Labor: " & rAfGes & " Sätze in laborneu über " & Switch(erfolg = 1, "SELECT INTO OUTFILE", erfolg = 2, "SQLinSQL", erfolg = 3, "csv", True, "laborlangsam") & " eingefügt."
    syscmd 4, meldTxt
    Debug.Print meldTxt
   End If ' erfolg <>0
  End If ' not laborlangsam
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in doPatvonMO/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
' in doPatvonMO und
Function laborges(ByRef labsql$, rAfGes&, erfolg%)
 Dim rsLb As ADODB.Recordset, i%, ErrNr&, ErrDes$
sql = _
"SELECT " & vbCrLf & _
...
";"
  Set rsLb = Nothing
  myFrag rsLb, sql, adOpenStatic, MOCon, adLockReadOnly, "9992147483647", rAf
  If Not rsLb.BOF Then
   tg1 = GetTickCount
   For i = 0 To 6
    If Not IsNull(rsLb.Fields(i)) Then
     If Right$(rsLb.Fields(i), 7) <> "VALUES " Then
      meldTxt = "Labor gesammelt: Füge Sätze aus " & rsLb.Fields(i).name & " ein ...."
      syscmd 4, meldTxt
      myEFrag rsLb.Fields(i), rAfGes, DBCn, , ErrNr, ErrDes
      If ErrNr Then
       InsKorr DBCn, rsLb.Fields(i), rAfGes, ErrDes, , ErrNr
      End If
      meldTxt = "Labor gesammelt: " & rAfGes & " Sätze aus " & rsLb.Fields(i).name & " eingefügt mit Fehler Nr. " & ErrNr & IIf(ErrNr <> 0, " " & ErrDes, "")
      Debug.Print meldTxt
      syscmd 4, meldTxt
      If i = 6 Then
       If rAfGes = 0 Then erfolg = 0 Else erfolg = 2
      End If
'        doPatvonMO = 3
'        Exit Function
'#If gehtnicht Then
...
'        Stop
     ...
     End If
   Next i
  End If ' Not rslb.BOF Then
End Function ' laborges(labsql$, rAfGes&, erfolg%)
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in holHAausMO/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in richtleist/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in MODiagnosen/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in richtHA/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in MOLeistungen/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#End If
 ...
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in moausgeb/" + AnwPfad)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
Public Function suchfi(pNr&, fI$, notObRlike%, MServ$)
 ...
 rst.Open "SELECT c.TABLE_NAME tn, c.COLUMN_NAME cn -- , a.column_name cn" & vbCrLf & _
          ...
          "JOIN information_schema.tables t ON t.table_catalog=c.table_catalog AND t.table_schema=c.table_schema AND t.table_name=c.table_name AND t.table_type='BASE TABLE'" & vbCrLf & _
...
'       Call moausgeb(MOCon, rst!Tn, False, "`" & rst!Cn & "`=" & pNr)
       Dim moausg$
       moausg = "`" & fn & "`="
       If fn <> "" Then moausg = moausg & rsu.Fields(fn)
       Call moausgeb(MOCon, rst!tn, False, moausg)
       gefu = True
       GoTo weiter
...
Public Function RightPad(s As String, nLen As Long) As String
    If Len(s) >= nLen Then
...
Public Function LeftPad(s As String, nLen As Long) As String
    If Len(s) >= nLen Then
        LeftPad = left(s, nLen)
    Else
