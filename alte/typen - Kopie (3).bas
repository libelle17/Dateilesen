...
'     Else ' j = 1 Then
      Set rs = myEFrag("SELECT COALESCE((SELECT MAX(fid) FID FROM `faelle` WHERE pat_id = " & rFa(i).Pat_id & " AND quartal = '" & rFa(i).Quartal & "' AND bhfb = " & DatFor_k(rFa(i).BhFB) & " AND bhfe1 = " & DatFor_k(rFa(i).BhFE1) & " AND ausgst = " & DatFor_k(rFa(i).ausgst) & "),(SELECT MAX(fid)+1 FID FROM `faelle`))")
'     End If
