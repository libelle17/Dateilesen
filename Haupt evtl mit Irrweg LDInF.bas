...
Function doMachSQL0$(TName$, NobAI%) ' NobAI = es existiert eine Zeile ohne Autoincrement
 ...
 NobAI = 0
 Tabl = LCase(TName)
' If Tabl = "forminhaltform_abk" Then Stop
 ...
 Do
  ...
  If rsAdSc.EOF Or obHierKAI Then
   ...
   If rsAdSc.EOF Then
    Exit Do
   Else
    VStr = Tbk(ti) & "(i)." & colW(rsAdSc!column_name)
    Select Case rsAdSc!data_type
'    Case 2: ' "integer"
...
Public Function PutReg(dlg As Dialog)
 ...
 Call ZeigDateien(dlg)
End Function ' PubReg
Public Function ZeigDateien(dlg As Dialog)
 dlg.hfrm.obAcc = dlg.obAcc
 dlg.hfrm.obMy = Not dlg.obAcc
 If dlg.obAcc Then
  dlg.hfrm.Ziel = dlg.MdB
 Else
  If dlg.obMyQuelle Then
   dlg.hfrm.Ziel = "Quelle"
  ElseIf dlg.obMyQuelle1 Then
   dlg.hfrm.Ziel = "Quelle1"
  ElseIf dlg.obMyQuelle2 Then
   dlg.hfrm.Ziel = "Quelle2"
  End If
 End If
...
Public Function key(KeyCode%, Shift%, frm As Form)
 On Error GoTo fehler
 If KeyCode = 27 Then
    If frm.Name = "Dialog" Then
     Call HolReg(frm)
    ElseIf frm.Name = "Lese" Then
     Call HolReg(frm.dlg)
    End If
    frm.Hide
...
Function holAB(frm As Lese)
 Const Quellmdb$ = "u:\anamnese\quelle.mdb"
 Dim rq As New ADODB.Recordset, f1 As ADODB.Field, f2 As ADODB.Field, Runde&, Pat_ID&
 frm.BytesBez = "Datens‰tze:"
 rq.Open "select count(*) as ct from anamnesebogen", CStrAcc & Quellmdb
 frm.GesBytes = rq!ct
 On Error GoTo fehler
 DBCn.BeginTrans
 If obMySQL Then Call DBCn.Execute("set foreign_key_checks = 0")
 rq.Close
 rq.Open "anamnesebogen", CStrAcc$ & Quellmdb
 Do While Not rq.EOF
  Runde = Runde + 1
  If Not rAnaAdo Is Nothing Then If rAnaAdo.State = 1 Then rAnaAdo.Close
  rAnaAdo.CursorLocation = adUseClient
  rAnaAdo.Open "select * from anamnesebogen where pat_id = " & rq!Pat_ID, DBCn, adOpenStatic, adLockOptimistic
  If Not rAnaAdo.BOF Then
   Call anaUpd(rq)
  Else
   Call anaIns(rq)
  End If
...
'  On Error GoTo f0
 ...
 If BrichAb Then
  frm.Ausgabe = "Kopieren der Anamnesebˆgen von '" & Quellmdb & "' nach: '" & frm.Ziel + "' abgebrochen." & vbCrLf + frm.Ausgabe
 Else
  frm.Ausgabe = "Fertig mit Kopieren der Anamnesebˆgen von '" & Quellmdb & "' nach: '" & frm.Ziel + "'" & vbCrLf + frm.Ausgabe
 End If
 DBCn.CommitTrans
 Exit Function
fehler:
Select Case MsgBox("FNr: " + CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.Source), "", CStr(Err.Source)) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "Aufgefangener Fehler in holAB/" + App.Path)
 Case vbAbort: Call MsgBox("Hˆre auf"): End
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Public Function anaUpd(rq As Recordset)
 Dim i&, sql$, raf&, Wert$
   On Error GoTo fehler
   sql = "update anamnesebogen set "
   For i = 0 To rq.Fields.Count - 1
    Select Case LCase(rq.Fields(i).Name)
     Case "pat_id", "prim"
     Case Else
      sql = sql & kla & rq.Fields(i).Name & klz & " = "
      Select Case rq.Fields(i).Type
         Case adBoolean
           If rq.Fields(i) Then Wert = "-1" Else Wert = "0"
         Case adBigInt, adCurrency, adDecimal, adDouble, adInteger, adLongVarBinary, adNumeric, adSingle, adSmallInt, adTinyInt, adUnsignedBigInt, adUnsignedInt, adUnsignedSmallInt, adUnsignedTinyInt
       ...
       Wert = ""
    End Select
    If Not obMySQL And i Mod 100 = 99 Then
   ...
   Exit Function
fehler:
Select Case MsgBox("FNr: " + CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.Source), "", CStr(Err.Source)) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "Aufgefangener Fehler in anaUpd/" + App.Path)
 Case vbAbort: Call MsgBox("Hˆre auf"): End
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Public Function anaIns(rq As Recordset)
 Dim i&, sql1$, sql2$, raf&, Wert$, mprim&, rs As ADODB.Recordset
   On Error GoTo fehler
