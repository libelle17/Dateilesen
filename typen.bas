...
setz:
  ...
  If SammelInsert = 0 Or i = UBound(rBr) Then
    altmode = myEFrag("SELECT @@global.sql_mode", , DBCn).Fields(0)
    myEFrag "SET GLOBAL sql_mode='STRICT_TRANS_TABLES'", , DBCn ' NO_ENGINE_SUBSTITUTION
    Open "v:\briefespeichern.txt" For Append As #151
    Print #151, csql.Value
    Close #151
    InsKorr DBCn, csql.Value, rAf, ErrD, , ErrN
    If ErrN Then
     If SammelInsert = 1 And ErrN = -2147217900 Then
...
",COALESCE(Bsnr,0) Bsnr,COALESCE(Ersteller,'') Ersteller,COALESCE(Änderer,'') Änderer,COALESCE(Kennzeichen_arzt_technik,'') Kennzeichen_arzt_technik" & _
",COALESCE(Auftragsschluessel,'') Auftragsschluessel,COALESCE(Auftragstext,'') Auftragstext FROM `leistungen` WHERE Pat_ID=" & pid & " ORDER BY `ZeitPunkt`"
 myFrag rs, sql
 If rs.EOF Then
  ReDim roLe(0)
 Else ' rs.EOF Then
...
Public Function leistungenSpeichern(SammelInsert%, BezfSp%)
 ...
 Call csql0.AppVar(Array(" INSERT ", sqlIGNORE, "INTO `leistungen` (FID,Pat_ID,ZeitPunkt," & _
     ...
     "Pruefzeit,Kalkzeit,Bsnr,Ersteller,Änderer,Kennzeichen_arzt_technik,Auftragsschluessel,Auftragstext)        VALUES"))
 If Not AllePat Then
...
setz:
  ...
  csql.AppVar Array("(", rLe(i).FID, ",", rLe(i).Pat_ID, ",", DatFor_k(rLe(i).Zeitpunkt), ",'", rLe(i).Leistung, "','", rLe(i).ArtdUs, "','", rLe(i).LAnzl, "','", rLe(i).LUhrz, "','", rLe(i).LfBegr, "','", _
   ...
   rLe(i).Pruefzeit, ",", rLe(i).Kalkzeit, ",", rLe(i).BSNR, ",'", rLe(i).Ersteller, "','", rLe(i).Änderer, "','", rLe(i).Kennzeichen_arzt_technik, "','", rLe(i).Auftragsschluessel, "','", rLe(i).Auftragstext, "')")
...
ElseIf ErrNumber = -2147217833 Or InStrB(ErrDescr, "Daten zu lang") <> 0 Or InStrB(ErrDescr, "Data too long") <> 0 Then
 ...
 For k = IIf(SammelInsert <> 0, 1, i) To IIf(SammelInsert <> 0, UBound(rLe), i)
  ...
  If Len(rLe(k).Kennzeichen_arzt_technik) > maxi(22) Then maxi(22) = Len(rLe(k).Kennzeichen_arzt_technik)
  If Len(rLe(k).Auftragsschluessel) > maxi(23) Then maxi(23) = Len(rLe(k).Auftragsschluessel)
  If Len(rLe(k).Auftragstext) > maxi(24) Then maxi(24) = Len(rLe(k).Auftragstext)
 Next k
 If obTrans <> 0 Then If myEFrag("SELECT COUNT(1) FROM information_schema.innodb_trx WHERE trx_mysql_thread_id = CONNECTION_ID()", , DBCn).Fields(0) <> 0 Then ComTrans ' DBCn.CommitTrans: obtrans = 0
nochmal:
 Set rsc = New ADODB.Recordset
    ...
    If maxL > 0 Then
     For k = IIf(SammelInsert <> 0, 1, i) To IIf(SammelInsert <> 0, UBound(rLe), i)
      Select Case m
       ...
       Case 22: Lese.Ausgeb "   Verkürze Inhalt von rLe.Kennzeichen_arzt_technik: '" & rLe(k).Kennzeichen_arzt_technik & "' -> '" & Left$(rLe(k).Kennzeichen_arzt_technik, maxL) & "'", True: rLe(k).Kennzeichen_arzt_technik = Left$(rLe(k).Kennzeichen_arzt_technik, maxL)
       Case 23: Lese.Ausgeb "   Verkürze Inhalt von rLe.Auftragsschluessel: '" & rLe(k).Auftragsschluessel & "' -> '" & Left$(rLe(k).Auftragsschluessel, maxL) & "'", True: rLe(k).Auftragsschluessel = Left$(rLe(k).Auftragsschluessel, maxL)
       Case 24: Lese.Ausgeb "   Verkürze Inhalt von rLe.Auftragstext: '" & rLe(k).Auftragstext & "' -> '" & Left$(rLe(k).Auftragstext, maxL) & "'", True: rLe(k).Auftragstext = Left$(rLe(k).Auftragstext, maxL)
      End Select
     Next
    ElseIf maxL < 0 Then
     GoTo nochmal
