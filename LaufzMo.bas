...
Type plzDaten ' für doPatientenlzMo
 pid As Long
 Datum As Date
 Uhrzeit As Date
 Arzt As String
...
' kommentiert aufgerufen in doPatientenlzMo
Private Sub doplink()
...
#If False Then
' kommentiert aufgerufen in doPatientenlzMo und rufThFestleg
...
Function doPatientenlzMo(Optional obohnerueckfrage% = 0, Optional obphp% = 0) ' aufgerufen aus: Patientenlaufzettel_Click() und MDIForm_Activate
    ' für PDF lesen
'#Const obadobe = 1
#If obadobe = 1 Then
   ...
   If Not rTerm.EOF Then
    ...
    If noch <> 0 Then
     ...
     For iru = 0 To UBound(plzDat)
      Call dodoplzMo(CStr(plzDat(iru).pid), plzVerz, plzDat(iru).Datum, CStr(plzDat(iru).Uhrzeit), , plzDat(iru).Arzt, , obphp)
      erstZ = erstZ + 1
      noch = noch - 1
...
'     Arzt = rTerm!Arzt
'     Call dodoplzMo(CLng(PID), plzVerz, Datum, Uhrzeit, , Arzt, , obphp)
'     erstZ = erstZ + 1
'     rTerm.MoveNext
'    Loop
'    Exit For
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vNS, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in doPatientenlzMo/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
End Function 'doPatientenlzMo
...
' letzte eGFR
Public Function letztGFRMo(pid&, Optional PAlter#, Optional obweibl%, Optional EthnicGroup% = 1) As labtyp
' letztGFRMo = hollwert(PID, "GFR','GFRCYS','GFRK','GFRM','GFRT','GFRW','CREACL','GFC','GFCK1','GFCK2','GFCK3','GFCK4','GFCM1','GFCW1','GFCW2','MDRD", "ml/m")
 ...
 If lc.Zp - LA.Zp > 1 Or (lc.WertSg <> "" And LA.WertSg = "") Then ' wenn letztes Kreatinin mehr als einen Tag jünger als die letzte eGFR ist
  ...
  If lc.WertSg <> "" Then letztGFRMo.WertSg = 175 * (lc.WertSg ^ -1.154) * altPalter ^ -0.203 * IIf(altobweibl, 0.742, 1) * IIf(EthnicGroup = 2, 1.212, 1)
  letztGFRMo.Abkü = "MDRD"
  letztGFRMo.Einheit = "ml/min"
  letztGFRMo.Zp = lc.Zp
 Else
  letztGFRMo = LA
 End If
End Function ' letztGFRMo#(PID&)
...
' in: doPLZeinzeln, PLZausListe_Click, los (Lese), Command1_Click (PatListe, 2x), PatListe.plz, MFG_Click (Patliste), doPatientenlzMo, Patientenlaufzettel_Click (PatAuswahl und AnBog)
Function dodoplzMo(Pat_id$, plzVerz$, Optional Datum As Date, Optional Uhrzeit$, Optional mitanzeig%, Optional Arzt$, Optional zmaxges% = 8, Optional obphp% = 0) ' Patientenlaufzettel
 Dim DN$, DatNr&
 Dim TI!(41)
 Dim m%, p%
...
' Dim wasAvan$ ' was genau verordnet wurde
 ...
 Dim dmtyp$
 Dim obdm%, obweibl%, obdement%, GebDat As Date
...
'  sql(i) = "SELECT zeitpunkt zp, IF(ISNULL(wert),IF(ISNULL(kommentar),'',kommentar),wert) wert FROM (" & lsql & ") AS innen WHERE abkü LIKE 'FT3%' ORDER BY zp DESC"
  ...
  If obdm Then
   ...
   sql(i) = "SELECT zeitpunkt zp, concat (art, ' <- ', inhalt) wert FROM `eintraege` WHERE pat_id = 1789 AND inhalt LIKE '%" & UmwfSQL(NName) & "%' ORDER BY zp DESC"
   nurpath(i) = 3
   i = i + 1
  End If ' obdm Then
...
'  IF Not rTh.BOF THEN metdos = rTh!mg
  Dim lGFR As labtyp
  lGFR = letztGFRMo(CLng(Pat_id), PAlter, obweibl, aRisk.EthnicGroup)
...
' Hier kommt das Labor rein!
  ...
  Call LaborInsPLZMo(Pat_id, SelbstStatus, raDatBOF, Matr, MForm, mBreiten)
...
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vNS, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in dodoplzMo/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
End Function ' dodoplzMo

' in dodoplzMo (ausgelagert)
...
' for dodoplzMo und LaborIns1 <- tuBriefStandAlone
Sub LaborInsPLZMo(Pat_id$, SelbstStatus%, raDatBOF%, ByRef Matr$(), ByRef MForm%(), ByRef mBreiten$()) 'Word.Document, Pat_id&)
...
#End If
 ...
 altgru = -1
 Dim raLwSource$
 raLwSource = raLw.source
 If raLw.State = 0 Then
  Set raLw = Nothing
  raLw.Open raLwSource, DBCn, adOpenStatic, adLockReadOnly
 End If
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vNS, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in LaborInsPLZMo/" + AnwPfad)
 Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
End Sub      ' LaborInsPLZMo
...
' ZeilZ = InputBox("Bitte gewünschte Tabellenzeilenzahl eingeben:", "Patientenlaufzettel Eingabe", ZeilZ)
 If IsNumeric(pid) And IsNumeric(ZeilZ) Then
  Call dodoplzMo(pid, plzVz, Now, Now - Int(Now), True, "", CDbl(ZeilZ))
 End If
