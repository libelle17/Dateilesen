...
Function AnpassForm()
 ...
 On Error Resume Next
 Call AnbogVar
 PatName = GesNamFn(Forms(Anmnbi).adoRS) ' Forms!anamnesebogen!Nachname & ", " & Forms!anamnesebogen.Vorname
 On Error GoTo fehler
...
' aufgerufen IN adoRS_MoveComplete
...
#If debu <> 0 Then
       ...
       Print #313, Now() & " " & frm.adoRS!Nachname & ", " & frm.adoRS!Vorname
       dnr = 0
       T0 = Timer
       Takt = T0
#End If
...
#End If
 frm.vTextB(188) = frm.adoRS!vorET
' IF (ISNULL(frm.adoRS!Ther1) OR frm.adoRS!Ther1 = vns) AND frm.adoRS!Diabetestyp <> "-" THEN
'  frm.vTextB(166) = TherUmw(TherArt(Pat_id, -1))
'  Debug.Print "frm.adoRS!Ther1:", frm.adoRS!Ther1
'  Dim ta$ ' 7.9.09
'  ta = TherUmw(TherArt(Pat_id, -1))
'  IF frm.adoRS!Ther1 <> ta THEN
...
' END IF
' IF (ISNULL(frm.adoRS!Ther1) OR frm.adoRS!Ther1 = vns) AND frm.adoRS!Diabetestyp <> "-" THEN
'  IF frm.adoRS!Ther1 <> TherUmw(TherArt(Pat_id, -1)) THEN frm.vTextB(166) = TherUmw(TherArt(Pat_id, -1))
' END IF
' IF (ISNULL(frm.adoRS!TherAkt) OR frm.adoRS!TherAkt = vns) AND frm.adoRS!Diabetestyp <> "-" THEN
'  IF frm.adoRS!TherAkt <> TherUmw(TherArt(Pat_id, 0)) THEN frm.vTextB(173) = TherUmw(TherArt(Pat_id, 0))
' END IF
...
' frm.adoRS!SchGr = raFa!SchGr
 ...
 End Select
 frm.MA0Zahl.BackColor = IIf(frm.adoRS!obkothny, &HFF0000, -2147483633)
 frm.MA1Zahl.BackColor = IIf(frm.adoRS!obschade, 65535, -2147483633)
 frm.MA2Zahl.BackColor = IIf(frm.adoRS!obwagner, 65535, -2147483633)
 frm.MA3Zahl.BackColor = IIf(frm.adoRS!obHammer, 65535, -2147483633)
#If debu <> 0 Then
       Tvor = Takt: Takt = Timer
       dnr = dnr + 1
       Print #313, Format(Takt - Tvor, "0.00") & "      " & Format(Takt - T0, "0.00") & " (" & dnr & ")"
...
#If debu <> 0 Then
       ...
       Print #313, Format(Takt - Tvor, "0.00") & "      " & Format(Takt - T0, "0.00") & " (" & dnr & ")"
#End If
 If IsNull(frm.adoRS!Diabetestyp) Or frm.adoRS!Diabetestyp = vNS Then ' Diabetestyp
  farbe = 16777215 ' weiß
 Else
...
' Vorbelegungen für (zusätzliche) Diagnosen im Feld mdi
 ...
 TabakStatus = doTabakSt(frm.adoRS!Pat_id)
 Select Case TabakStatus
  Case 0
  Case 1
   Call KRAdd(frm, frm.adoRS!tabakex, "Nikotinabusus", "F17.1", Zn, "F17.1", , , , "vTextB", 184) ' Tabak
  Case 3
   Call KRAdd(frm, frm.adoRS!tabakex, "Nikotinabusus vor langem", "F17.1", Zn, "F17.1", , , , "vTextB", 184) ' Tabak
  Case 2
   Call KRAdd(frm, frm.adoRS!tabakakt, "Nikotinabusus", "F17.1", gesi, "F17.1", , , , "vTextB", 186) ' "Tabak"
 End Select
#If debu <> 0 Then
       Tvor = Takt: Takt = Timer
       dnr = dnr + 1
...
'  IF NPGrund = "KW" OR NPGrund = "MER" THEN
 ...
 verform = IIf(IsNull(frm.adoRS!Verformungen), vNS, LCase(frm.adoRS!Verformungen))
 BewEinsch = IIf(IsNull(frm.adoRS!Bewegungseinschränkungen), vNS, LCase(frm.adoRS!Bewegungseinschränkungen))
...
#End If
 ...
 obAutNP = 0
 HarnB = 0
 EntlSt = IIf(IsNull(frm.adoRS![Entleerungsstörungen Magen]), vNS, LCase(frm.adoRS![Entleerungsstörungen Magen]))
 If Left(EntlSt, 1) = "j" Then
  obAutNP = obAutNP - 1
  Call doGilbE(frm, "G99", "vTextB", 95) ' Entleerungsstörungen Magen
 End If
 EntlSt = IIf(IsNull(frm.adoRS![Entleerungsstörungen Harnblase]), vNS, LCase(frm.adoRS![Entleerungsstörungen Harnblase]))
 If Left(EntlSt, 1) = "j" Then
  obAutNP = obAutNP - 1
  Call doGilbE(frm, "G99", "vTextB", 96) ' Entleerungsstörungen Harnblase
  HarnB = -1
 End If
 EntlSt = IIf(IsNull(frm.adoRS!Sexualstörung), vNS, LCase(frm.adoRS!Sexualstörung))
 If Left(EntlSt, 1) = "j" Then
...
#End If
 ...
 Else
  Dim Geschwür$
  Geschwür = IIf(IsNull(frm.adoRS!Geschwür), vNS, LCase(frm.adoRS!Geschwür))
...
#End If
 ...
 HSchw = IIf(IsNull(frm.adoRS!Herzschwäche), vNS, LCase(frm.adoRS!Herzschwäche))
 If InStrB(HSchw, "j") <> 0 Or InStrB(HSchw, "dyspn") <> 0 Then
  herzin = True
  HIGrund = "Frgb"
 End If
 Lunge = IIf(IsNull(frm.adoRS!Lunge), vNS, LCase(frm.adoRS!Lunge))
 ...
 WS = IIf(IsNull(frm.adoRS!WS), vNS, LCase(frm.adoRS!WS))
...
#End If
 ...
 If InStrB(LCase(frm.adoRS![Entleerungsstörungen Harnblase]), "j") <> 0 And InStrB(frm.vTextB(147), "N31") = 0 And InStrB(frm.vTextB(147), "N39") = 0 Then
  Call KRAdd(frm, "Anamn", "Störung der Harnblase", "N31.9", gesi, "N31", "N39", , , IIf(InStrB(LCase(frm.adoRS![Entleerungsstörungen Harnblase]), "j"), "Entleerungsstörungen Harnblase", vNS))
 End If
...
#End If
 Dim zähn$
 zähn = IIf(IsNull(frm.adoRS!Zähne), vNS, LCase(frm.adoRS!Zähne))
 If InStrB(zähn, "ungsbed") <> 0 Then
  Call KRAdd(frm, "Unters", "Kariöse Zähne", "K02.9", gesi, "K02", , , , "vTextB", 133) ' Zähne
 End If
 
 Dim beinöd$
 If IsNull(frm.adoRS!BeinödVen) Then
  beinöd = vNS
 Else
  beinöd = LCase$(frm.adoRS!BeinödVen)
 End If
 ...
 End If
' beinöd = IIF(ISNULL(frm.adoRS!Beinbefund), vns, LCase$(frm.adoRS!Beinbefund))
 If IsNull(frm.adoRS!Beinbefund) Then
  beinöd = vNS
 Else
  beinöd = LCase$(frm.adoRS!Beinbefund)
 End If
...
'If Ther1 <> frm.adoRS!Ther1 THEN frm.vTextB(166) = Ther1
 frm.Erklärung = vNS
 frm.Inhalt = vNS
...
#If True Then
 Call LabwertA(frm, LA_AlbCre, ">", 20, "Nephropathie", "N08.3", gesi, "mg/gCrea", "N08.3", "N08.3", 50)
#Else
...
' raLau.Open sql & " AND abkü = ""COR_S"" " & sql1, DBCn, adOpenDynamic, adLockReadOnly
 myFrag raLau, sql & " AND abkü = ""COR_S"" " & sql1
#If debu <> 0 Then
       Tvor = Takt: Takt = Timer
       dnr = dnr + 1
...
#End If
' Dim bmi#
' IF frm.adoRS!Gewicht < 0 THEN frm.vTextB(10) = -frm.adoRS!Gewicht
' IF NOT ISNULL(frm.adoRS!Gewicht) AND NOT ISNULL(frm.adoRS!Größe) THEN
'  IF frm.adoRS!Größe > 0 THEN
'   bmi = frm.vTextB(10) * IIF(frm.vTextB(10) < 3, 100, 1) / frm.adoRS!Größe / frm.adoRS!Größe ' * 10000
'   Do
...
'   IF frm.adoRS!bmi >= 28 THEN
'    Call KRAdd(frm, "BMI = " + Format$(frm.adoRS!bmi, "##,#"), "Übergewicht", "E66.99", gesi, "E66", , , , "vTextB", 11) ' Gewicht
'   END IF
   Select Case frm.adoRS!bmi
    Case Is >= 40
    Call KRAdd(frm, "BMI = " + Format$(frm.adoRS!bmi, "##,#"), "Übergewicht BMI>=40", "E66.92", gesi, "E66", , , , "vTextB", 11) ' Gewicht
    Case Is >= 35
    Call KRAdd(frm, "BMI = " + Format$(frm.adoRS!bmi, "##,#"), "Übergewicht BMI 35-<40", "E66.91", gesi, "E66", , , , "vTextB", 11) ' Gewicht
    Case Is >= 30
    Call KRAdd(frm, "BMI = " + Format$(frm.adoRS!bmi, "##,#"), "Übergewicht BMI 30-<35", "E66.90", gesi, "E66", , , , "vTextB", 11) ' Gewicht
   End Select
'  END IF
' END IF
...
'  aktSt = LCase$(frm.adoRS.Fields(AugFeld(i)))
  If IsNull(frm.adoRS.Fields(AugFeld(i))) Then
   aktSt = vNS
  Else
   aktSt = LCase$(frm.adoRS.Fields(AugFeld(i)))
  End If
 ...
 obMI = 0
 obPTCA = 0
 Anginap = IIf(IsNull(frm.adoRS![Angina pectoris]), vNS, LCase(frm.adoRS![Angina pectoris]))
 ...
 ElseIf InStrB(Anginap, "leicht") <> 0 Or InStrB(Anginap, "z.n.") <> 0 Or InStrB(Anginap, "evtl") <> 0 Or InStrB(Anginap, "manchmal") <> 0 Then
  ...
  obKHK = 1
 End If
 mi = IIf(IsNull(frm.adoRS!Herzinfarkt), vNS, LCase(frm.adoRS!Herzinfarkt))
 If Left(mi, 1) = "j" Or InStrB(mi, "stummer") <> 0 Or InStrB(mi, "vwi") <> 0 Or InStrB(mi, "hwi") <> 0 Then
  ...
  obKHK = 2
 End If
 ptca = IIf(IsNull(frm.adoRS![PTCA oder Stent]), vNS, LCase(frm.adoRS![PTCA oder Stent]))
 If InStrB(ptca, "j") <> 0 Then
  Call doGilbE(frm, "Z95.5Z", "vTextB", 78) ' PTCA oder Stent
  obPTCA = 1
  obKHK = 2
 End If
 If frm.adoRS![Bypaß kardial] <> 0 Then
  Call doGilbE(frm, "Z95.1", "vCheckB", 3) ' Bypaß kardial
  obKHK = 2
 End If
 hkBschr = IIf(IsNull(frm.adoRS![Herzkrankheit Beschreibung]), vNS, LCase(frm.adoRS![Herzkrankheit Beschreibung]))
 ...
 End If
 If frm.adoRS![Bypaß kardial] <> 0 Then Call KRAdd(frm, "Fragebogen", "Bypaß kardial", "Z95.1", Zn)
 If obMI = 1 Then Call KRAdd(frm, "Fragebogen", "Myokardinfarkt", "I21.9", Zn, "I21", "I25.2")
 
 Dim hdb$
 hdb = IIf(IsNull(frm.adoRS!Hirndurchblutungsstörung), vNS, LCase(frm.adoRS!Hirndurchblutungsstörung))
 ...
 End If
 Dim Schlag$
 Schlag = IIf(IsNull(frm.adoRS!Schlaganfall), vNS, LCase(frm.adoRS!Schlaganfall))
 ...
 End If
 Dim amput$
 amput = IIf(IsNull(frm.adoRS![Amputation]), vNS, LCase(frm.adoRS![Amputation]))
...
'Call Lese.ProgStart
  sql = "SELECT n.pat_id, " & _
        ...
        "LEFT JOIN diagnosen d ON d.pat_id = n.pat_id AND d.gicd RLIKE '^I7[034]\.' AND d.id1 = (SELECT MAX(id1) FROM diagnosen d0 WHERE d0.pat_id = d.pat_id AND (d.gicd RLIKE '^I7[034]\.')) " & _
...
'   IF frm.NoMatch THEN Exit Function
  ...
  If Not obLH Then
   ...
   If Not obLH Then
    liph = IIf(IsNull(frm.adoRS![Liphypertrophien Arme]), vNS, LCase(frm.adoRS![Liphypertrophien Arme]))
    obLH = Not obNein(liph)
    If obLH Then
...
'   IF frm.NoMatch THEN Exit Function
 ...
 Dim ameis$
 ameis = IIf(IsNull(frm.adoRS!Ameisenlaufen), vNS, LCase(frm.adoRS!Ameisenlaufen))
  ...
  For Each ctl In frm.Controls
   ...
   If fehler = 0 Then
    If ctl.DataField Like art Then
     If raDok.BOF Or PcDokPfad = vNS Then
...
' Hier noch einfügen, wenn Tabelle rfDE abgefragt werden soll
  ...
  Else
   ...
   Set rfDE = Nothing
   If LVobMySQL Then
'    Call rfDE.Open("SELECT * FROM `fuerdiagexp` WHERE ISNULL(status) AND pat_id = " & frm.adoRS!Pat_id & " AND icd RLIKE '" & icdlike & "' AND diagnose LIKE '%" & LTrim$(REPLACE$(LTrim$(REPLACE$(MDIDiag(nr), "V.a.", vNS)), "Z.n.", vNS)) & "'", DBCn, adOpenDynamic, adLockReadOnly) ' rfDE = TabÖff("fuerDiagExp", "Suche") ' pat_id, icd
    myFrag rfDE, "SELECT * FROM `fuerdiagexp` WHERE ISNULL(status) AND pat_id = " & frm.adoRS!Pat_id & " AND icd RLIKE '" & icdlike & "' AND diagnose LIKE '%" & LTrim$(REPLACE$(LTrim$(REPLACE$(MDIDiag(nr), "V.a.", vNS)), "Z.n.", vNS)) & "'"
   Else
'    Call rfDE.Open("SELECT * FROM `fuerdiagexp` WHERE ISNULL(status) AND pat_id = " & frm.adoRS!Pat_id & " AND (icd = '" & MDIICD(nr) & "' OR icd = '" & MDIICD(nr) & "V' OR icd = '" & MDIICD(nr) & "Z' OR icd = '" & MDIICD(nr) & "VZ' OR icd = '" & MDIICD(nr) & "ZV') AND diagnose LIKE '%" & LTrim$(REPLACE$(LTrim$(REPLACE$(MDIDiag(nr), "V.a.", vNS)), "Z.n.", vNS)) & "'", DBCn, adOpenDynamic, adLockReadOnly) ' rfDE = TabÖff("fuerDiagExp", "Suche") ' pat_id, icd
    myFrag rfDE, "SELECT * FROM `fuerdiagexp` WHERE ISNULL(status) AND pat_id = " & frm.adoRS!Pat_id & " AND (icd = '" & MDIICD(nr) & "' OR icd = '" & MDIICD(nr) & "V' OR icd = '" & MDIICD(nr) & "Z' OR icd = '" & MDIICD(nr) & "VZ' OR icd = '" & MDIICD(nr) & "ZV') AND diagnose LIKE '%" & LTrim$(REPLACE$(LTrim$(REPLACE$(MDIDiag(nr), "V.a.", vNS)), "Z.n.", vNS)) & "'"
   End If
...
'    Call rfDE.Open("SELECT * FROM `fuerdiagexp` WHERE pat_id = " & frm.adoRS!Pat_id & " AND icd LIKE '" & icdlike & "'", DBCn, adOpenDynamic, adLockReadOnly) ' rfDE = TabÖff("fuerDiagExp", "Suche") ' pat_id, icd
    myFrag rfDE, "SELECT * FROM `fuerdiagexp` WHERE pat_id = " & frm.adoRS!Pat_id & " AND icd LIKE '" & icdlike & "'"
...
'  rsP.Seek "=", Pat_id, IIF(Zp = 0, Zeitpunkt, Zp), RRSyst, RRDiast
  ...
  If rsP.BOF Then
   InsKorr DBCn, DBCnS, "INSERT INTO `rrparse`(pat_id,zeitpunkt,rrsyst,rrdiast,quelle) VALUES(" & Pat_id & "," & DatFor_k(IIf(Zp = 0, Zeitpunkt, Zp)) & "," & RRsyst & "," & RRdiast & ",'" & REPLACE$(Quelle, "'", "''") & "')", rAF
  End If
 Exit Function
fehler:
 Dim AnwPfad$
...
'  rsNa.Seek "=", frm.adors!Pat_id
  ...
  myFrag rsNa, "SELECT * FROM `namen` WHERE pat_id = " & Pat_id
  If Not rsNa.EOF Then
   frm.adoRS!PatName = GesNamFn(rsNa) ' rNa!Nachname & " " & rNa!Vorname
  End If
  If Not IsNull(frm.adoRS!DokPfad) Then
'   raDok.Seek "=", CLng(frm.adors!Pat_id), CStr(frm.adors!DokPfad)
'   IF Not raDok.NoMatch THEN
' frm.adoRS!Pat_id
'   raDok.Open "SELECT * FROM `dokumente` WHERE pat_id = " & Pat_id & " AND dokpfad = '" & frm.adoRS!DokPfad & "'", DBCn, adOpenDynamic, adLockReadOnly
   myFrag raDok, "SELECT * FROM `dokumente` WHERE pat_id = " & Pat_id & " AND dokpfad = '" & frm.adoRS!DokPfad & "'"
...
'    END IF
    ...
    syscmd acSysCmdSetStatus, "Erstelle Brief für " & raAn!GesName & ": 2)a) Diagnosen prüfen ..."
    obkNeph = obKeineNephropathie(Pat_id, obMakroAlb)
...
'      Call Shell("cmd /c echo ""Achtung doppelte Diagnose"" ", vbNormalNoFocus)
    ...
    If InStrB(behD, "am") <> 0 Then
     ...
     On Error GoTo fehler
    End If
    syscmd acSysCmdSetStatus, "Erstelle Brief für " & raAn!GesName & ": 3) Anamnese ..."
...
'      SET aktra = dc.Range(aktra.Start + 2, dc.Bookmarks("Urin").End)
    ...
    syscmd acSysCmdSetStatus, "Erstelle Brief für " & raAn!GesName & ": 5) Labor..."
    Call LaborIns1(dc, Pat_id, nurLabor)
    If nurLabor = 0 Then
    syscmd acSysCmdSetStatus, "Erstelle Brief für " & raAn!GesName & ": 6) Medikation..."
    Wapp.Visible = True
    Call letzteMed(dc, Pat_id)
    syscmd acSysCmdSetStatus, "Erstelle Brief für " & raAn!GesName & ": 7) Epikrise..."
...
' bei Wiederholungsbriefen
    syscmd acSysCmdSetStatus, "Erstelle Brief für " & raAn!GesName & ": 9) Prüfen von Vorberichten..."
    Call WiederHolungsBrief(rsNa!Nachname, VorDat, Wapp, dc)
    syscmd acSysCmdSetStatus, "Erstelle Brief für " & raAn!GesName & ": 10) Korrekturen..."
   ...
   syscmd acSysCmdSetStatus, "Erstelle Brief für " & raAn!GesName & ": 11) Speichern..."
   Call ZwischenSpeichern(sverz, rsNa!Nachname, rsNa!Vorname, Pat_id, infos) ' infos(4, 0), infos(15, 0)) ' Faxnr, Mail
  End With ' wapp
  On Error Resume Next
  If obStumm Then
...
'     rDT.Open "SELECT ICD, DiagSicherheit FROM `diagnosen` d WHERE d.pat_id = " & Pat_id & "  AND COALESCE(d.f6010,0)=0 ", DBCn, adOpenDynamic, adLockReadOnly
     myFrag rDT, "SELECT ICD, DiagSicherheit FROM `diagnosen` d WHERE d.pat_id = " & Pat_id & "  AND COALESCE(d.f6010,0)=0 "
   ...
   Select Case i
    Case 0
     If VorDat = CDate(0) Then
      If IsNull(rsAnam!Ther1) Then
      Else
       TherapieStr = rsAnam!Ther1
      End If
     Else
...
'   DiabTyp = IIf(IsNull(rsAnam!Diabetestyp), "", rsAnam!Diabetestyp)
   thrang(i) = ThaRang(TherapieStr, thLang, rsAnam!dmtyp) ' , DiabTyp)
   Therlang(i) = thLang
  Next i
...
'  rDT.Open "SELECT 0 FROM `diagnosen` d WHERE d.pat_id = " & Pat_id & " AND (d.icd LIKE ""N12%"" OR d.icd LIKE ""N30%"" OR d.icd LIKE ""N39%"") AND d.diagsicherheit <> 'A' AND COALESCE(d.f6010,0)=0 ", DBCn, adOpenDynamic, adLockReadOnly
  myFrag rDT, "SELECT 0 FROM `diagnosen` d WHERE d.pat_id = " & Pat_id & " AND (d.icd LIKE ""N12%"" OR d.icd LIKE ""N30%"" OR d.icd LIKE ""N39%"") AND d.diagsicherheit <> 'A' AND COALESCE(d.f6010,0)=0"
...
'     rDT.Open "SELECT d.icd AS icd, diagtext,g1.gruppe dg1 FROM `diagnosen` d LEFT JOIN `diagreihe` dr ON d.icd = dr.icd LEFT JOIN diagg1 g1 ON g1.lfdnr=dr.gi1 WHERE pat_id = " & Pat_id & " AND d.diagsicherheit <> 'A' AND COALESCE(d.f6010,0)=0 ", DBCn, adOpenDynamic, adLockReadOnly
     myFrag rDT, "SELECT d.icd AS icd, diagtext,g1.gruppe dg1 FROM `diagnosen` d LEFT JOIN `diagreihe` dr ON d.icd = dr.icd LEFT JOIN diagg1 g1 ON g1.lfdnr=dr.gi1 WHERE pat_id = " & Pat_id & " AND d.diagsicherheit <> 'A' AND COALESCE(d.f6010,0)=0 "
     Do While Not rDT.EOF
  ' Leberkrankheiten außer Leberzirrhose, Obstipation
     ...
     Loop
     If keinAss = 0 Then
'      IF rsAnam!dmtyp <> "-" AND (Now - rsAnam!GebDat) > 365 * 40 THEN
...
'   IF sKrea = vNS THEN sKrea = "0"
   lKrea = CDbl(sKrea)
   If lKrea <> 0 Then
    GFR = (140 - Alter) * rsAnam!Gewicht * IIf(rsAnam!Gewicht < 3, 100, 1) * IIf(rnam!geschlecht = "w", 0.85, 1) / lKrea / 72
    DialZt = (GFR - 10) / 3
    ...
    If obNiereWichtig Or obNephrologe Then
     ...
     Else
      Set rDial = Nothing
      myFrag rDial, "SELECT 0 FROM `diagnosen` d WHERE d.pat_id = " & Pat_id & " AND d.icd LIKE 'Z49%' AND d.diagsicherheit<>'A' AND COALESCE(d.f6010,0)=0 "
...
'    END IF
      ...
      Epi = Epi + IIf(rsAnam!dmtyp = "1", " Bei", " Nur bei") + " Typ-1-Diabetikern ist außerdem der Nutzen einer Eiweißrestriktion belegt."
     Else
...
'  rAu.Seek "=", Pat_ID
  ...
  If rsAnam!dmtyp <> "-" And rsAnam!dmtyp <> "g" Then Epi = Epi + nzw + "Kontrolluntersuchungen gemäß dem Gesundheitspass Diabetes werden empfohlen."
  Epi = Epi + nzw
  ...
  dc.bookmarks!kopfgrenze.Range = REPLACE$(REPLACE$(IIf(rsAnam!dmtyp = "-", dc.bookmarks!kopfgrenze.Range, REPLACE$(dc.bookmarks!kopfgrenze.Range, "Diabetologischer ", vNS)), "Patname", Akklang), "Patgeb", Format$(rsAnam!GebDat, "dd/mm/yyyy"))
  Exit Sub
  Dim ii%, jj%
  For ii = 1 To dc.sections.COUNT
    For jj = 1 To dc.sections(ii).Headers.COUNT
     ...
     If rsAnam!dmtyp = "-" Then dc.sections(ii).Headers(jj).Range = REPLACE$(dc.sections(ii).Headers(jj).Range, "Diabetologischer ", vNS)
    Next
  Next ii
 Exit Sub
fehler:
...
Do
 FristS = InputBox("Bitte Frist eingeben:", "Frist nach ENDe des Quartals zum Rückgriff auf das alte", 21)
...
VN = "labor2a"
Vsql = "SELECT Pat_id, eingang Zeitpunkt, befart FertigStGrad, w.Abkü, w.langtext Langtext" & vbCrLf & _
...
"n.Einheit," & vbCrLf & _
"IF(ISNULL(k.Kommentar),'',k.Kommentar) Kommentar," & vbCrLf & _
...
If LVobMySQL Then
 VN = "aktfaelle"
  Vsql = "SELECT f.pat_id AS pid, notiz, stru.leistung AS stru, chron.leistung AS chron, kt.ct AS kt, ebm.leistung AS verspau " & vbCrLf & _
 ",(SELECT icd FROM diagnosen d WHERE d.pat_id=f.pat_id AND d.gicd RLIKE '^E1[0-4]\.|^O24\.' AND (d.obdauer <> 0 OR d.fid = f.fid) ORDER BY icd LIMIT 1) icd " & vbCrLf & _
...
' aktfaelle verschieden mit Therapieart
 ...
 ", (SELECT MAX(icd) FROM diagnosen WHERE pat_id=f.pat_id AND ((gicd RLIKE '^E1[0-4]\.' AND obdauer<>0) OR (gicd='O24.4' AND obdauer=0 AND diagdatum BETWEEN qanf() AND qend()))) dicd " & vbCrLf & _
 ", f.* " & vbCrLf & _
...
VN = "qsumme"
Vsql = "SELECT SUM(`_qsumme`.`wert`) AS `summe`,`_qsumme`.`quartal` AS `quartal` FROM `_qsumme` GROUP BY `_qsumme`.`quartal` ORDER BY SUBSTR(`_qsumme`.`quartal`,2) DESC,`_qsumme`.`quartal` DESC"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1

VN = "CSII bei Typ 2"
Vsql = "SELECT `a`.`Pat_id` AS `pat_id`,`a`.`Nachname` AS `nachname`,`a`.`Vorname` AS `vorname`,`d`.`gICD` AS `icd` FROM (`anamnesebogen` `a` LEFT JOIN `diagnosen` `d` on(((`a`.`Pat_id` = `d`.`Pat_id`) AND (`d`.`gICD` REGEXP '^E1[1234].|^O24.4') ))) WHERE ((`a`.`Ther1` = 'CSII') AND (`d`.`ICD` IS NOT NULL))"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
Vsql = "SELECT IF(a.DsJ='',COALESCE(erstandm,vorgestellt),a.DsJ) >= MID(aktq(),2)-IF(LEFT(aktq() COLLATE utf8_general_ci,1)='1',1,0)" & vbCrLf & _
       ...
       " LEFT JOIN (SELECT pat_id pid FROM diagnosen WHERE gicd LIKE 'E1%' GROUP BY pat_id) d ON d.pid=a.pat_id" & vbCrLf & _
       ") a;"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
VN = "anaktk"
Vsql = "" & _
"SELECT a.`Prim` `Prim`,a.`Pat_id` `Pat_id`" & vbCrLf & _
", COALESCE((SELECT voret FROM sws WHERE pat_id=a.pat_id AND voret>qanf() ORDER BY voret DESC LIMIT 1),'') voret " & vbCrLf & _
", COALESCE((SELECT 1 FROM desktop WHERE pat_id = a.pat_id AND iconpath RLIKE '4eckblau' AND showasnote=0 LIMIT 1),0) obKothny " & vbCrLf & _
", COALESCE((SELECT 1 FROM desktop WHERE pat_id = a.pat_id AND iconpath RLIKE '4eckgelb' AND showasnote=0 LIMIT 1),0) obSchade " & vbCrLf & _
", COALESCE((SELECT 1 FROM desktop WHERE pat_id = a.pat_id AND iconpath RLIKE '4eckmagneta' AND showasnote=0 LIMIT 1),0) obWagner " & vbCrLf & _
", COALESCE((SELECT 1 FROM desktop WHERE pat_id = a.pat_id AND iconpath RLIKE '4EckHellgruen' AND showasnote=0 LIMIT 1),0) obHammer " & vbCrLf & _
"FROM anamnesebogen a;"
Call DtbCreateQueryDef(VN, Vsql)
vz = vz + 1
...
sql = "CREATE DEFINER=`praxis`@`%` FUNCTION `quelldat`(name VARCHAR(1000)) RETURNS DATE " & _
    ...
    " DECLARE Fl VARCHAR(10) DEFAULT 'Fremdlabor';" & vbCrLf & _
    " DECLARE muster VARCHAR(1000) DEFAULT '[0123456789]';" & vbCrLf & _
    ...
    " SET posf=INSTR(name,Fl);" & vbCrLf & _
    " IF POSf>0 THEN" & vbCrLf & _
    "  SET i=posf+char_LENGTH(fl);" & vbCrLf & _
    " END IF;" & vbCrLf & _
...
sql = sql & _
    ...
    "  UNION " & vbCrLf & _
    "  SELECT DATE(k.zeitpunkt) dt, k.zeitpunkt zp,f.pat_id,f.fid,schgr,vknr,ik,f.lanrid, 'F' Typ, form_abk Art " & vbCrLf & _
    "   FROM aktfvs f " & vbCrLf & _
    ...
    Vsql = Vsql & _
    "  UNION " & vbCrLf & _
    "  SELECT DATE(l.zeitpunkt) dt, l.zeitpunkt zp,f.pat_id,f.fid,schgr,vknr,ik,f.lanrid, 'L' Typ, CAST(leistung AS char) Art " & vbCrLf & _
    "   FROM aktfvs f " & vbCrLf & _
    ...
    "  UNION " & vbCrLf & _
    "  SELECT DATE(m.zeitpunkt) dt, m.zeitpunkt zp,f.pat_id,f.fid,schgr,vknr,ik,f.lanrid, 'M' Typ, 'MP' Art " & vbCrLf & _
    "   FROM aktfvs f " & vbCrLf & _
...
"  (SELECT MIN(gicd) gicd FROM diagnosen d WHERE pat_id = pid AND gicd REGEXP '^E1[0-4]\.|^O24|^R73' " & vbCrLf & _
"  ) i );" & vbCrLf & _
"RETURN typ;" & vbCrLf & _
"END"
...
" FROM namen n " & vbCrLf & _
" LEFT JOIN diagnosen d ON d.pat_id=n.pat_id AND d.gICD REGEXP '^E1[0-4]\.|^O24|^R73' GROUP BY n.pat_id,d.gicd,d.diagtext " & vbCrLf & _
") i " & vbCrLf & _
"WHERE rang=1;"
DBCn.Execute sql
...
"DECLARE muster VARCHAR(13) DEFAULT '[0123456789]';" & vbCrLf & _
"DECLARE ch VARCHAR(1);" & vbCrLf & _
"DECLARE ges VARCHAR(6) DEFAULT '';" & vbCrLf & _
...
Sub LaborIns1(dc As Object, Pat_id$, nurLabor%) ' nur in tuBriefStandalone
 ...
 ReDim MForm2(MAX(UBound(MForm, 2) - DiffBr, 1), UBound(Matr, 3)) ' erst: 26.7.11
...
' kvä.Open "kvaerzte.hae", DBCn, adOpenDynamic, adLockReadOnly 'haecn
 myFrag kvä, "SELECT * FROM kvaerzte.hae"
...
'    frm.adoRS.Open "SELECT a.*, CONCAT(nachname,' ', vorname) AS gesname FROM `anamnesebogen` WHERE exists (SELECT * FROM `aktfv` WHERE pat_id = `anamnesebogen`.pat_id) ORDER BY pat_id DESC", db, adOpenDynamic, adLockOptimistic
    Dim sqlspl$(), orderby$
...
'    frm.adoRS.Open frm.anBogCS, db, adOpenDynamic, adLockOptimistic
    myFrag frm.adoRS, frm.anBogCS, adOpenDynamic, db
   Else
    db.CursorLocation = adUseClient
'    frm.adoRS.Open "SELECT *,IIF(größe=0,'',gewicht/größe/größe*iIF(größe>3,10000,1)) bmi, GesName(pat_id) gesname  FROM `anamnesebogen` ORDER BY pat_id DESC", db, adOpenDynamic, adLockOptimistic
    myFrag frm.adoRS, "SELECT *,IIF(größe=0,'',gewicht/größe/größe*iIF(größe>3,10000,1)) bmi, GesName(pat_id) gesname  FROM `anamnesebogen` ORDER BY pat_id DESC", adOpenDynamic, db
   End If
   frm.obStumm = False
   On Error Resume Next
...
'    Err.Clear
'    ON Error Resume Next
'    IF ctl.DataSource <> frm.adoRS THEN
'     SET ctl.DataSource = frm.adoRS
'    END IF
'   Next ctl
...
Function AnBogUnload(frm As AnBog)
  ' frm.adoRS!Pat_id
 If Not frm.adoRS.EOF And Not frm.adoRS.BOF Then _
...
'     Dim rAF&, rAfL&
'     IF Not obTest THEN
'      InsKorr DBCn, DBCnS, "INSERT INTO `diagnosen`(pat_id,gesName,ICD,diagdatum,diagsicherheit,diagtext,obdauer,aktzeit) VALUES(" & q!Pat_id & ",'" & UmwfSQL(q!Name) & "','" & q!ICD & "'," & DatFor_k(aktdat) & ",'" & DiagSi & "','" & DiagText & "'," & IIF(BDTFeld = "3650", 1, 0) & "," & DatFor_k(BDT.üzpt) & ")", rAF
'      IF rAF <> 1 THEN
...
Function test_fdübertrag()
 ...
 Do While Not q.EOF
  InsKorr z, JCn, "INSERT INTO `fuerdiagexp`(name,pat_id,icd,diagnose,nurquart,zeitpunkt) VALUES('" & UmwfSQL(q!name) & "'," & q!Pat_id & ",'" & q!ICD & "','" & q!Diagnose & "'," & q!nurquart & "," & DatFor_k(Now()) & ")", rAF
  q.Move 1
 Loop
