...
' 23.1.10: beim Seitenumbruch kann auch nach der Fremdsprache noch ein Teil des Textes der genehmigten Ziffern kommen: Dr. Peter Bernius
     ...
     If Absch(aktab).art = 0 Then ' Wenn keine ermächtigte Einrichtung
      ...
      Else
       HACn.Execute "UPDATE `arzt` SET `titel_id` = " & Arzt.titel_id & ", `nlart_id` = " & Arzt.nlart_id & ", `Namenszusatz` = '" & Arzt.Namenszusatz & "', `aktzeit` = " & DatFor_k(KVDate) & ", `Nachname` = '" & doUmwfSQL(Arzt.Nachname, LVobMySQL) & "', `Vorname` = '" & doUmwfSQL(Arzt.Vorname, LVobMySQL) & "' WHERE LANR = " & Arzt.Lanr, rAF
       If rAF > 1 Then
...
'      HACn.CommitTrans
'      HACn.Execute "begin"
      HACn.Execute "DELETE FROM `arzt_has_leistungsumfang` WHERE `arzt_id` = " & idIns2, rAF
...
'  HACn.Execute "INSERT INTO `arzt`(nachname,vorname,titel_id,fachrichtung_id,lanr,nlart_id) VALUES('" & Arzt.Nachname & "','" & Arzt.Vorname & "'," & Arzt.titel_id & "," & Arzt.fachrichtung_id & "," & Arzt.LANR & " ," & Arzt.nlart_id & ") ON duplicate key update nachname = '" & Arzt.Nachname & "', vorname = '" & Arzt.Vorname & "', titel_id = " & Arzt.titel_id & ", fachrichtung_id = " & Arzt.fachrichtung_id & ", nlart_id = " & Arzt.nlart_id, rAF
  ...
  If rs.BOF Then
   ...
   Set rs = HACn.Execute("SELECT last_insert_id()")
  Else
   HACn.Execute "UPDATE `bs` SET `name` = '" & doUmwfSQL(BS.name, LVobMySQL) & "',`straße` = '" & doUmwfSQL(BS.Straße.Value, LVobMySQL) & "',`hausnr` = '" & BS.Hausnr & "',`plz` = '" & BS.plz & "',`ort_id` = " & BS.Ort_id & ",`bsart_id` = " & BS.bsart_id & ",`sprechzeiten_id` = " & BS.sprechzeiten_id & ",`rollst` = " & BS.Rollst & ",`aktzeit` = " & DatFor_k(KVDate) & " WHERE `bsnr` = '" & BS.BSNR & "'", rAF
   If rAF > 1 Then
  ...
  idIns1 = rs.Fields(0)
  HACn.BeginTrans
  HACn.Execute "DELETE FROM `tel` WHERE `bs_id` = " & idIns1, rAF
  For jj = 0 To telz
   Set rs = Nothing
   ...
   End If
  Next jj
  HACn.Execute "DELETE FROM `fax` WHERE `bs_id` = " & idIns1, rAF
  For jj = 0 To faxz
   Set rs = Nothing
   ...
   End If
  Next jj
  HACn.Execute "DELETE FROM `mail` WHERE `bs_id` = " & idIns1, rAF
  For jj = 0 To mailz
   Set rs = Nothing
  ...
  If Absch(aktab).art = 0 Then ' Wenn keine ermächtigte Einrichtung
   HACn.Execute "UPDATE `arzt_has_bs` SET aktzeit = " & DatFor_k(KVDate) & ", obneben = " & BS.obNBS & ", obang = " & BS.obAng & " WHERE `bs_id` = " & idIns1 & " AND `arzt_id` = " & idIns2, rAF
   If rAF <> 1 Then
    HACn.BeginTrans
...
'      HACn.Execute "begin"
      HACn.Execute "DELETE FROM `bs_has_leistungsumfang` WHERE `bs_id` = " & idIns1, rAF
      For jj = 1 To LeiUz
       idIns = indIns(HACn, HACnS, "leistungsumfang", "leistungsumfang", LeiU(jj).Value, "idleistungsumfang")
'       ON Error Resume Next
...
'       ON Error GoTo fehler
      Next jj
      HACn.Execute "DELETE FROM `bs_has_genehmigung` WHERE `bs_id` = " & idIns1, rAF
'      Dim dop%
'      Dim jjjj%
      For jj = 1 To Genehz
       dop = 0
       ...
       End If
      Next jj
      HACn.Execute "DELETE FROM `bs_has_vertragsangebot` WHERE `bs_id` = " & idIns1, rAF
      For jj = 1 To VertAz
       dop = 0
'       IF jj = 20 THEN Stop
'       IF VertA(jj).Value = "Franz-Josef-Strauß-Allee 11" THEN Stop
