...
Option Explicit
Public Lanr&, Nachname$, Vorname$, Titel$, Stra$, HNR$, ZIP$, CTY$, TEL$
...
Private Sub OK_Click()
 ...
 For i = 0 To Option1.COUNT - 1
  If Option1(i).Value <> 0 Then
   Lanr = Option1(i).Tag
   Set rserg = myEFrag("SELECT MAX(nachname)nn,MAX(vorname)vn,MAX(titel)t,MAX(strasse)s,MAX(Hausnummer)HNR,MAX(plz)ZIP,MAX(Stadt)CTY,MAX(Telefon)TEL FROM lanrpraxis WHERE lanr=" & Lanr, rAf)
   If rAf Then
    Nachname = rserg!NN
    Vorname = rserg!VN
    Titel = rserg!t
    ...
    TEL = rserg!TEL
   Else ' rAF Then
    Nachname = "": Vorname = "": Titel = "": Stra = "": HNR = "": ZIP = "": CTY = "": TEL = ""
   End If ' rAF Then Else
   Exit For
  End If ' Option1(i).Value <> 0 Then
 Next i
 Unload Me
End Sub ' OK_Click
