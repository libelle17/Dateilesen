...
'    Call DBCn.Execute("UPDATE `anamnesebogen` SET " & "`" & SpName & "`" & " = """ & replace$(NeuInh, """", """""") & """ WHERE pat_id = " & rNa(0).Pat_id, AfN)
'   END IF
   If Not SpMod(Len(NeuInh), "anamnesebogen", rsc, NeuInh) Then ' dann Memo-Feld
    Call DBCn.Execute("UPDATE `anamnesebogen` SET `" & SpName & "` = """ & REPLACE$(NeuInh, """", """""") & """ WHERE pat_id = " & rNa(0).Pat_id, AfN)
   End If
   If rsAnm.State = 0 Then rsAnm.Open ZCStr, DBCn, adOpenStatic, adLockOptimistic
   MerkNeuInh = NeuInh
...
If rsAnm.State = 0 Then
 rsAnm.Open "SELECT * FROM `anamnesebogen` WHERE pat_id = " & rNa(0).Pat_id, DBCn, adOpenStatic, adLockOptimistic
 Resume
End If
