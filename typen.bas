...
Public Function laboryusSpeichern(SammelInsert%, BezfSp%, j&, Optional rAf&, Optional sfkco%, Optional Tbnm = "laboryus")
 ...
 Call csql0.AppVar(Array(" INSERT ", sqlIGNORE, "INTO `" & LCase$(Tbnm) & "` (UsLfd,DatID,SatzID," & _
     ...
     "Titel,NVorsatz,NVors2,BefArt,Abrechnungstyp,GebüOrd,Auftraggeber,Patienteninformation,Geschlecht,Pat_idUrsp," & _
     "Pat_id_0,Pat_id_1,Pat_id_2,Pat_id_3,Pat_id_4,Pat_id_5,Pat_id_6,Pat_id_7,ZeitpunktLaborneu,Pat_id_Laborneu," & _
     "verglichen,AfN,z7,SQL7,namsp,wertsp,termsp,Termine,TM_Pat_id,Spalte_16)        VALUES"))
sql:
 csql.m_Len = 0
 For i = j To j
...
setz:
  ...
  csql.AppVar Array("(", rLu(i).UsLfd, ",", rLu(i).DatID, ",", rLu(i).SatzID, ",'", rLu(i).Satzart, "','", rLu(i).Satzlänge, "','", rLu(i).Auftragsnummer, "','", rLu(i).Auftragsschlüssel, "',", DatFor_k(rLu(i).Eingang), ",", DatFor_k( _
   ...
   rLu(i).Pat_idUrsp, "','", rLu(i).Pat_id_0, "','", rLu(i).Pat_id_1, "','", rLu(i).Pat_id_2, "','", rLu(i).Pat_id_3, "','", rLu(i).Pat_id_4, "','", rLu(i).Pat_id_5, "','", _
   rLu(i).Pat_id_6, "','", rLu(i).Pat_id_7, "',", DatFor_k(rLu(i).ZeitpunktLaborneu), ",'", rLu(i).Pat_id_Laborneu, "',", DatFor_k(rLu(i).verglichen), ",", rLu(i).AfN, ",", rLu(i).z7, ",'", _
   rLu(i).SQL7, "',", rLu(i).namsp, ",", rLu(i).wertsp, ",", rLu(i).termsp, ",'", rLu(i).Termine, "',", rLu(i).TM_Pat_ID, ",", DatFor_k(rLu(i).Spalte_16), ")")
...
ElseIf ErrNumber = -2147217833 Or InStrB(ErrDescr, "Daten zu lang") <> 0 Or InStrB(ErrDescr, "Data too long") <> 0 Then
 ...
 For k = IIf(SammelInsert <> 0, j, i) To IIf(SammelInsert <> 0, j, i)
  ...
  If Len(rLu(k).Pat_idUrsp) > maxi(15) Then maxi(15) = Len(rLu(k).Pat_idUrsp)
  If Len(rLu(k).Pat_id_0) > maxi(16) Then maxi(16) = Len(rLu(k).Pat_id_0)
  If Len(rLu(k).Pat_id_1) > maxi(17) Then maxi(17) = Len(rLu(k).Pat_id_1)
  If Len(rLu(k).Pat_id_2) > maxi(18) Then maxi(18) = Len(rLu(k).Pat_id_2)
  If Len(rLu(k).Pat_id_3) > maxi(19) Then maxi(19) = Len(rLu(k).Pat_id_3)
  If Len(rLu(k).Pat_id_4) > maxi(20) Then maxi(20) = Len(rLu(k).Pat_id_4)
  If Len(rLu(k).Pat_id_5) > maxi(21) Then maxi(21) = Len(rLu(k).Pat_id_5)
  If Len(rLu(k).Pat_id_6) > maxi(22) Then maxi(22) = Len(rLu(k).Pat_id_6)
  If Len(rLu(k).Pat_id_7) > maxi(23) Then maxi(23) = Len(rLu(k).Pat_id_7)
  If Len(rLu(k).Pat_id_Laborneu) > maxi(24) Then maxi(24) = Len(rLu(k).Pat_id_Laborneu)
  If Len(rLu(k).SQL7) > maxi(25) Then maxi(25) = Len(rLu(k).SQL7)
  If Len(rLu(k).Termine) > maxi(26) Then maxi(26) = Len(rLu(k).Termine)
 Next k
nochmal:
 Set rsc = New ADODB.Recordset
 Set rsc = DBCnOSchema(adSchemaColumns, Array(Empty, Empty, LCase$(Tbnm), Empty))
    ...
    If maxL > 0 Then
     For k = IIf(SammelInsert <> 0, j, i) To IIf(SammelInsert <> 0, j, i)
      Select Case m
       ...
       Case 15: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_idUrsp: '" & rLu(k).Pat_idUrsp & "' -> '" & left$(rLu(k).Pat_idUrsp, maxL) & "'", True: rLu(k).Pat_idUrsp = left$(rLu(k).Pat_idUrsp, maxL)
       Case 16: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_id_0: '" & rLu(k).Pat_id_0 & "' -> '" & left$(rLu(k).Pat_id_0, maxL) & "'", True: rLu(k).Pat_id_0 = left$(rLu(k).Pat_id_0, maxL)
       Case 17: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_id_1: '" & rLu(k).Pat_id_1 & "' -> '" & left$(rLu(k).Pat_id_1, maxL) & "'", True: rLu(k).Pat_id_1 = left$(rLu(k).Pat_id_1, maxL)
       Case 18: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_id_2: '" & rLu(k).Pat_id_2 & "' -> '" & left$(rLu(k).Pat_id_2, maxL) & "'", True: rLu(k).Pat_id_2 = left$(rLu(k).Pat_id_2, maxL)
       Case 19: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_id_3: '" & rLu(k).Pat_id_3 & "' -> '" & left$(rLu(k).Pat_id_3, maxL) & "'", True: rLu(k).Pat_id_3 = left$(rLu(k).Pat_id_3, maxL)
       Case 20: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_id_4: '" & rLu(k).Pat_id_4 & "' -> '" & left$(rLu(k).Pat_id_4, maxL) & "'", True: rLu(k).Pat_id_4 = left$(rLu(k).Pat_id_4, maxL)
       Case 21: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_id_5: '" & rLu(k).Pat_id_5 & "' -> '" & left$(rLu(k).Pat_id_5, maxL) & "'", True: rLu(k).Pat_id_5 = left$(rLu(k).Pat_id_5, maxL)
       Case 22: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_id_6: '" & rLu(k).Pat_id_6 & "' -> '" & left$(rLu(k).Pat_id_6, maxL) & "'", True: rLu(k).Pat_id_6 = left$(rLu(k).Pat_id_6, maxL)
       Case 23: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_id_7: '" & rLu(k).Pat_id_7 & "' -> '" & left$(rLu(k).Pat_id_7, maxL) & "'", True: rLu(k).Pat_id_7 = left$(rLu(k).Pat_id_7, maxL)
       Case 24: Lese.Ausgeb "   Verkürze Inhalt von rLu.Pat_id_Laborneu: '" & rLu(k).Pat_id_Laborneu & "' -> '" & left$(rLu(k).Pat_id_Laborneu, maxL) & "'", True: rLu(k).Pat_id_Laborneu = left$(rLu(k).Pat_id_Laborneu, maxL)
       Case 25: Lese.Ausgeb "   Verkürze Inhalt von rLu.SQL7: '" & rLu(k).SQL7 & "' -> '" & left$(rLu(k).SQL7, maxL) & "'", True: rLu(k).SQL7 = left$(rLu(k).SQL7, maxL)
       Case 26: Lese.Ausgeb "   Verkürze Inhalt von rLu.Termine: '" & rLu(k).Termine & "' -> '" & left$(rLu(k).Termine, maxL) & "'", True: rLu(k).Termine = left$(rLu(k).Termine, maxL)
      End Select
     Next
    ElseIf maxL < 0 Then
     GoTo nochmal
