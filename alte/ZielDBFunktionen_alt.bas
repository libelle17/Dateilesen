...
Public Type dmptyp
 ...
 FE(14) As Integer ' Folgeerkrankungen bekannt
 FEn(14) As Integer ' Folgeerkrankungen bekannt
 ...
 VorM(9) As Integer
 obGlib As AndiDiabMedType
 obmetf As AndiDiabMedType
 obGlucI As AndiDiabMedType
 obSHGlin As AndiDiabMedType
 obGlit As AndiDiabMedType
 insz As Integer
 obIns As Boolean
 obAnal As Boolean
 obHMG As Boolean
...
' Dim dt.obglib As Boolean, dt.obmetf As Boolean, dt.obgluci As Boolean, dt.obshglin As Boolean, dt.obglit As Boolean, _
...
If diI("E10", pid) Then DT.dtyp = "1" Else If diI("E11", pid) Then DT.dtyp = "2"
...
With raAna
 ...
 End If
 If diI("Z34", pid) Or diI("Z33", pid) Then
  TabPr "Schwangerschaft laut ICD-Diagnosen:", "ja"
  DT.obSchw = True
 End If
 Dim vordat As Date '(nur Attrappe)
 Call TherAuskunft(pid, 0, DT.insz, vordat, DT.obIns, DT.obAnal, DT.obGlib, DT.obmetf, DT.obGlucI, DT.obSHGlin, DT.obGlit, DT.obHMG, DT.obAntihyp, DT.obThro)
 ...
 If DT.dtyp = "1" And Not DT.obIns And Not DT.obAnal Then ' Medikamentenplan fehlt oder keine Dosierung angegeben
  ...
  rez.Close
 End If
 DT.dspmed = (DT.obIns Or DT.obAnal Or DT.obGlib Or DT.obmetf Or DT.obGlucI Or DT.obSHGlin Or DT.obGlit)
...
' End If
 ...
 End If
 DT.PrRR = GetPrRR(raAna, RRsyst%, RRdiast%)
 If DT.dtyp = "2" Then
  If DT.PrRR <> "" Then
...
' Handlungsbedarf: Krankenhauseinweisungen
 If obPosi(![Keto pa]) Then
  TabPr "Ketoazidosen /12 Mon: ", CStr(![Fremde Hilfe pa])
 End If
...
'   If bmi > 24.9 Then dt.ernb = True
 ...
 Dim rrEmpfehlung$
 rrEmpfehlung = rrEmpf(RRsyst, RRdiast, pid)
...
' Dim rEin As DAO.Recordset
 If lies.obMySQL Then
  sql = "select * from eintraege where pat_id = " & CStr(pid) & " and (" + artspez + " and ((inhalt like ""%augenb%"" and not inhalt like ""%augenbl%"" and not inhalt like ""%augen"") or (inhalt like ""%augenarzt%"" or inhalt like ""%augenärzt%"") or (inhalt like ""% aa%"" and not inhalt like ""% aag%""))) or (art = ""aa"" or art = ""augen""))" & " and zeitpunkt > " & datform(Now() - 550) & " order by zeitpunkt desc limit 3"
 Else
  sql = "select top 3 * from eintraege where pat_id = " & CStr(pid) & " and (" + artspez + " and (inhalt like ""%augenb[!l]%"" or inhalt like ""%augen[aä]rzt%"" or inhalt like ""% aa[!g]%"")) or (art = ""aa"" or art = ""augen""))" + " and zeitpunkt > " & datform(Now() - 550) & " order by zeitpunkt desc"
 End If
...
' Call LeistungsExport0
 ...
 Do While LenB(Datei) <> 0
  If InStrB(Datei, " PID ") <> 0 Then
   ...
   If DateiDatum >= QAnf(ZQuart(Now() - 14)) Then
...
Function TherAuskunft(Pat_id, obanf%, Optional insz%, Optional vordat As Date, Optional obIns, Optional obAnal, Optional obGlib, Optional obmetf, Optional obGlucI, Optional obSHGlin, Optional obGlit, Optional obHMG, Optional obAntihyp, Optional obThro, Optional obAntikoag, Optional Qmax$, Optional obRezIns As Boolean, Optional DT$) As TherapieArt
...
'Medikamente:
 ...
 For i = 1 To MPz 'MPNrl = MPNrA To MPNrE
  ...
  End If
  obGlib = 0: obmetf = 0: obGlucI = 0: obSHGlin = 0: obGlit = 0
  obIns = 0: obAnal = 0: obHMG = 0: obAntihyp = 0: obThro = 0
  insz = 0
  obNIns = 0
...
'    rMA.Seek "=", medi
    Set raMa = Nothing
    raMa.Open "select -hmg as j_hmg, -hypt as j_hypt, -thro as j_thro, -antikoag as j_antikoag, -glib as j_glib, -metf as j_metf, -gluci as j_gluci, -shglin as j_shglin, -glit as j_glit, -ins as j_ins, -AnAl as j_anal, m.* from medarten m where medikament = '" & medi & "'", DBCn, adOpenDynamic, adLockReadOnly
    If raMa.BOF Then
     If DosH > 0 Then
'      MsgBox "Medikament: " + Medi + " noch nicht in Tabelle MedArten erfaßt"
      ...
      DBCn.Execute ("insert into medarten(langname,medikament,hinzugefueft,pat_id) values('" & Med(AktMz) & "','" & UCase$(medi) & "'," & datform(Now) & "," & Pat_id & ")")
     End If
    Else
  ...
  Else
    Select Case insz
     Case 0
      ...
      End If
     Case 1
      If obGlib Or obGlucI Or obSHGlin Or obGlit Or obmetf Then
       Thfakt = komb
      Else
       Thfakt = ct
      End If
...
#End If
 Select Case MsgBox("FNr: " + CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vbNullString, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in TherAuskunft/" + AnwPfad)
  ...
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
End Function ' TherAuskunft(pat_id, obanf%, insz, Optional obIns, Optional obAnal, Optional obGlib, Optional obMetf, Optional obGlucI, Optional obSHGlin, Optional obGlit, Optional obHMG, Optional obAntihyp, Optional obThro)
Function diI(icd_str, Optional pid) As Boolean
 ...
 If Pat_id = 0 Then
  ...
  Exit Function
 End If
 diIsql = "select " & IIf(lies.obMySQL, "", "top 1 ") & "* from diagnosen where pat_id = " & Pat_id & " and icd like '" & icd_str & "%" & "' and diagsicherheit in (""G"",""V"")"
 Dim lddat As Date
