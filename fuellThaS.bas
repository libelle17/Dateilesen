...
BEGIN
DECLARE rc INTEGER(10); SET @inpid=inpid; DELETE FROM therarten WHERE pat_id=@inpid; SET @vzahl = ROW_COUNT(); INSERT INTO therarten(pat_id,zp,mpnr,therart,insart,grund,abspos,aktzeit,stbyte) 
SELECT pid,zp,mpnr,thart,ia,gru,abspos,NOW()aktzeit,stbyte FROM(
WITH RECURSIVE ausw AS(
WITH dsort AS( 
 SELECT ROW_NUMBER()OVER(PARTITION BY pid ORDER BY zp,MPNr)rang
  , COALESCE(LAG(zp,1) OVER (PARTITION BY pid ORDER BY zp,MPNr),'1900-01-01') lzp
  , COALESCE(LAG(ia,1) OVER (PARTITION BY pid ORDER BY zp,MPNr),'-10') lia 
  , i.* FROM ( 
  ...
  WHERE insulinpumpe<>0 AND (x.pat_id=@inpid)
 UNION 
 ...
 UNION 
  ...
  WHERE form_abk IN ('rp','lar','prp','plar') AND feld IN ('medikament','txtMedKey','VerordnungsZeile') AND feldinh RLIKE 'fine|Nad|micro fi|[0-9] {0,1}mm|lantus|tresiba|levemir|basal|protaphan|semglee|abasaglar|semilente|awiqli' AND NOT feldinh RLIKE 'fine {0,1}touch|Katheter|Paradigm|Mio|Flexl|Tender|d li|link|autosoft|Minimed|Sohlen|Oberarm|Fußbett|Schuh|Wanderh|Lancets fine|easy(-release| set)|quick {0,1}set|mmHg|insight|mylife|inset|insulinset|infusion|polster|szinti|dana|fexo|enadura|infektionsnadeln|port|magnes|medtronic|sicherheitslan|omnipod|orbisoft|orbit soft|nadellanz|schlauch|sure|tamponade|trusteel|varisoft|knoten|fine point|flex link|microlet fine|verkürz|vasofix|truesteel|sterile lanzetten|stahlnad|[68]0 {0,1}cm|alkohol|thin lanc|haut|TESTSTR|variosoft|Nadellänge|^BD Micro Fine Lancetten G 33 200 Stück$'
  AND (x.pat_id=@inpid)
 UNION 
 ...
 UNION 
  ...
  FROM eintraege x WHERE ((x.pat_id=@inpid) AND (art IN ('334','335','iplan') ))
 UNION (
	...
	WHERE art='iplan' AND (x.pat_id=@inpid) AND inhalt LIKE 'Insulinplan für%'
 ...
 UNION 
  ...
  FROM eintraege x FORCE INDEX (patid_inhalt) WHERE (x.pat_id=@inpid) AND inhalt RLIKE '^Insulinplan(?! für)'
  GROUP BY pid, zp) 
 UNION 
 ...
 UNION 
  ...
  WHERE (x.pat_id=@inpid) AND insdat NOT IN(0,18991230)
 ) i 
 ) SELECT 0 Vorl,SPACE(7)VorTha, d1.* FROM dsort d1 WHERE rang=1
 UNION SELECT aw.rang, aw.Thart, d2.* FROM dsort d2 
 JOIN ausw aw ON TRUE AND d2.rang=(SELECT MIN(rang)FROM dsort WHERE rang>aw.rang AND zp>aw.bis)
) 
SELECT COALESCE(LAG(ThArt)OVER(PARTITION BY pid ORDER BY rang),SPACE(7))lthart, aw.* FROM ausw aw
 WHERE TRUE
 AND NOT (Thart='Diät' AND VorTha IN ('CSII','ICT','GLP1ICT') AND NOT EXISTS(SELECT 1 FROM sws WHERE pat_id=aw.pid AND voret BETWEEN aw.lzp AND aw.zp))
 AND NOT (lia=-2 AND ia=2 AND NOT (thart<>VorTha AND thart IN('GLP1','GLP1Ins','GLP1ICT')) AND zp BETWEEN lzp AND ADDDATE(lzp,92)) 
 AND NOT (VorTha='CSII' AND ia<>4)
)i
WHERE thart<>lthart
; SET rc=ROW_COUNT(); UPDATE anamnesebogen x SET ther1=(SELECT therart FROM therarten WHERE pat_id=x.pat_id ORDER BY zp, mpnr LIMIT 1), therakt =(SELECT therart FROM therarten WHERE pat_id=x.pat_id ORDER BY zp DESC, mpnr DESC LIMIT 1) WHERE (x.pat_id=@inpid); RETURN rc; END
