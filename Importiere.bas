...
Public Const ExtraPatienten$ = "'^zutun|^bestellungen|^seminar|^cgms ger|^fehler|^teambesprechung|^ideen|^einladungen|^neues'"
...
Public Function holiAzu&(ByVal Lanr&) ' hole interne Arztzuordnungsnummer
  ...
  If SafeArrayGetDim(iLanr) <> 0 Then ' wenn iLanr initialisiert
   ...
   Next i
  End If
  DBCn.Execute "INSERT INTO `lanrpraxis`(lanr,Nachname,Vorname,Titel,Anschrz,Strasse,Hausnummer,PLZ,Stadt,Telefon,Telefax,Abteilung,Name,KVNr,BSNR,Betriebsstaettename) VALUES(" & Lanr & ",'','','','','','','','','','','','','','','')", rAF
  If rAF = 1 Then
   holiAzu = DBCn.Execute("SELECT last_insert_id() id")!id
' folgende Alternative brauchts ned, wenn DBCn nicht gleichzeitig von zweitem Thread bedient wird:
'   holiAzu = DBCn.Execute("SELECT id FROM lanrpraxis WHERE lanr=" & LANR)!id
  Else
   holiAzu = -1
  End If
  If SafeArrayGetDim(iLanr) = 0 Then ReDim iLanr(0) Else ReDim Preserve iLanr(UBound(iLanr) + 1)
...
' Call rs.Open("quelle.`forminhaltfeld`", cn, adOpenStatic, adLockReadOnly)
 Set rs = DBCn.Execute("SELECT abkü, labor, langtext, einheit FROM `laborparameter`")
 Do While Not rs.EOF
  Set sLp = New sLpar
...
' END IF
 ...
 Set rs = DBCn.Execute("SELECT DISTINCT kennung FROM `unbekannte kennungen`", rAF)
 Do While Not rs.EOF
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & ", ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + IIf(IsNull(Err.source), vNS, CStr(Err.source)) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler IN doFormVorb/" + AnwPfad)
...
Function eintrhist()
 ...
 On Error GoTo fehler
 Set rq = DBCn.Execute("SELECT GROUP_CONCAT(pat_id) FROM `namen` WHERE nachname RLIKE " & ExtraPatienten)
 ...
 If LenB(hist) <> 0 Then
  ...
  Do While Not rq.EOF
   ...
   If rz2.BOF Then
    DBCn.Execute "INSERT INTO `eintrhist2`(fid,abspos,aktzeit,stbyte,id) VALUES(" & rq!FID & "," & rq!absPos & "," & DatFor_k(rq!AktZeit) & "," & rq!StByte & "," & rz1!id & ")", rAF
   End If
   rq.Move 1
...
' Dim i1s As New CString, i2s As New CString ' Inhalt 1 (-> Patient 1), Inhalt 2 (->Patient 2)
 ...
 If frm.dlg.obBDT Then
  If frm.dlg.ÜberTabelle <> 0 Then
   ...
   DBCn.Execute "DROP TABLE IF EXISTS `inlalt`", rAF
   frm.Ausgeb "Kopiere Tabelle `" & DefDB(DBCn) & "`.`inlk` IN Vortabelle `" & DefDB(DBCn) & "`.`inlalt` ...", False
   On Error Resume Next
   DBCn.Execute "RENAME TABLE `inlk` to `inlalt`", rAF
   On Error GoTo fehler
   frm.Ausgeb "Erstelle Tabelle `" & DefDB(DBCn) & "`.`inlk` ...", False
   DBCn.Execute "CREATE TABLE IF NOT EXISTS `inlk` (id int(10) UNSIGNED NOT NULL AUTO_INCREMENT, stbyte int(10), breite char(3) NOT NULL, kennung char(4) NOT NULL, inhalt varchar(10000) NOT NULL, PRIMARY KEY (id), KEY kennunginhalt (kennung,inhalt(6))) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_german2_ci", rAF
   frm.Ausgeb "Leere Tabelle `" & DefDB(DBCn) & "`.`inlk` ...", False
   DBCn.Execute "DELETE FROM inlk", rAF
   frm.Ausgeb "Stelle Zähler IN Tabelle `" & DefDB(DBCn) & "`.`inlk` ...", False
   DBCn.Execute "ALTER TABLE `inlk` auto_increment=0", rAF
   frm.Ausgeb "Lade Daten IN Tabelle `" & DefDB(DBCn) & "`.`inlk` (mit: LOAD DATA INFILE), kann dauern ...", False
   DBCn.Execute "LOAD DATA INFILE '/DATA/eigene Dateien/TMExport/" & BDTName & "' INTO TABLE `inlk` fields terminated by '' enclosed by '' lines terminated by '\r\n'  (breite,kennung,inhalt)", rAF
      ...
      Select Case cKenn(ind)
       Case 3000
        If cInha(ind) <> altpat Then ' oder am Schluss!
         obNeuerPat = True ' eingelesene Zeile trug neuen Patienten
...
' Folgendes aus vermuteten Performancegründen hier statt beim einzeln Speichern
  If frm.dlg.obBDT Then
   ...
   DBCn.Execute "UPDATE `anamnesebogen` a INNER JOIN `namen` n ON a.pat_id = n.pat_id  SET a.nachname = n.nachname, a.vorname = n.vorname, a.anrede = " & sqliif & "(n.geschlecht='m','Herr','Frau'), a.titel = n.titel, a.gebdat = n.gebdat WHERE (a.nachname = '' OR ISNULL(a.nachname))", rAF
...
'     DBCn.Execute "DELETE FROM `forminhfeld` WHERE NOT EXISTS (SELECT foid FROM `forminhkopf` WHERE foid = `forminhfeld`.foid)", rAF
'    Else
'     IF lies.obMySQL THEN
'      DBCn.Execute "DROP TABLE IF EXISTS `forminhfeld2`", rAF
'     Else
'      ON Error Resume Next
'      DBCn.Execute "DROP TABLE `forminhfeld2`", rAF
'      ON Error GoTo fehler
'     END IF
'     Dim VorZahl&, GelZahl&
'     DBCn.Execute "CREATE TABLE `forminhfeld2` LIKE `forminhfeld`", rAF
'     rs.Open "SELECT COUNT(0) ct FROM `forminhfeld`", DBCn
'     VorZahl = rs!ct
'     DBCn.Execute "INSERT INTO `forminhfeld2` SELECT fif.* FROM `forminhkopf` fik LEFT JOIN `forminhfeld` fif ON fif.foid = fik.foid", rAF
'     GelZahl = VorZahl - rAF
'     DBCn.Execute "SET foreign_key_checks=0"
'     DBCn.Execute "DROP TABLE `forminhfeld`"
'     DBCn.Execute "ALTER TABLE `forminhfeld2` rename `forminhfeld`"
...
' Dim medSL As New SortierListe
 ...
 mZ = DBCn.Execute("SELECT COUNT(0) FROM (SELECT 0 FROM medplan GROUP BY medanfang) i").Fields(0)
...
#If medikorr Then
  If rMe!MedAnfang <> medi.Med Then
   DBCn.Execute "UPDATE `medplan` SET medanfang = UPPER('" & medi.Med & "') WHERE medikament = '" & rMe!Medikament & "'", rAF
  End If
#End If
...
' Call ForeignNo
  ...
  If LenB(Med) <> 0 And Med <> """" Then
   Set rs = DBCn.Execute("SELECT 0 FROM `medarten` WHERE medikament = '" & REPLACE(Med, "'", "\\\'") & "'")
   If rs.BOF Then
   ...
   Else
    Set rs = DBCn.Execute("SELECT 0 FROM `medarten` WHERE medikament = '" & Med & "' AND (pat_id = 0 OR ISNULL(pat_id))")
    If Not rs.BOF Then
     Call DBCn.Execute("UPDATE `medarten` SET pat_id = " & medSL.Item(i).Pat_id & " WHERE medikament = '" & Med & "'", rAF)
    End If
' , Langname = '" & medSL.Item(i).LT & "'
    Set rs = DBCn.Execute("SELECT 0 FROM `medarten` WHERE medikament = '" & Med & "' AND (langname = '' OR ISNULL(langname))")
    If Not rs.BOF Then
     Call DBCn.Execute("UPDATE `medarten` SET LangName = '" & medSL.Item(i).LT & "' WHERE medikament = '" & Med & "'", rAF)
    End If
    Call DBCn.Execute("UPDATE `medarten` SET anzahl = " & medSL.Item(i).mvPatZahl & " WHERE medikament = '" & Med & "'", rAF)
   End If
  End If
 Next i
' Call ForeignYes
...
Function testload()
 ...
 Dim T1!, T2!
 Call Lese.ProgStart
 Call rdn.Open("SELECT stbyte, datei FROM `namen` LEFT JOIN `eintragszahlen` USING (stbyte) GROUP BY stbyte", DBCn)
 Call getAktByte
 Do While Not rdn.EOF
  erg = Dir(rdn!Datei)
  If LenB(erg) <> 0 Then
'   DBCn.Execute "LOAD DATA INFILE '/DATA/eigene Dateien/TMExport/" & erg & "' INTO TABLE `inlk` fields terminated by '' enclosed by '' lines terminated by '\r\n'  (breite,kennung,inhalt) SET StByte = " & AktByte & ", pat_id = IF(kennung=3000,inhalt,0)", rAF
   T1 = Timer
   DBCn.Execute "LOAD DATA INFILE '/DATA/eigene Dateien/TMExport/" & erg & "' INTO TABLE `inlk` fields terminated by '' enclosed by '' lines terminated by '\r\n'  (breite,kennung,inhalt) SET StByte = " & AktByte, rAF
'  DBCn.Execute "LOAD DATA INFILE '/DATA/eigene Dateien/TMExport/Temiz x0119153.BDT' INTO TABLE `inlk` fields terminated by '' enclosed by '' lines terminated by '\r\n'  (breite,kennung,inhalt) SET StByte = " & AktByte & ", pat_id = IF(kennung=3000,inhalt,0)", rAF
   T2 = Timer
   DoEvents
...
Function getAktByte()
 Dim rs As New Adodb.Recordset
 On Error GoTo fehler
 Set rs = DBCn.Execute("SELECT stbyte FROM `eintragszahlen` ORDER BY stbyte DESC")
 Call Lese.ProgStart
 If rs.BOF Then
  VorByte = 0
 Else
...
' 8.1.16 diese Bedingung scheint aktuell IN Leere zu laufen, da zu deren Erfüllung oder Ablehnung aktuell kein Pumpenrezept
...
#Else
 ...
 Do While Not rsa.EOF
  ...
  If Not rs1.BOF Then
   DBCn.Execute "UPDATE `anamnesebogen` SET therakt = '" & rs1!therart & "' WHERE pat_id = " & rsa!Pat_id & " AND therakt <> '" & rs1!therart & "'", rAF
...
'       Call CnOpen(False, zwiCStr, frm)
       Call acon(quelleT)
'       IF lies.obmysql THEN Call DBCn.Execute("use " & myDB) ' nicht nötig, da hier kein lies.obmysql
...
'      myfrag rsAdo, "SELECT (MAX(pat_id) + 1) AS pid FROM `namen`"
     ...
     End If
     DBCn.Execute ("update namen SET stbytea = " & AktByte & " WHERE pat_id = " & rInhalt) ' 17.11.19 um Programmabbrüchen während Import nachspüren zu können
...
'      rFa(UBound(rFa)).letzteRegel = rInhalt
   ...
   ' 10.1.10: Muß sich irgendwann auf 4285 geändert haben
     ...
     If ArraInd >= 5 Then
      ...
      Dim uers As New Adodb.Recordset, uerunde%, insrunde%, Tabl$
      Dim nnafeld$
      For insrunde = 1 To 2
       If insrunde = 1 Then Tabl = "liuez": nnafeld = "name" Else Tabl = "ueberwvon": nnafeld = "nachname"
       For uerunde = 1 To 2
        Set uers = Nothing
        myFrag uers, "SELECT id FROM " & Tabl & " WHERE kvnr = '" & Arra(5) & "' AND titel = '" & Arra(1) & "' AND vorname = '" & Arra(2) & "' AND zusatz = '" & Arra(3) & "' AND " & nnafeld & " = '" & Arra(4) & "'"
        If uers.EOF Then
         InsKorr DBCn, DBCnS, "INSERT INTO " & Tabl & "(kvnr,titel,vorname,zusatz," & nnafeld & ") VALUES('" & Arra(5) & "','" & Arra(1) & "','" & Arra(2) & "','" & Arra(3) & "','" & Arra(4) & "')", rAF
        Else ' uers.EOF
         Exit For
        End If ' uers.EOF
       Next ' uerunde
...
'    Debug.Print aFäl!Pat_id, aFäl!Nachname, aFäl!Vorname, aFäl!SchGr, aFäl!FID, aFäl!Quartal, aFäl!BhFB
   ...
   Case 5000, 6200 ' 6200 = Tagesdatum (BDT-Beschreibung ZI 1994)
    ...
    messDatumD = messDatum
    If obHausBesuch Then
     If UBound(rFa) = 0 Then Call neuQuartal(frm, QuartalStr(messDatum))
...
'       rRr(UBound(rRr)).Bemerkung = "" ' 21.9.20; stimmt doch nicht
  ...
  Case 6260 ' Therapie, bei uns: Eintrag Tamara Hartmann
   ...
   Case 6295 ' Formularabkürzung
    FormAbk = rInhalt
'    IF FormAbk LIKE "eDMP*" THEN Halt
    Select Case rInhalt
           ...
           If rMe(UBound(rMe)).Medikament = "" Then
            ...
            Else ' Not rpzn.BOF THEN
             ...
             For medrunde = 0 To 1
              ...
              myFrag rsP, "SELECT pck_rezeptinfo FROM `glalle` WHERE pck_pzn='" & gef & "'"
...
'     Debug.Print "Formbezeichnung: " + FormBez
   ...
   Case 6324 ' Brief mit Pfad
    ...
    rBr(UBound(rBr)).Quelldatum = doQuelldatum(rBr(UBound(rBr)).name)
...
'    Call Laboreintr1
  ...
  Case 9602 ' Beginn der Übertragung
   ...
   Call DBCn.Execute("UPDATE `eintragszahlen` SET speicherzt = " & DatFor_k(SpeicherZt) & " WHERE beginn = " & DatFor_k(rEzäBeg) & vNS, numA)
'  Case 4211 ' Ankreuzfeld HAH
'  Case 5010 ' Medikament
...
Function test7()
 ...
 Set rs = DBCn.Execute("SELECT Inhalt FROM `eintraege` WHERE pat_id = 1786 AND art=""andm""")
...
'  Call rsAnm.Open("SELECT -obmednetz AS j_obmednetz, -tkz AS j_tkz, a.* FROM `anamnesebogen` a WHERE pat_id = " & rNa(0).Pat_id, DBCn, adOpenDynamic, adLockOptimistic)
  ...
  If rsAnm.BOF Then
   Dim primnr&
   If Not rsAdo Is Nothing Then If rsAdo.State = 1 Then rsAdo.Close
   myFrag rsAdo, "SELECT MAX(prim) AS mprim FROM `anamnesebogen`"
...
Function kvnrpruef()
 ...
 ' nicht: schwerpunkt,zusatzbezeichnung,bemerkung,beme,sprechstunden,von_bis,internetadressen,`Behandlung IN Fremdsprachen`,`Rollstuhlgerechte Praxis`,verkehrsmittel,linie,`Haltestelle Parkplätze`,Wegbeschreibung,`Entfernung zur Praxis`,zahl,nichtmehr,gelöscht
  ...
  If rs.BOF Then
   DBCn.Execute "INSERT INTO `hausaerzte`(kvnr) VALUES('" & rNa(0).KVNr & "')", rAF
  Else
   On Error Resume Next
   DBCn.Execute "INSERT INTO `hausaerzte`(überschrift,name,vorname,nachname,anschrift,kvnr,telefon,telefax,e_mail,zulassungsgebiet,arzttyp,`Gemeinschaftspraxis mit`,titel,geschlecht,`straße`,plz,ort,dmpt2,dmpt1) SELECT überschrift, CONCAT_WS(' ',anrede,titelt,vorname,name),vorname,name,CONCAT_WS(', ',strasse,CONCAT_WS(' ',plz,ort)),kvnr,telefon,fax,email,fachgruppe,arzttyp,gemmit,titel,geschlecht,strasse,plz,ort,dmpt2,dmpt1  FROM `aktlue` WHERE kvnro = '" & rNa(0).KVNr & "'", rAF
   On Error GoTo fehler
  End If
  Debug.Print rAF
 End If
...
' Call ForeignNo
 ...
 Call ForeignNo0
 Call ForeignNo1
 DBCn.Execute "UPDATE `medarten` SET pat_id = 0 WHERE pat_id = " & rNa(0).Pat_id, rAF
 Call ForeignYes0
 Call ForeignYes1
 Dim medi As MediCl
 For i = 1 To UBound(rMe)
 ...
 For i = 1 To UBound(rFm)
  ...
  If obVorb Then
   ...
   If fi2 Is Nothing Then
    DBCn.Execute "INSERT INTO `forminhaltfeldinh` (feldinh,stbyte) VALUES('" & rFm(i).FeldInh & "'," & AktByte & ")", rAF
'nochmal:
    Set rsAdo = DBCn.Execute("SELECT feldinhvw,feldinh FROM `forminhaltfeldinh` WHERE feldinh = '" & rFm(i).FeldInh & "'")
    If rsAdo.EOF Then
     Set rsAdo = DBCn.Execute("SELECT feldinhvw,feldinh FROM `forminhaltfeldinh` ORDER BY feldinhvw DESC")
    End If
...
'  Dim rKv1() AS kvnrue
 ...
 For i = 0 To UBound(Infos12, 2)
'  IF i < 3 THEN
'   DBCn.Execute "UPDATE `namen` SET getha" & CStr(i) & " = " & IIf(Infos12(12, i) = vNS, 0, Infos12(12, i)) & " WHERE pat_id = " & rNa(0).Pat_id, rAF
'  END IF
  If Infos12(12, i) <> vNS Then
   DBCn.Execute "SELECT kvnr FROM `hareal` WHERE kvnr = " & Infos12(12, i), rAF
   If rAF = 0 Then
...
#End If
 ...
 DBCn.Execute "update namen n LEFT JOIN (SELECT pat_id, MIN(bhfb) bhfb, MIN(fanf) fanf FROM faelle f GROUP BY pat_id) f ON n.pat_id=f.pat_id SET kAufDat=date(IF(fanf>bhfb,fanf,bhfb)) WHERE f.pat_id=" & CStr(rNa(0).Pat_id), rAF
...
' DBCn.Execute "update quelle.`namen` SET obhierdmp = " & ohd & ", obdmp = " & od & " WHERE pat_id = " & rs!Pat_id, rAf
' Debug.Print rAf
' rs.Move 1d
'Loop
'End FUNCTION ' testa
...
'   Stop ' aus 6299 übernehmen
   ...
   rFa(i).KID = DBCn.Execute("SELECT last_insert_id()").Fields(0)
...
Function getDTyp$()
 ...
 For i = 1 To UBound(rDi)
  If (rDi(i).DiagSicherheit = "G" Or rDi(i).DiagSicherheit = "" Or rDi(i).DiagSicherheit = " ") And rDi(i).f6010 = 0 Then
   ...
   ElseIf rDi(i).ICD Like "E1*" Then
    If (rDi(i).DiagText Like "*pankr*" Or rDi(i).DiagText Like "*pancr*" Or rDi(i).DiagText Like "*sekund*") And (rDi(i).DiagSicherheit = "G" Or rDi(i).DiagSicherheit = " ") And rDi(i).f6010 = 0 Then
     getDTyp = "s"
     Exit For
    Else
...
Function DiagString$(Pat_id$, DiagTab() As CString, Optional VorDat As Date, Optional obBrief%, Optional dmseit$) ' für dynDiag, tubriefStandalone und dodoPLZ
 ...
 sql = "SELECT DiagSicherheit, DiagText, DiagSeite, DiagAttr, d.ICD, obdauer, COALESCE(d.f6010,0) f6010, obDauer<>0 j_obdauer,g1.rf,r.gi2 " & vbCrLf & _
       ...
       "ORDER BY COALESCE(d.f6010,0),g1.rf,gi2 DESC,icd;"
...
' SET rsAdo = DBCn.Execute("SELECT IF(ISNULL(wert),IF(ISNULL(kommentar),'',kommentar),wert) AS erg FROM (" & sql & ") AS sql1 WHERE abkü IN ('ALBCRE','ALBKRE','ALBQ','ALBUM','ALBUP') AND (abkü <> 'ALBU' OR wert LIKE '%<%') ORDER BY zeitpunkt DESC")
...
' SET rsAdo = DBCn.Execute("SELECT iif(ISNULL(wert),iif(ISNULL(kommentar),"""",kommentar),wert) AS erg FROM (" + sql + ") AS sql1 WHERE abkü IN (""CREAT"",""KRE02"", ""KREA"", ""KREA02"", ""KRES"") ORDER BY zeitpunkt DESC")
...
Function formInhMach()
 ...
 Call myA.Execute("DELETE FROM `forminhfeld`")
 Call myA.Execute("DELETE FROM `forminhkopf`")
...
Function doFLQuelldatum(DokName$) As Date
 Dim posfl%, p0%, ndok$
 Dim Spli$(), i&, buch$, cont As String * 10, auffuell%, auffStand%
 Const Fl$ = "Fremdlabor"
...
'    rdo.Update
  ...
  ReDim Spli(0)
 Else
  doFLQuelldatum = doQuelldatum(DokName)
 End If
End Function
   ...
   Case 130
    ...
    If DBlen > 0 Then
     ...
     If ZLen < DBlen Then
      ...
      rslen.Close
      On Error Resume Next
      Call DBCn.Execute("ALTER TABLE `" & TName & "` " & sqlALTER & " Column " & " `" & rsc!COLUMN_NAME & "` " & sqlText & "(" & ZLen & ") " & " DEFAULT " & IIf(IsNull(rsc!Default), "NULL", rsc!Default) & IIf(IsNull(rsc!collation), vNS, " COLLATE " & rsc!collation) & " COMMENT '" & rsc!Comment & "'")
...
'  Call acon(quelleT)
'  IF lies.obMySQL THEN Call DBCn.Execute("USE `" & lies.MyDB & "`")
'  Call ForeignNo0
'  Call ForeignNo1
'  IF DBCn.State = 0 THEN
'   DBCnOpen
...
'    Call DBCn.Execute("ALTER TABLE `" & TName & "`" & sqlALTER & " COLUMN `" & SpName & "` " & "MEMO")
'   Else
'    ON Error Resume Next
'nochmal:
'    Err.Clear
...
'     Call DBCn.Execute("ALTER TABLE `" & TName & "`" & sqlALTER & " COLUMN `" & SpName & "` " & sqlText & "(" & Len(SpVal) & ") " & IIf(ISNULL(rsc!collation), vNS, " COLLATE " & rsc!collation) & " default " & IIf(ISNULL(rsc!Default), " null", rsc!Default) & " comment '" & rsc!Comment & "'", rAF)
'    Else
'     Call DBCn.Execute("ALTER TABLE `" & TName & "`" & sqlALTER & " COLUMN `" & SpName & "` " & sqlText & "(" & Len(SpVal) & ")", rAF)
'    END IF
'    Dim FNr&
'    FNr = Err.Number
'    Dim FText$
...
''  DBCn.Close
''  DBcnOpen ZCStr
''  IF lies.obmysql THEN Call DBCn.Execute("use " & myDB)
''  call foreignno
''  Call DBCn.Execute("ALTER TABLE " & "`" & Tname & "`" & " " & IIf(lies.obmysql, "MODIFY", "ALTER") & " Column " & "`" & SpName & "`" & " " & "MEMO")
...
'  rlaL.Seek "=", rlAb!LangtextVW
  Set rlaL = Nothing
  rlaL.Open "SELECT * FROM `laborlangtext` WHERE langtextvw = " & rlAb!LangtextVW, DBCn, adOpenDynamic, adLockReadOnly
