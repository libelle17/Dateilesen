...
Option Explicit
Public Const FristS$ = "25" ' f¸r
#Const ohneDAO = True
Public Const vNS$ = vbNullString
Public Const vorsil$ = "labory" ' koordinieren mit labimp und labpath
...
Public FSO As New FileSystemObject ' Microsoft Scripting Runtime
...
Public Const TbZ1% = 26 ' 27 (FormInhaltForm_Abk) ' 28 (therarten)
Public Const TbZ2% = TbZ1 + 8
'Public Lese1
Public AllePat% ' alle Patienten sind in der Datei
Public obVorber% ' ob Formularvorbereitung
Public tbn$(TbZ2), Tbk$(TbZ2), noDup%(TbZ2)
Public ZielDbS$
...
' tbn(27) = "vopl"
' tbn(26) = "therarten"
 tbn(TbZ1 + 1) = vorsil & "saetze"
 tbn(TbZ1 + 2) = vorsil & "eingel"
 tbn(TbZ1 + 3) = vorsil & "us"
 tbn(TbZ1 + 4) = vorsil & "bakt"
 tbn(TbZ1 + 5) = vorsil & "wert"
 tbn(TbZ1 + 6) = vorsil & "leist"
...
'   Case "Formulare": Tbk(i) = "Fo"
   Case vorsil & "saetze": Tbk(i) = "Ls"
'   Case vorsil & "bakt":   Tbk(i) = "Lo"
   Case vorsil & "leist":  Tbk(i) = "LL"
   Case vorsil & "wert":   Tbk(i) = "Lw"
   Case vorsil & "us":     Tbk(i) = "Lu"
   Case vorsil & "eingel": Tbk(i) = "Lg"
'   Case vorsil & "pneu": Tbk(i) = "Lg"
'   Case vorsil & "plab": Tbk(i) = "Lg"
'   Case vorsil & "pnb": Tbk(i) = "Lg"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in Zeichensatz/" + AnwPfad)
...
'   Case 11:              Art = Art + "boolean" kˆnnte in String "Wahr" o.‰. umgewandelt werden
   ...
   Case "bigint"  ' neu 18.9.24 f¸r faealle->vermicode
          art = art + "double"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in doMacheTypen/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in obAutoIncr/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' in MacheTypen, doMachSQL0, doMachSQL1, doMachSQL2, doMachSQL3, doMacheTypen
Function colW$(col$)
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in doMachSQL0/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in doMachSQL1/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
'  sql = "  csql.AppVar Array(""("
      ptxt = ptxt & IIf(left$(tza, 1) = "'", "'", "") & """)" & vbCrLf & "  csql.AppVar Array(" ' Mid$(tza, 5)
     Else
      ptxt = ptxt & tza
      ptxt = ptxt & " _" & vbCrLf & "   "
     End If
...
'       ptxt = ptxt & "replace$(" & VStr & ",""'"",""\'"")" & tze1
      Case Else
       ptxt = ptxt & VStr & tze1
     End Select
    End If ' Tabl = "namen" ... else
   End If ' rsAdSc.EOF Then else
  End If ' rsAdSc.EOF Or obHierKAI Then
  rsAdSc.Move 1
 Loop '  If rsAdSc.BOF Then Exit Do
 Exit Function
fehler:
 Dim AnwPfad$
#If VBA6 Then
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in doMachSQL2/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in doMachSQL3/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' Call myEFrag("use " & myDB)
 ...
 Print #257, "Declare Function GetTickCount& Lib ""kernel32"" ()"
...
'    Print #257, " IF wdh = 0 THEN ReDim r" + Tbk(i) + "(0)"
 ...
 Print #257, "Public Sub LˆschePat(PID&, Optional obAnzeig%, Optional ohneLabor%)"
 Print #257, " Dim Tb, tbn, rAf&, ergeb$"
 Print #257, " syscmd 4, ""Lˆsche Pat. "" & pid"
 Dim tbnS$
 tbnS = " tbn = Array("
 For i = 0 To TbZ1
...
' tbn = Array("namen", "faelle", "au", "briefe", "diagnosen", "dokumente", "eintraege", "forminhkopf", "kheinweis", "lbanforderungen", "leistungen", "medplan", "rezepteintraege", "rr", "kvnrue", "dmpreihe", "desktop", "usdm", "fuss", "ulcus", "vkgd", "sws", "vopl")
 ...
 Print #257, " syscmd 4, ""Fertig mit Lˆschen von Pat. "" & pid"
 Print #257, " Exit Sub"
 Print #257, "fehler:"
 ...
 Print #257, " #END IF"
 Print #257, " SELECT CASE MsgBox(""FNr: "" + CStr(Err.Number) + vbCrLf + ""LastDLLError: "" + CStr(Err.LastDllError) + vbCrLf + ""Source: "" & cstr(nz(err.source,"""")) + vbCrLf + ""Description: "" + Err.Description, vbAbortRetryIgnore, ""aufgefangener Fehler in LˆschePat/"" + AnwPfad)"
 ...
 Print #257, " AnwPfad = App.Path"
 Print #257, " #END IF"
 Print #257, " SELECT CASE MsgBox(""FNr: "" + CStr(Err.Number) + vbCrLf + ""LastDLLError: "" + CStr(Err.LastDllError) + vbCrLf + ""Source: "" & cstr(nz(err.source,"""")) + vbCrLf + ""Description: "" + Err.Description, vbAbortRetryIgnore, ""aufgefangener Fehler in allesLˆsch/"" + AnwPfad)"
 Print #257, "  Case vbAbort: Call MsgBox("" Hˆre auf ""): Progende"
 Print #257, "  Case vbRetry: Call MsgBox(""Versuche nochmal""): Resume"
 Print #257, "  Case vbIgnore: Call MsgBox("" Setze fort ""): Resume Next"
 Print #257, " END SELECT"
 ...
 For i = TbZ2 - 1 To TbZ1 + 1 Step -1
  Print #257, " call doEntleer(frm, """ & LCase$(tbn(i)) & """)"
 Next i
 Print #257, " call doentleer(frm, vorsil & ""plab"")"
 Print #257, " call doentleer(frm, vorsil & ""pneu"")"
 Print #257, " call doentleer(frm, vorsil & ""pnb"")"
 Print #257, " call ForeignYes0"
 Print #257, " call ForeignYes1"
 Print #257, " Exit Function"
 Print #257, "fehler:"
 ...
 Print #257, " AnwPfad = App.Path"
 Print #257, " #END IF"
 Print #257, " SELECT CASE MsgBox(""FNr: "" + CStr(Err.Number) + vbCrLf + ""LastDLLError: "" + CStr(Err.LastDllError) + vbCrLf + ""Source: "" & cstr(nz(err.source,"""")) + vbCrLf + ""Description: "" + Err.Description, vbAbortRetryIgnore, ""aufgefangener Fehler in LabLˆsch/"" + AnwPfad)"
 Print #257, "  Case vbAbort: Call MsgBox("" Hˆre auf ""): Progende"
 Print #257, "  Case vbRetry: Call MsgBox(""Versuche nochmal""): Resume"
 Print #257, "  Case vbIgnore: Call MsgBox("" Setze fort ""): Resume Next"
 Print #257, " END SELECT"
...
#End If
 For i = 0 To TbZ2 - 1
  aktTbn = LCase$(REPLACE$(tbn(i), " ", "_"))
  Select Case aktTbn
   Case "formulare", "forminhfeld", "kvnrue", vorsil & "eingel", vorsil & "us", vorsil & "bakt", vorsil & "wert", vorsil & "wert", vorsil & "leist", vorsil & "saetze", "liuez", "unbek_kenn" ' "forminhaltform_abk",
   Case Else
   aktTbk = Tbk(i)
   Print #257, ""
...
'    replace$(doUmwfSQL(rs!diagtext,lies.obMySQL,false), "≥", "¸")
 ...
 syscmd 4, "Mache Typen (10) ..."
   ...
   Print #257, " AnwPfad = App.Path"
   Print #257, " #END IF"
   Print #257, " SELECT CASE MsgBox(""FNr: "" + CStr(Err.Number) + vbCrLf + ""LastDLLError: "" + CStr(Err.LastDllError) + vbCrLf + ""Source: "" & cstr(nz(err.source,"""")) + vbCrLf + ""Description: "" + Err.Description, vbAbortRetryIgnore, ""aufgefangener Fehler in " & aktTbn & "Laden/"" + AnwPfad)"
...
End If ' aktTbn = "faelle" Then
   ...
   Print #257, " AnwPfad = App.path"
   Print #257, " #END IF"
   Print #257, " Select Case MsgBox(""FNr: "" + CStr(Err.Number) + vbCrLf + ""LastDLLError: "" + CStr(Err.LastDllError) + vbCrLf + ""Source: "" & cstr(nz(err.source,"""")) + vbCrLf + ""Description: "" + Err.Description, vbAbortRetryIgnore, ""aufgefangener Fehler in " & aktTbn & "Einf/"" + AnwPfad)"
   Print #257, "  Case vbAbort: Call MsgBox("" Hˆre auf ""): Progende"
   Print #257, "  Case vbRetry: Call MsgBox("" Versuche nochmal""): Resume"
   Print #257, "  Case vbIgnore: Call MsgBox("" Setze fort ""): Resume Next"
   Print #257, " END Select"
...
'    """'"" & rFa(i)." & "rsAdSc!COLUMN_NAME & ""'"""
  ...
  Select Case aktTbn
   Case vorsil & "us", vorsil & "wert", vorsil & "bakt", vorsil & "leist", vorsil & "saetze"
   Case Else
    Print #257, " ' in LaborDirektImport"
  End Select
  pText = "Public Function " & aktTbn & "Speichern(SammelInsert%, BezfSp%"
  Select Case aktTbn
   Case vorsil & "us" ' , vorsil & "wert", vorsil & "bakt", vorsil & "leist"
    pText = pText + ", j&"
  ...
  Select Case aktTbn
   Case vorsil & "us", vorsil & "wert", vorsil & "bakt", vorsil & "leist", vorsil & "saetze"
   Case Else
    Print #257, " syscmd 4, pid & "": Speichere "" & Ubound(r" & Tbk(i) & ")+" & IIf(aktTbn = "namen", 1, 0) & " & "" S‰tze in `"" & tbnm & ""`"""
  End Select
...
' INTO `" & LCase$(tbn(i)) & "`
  ...
  Dim ianf$, iend$
  Select Case aktTbn
   Case "namen", vorsil & "saetze": ianf = "0"
   Case "formulare": ianf = "rFo1 + 1"
'   Case "forminhaltform_abk": ianf = "rFi1 + 1"
   Case "unbek_kenn": ianf = "rUn1 + 1"
   Case vorsil & "us": ianf = "j"
'   Case vorsil & "wert", vorsil & "bakt", vorsil & "leist": iAnf = "j"
   Case Else: ianf = "1"
  End Select
  Select Case aktTbn
   Case vorsil & "us": iend = "j"
'   case vorsil & "wert", vorsil & "bakt", vorsil & "leist": iend = "j"
...
'  Print #257, "  ON error resume next"
   Select Case aktTbn
    Case "formulare", "forminhfeld", "unbek_kenn" ' , "forminhaltform_abk"
    Case Else
' Folgendes aus Performancegr¸nden vor Compactdatabase gesammelt ' 25.10.24: wieder so ‰hnlich aktiviert!
     Select Case aktTbn
      Case "forminhkopf"
       Print #257, "   myEFrag ""DELETE fif FROM forminhfeld fif LEFT JOIN forminhkopf fk USING (foid) WHERE pat_ID="" & CStr(rNa(0).Pat_ID), rAfi"
...
#End If
  ...
  If noDup(i) Then ' zur Zeit vorsil & "wert"
   Print #257, "    Dim j&"
...
'   Print #257, "     myFrag rsdop, ""SELECT 0 FROM " & vorsil & "wert WHERE refnr = "" & rLw(i).RefNr & "" AND abk¸ = '"" & rLw(i).Abk¸ & ""'"""
   ...
   Print #257, "    Next j"
  End If ' noDup(i)
  Print #257, "  If i Mod 10 = 0 Then syscmd 4, pid & "": Speichere Satz "" & i & ""/"" & UBound(r" & Tbk(i) & ") + 0 & "" in `"" & Tbnm & ""`"""
...
'  Print #257, "'   Call DBCn.Execute(csql.Value, rAF) ', , adAsyncExecute) ' ausrangiert 16.6.24 wegen fehlender Feldverl‰ngerung"
  Print #257, "    InsKorr DBCn, csql.Value, rAf, ErrD, , ErrN, sfkco"
  If aktTbn = "formulare" Then
...
'  Print #257, "'    If ErrN Then Error ErrN"
  ...
  End If ' Not rfld Is Nothing Then
  Print #257, "     Else ' SammelInsert = 1 And ErrN = -2147217900 Then elseif"
  Print #257, "      Error ErrN"
  Print #257, "     End If ' SammelInsert = 1 And ErrN = -2147217900 Then else"
  Print #257, "    End If ' ErrN"
...
'  Print #257, "   END IF ' lese.obmysql AND obmitAlterTab"
  If aktTbn = "faelle" Then
   ...
   Print #257, "      End If ' rFa(i).FID <> 0 Then"
...
'   Case "forminhaltform_abk": iiAnf = "rFi1"
  ...
  Print #257, " Exit Function"
  Print #257, "fehler:"
  Print #257, "If ErrN = 0 Then"
  Print #257, " ErrDescr = Err.Description"
  Print #257, " ErrNumber = Err.Number"
  Print #257, "Else ' ErrN = 0 Then"
  Print #257, " ErrDescr = errd"
  Print #257, " ErrNumber = ErrN"
  Print #257, "End If ' ErrN = 0 Then else"
...
'   Print #257, "   If FoIDv = 0 Then FoIDv = DBCn.Execute(""SELECT (MAX(foid)+1) FROM forminhkopf"").fields(0) Else FoIDv = FoIDv + 1"
  ...
  Print #257, "IF ErrNumber = -2147217900 AND (InStrB(ErrDescr, ""Doppelter Eintrag"") <> 0 Or InStrB(ErrDescr, ""Duplicate"") <> 0) Then"
  ...
  Print #257, "  myEFrag ""SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ"", rAFi"
  Print #257, " Else"
...
'  Print #257, " IF lese.obMysql THEN Resume next ELSE resume" ' ge‰ndert 20.8.23
  ...
  If aktTbn = "faelle" Then
   ...
   Print #257, " If rFa(i).SchGr = 90 Or rFa(i).SchGr = 89 Then kat = ""PRI"" Else kat = holKat(uKas)"
   Print #257, " sqlquer = ""INSERT INTO `kassenliste`(name,kurzname,`GO`,`VKNR`,`IK`,`eingef`,pid,kateg) VALUES ("" & ""'"" & rFa(I).kasse & ""', '"" & rFa(I).kkasse_2 & ""', '"" & rFa(I).GOƒKatName & ""', '"" & rFa(I).VKNr & ""', '"" & rFa(I).IK & ""',"" & Format(NOW(), ""yyyymmddHHMMSS"") & "","" & rFa(I).Pat_id & "","" & kat & "")"""
  ...
  Print #257, " AnwPfad = App.Path"
  Print #257, "#END IF"
  Print #257, " SELECT CASE MsgBox(""FNr: "" + CStr(ErrNumber) + vbCrLf + ""LastDLLError: "" + CStr(Err.LastDllError) + vbCrLf + ""Source: "" & cstr(nz(err.source,"""")) + vbCrLf + ""Description: "" + Err.Description, vbAbortRetryIgnore, ""aufgefangener Fehler in " & aktTbn & "Speichern" + "/"" + AnwPfad)"
  Print #257, "  Case vbAbort: Call MsgBox("" Hˆre auf ""): Progende"
  Print #257, "  Case vbRetry: Call MsgBox(""Versuche nochmal""): Resume sql: If ErrNumber = 998 Then Resume sql Else Resume"
  Print #257, "  Case vbIgnore: Call MsgBox("" Setze fort ""): Resume Next"
  Print #257, " END SELECT"
 ...
 For i = 0 To TbZ1
  ...
  Select Case aktTbn
   Case "laborneu"
    Print #257, " If Not ohneLabor Then"
    Print #257, "  tg2 = GetTickCount"
    Print #257, "  tg0 = tg0 + tg2 - tg1"
    Print #257, "  tg1 = tg2"
    Print #257, "  Call " & aktTbn & "Speichern(SI, BfS)"
    Print #257, "  tg2 = GetTickCount"
    Print #257, " End If ' Not ohneLabor"
   Case Else
    Print #257, " Call " & aktTbn & "Speichern(SI, BfS)"
  End Select
  If aktTbn = "faelle" Then
...
'    Print #257, "   END IF ' not lese.obmysql"
 ...
 Print #257, "  Resume"
 Print #257, " END IF"
 Print #257, " SELECT CASE MsgBox(""FNr: "" & CStr(Err.Number) & vbCrLf & ""LastDLLError: "" & CStr(Err.LastDllError) & vbCrLf & ""Source: "" & cstr(nz(err.source,"""")) & vbCrLf & ""Description: "" & Err.Description, vbAbortRetryIgnore, ""aufgefangener Fehler in tuSpeichern/"" + AnwPfad)"
 Print #257, "  Case vbAbort: Call MsgBox("" Hˆre auf ""): Progende"
 Print #257, "  Case vbRetry: Call MsgBox(""Versuche nochmal""): Resume"
 Print #257, "  Case vbIgnore: Call MsgBox("" Setze fort ""): Resume Next"
 Print #257, " END SELECT"
 ...
 Print #257, ""
 Print #257, "Public Function tuStutzen() "
 Print #257, " Dim Errnr&, ErrD$, rAf&"
 Print #257, " Call Lese.ProgStart"
 For i = 0 To TbZ2
  aktTbn = LCase$(REPLACE$(tbn(i), " ", "_"))
  Set rfld = myEFrag("SELECT * FROM information_schema.columns WHERE table_catalog='def' AND table_schema='quelle' AND TABLE_NAME='" & aktTbn & "' AND COLUMN_NAME='pat_id'", , DBCn, , ErrNr, ErrD)
  If Not rfld.EOF Then
   Print #257, " Call myEFrag(""DELETE FROM `" & aktTbn & "` WHERE pat_id>100000"", rAf, DBCn, , ErrNr, ErrD)"
  End If
 Next i
 Print #257, "End Function ' tuStutzen(frm AS Lese, SI%, BfS%) "
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " & CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in MacheTypen/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' Call hlese.ConstrFestleg(0, hlese)
 ...
 reg = getReg(1, RegStelle, "MORes")
 If reg <> "" Then hlese.MORes = reg Else If hlese.MOBetr = 1 Then hlese.MORes = "wres"
 obStart = False
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in HolReg/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#End If
 ...
 Call fStSpei(HCU, RegStelle, "MORes", hlese.MORes)
' Call hlese.ZeigDateien
End Function ' PubReg
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in BooleanFld/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in Vergleiche/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in Tabf¸ll/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in ZSU1/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' AnwPfad = App.Path
'#END IF
'SELECT CASE MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in ProgrammLauf/" + AnwPfad)
' Case vbAbort: Call MsgBox("Hˆre auf"): Progende
' Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
' Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
'End SELECT
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in adoxtest/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in getDokPfad/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Public Function PAuswahl&(Optional Zweck$ = "Patientenauswahl", Optional PatName$)
 ...
 paus.ohne‹bertr.Visible = False
 paus.PlzMitImp.Visible = False
 paus.‹bertragung.Visible = False
 ...
 paus.Height = 8400
 altakt = Lese.Aktion
 Lese.Aktion = zielpatient
 paus.Show vbModal
 Lese.Aktion = altakt
 If paus.abgebrochen Or paus.Pat_ID = "" Then
  PAuswahl = -1
 Else
  PAuswahl = paus.Pat_ID
  PatName = paus.pat_idDaten
 End If
End Function ' PAuswahl
...
Ende:
 Exit Function
fehler:
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in key/" + App.path)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' SetDBCn Nothing
 ...
 Resume
End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in ForeignNo0/" + App.path)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in BezLˆschA/" + AnwPfad)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in BezHerstA/" + AnwPfad)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
' If obTrans <> 0 Then DBCn.CommitTrans: obTrans = 0
 ...
 Exit Function
fehler:
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in holAB/" + App.path)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' in ListeDerFehlendenDokumente_Click
...
Function doHABKK(frm As Lese)
 Const sql$ = "SELECT COUNT(0) Zahl, CONCAT(l.name,' ',l.vorname,' ',l.fachgruppe,' ',l.zulg,' ',l.ort) Hausarzt, l.telefon, l.fax, ¸bwr ¸w FROM `aktf` a LEFT JOIN `faelle` f ON a.fid = f.fid LEFT JOIN `kassenliste` k ON f.kid=k.id LEFT JOIN `aktlue` l ON ¸bwr = l.kvnro WHERE kateg = 'BKK' AND ¸bwr <> '' AND l.name <> 'Schade' GROUP BY ¸w ORDER BY Zahl DESC;"
 Dim rab As New ADODB.Recordset
...
Function doF‹wS(frm As Lese) ' fehlende ‹berweisungsscheine
 ...
 sql = "SELECT n.Pat_id, gesname(n.pat_id) Name, GROUP_CONCAT(DISTINCT CAST(f.schgr AS char)) Schgr, LEFT(GROUP_CONCAT(DISTINCT CONCAT_WS(', ', l2.Name, LEFT(l2.vorname,1),l2.ort, l2.telefon)),31) ‹berweiser, LEFT(CONCAT_WS(', ',l.Name, LEFT(l.vorname,1),l.ort, l.telefon),31) Hausarzt FROM `aktf` f LEFT JOIN `tmbrie` b ON f.pat_id = b.pat_id AND b.name LIKE '%¸w%' AND b.name LIKE '%" & left$(aktQ, 1) & "%' AND b.name LIKE '%" & Right$(aktQ, 2) & "%' LEFT JOIN `namen` n ON f.pat_id = n.pat_id LEFT JOIN `aktlue` l ON l.kvnr = n.kvnr LEFT JOIN `faelle` f2 ON f2.fid = f.fid AND ¸bwr <> '' LEFT JOIN `aktlue` l2 ON l2.kvnro = ¸bwr WHERE ISNULL(b.name) AND f.schgr<>'0' GROUP BY f.pat_id"
 Dim rab As New ADODB.Recordset
' rAb.Open sql, DBCn, adOpenDynamic, adLockReadOnly
 myFrag rab, sql
...
' Const sql$ = "SELECT v.*,n.Nachname,n.Vorname,Gebdat, d.icd, IF(¸bwv = '', and¸w, ¸bwv) AS ¸w, k.kvnr, n.info FROM `aktfv` v LEFT JOIN `namen` n ON v.pat_id = n.pat_id LEFT JOIN `faelle` f ON v.fid = f.fid LEFT JOIN `diagnosen` d ON v.pat_id = d.pat_id AND d.icd LIKE 'E1%' LEFT JOIN kvnrue k ON v.pat_id = k.pat_id WHERE notiz = '' OR ISNULL(notiz) OR k.kvnr = '' OR ISNULL(k.kvnr) GROUP BY v.pat_id ORDER BY v.pat_id, k.lfdnr"
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in ergEBM/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' Exit Function
'fehler:
'Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in holKassen/" + App.path)
' Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
' Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
' Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
'End Select
...
' If obTrans <> 0 Then DBCn.CommitTrans: obTrans = 0
 ...
 Exit Function
fehler:
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in holDA/" + App.path)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
' rs.Open "SELECT pat_id, nachname, vorname, zpkt FROM (SELECT pat_id, nachname, vorname, (SELECT MAX(fid) AS mfid FROM `faelle` WHERE pat_id = `namen`.pat_id) AS mmfid FROM `namen` WHERE exists (SELECT * FROM `diagnosen` WHERE icd LIKE 'L89.%' AND NOT icd LIKE 'L89.1%'  AND diagsicherheit <> 'A' AND COALESCE(Dggel,0)=0 AND pat_id = `namen`.pat_id AND fid <> (SELECT MAX(fid) FROM `faelle` WHERE pat_id = `namen`.pat_id)) AND NOT EXISTS (SELECT * FROM `diagnosen` WHERE icd LIKE 'L89.1%' AND diagsicherheit NOT IN ('A','Z') AND COALESCE(Dggel,0)=0 AND pat_id = `namen`.pat_id)) AS innen LEFT JOIN (SELECT MIN(zeitpunkt) AS zpkt, MIN(fid) AS mfid FROM `leistungen` GROUP BY fid) AS lei ON innen.mmfid = lei.mfid ORDER BY zpkt DESC", DBCn, adOpenDynamic, adLockOptimistic
 Screen.MousePointer = vbHourglass
 Set rs = Nothing
' rs.Open sql, DBCn, adOpenDynamic, adLockOptimistic
 myFrag rs, sql
...
' SetDBCn Nothing
 ...
 Exit Function
fehler:
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in dokPfadƒnder/" + App.path)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
'     myEFrag ("KILL " & rtok!id) ' auskommentiert 6.7.26, da Ursache f¸r nicht verbundene Verbingungsabbr¸che
    End If
...
EndeSchlaf:
   ...
   Next uru
'  If rAf <> 0 Then
   Err.Raise 999, , "Fehler " & ErrNr & "(" & ErrDes & ") in anaUpd:" & vbCrLf & "Zahl an Datens‰tzen aktualisiert: " & rAf & vbCrLf & "bei: " & csql.Value
'  End If ' rAf <> 0
Ende:
   Exit Function
SchlafFehler:
...
If Err.Number = -2147217871 Or Err.Number = -2147467259 Then
 ...
 Resume
End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in anaUpd/" + App.path)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): If Err.Number = 999 Then Resume EndeSchlaf Else Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
'            Feldl‰nge = rz.Fields(rq.Fields(i).Name).Properties.Item("OCTETLENGTH")
   ...
   Exit Function
fehler:
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in TUpd/" + App.path)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Public Function anaIns(rq As Recordset)
 ...
 Dim i&, sql1$, sql2$, rAf&, Wert$, mprim&, rs As ADODB.Recordset, pos0&
   ...
   Exit Function
fehler:
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in anaIns/" + App.path)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " & CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in TIns/" & AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in getLDatei/" + AnwPfad)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in ‹bertragFormulare/" + AnwPfad)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
''   Const ConString$ = """PROVIDER=MSDASQL;driver={MySQL ODBC 3.51 Driver};server=mitte;uid=praxis;pwd=...;database=quelle;"""
'   Dim SelString$
...
On Error GoTo fehler
 ...
 Exit Sub
fehler:
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description + vbCrLf + "Fehlerposition: " + CStr(FPos), vbAbortRetryIgnore, "aufgefangener Fehler in SizeColumns/" + App.path)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
' Dim lies As New Lese
 ...
 Call acon(quelleT)
' Call Faelle.Open("SELECT DISTINCT pat_id FROM `faelle` WHERE quartal LIKE '_2005' AND schgr NOT IN(89,90)", DBCn, adOpenDynamic, adLockReadOnly)
 myFrag Faelle, "SELECT DISTINCT pat_id FROM `faelle` WHERE quartal LIKE '_2005' AND schgr NOT IN(89,90)"
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in doHilfsmittelklassifikationen/" + AnwPfad)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
Function fzsfuell(frm As Lese, abstand&, Optional obgestern) ' Abstand: 999 => unbekannt, 9999 => ohne abstand
  ...
  myEFrag ("DELETE fz FROM fallzahlstand fz LEFT JOIN" & vbCrLf & _
     ...
     "WHERE schgr NOT IN(89,90) " & QBed & vbCrLf & _
  ...
  sql = "SELECT i.quartal," & abstand & " tage,i.kassenpat,fz.kassenpat " & _
   ...
   "FROM `faelle` f " & _
   "WHERE schgr NOT IN(89,90) " & QBed & " AND substr(quartal,2,4)> '2003' AND quartal <> '12004' AND quartal <> '22004' GROUP BY quartal, pat_id) i " & _
...
'    myEFrag sql, rAF
    ...
    ",COALESCE((SELECT COUNT(DISTINCT pat_id) FROM faelle WHERE quartal=" & QT & " AND schgr NOT IN(89,90) AND fanf BETWEEN " & qbg & " AND " & qed & "),0) Kassenpat" & vbCrLf & _
    ",COALESCE((SELECT COUNT(DISTINCT pat_id) FROM faelle WHERE quartal=" & vqu & " AND schgr NOT IN(89,90) AND fanf BETWEEN " & vqbg & " AND " & vqed & "),0) Kassenpatvor" & vbCrLf & _
    ",COALESCE((SELECT COUNT(DISTINCT pat_id) FROM faelle f WHERE quartal=" & QT & " AND schgr NOT IN(89,90) AND fanf BETWEEN " & qbg & " AND " & qed & " AND EXISTS (SELECT 0 FROM `diagnosen` d WHERE d.pat_id = f.pat_id AND d.icd RLIKE '^E1[0-4]\.' AND d.diagsicherheit IN ('G',' ') AND obdauer<>0) GROUP BY quartal),0) DM" & vbCrLf & _
    ",COALESCE((SELECT COUNT(DISTINCT pat_id) FROM faelle f WHERE quartal=" & QT & " AND schgr NOT IN(89,90) AND fanf BETWEEN " & qbg & " AND " & qed & " AND EXISTS (SELECT 0 FROM eintraege WHERE pat_id= f.pat_id AND zeitpunkt BETWEEN " & qbg & " AND " & qed & " AND (art IN ('gs','doppler','dop','duplex','dup') OR (art='tb' AND ersteller='gs') OR inhalt LIKE '%(gs)%') AND NOT inhalt LIKE '%(wd)%' AND NOT inhalt LIKE '%(ah)%') GROUP BY quartal),0) davonSchade" & vbCrLf & _
    ",COALESCE((SELECT COUNT(DISTINCT pat_id) FROM faelle f WHERE quartal=" & QT & " AND schgr NOT IN(89,90) AND fanf BETWEEN " & qbg & " AND " & qed & " AND EXISTS (SELECT 0 FROM eintraege WHERE pat_id= f.pat_id AND zeitpunkt BETWEEN " & qbg & " AND " & qed & " AND (art='tk' OR (art='tb' AND ersteller='tk') OR inhalt LIKE '%(tk)%')) GROUP BY quartal),0) davonKothny" & vbCrLf & _
    ...
    ",COALESCE((SELECT COUNT(DISTINCT pat_id) FROM faelle f WHERE quartal=" & QT & " AND schgr NOT IN(89,90) AND fanf BETWEEN " & qbg & " AND " & qed & " AND EXISTS (SELECT 0 FROM eintraege WHERE pat_id= f.pat_id AND zeitpunkt BETWEEN " & qbg & " AND " & qed & " AND (SELECT MIN(fanf) FROM faelle WHERE pat_id=f.pat_id)=f.fanf) GROUP BY quartal),0) neue" & vbCrLf & _
    ...
    ",COALESCE((SELECT COUNT(0) FROM tmbrie b INNER JOIN diagview d ON d.pat_id=b.pat_id AND d.gicd RLIKE '^E1[0-4]\.' AND obdauer<>0 WHERE autor='tk' AND (name LIKE '%brief%' OR name LIKE '%nachricht%') AND quelldatum BETWEEN " & qbg & " AND " & qed & "),0) dmbriefeKothny" & vbCrLf & _
    ") i"
'   sql = sql & _
 ...
 '    sql = sql & vbCrLf & _
     ...
     "(SELECT COUNT(DISTINCT pat_id) FROM `faelle` WHERE schgr NOT IN(89,90) AND quartal = vorquartal " & VQBed & " GROUP BY quartal) Kassenpatvor,  " & vbCrLf & _
     "(SELECT COUNT(DISTINCT pat_id) FROM `faelle` f WHERE schgr NOT IN(89,90) AND quartal = i.quartal " & QBedi & " AND EXISTS (SELECT 0 FROM `diagnosen` d WHERE d.pat_id = f.pat_id AND d.icd RLIKE '^E1[0-4]\.|^O24\.' AND d.diagsicherheit RLIKE '[^Z^A^V]' AND COALESCE(d.Dggel,0)=0) GROUP BY quartal) Diabetes, " & vbCrLf & _
     "(SELECT COUNT(DISTINCT e.pat_id) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.pat_id = f.pat_id WHERE schgr NOT IN(89,90) AND (art IN ('" & IIf(fctQAnf(Quartal) > #1/1/2006#, "gs", "gs','notiz") & "','doppler','dop','duplex') OR inhalt LIKE '%(gs)%') " & zpQBed & " AND CONCAT((month(e.zeitpunkt)+2) div 3, YEAR(e.zeitpunkt)) = i.Quartal) davonSchade, " & vbCrLf & _
     "(SELECT COUNT(DISTINCT e.pat_id) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.pat_id = f.pat_id WHERE schgr NOT IN(89,90) AND (art = 'tk' OR inhalt LIKE '%(tk)%') " & zpQBed & " AND CONCAT((month(e.zeitpunkt)+2) div 3, YEAR(e.zeitpunkt)) = i.Quartal) davonKothny, " & vbCrLf & _
     ...
     "(SELECT COUNT(DISTINCT e.pat_id) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.pat_id = f.pat_id WHERE schgr NOT IN(89,90) AND (art IN ('gs','doppler','dop','duplex') OR inhalt LIKE '%(gs)%') " & zpQBed & " AND CONCAT((month(e.zeitpunkt)+2) div 3, YEAR(e.zeitpunkt)) = i.Quartal AND (SELECT MIN(fanf) FROM `faelle` f1 WHERE f1.pat_id = e.pat_id) = (SELECT MIN(fanf) FROM `faelle` f1 WHERE f1.pat_id = e.pat_id AND fanf >= (CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01')) )) neueSchade, " & vbCrLf & _
     "(SELECT COUNT(DISTINCT e.pat_id) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.pat_id = f.pat_id WHERE schgr NOT IN(89,90) AND (art IN ('tk') OR inhalt LIKE '%(tk)%') " & zpQBed & " AND CONCAT((month(e.zeitpunkt)+2) div 3, YEAR(e.zeitpunkt)) = i.Quartal AND (SELECT MIN(fanf) FROM `faelle` f1 WHERE f1.pat_id = e.pat_id) = (SELECT MIN(fanf) FROM `faelle` f1 WHERE f1.pat_id = e.pat_id AND fanf >= (CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01')) )) neueKothny, " & vbCrLf & _
     ...
     "(SELECT COUNT(0)<>0 FROM `faelle` f1 WHERE f1.fid = f.fid AND f1.fanf = (SELECT MIN(fanf) FROM `faelle` f2 WHERE f2.pat_id = f1.pat_id) AND EXISTS (SELECT 0 FROM `diagnosen` d WHERE d.pat_id = f1.pat_id AND d.icd RLIKE '^E1[0-4]\.|^O24\.' AND d.diagsicherheit RLIKE '[^Z^A^V]' AND COALESCE(d.Dggel,0)=0)) erstdm " & vbCrLf & _
     "FROM `faelle` f " & vbCrLf & _
     "WHERE quartal = " & Qt & " AND schgr NOT IN(89,90) " & QBed & " GROUP BY quartal, pat_id) i " & vbCrLf & _
     "GROUP BY quartal) i"
...
'    sql = "SELECT eart, COUNT(0) zahl FROM (SELECT * FROM (SELECT name,pat_id,DATE(zeitpunkt) zp,(SELECT MAX(art) FROM `eintraege` e WHERE e.pat_id = `tmbrie`.pat_id AND art IN ('tk','gs') AND zeitpunkt = (SELECT MAX(zeitpunkt) FROM `eintraege` e WHERE e.pat_id = `tmbrie`.pat_id AND art IN ('tk','gs') AND zeitpunkt < `tmbrie`.zeitpunkt)) eart FROM `tmbrie` WHERE not name LIKE '%dmp-%' AND (name LIKE '%brief%' OR name LIKE '%nachricht%') AND art IN ('wbr','brief')) i WHERE NOT ISNULL(eart) GROUP BY pat_id, eart, zp) i WHERE zp BETWEEN '" & Format(fctQAnf(quartal), "yyyy-mm-dd") & "' AND '" & IIf(abstand = 9999, Format(fctQEnd(quartal), "yyyy-mm-dd"), Format(fctQAnf(quartal) + abstand, "yyyy-mm-dd")) & "' GROUP BY eart"
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in fzsfuell/" + AnwPfad)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
Function dofallzahlstand(frm As Lese, Optional obgestern$) ' in GesLies, AlleFallzahlst‰nde_Click und Fallzahlstand_Click
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in dofallzahlstand/" + AnwPfad)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
''   sql = "SELECT Quartal, COUNT(0) - COUNT(erst) AS `neue Pat.`, COUNT(0) AS `F‰lle` FROM `faelleverschiedenneu` f " & _
  "WHERE schgr NOT IN(89,90) AND to_days(fanf)-to_days(cdate(CONCAT(MID(quartal,2,4)°'-'°(LEFT(quartal,1)-1)*3+1°'-01'))) " & _
...
'    sql = "SELECT i.quartal, CONCAT(CAST(kassenpat AS char),' (',CONCAT(IF(kassenpat>kassenpatvor,'+',''),CAST(round(round(kassenpat / Kassenpatvor,3)*100-100,1) AS char),'%'),')') `Kassenpat.,`, CONCAT(CAST(davonSchade AS char),' (',CONCAT(CAST(round(round(davonSchade / Kassenpat,3)*100,1) AS char),'%'),')') `davon Schade,`, CONCAT(CAST(davonKothny AS char),' (',CONCAT(CAST(round(round(davonKothny / Kassenpat,3)*100,1) AS char),'%'),')') `Kothny,`, CONCAT(CAST(round(round(Diabetes / kassenpat,3)*100,1) AS char),'%') `D.m.,`, GDM `GDM,`, neue `Neue,`, '' `dav.: `, CONCAT(CAST(round(round(neuedm / neue,3)*100,1) AS char),'%') `D.m.,`, neueSchade `Schade,`, neueKothny `Kothny;`, Doppler `Kasse+Privat: Doppler`, Duplex, Sonos, Schul " & _
          "FROM (SELECT Quartal, COUNT(0) kassenpat,  " & _
          "(SELECT COUNT(DISTINCT pat_id) FROM `faelle` WHERE schgr NOT IN(89,90) AND quartal = vorquartal AND to_days(fanf)-to_days((CONCAT(MID(vorquartal,2,4),'-',(LEFT(vorquartal,1)-1)*3+1,'-01'))) BETWEEN 0 AND " & abstand & " GROUP BY quartal) Kassenpatvor,  " & _
          "(SELECT COUNT(DISTINCT pat_id) FROM `faelle` f WHERE schgr NOT IN(89,90) AND quartal = i.quartal AND to_days(fanf)-to_days((CONCAT(MID(i.quartal,2,4),'-',(LEFT(i.quartal,1)-1)*3+1,'-01'))) BETWEEN 0 AND " & abstand & " AND exists (SELECT 0 FROM `diagnosen` d WHERE pat_id = f.pat_id AND d.icd RLIKE '^E1[0-4]\.|^O24\.' AND diagsicherheit RLIKE '[^Z^A^V]' AND COALESCE(Dggel,0)=0) GROUP BY quartal) Diabetes, " & _
          "(SELECT COUNT(DISTINCT e.pat_id) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.pat_id = f.pat_id WHERE schgr NOT IN(89,90) AND (art IN ('gs','doppler','dop','duplex') OR inhalt LIKE '%(gs)%') AND to_days(e.zeitpunkt)-to_days((CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01'))) BETWEEN 0 AND " & abstand & " AND CONCAT((month(e.zeitpunkt)+2) div 3, YEAR(e.zeitpunkt)) = i.Quartal) davonSchade, " & _
          "(SELECT COUNT(DISTINCT e.pat_id) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.pat_id = f.pat_id WHERE schgr NOT IN(89,90) AND (art = 'tk' OR inhalt LIKE '%(tk)%') AND to_days(e.zeitpunkt)-to_days((CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01'))) BETWEEN 0 AND " & abstand & " AND CONCAT((month(e.zeitpunkt)+2) div 3, YEAR(e.zeitpunkt)) = i.Quartal) davonKothny, " & _
          ...
          "(SELECT COUNT(DISTINCT e.pat_id) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.pat_id = f.pat_id WHERE schgr NOT IN(89,90) AND (art IN ('gs','doppler','dop','duplex') OR inhalt LIKE '%(gs)%') AND to_days(e.zeitpunkt)- to_days((CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01'))) BETWEEN 0 AND " & abstand & " AND CONCAT((month(e.zeitpunkt)+2) div 3, YEAR(e.zeitpunkt)) = i.Quartal AND (SELECT MIN(fanf) FROM `faelle` f1 WHERE f1.pat_id = e.pat_id) = (SELECT MIN(fanf) FROM `faelle` f1 WHERE f1.pat_id = e.pat_id AND fanf >= (CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01')) )) neueSchade, " & _
          "(SELECT COUNT(DISTINCT e.pat_id) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.pat_id = f.pat_id WHERE schgr NOT IN(89,90) AND (art IN ('tk') OR inhalt LIKE '%(tk)%') AND to_days(e.zeitpunkt)-to_days((CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01'))) BETWEEN 0 AND " & abstand & " AND CONCAT((month(e.zeitpunkt)+2) div 3, YEAR(e.zeitpunkt)) = i.Quartal AND (SELECT MIN(fanf) FROM `faelle` f1 WHERE f1.pat_id = e.pat_id) = (SELECT MIN(fanf) FROM `faelle` f1 WHERE f1.pat_id = e.pat_id AND fanf >= (CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01')) )) neueKothny, " & _
          ...
          "(SELECT COUNT(0)<>0 FROM `faelle` f1 WHERE f1.fid = f.fid AND f1.fanf = (SELECT MIN(fanf) FROM `faelle` f2 WHERE f2.pat_id = f1.pat_id) AND exists (SELECT 0 FROM `diagnosen` d WHERE pat_id = f1.pat_id AND d.icd RLIKE '^E1[0-4]\.|^O24\.' AND d.diagsicherheit RLIKE '[^Z^A^V]' AND COALESCE(d.Dggel,0)=0 )) erstdm " & _
          "FROM `faelle` f WHERE schgr NOT IN(89,90) AND to_days(fanf)-to_days((CONCAT(MID(quartal,2,4),'-',(LEFT(quartal,1)-1)*3+1,'-01'))) BETWEEN 0 AND " & abstand & " AND substr(quartal,2,4)> '2008' AND quartal <> '12009'  GROUP BY quartal, pat_id) i GROUP BY quartal ORDER BY substr(Quartal,2,4) DESC,LEFT(Quartal,1) DESC) i"

'' sql = _
 "SELECT Quartal, COUNT(0) `Kassenpat.,`, " & _
 "(SELECT COUNT(distinct(e.pat_id)) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.fid = f.fid WHERE schgr NOT IN(89,90) AND art IN ('gs','doppler','dop','duplex') " & _
 ...
 "(SELECT COUNT(distinct(e.pat_id)) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.fid = f.fid WHERE schgr NOT IN(89,90) AND art = 'tk' " & _
 "and to_days(e.zeitpunkt)-to_days((CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01'))) BETWEEN 0 AND " & abstand & " " & _
 ...
 "(SELECT COUNT(distinct(e.pat_id)) Zahl FROM `eintraege` e LEFT JOIN `faelle` f ON e.fid = f.fid WHERE schgr NOT IN(89,90) AND art IN ('tk') " & _
 "and to_days(e.zeitpunkt)-to_days((CONCAT(YEAR(e.zeitpunkt),'-',month(e.zeitpunkt) - ((month(e.zeitpunkt)-1) mod 3),'-01'))) BETWEEN 0 AND " & abstand & " " & _
...
"FROM `faelle` f WHERE schgr NOT IN(89,90) AND to_days(fanf)-to_days((CONCAT(MID(quartal,2,4),'-',(LEFT(quartal,1)-1)*3+1,'-01'))) BETWEEN 0 AND " & abstand & " AND substr(quartal,2,4)> '2008' AND quartal <> '12009'  GROUP BY quartal, pat_id) i GROUP BY quartal ORDER BY substr(Quartal,2,4) DESC,LEFT(Quartal,1) DESC"
...
' ON Error GoTo fehler
' sql = "SELECT time(zp) AS uzeit, i2.* FROM (SELECT IF(minleizp > zeitpunkt, minleizp, zeitpunkt) AS zp, innen.* FROM (SELECT b.pat_id, (SELECT adddate(min(zeitpunkt),INTERVAL 1 minute) FROM `leistungen` WHERE fid = f.fid) AS minleizp, b.zeitpunkt AS zeitpunkt, f.fid, '40120' AS leistung, schgr,GOƒKatName,GOƒKatNr,‹WZiel,And‹w,‹bwV,BhFB,BhFE1,TMFNr,AbrGb,GebOr,KVKserg,KVKs,IK,AbrAr,lVorl,KtrAbrB, name FROM `tmbrie` b LEFT JOIN lfaellev f using(pat_id) LEFT JOIN (SELECT * FROM `leistungen` WHERE leistung LIKE '4012%') AS l ON l.pat_id = b.pat_id AND DATE(l.zeitpunkt)= DATE(b.zeitpunkt) WHERE (name LIKE '%brief%' OR name LIKE '%DMP-Daten%') AND b.zeitpunkt BETWEEN CONCAT(YEAR(SUBDATE(NOW(),INTERVAL 14 DAY)),'-',((month(SUBDATE(NOW(),INTERVAL 14 DAY))-1)div 3)*3+1,'-01') AND adddate(CONCAT(YEAR(SUBDATE(NOW(),INTERVAL 14 DAY)),'-',((month(SUBDATE(NOW(),INTERVAL 14 DAY))-1)div 3)*3+1,'-01'), INTERVAL 3 MONTH) AND ISNULL(leistung) AND schgr NOT IN(89,90)) AS innen) AS i2x;"
' myFrag rs, sql
' IF Not rs.BOF THEN
...
''     op = format$(3 + 4 + Len("TM#" + nz(rs!TMFNr,space$(11))), "000") + "4144" + "TM#" + nz(rs!TMFNr,space$(11))
''     Print #310, ZSU(op)
...
' AnwPfad = App.Path
'#END IF
' SELECT CASE MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in doPorto/" + AnwPfad)
'  Case vbAbort: Call MsgBox("Hˆre auf"): Progende
'  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
'  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
' END SELECT
...
#Else
 AnwPfad = App.path
#End If
Select Case MsgBox("FNr: " & FNr & ", ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in fallzeig/" + AnwPfad)
 Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
 Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
 Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
End Select
...
Public Function labtest()
 ...
 Lese.ProgStart
 Dim rs As New ADODB.Recordset
 myFrag rs, "SELECT (SELECT COUNT(0) FROM `" & vorsil & "us` u WHERE datid = e.datid) zahl,datid,pfad,name FROM " & vorsil & "eingel e"
 Do While Not rs.EOF
  erg = Dir(rs!Pfad)
  If LenB(erg) = 0 Then
   erg = Dir("\\anmeldr\biowinbackup\" & rs!name)
   If LenB(erg) <> 0 Then
    myEFrag "UPDATE " & vorsil & "eingel SET pfad = CONCAT('\\\\anmeldr\\biowinbackup\\',name) WHERE datid = " & rs!DatID, rAf
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in holFrist/" + AnwPfad)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
Public Function ZahlEintrag() ' &Zahlen aus Eintr‰gen Feld inhnum eintragen
 Dim rn As New ADODB.Recordset, rf As New ADODB.Recordset, rE As New ADODB.Recordset, gew0 As New CString, gewStr As New CString, buch$, i&, gew#, za%, kz%, ZZ%, kzm% ' Ziffer (als letztes) angeh‰ngt, Kommazahl, Ziffernzahl, keine Ziffer mehr
...
sql = "CREATE TABLE `kassenliste` (" & vbCrLf & _
  "`ID` int(10) NOT NULL KEY AUTO_INCREMENT," & vbCrLf & _
...
#Else
 AnwPfad = App.path
#End If
 Select Case MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + CStr(nz(Err.Source, "")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "aufgefangener Fehler in lieseExcel/" + AnwPfad)
  Case vbAbort: Call MsgBox("Hˆre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Resume Next
 End Select
...
If Not rs.BOF Then
 Do While Not rs.EOF
  ...
  sql = "INSERT INTO kassenliste(VKNR,IK,NAME,Kateg,AnzahlIK,AnzahlKTUG,G¸ltigVon,G¸ltigBis,GO,Kurzname,rName,Lantus2,Levemir2,Humalog,Liprolog,Novorapid,Apidra,eingef,geaen,pid)VALUES('" & _
  rs!VKNr & "','" & rs!IK & "','" & rs!name & "','" & rs!kateg & "','" & rs!AnzahlIK & "','" & rs!AnzahlKTUG & "','" & rs!G¸ltigVon & "','" & rs!G¸ltigBis & "','" & rs!GO & "','" & rs!kurzname & "','" & rs!rname & "','" & rs!Lantus2 & "','" & rs!Levemir2 & "','" & rs!Humalog & "','" & rs!Liprolog & "','" & rs!Novorapid & "','" & rs!Apidra & "','" & rs!eingef & "','" & rs!geaen & "','" & rs!pid & _
  "')"
