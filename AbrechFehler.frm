...
"NOT (diabetestyp IN ('-','p','','?','g') AND (ISNULL(icd) OR icd LIKE'R73.0%')) AND " & vbCrLf & _
...
",f.übwvbsnr kvnr, CONCAT(ea.FNachname,', ',ea.FVorname) Üw " & vbCrLf & _
...
sql(AWlf) = sql(AWlf) & _
"LEFT JOIN namen n USING (pat_id) " & vbCrLf & _
"LEFT JOIN (SELECT * FROM earzt GROUP BY farztnr)ea ON ea.FArztnr=f.übwvlanr AND ea.farztnr<>''" & vbCrLf & _
...
' 18
AwN(AWlf) = "LANR von Überweisern, die bei externen Ärzten nicht auffindbar sind"
sql(AWlf) = _
"SELECT f.pat_id,gesnameg(f.pat_id)PName,f.quartal,ÜbWVLANR,ÜWZiel,ausgst,BhFB,BhFE1,Kasse,Auftrag,Verdacht,Befund" & vbCrLf & _
" FROM aktfv fa" & vbCrLf & _
" JOIN faelle f USING(pat_id,fid)" & vbCrLf & _
" LEFT JOIN (SELECT * FROM earzt GROUP BY farztnr)ea ON ea.FArztnr=f.übwvlanr AND ea.farztnr<>''" & vbCrLf & _
" WHERE farztnr Is Null And f.SchGr <> 0"
 mins(AWlf) = 10
 maxs(AWlf) = 80
 AWlf = AWlf + 1

' 19
AwN(AWlf) = "BSNR von Überweisern, die bei externen Ärzten nicht auffindbar sind"
sql(AWlf) = _
"SELECT f.pat_id,gesnameg(f.pat_id)PName,f.quartal,übwvbsnr,ÜWZiel,ausgst,BhFB,BhFE1,Kasse,Auftrag,Verdacht,Befund" & vbCrLf & _
" FROM aktfv fa" & vbCrLf & _
" JOIN faelle f USING(pat_id,fid)" & vbCrLf & _
" LEFT JOIN (SELECT * FROM epraxis GROUP BY FBetriebsnr)ep ON ep.FBetriebsnr =f.übwvbsnr AND ep.FBetriebsnr<>''" & vbCrLf & _
" WHERE FBetriebsnr IS NULL AND f.schgr<>0"
 mins(AWlf) = 10
 maxs(AWlf) = 80
 AWlf = AWlf + 1
...
"  , CASE " & vbCrLf & _
"   WHEN eintr<>''AND Azu<>''AND dt<>''AND azu NOT RLIKE Eintr AND dt NOT RLIKE Eintr THEN 'Azu+Desktop falsch?' " & vbCrLf & _
"   WHEN eintr<>''AND Azu<>''AND azu NOT RLIKE Eintr THEN'Azu falsch?'" & vbCrLf & _
"   WHEN dt<>''AND Azu<>''AND dt NOT RLIKE Eintr THEN'Desktop falsch?'" & vbCrLf & _
"   WHEN termin<>'' AND (e_t OR d_t) /*AND NOT ungt*/ THEN ' Termin falsch?' " & vbCrLf & _
"   ELSE 'bitte prüfen' END `mögl.Fehler` " & vbCrLf & _
"  ,gein,dt/*, gsz gszahl,gszul gszuletzt,tkz tkzahl,tkzul tkzuletzt,ahz ahzahl,ahzul ahzuletzt*/" & vbCrLf & _
"  FROM ( " & vbCrLf & _
"   SELECT i.*" & vbCrLf & _
"   , eintr<>'' AND dt RLIKE 'tk|gs|ah' AND INSTR(dt,eintr)=0 e_d" & vbCrLf & _
"   , Azu<>'' AND dt RLIKE 'tk|gs|ah' AND INSTR(dt,Azu)=0 a_d" & vbCrLf & _
...
"   FROM (" & vbCrLf & _
"    SELECT f.*, gein /*CASE WHEN gszul>tkzul AND gszul>ahzul AND gsz>tkz AND gsz>ahz THEN 'gs'" & vbCrLf & _
"                     WHEN tkzul>gszul AND tkzul>ahzul AND tkz>gsz AND tkz>ahz THEN 'tk'" & vbCrLf & _
"                     WHEN ahzul>gszul AND ahzul>tkzul AND ahz>gsz AND ahz>tkz THEN 'ah'" & vbCrLf & _
"                     WHEN gszul>tkzul AND gszul>ahzul THEN 'gs'" & vbCrLf & _
"                     WHEN tkzul>gszul AND tkzul>ahzul THEN 'tk'" & vbCrLf & _
"                     WHEN ahzul>gszul AND ahzul>tkzul THEN 'ah'" & vbCrLf & _
"                     ELSE '' END*/ Eintr " & vbCrLf & _
"    FROM ( " & vbCrLf
sql(AWlf) = sql(AWlf) & _
"     SELECT f.pat_id " & vbCrLf & _
...
"/*     , (SELECT COUNT(0) FROM eintraege WHERE (((art IN ('gs','doppler','dop','duplex','dup','tb','th')AND ersteller='gs') AND NOT inhalt LIKE '%(tk)%') OR inhalt LIKE '%(gs)%') AND pat_id = f.pat_id AND zeitpunkt>SUBDATE(CURRENT_TIMESTAMP()," & eab & ")) gsz " & vbCrLf & _
"     , COALESCE((SELECT DATE(MAX(zeitpunkt)) FROM eintraege WHERE (((art IN ('gs','doppler','dop','duplex','dup','tb','th') AND ersteller='gs') AND NOT inhalt RLIKE '\((ah|tk)\)') OR inhalt LIKE '%(gs)%') AND pat_id = f.pat_id AND zeitpunkt>SUBDATE(CURRENT_TIMESTAMP()," & eab & ")),0) gszul " & vbCrLf & _
"     , (SELECT COUNT(0) FROM eintraege WHERE ((art IN ('tk','ARCHIE2','APK') OR (art IN('tb','th')AND ersteller='tk')) OR inhalt LIKE '%(tk)%') AND pat_id = f.pat_id AND zeitpunkt>SUBDATE(CURRENT_TIMESTAMP()," & eab & ")) tkz " & vbCrLf & _
"     , COALESCE((SELECT DATE(MAX(zeitpunkt)) FROM eintraege WHERE ((art IN ('tk','ARCHIE2','APK') OR (art IN('tb','th')AND ersteller='tk')) OR inhalt LIKE '%(tk)%') AND pat_id = f.pat_id AND zeitpunkt>SUBDATE(CURRENT_TIMESTAMP()," & eab & ")),0) tkzul " & vbCrLf & _
"     , (SELECT COUNT(0) FROM eintraege WHERE (((art IN ('ah','doppler','dop','duplex','dup','tb','th')AND ersteller='ah') AND NOT inhalt RLIKE '\((gs|tk)\)') OR inhalt LIKE '%(ah)%') AND pat_id = f.pat_id AND zeitpunkt>SUBDATE(CURRENT_TIMESTAMP()," & eab & ")) ahz " & vbCrLf & _
"     , COALESCE((SELECT DATE(MAX(zeitpunkt)) FROM eintraege WHERE (((art IN ('gs','doppler','dop','duplex','dup','tb','th') AND ersteller='gs') AND NOT inhalt RLIKE '\((gs|tk)\)') OR inhalt LIKE '%(ah)%') AND pat_id = f.pat_id AND zeitpunkt>SUBDATE(CURRENT_TIMESTAMP()," & eab & ")),0) ahzul */" & vbCrLf & _
"     , COALESCE((SELECT GROUP_CONCAT(DISTINCT ersteller SEPARATOR '|')FROM eintraege WHERE(art IN('gs','doppler','dop','duplex','dup','tb','th')AND ersteller IN('gs','tk','ah'))AND pat_id = f.pat_id AND zeitpunkt>SUBDATE(CURRENT_TIMESTAMP()," & eab & ")),'')gein" & vbCrLf & _
"     , COALESCE((SELECT CONCAT(CASE WHEN raum='Schade' THEN 'gs' WHEN raum='Kothny' THEN 'tk' WHEN raum='Wagner' THEN 'wd' WHEN raum LIKE 'Hamm%' THEN 'ah' ELSE '' END,' ', DATE_FORMAT(zp,'%e.%c.%y'),' ',REPLACE(REPLACE(LEFT(zusatz,IF(LENGTH(zusatz)-1<25,LENGTH(zusatz)-1,25)),CHR(10),''),CHR(13),'')) FROM termine t WHERE pid = f.pat_id AND raum<>'Labor' AND zp =(SELECT MIN(zp) FROM termine WHERE zp>=CURRENT_TIMESTAMP() AND pid=t.pid AND raum RLIKE 'Kothny|Schade|Wagner|^Hamm' AND NOT zusatz RLIKE 'doppler|duplex|dup|car|belastung|Sono|Arterien|Venen') AND aktzeit=(SELECT MAX(aktzeit) FROM termine WHERE pid=t.pid AND raum<>'Labor'AND zp=t.zp) LIMIT 1),'')Termin" & vbCrLf
sql(AWlf) = sql(AWlf) & _
...
' 304
AwN(AWlf) = "Doppelte Patienten aktueller Fälle (zumindest in Patientendaten)"
sql(AWlf) = "SELECT gesname(pat_id)PName, DATE_FORMAT(Gebdat,'%d.%m.%y')Geb" & vbCrLf & _
...
",(SELECT GROUP_CONCAT(pat_id)PID FROM namen WHERE gebdat=n.gebdat AND(nachname RLIKE n.nachname OR n.nachname RLIKE nachname)AND(vorname RLIKE n.vorname OR n.vorname RLIKE vorname))PIDs" & vbCrLf & _
"FROM aktfv f JOIN namen n USING(pat_id)" & vbCrLf & _
"HAVING zahl>1;"
 mins(AWlf) = 10
 maxs(AWlf) = 60
 AWlf = AWlf + 1
...
' 305
AwN(AWlf) = "Doppelte Patienten fallunabhängig (zumindest in Patientendaten), danach 401"
sql(AWlf) = "SELECT gesname(pat_id)PName,n.Pat_id,DATE_FORMAT(Gebdat,'%d.%m.%y')Geb" & vbCrLf & _
",(SELECT COUNT(0)FROM namen WHERE gebdat=n.gebdat AND (nachname RLIKE n.nachname OR n.nachname RLIKE nachname)AND(vorname RLIKE n.vorname OR n.vorname RLIKE vorname))Zahl" & vbCrLf & _
...
"AND inhalt not RLIKE 'laborbefund|Ã¤|Ã¼|Ã¶|Charge|Vax|Duplikat gedruckt|Encep|Markierung gesetzt|Tonaudio'" & vbCrLf & _
...
' AND COALESCE(gd.Dggel,0)=0
 sql(AWlf) = _
"SELECT i.Pat_id PID, gesname(i.pat_id) Name, CONCAT(F0,IF(F0='' OR F1='','',', '),F1) LEIFehler, IF(i.obChrPa,'ja','nein') chron, IF(i.obvorkoz,'ja','nein') 3VorKte, i.aktkoz aktKtzl, i.l0z 03220zl, i.l1z 03221zl, sonst Sonstige_Diagnosen,DATE(IF(F0 RLIKE'03220 dazu',erst,letzt))LEIDAT,LANRID,czp Kontakte,cart KontaktArt" & vbCrLf & _
"FROM ( " & vbCrLf & _
...
' AND COALESCE(gd.Dggel,0)=0
 sql(AWlf) = _
"SELECT i.Pat_id PID, gesname(i.pat_id) Name, CONCAT(F0,IF(F0='' OR F1='','',', '),F1) LEIFehler, IF(i.obChrPa,'ja','nein') chron, IF(i.obvorkoz,'ja','nein') 3VorKte, i.aktkoz aktKtzl, i.l0z 03220zl, i.l1z 03221zl,DATE(IF(F0 RLIKE'03220 dazu',erst,letzt))LEIDAT,LANRID,czp Kontakte,cart KontaktArt,sonst Sonstige_Diagnosen" & vbCrLf & _
"FROM ( " & vbCrLf & _
"SELECT i.Pat_id, i.obChrPa, i.obvorkoz, i.aktkoz, i.l0z, i.l1z, i.sonst " & vbCrLf & _
", CASE " & vbCrLf & _
"    WHEN obChrPa=1 AND obvorkoz=1 THEN " & vbCrLf & _
...
' neuere Updates
     ...
     myEFrag (" UPDATE anamnesebogen a" & vbCrLf & _
      "LEFT JOIN (SELECT pat_id,min(gicd) gicd FROM diagview where gicd LIKE'R73.0%' GROUP BY pat_id) pd ON a.pat_id=pd.pat_id" & vbCrLf & _
...
'  If rE Is Nothing Then' auskommentiert 26.3.23
  ...
  If obmo Then
   If MOConInit(, "AbrFausg: " & name) Then Exit Function
   Set rc = MOCon
...
' AnwPfad = App.path
'#END IF
'SELECT CASE MsgBox("FNr: " & FNr & "ErrNr: " & CStr(Err.Number) + vbCrLf + "LastDLLError: " + CStr(Err.LastDllError) + vbCrLf + "Source: " + cstr(nz(err.source,"")) + vbCrLf + "Description: " + Err.Description, vbAbortRetryIgnore, "Aufgefangener Fehler in do_ausg/" + AnwPfad)
' Case vbAbort: Call MsgBox("Höre auf"): Progende
