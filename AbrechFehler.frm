...
"OR (NOT ISNULL(dd.icd) AND NOT ISNULL(gd.icd))"
 ...
 sql(AWlf) = "SELECT d.pat_id,GesNameg(d.pat_id) Name,d.diagdatum Datum,d.diagsicherheit Si, d.DiagText,d.ICD FROM `diagnosen` d LEFT JOIN faelle f ON d.fid = f.fid LEFT JOIN `diagnosen` r ON d.pat_id = r.pat_id AND r.icd RLIKE '^E1[01]' WHERE d.gzICDok RLIKE '^E1[234]' AND ISNULL(r.icd) AND schgr<> 90" ' AND COALESCE(d.f6010,0)=0
 mins(AWlf) = 5
 maxs(AWlf) = 18
 AWlf = AWlf + 1
