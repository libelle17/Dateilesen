...
#If gehtgenauso Then
   ...
   "   SELECT DATE_ADD(date,INTERVAL-3 MONTH) FROM nrows WHERE DATE>CURRENT_DATE-INTERVAL " & CStr((opt.Dokuzahl - 1) * 3) & " MONTH)" & vbCrLf & _
   "   SELECT DATE, quartal(DATE)q FROM nrows) qr" & vbCrLf & _
   "   LEFT JOIN dmpreihe dr ON dr.Pat_id=n.pat_id AND dr." & dmpasp & " AND quartal(dr.dokudatum)=qr.q" & vbCrLf & _
   "    AND dr.dokudatum=(SELECT MAX(dokudatum)FROM dmpreihe WHERE pat_id=f.pat_id AND " & dmpasp & "AND quartal(dokudatum)=qr.q)" & vbCrLf & _
   "  ) dokus" & vbCrLf & _
   " ,(SELECT qbeg(MAX(dokudatum))FROM dmpreihe WHERE pat_id=f.pat_id AND " & dmpasp & ") qb" & vbCrLf & _
   "-- ,ROW_NUMBER()OVER(PARTITION BY f.pat_id ORDER BY dr.DokuDatum DESC) rn" & vbCrLf & _
   " ,COALESCE((SELECT MIN(art)='ED' FROM dmpreihe dr WHERE pat_id=f.pat_id AND " & dmpasp & "),0) oberst" & vbCrLf & _
...
'       Call FertigStellen(Me.MFG.MouseRow, True) ' in Turbomed anzeigen
        waehleinMO (Me.MFG.TextMatrix(Me.MFG.MouseRow, Pat_IDSp))
...
Public Function waehleinMO(Pat_id&)
    ...
    Else
     ...
     Pausenlänge = 10
     For j = 500 To 500 Step 100
      For k = 20 To 20 Step 10
       Mausklick 0, j, k ' Screen.Width / 2, Screen.Height / 2
...
mofehler:
 Select Case MsgBox("Fehler beim Aufruf von " & Pat_id & " in Medical Office. Bitte diese Anwendung überprüfen!", vbAbortRetryIgnore, "Aufgefangener Fehler in waehleinMO/" + App.path)
  Case vbAbort: Call MsgBox("Höre auf"): ProgEnde
  Case vbRetry: Call MsgBox("Versuche nochmal"): Resume
  Case vbIgnore: Call MsgBox("Setze fort"): Exit Function
 End Select
fehler:
 Dim AnwPfad$
#If VBA6 Then
