...
Private Sub DokuBeliebig() ' Doku zu beliebigem Patienten
 Dim erg As Variant, VorDoku$, NachN$, VorN$
 Select Case PLArt
  ...
  Case artpat ' DMP hier Liste
   ...
   If IsNumeric(erg) Then
    With MFG
     ...
     altC = .col
     .col = NachNameSp
     Call callMachDMPBogen(CLng(erg), NachN, VorN, .CellBackColor = vbWhite, .CellBackColor = HellRot, .TextMatrix(.Row, ICDSp), True)
     .col = altC
     FNr = 9
     .SetFocus
...
' auskommentiert am 30.6.15, da bei Woltmann Fehler dadurch erzeugt ("Wenn eine Angabe im Feld KVK-Einlesedatum erfolgte, dann darf die Angabe der Statusergänzung nicht fehlen.")
 ...
 Dim mofl$
 mofl = movz & "\641915300_" & Pat_ID & Format(DokuDat, "yyyymmdd") & ".E" & IIf(obErstD, "E", "V") & "D" & DmT
...
#End If
 ...
 Print #176, "<sciphox:Beobachtung><sciphox:Parameter DN=""HbA1c""/><sciphox:Ergebniswert V=""" & REPLACE$(aktDC.x_DMPHbA1c, ",", ".") & """ U=""%""/></sciphox:Beobachtung>"
' dmpstring()
 Print #176, "<sciphox:Beobachtung><sciphox:Parameter DN=""eGFR""/><sciphox:Ergebniswert V=""" & REPLACE$(aktDC.eGFR, ",", ".") & """ U=""ml/min/1,73m2KOF""/></sciphox:Beobachtung>"
 ...
 Print #176, "<sciphox:Beobachtung><sciphox:Parameter DN=""Weiteres Risiko für Ulcus""/><sciphox:Ergebnistext V=""" & "" & """/>"
 ...
 Print #176, "<sciphox:Beobachtung><sciphox:Parameter DN=""(Wund)Infektion""/><sciphox:Ergebnistext V=""" & aktDC.Infekt & """/></sciphox:Beobachtung>"
 ...
 Print #176, "<sciphox:Beobachtung><sciphox:Parameter DN=""Empfohlene Schulung(en) wahrgenommen""/><sciphox:Beobachtungen>"
 Print #176, "<sciphox:Beobachtung><sciphox:Parameter DN=""Diabetes-Schulung""/><sciphox:Ergebnistext V=""" & Switch(aktDC.dmpDMSchulWahrg = "j", "Ja", aktDC.dmpDMSchulWahrg = "n", "Nein", aktDC.dmpDMSchulWahrg = "War aktuell nicht möglich", True, "Bei letzter Dokumentation keine Schulung empfohlen") & """/></sciphox:Beobachtung>"
 Print #176, "<sciphox:Beobachtung><sciphox:Parameter DN=""Hypertonie-Schulung""/><sciphox:Ergebnistext V=""" & Switch(aktDC.dmpHypertSchulWahrg = "j", "Ja", aktDC.dmpHypertSchulWahrg = "n", "Nein", aktDC.dmpHypertSchulWahrg = "War aktuell nicht möglich", True, "Bei letzter Dokumentation keine Schulung empfohlen") & """/></sciphox:Beobachtung>"
 Print #176, "</sciphox:Beobachtungen></sciphox:Beobachtung></sciphox:Beobachtungen></sciphox:sciphox-ssu></local_markup></content></paragraph>"
 Print #176, ""
 Print #176, ""
 Print #176, ""
 Print #176, ""
 Print #176, ""
 Print #176, ""
 Print #176, "</section></body></levelone>"
 Print #176, ""
 Print #176, ""
 Print #176, ""
 Print #176, ""
...
#Else
      ...
      "   ,a.tkz, n.nachname, n.vorname, d.icd, k.kurzname,dr.art,dr.ok,dr.exportiert,n.SDatum" & vbCrLf & _
      "   FROM `namen` n" & vbCrLf & _
      "   LEFT JOIN `diagview` d ON n.pat_id = d.pat_id AND gicd RLIKE '^E1[0-4]'" & vbCrLf & _
      ...
      If Me.ohneTermine Then sql = sql & "   LEFT JOIN termine t ON t.pid = n.pat_id AND DATE(t.zp) BETWEEN NOW() AND qende(NOW()) " & vbCrLf
      sql = sql & _
      "   LEFT JOIN dmpreihe dr ON dr.Pat_id=n.pat_id AND dr.Abk  RLIKE '^(eDMPDM|DMPDTYP)'" & vbCrLf & _
      "   WHERE (/*notiz LIKE '%DMP hier%' OR */dmpklass = 3) AND NOT ISNULL (f.bhfb)" & vbCrLf & _
      "     AND (SDatum IS NULL OR SDatum=18991230 OR SDatum>qbeg(qbeg(NOW() - INTERVAL " & Verspätung & " DAY)- INTERVAL 1 DAY))" & vbCrLf & _
      "     AND NOT (f.bhfb<qbeg(qbeg(NOW() - INTERVAL " & Verspätung & " DAY)- INTERVAL 1 DAY) AND tkz)" & vbCrLf & _
      "     AND n.pat_id<" & pidoffs & "" & vbCrLf & _
