...
#Else
   ...
   " FROM faelle f" & vbCrLf & _
   " LEFT JOIN namen n USING (pat_id)" & vbCrLf & _
   " LEFT JOIN dmpreihe dr ON dr.Pat_id=f.pat_id AND dr.dmpart IN(1,2) AND dokudatum=(SELECT MAX(dokudatum) FROM dmpreihe WHERE pat_id=dr.pat_id AND dmpart IN (1,2)) -- dr.Abk  RLIKE '^(eDMPDM|DMPD(TYP|M2)|(Erst|Verlaufs)-Dokumentation Diabetes)'" & vbCrLf & _
   ...
   " LEFT JOIN diagview dv ON dv.pat_id=n.pat_id AND dv.gicd RLIKE '^E1[0-4]' AND id1=(SELECT MIN(id1) FROM diagview WHERE pat_id=dv.pat_id AND gicd RLIKE '^E1[0-4]')" & vbCrLf
   sql = sql & _
